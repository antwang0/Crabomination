...
2. **Gates at the closing tip `1e21fa73`:** suite **19,620 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); clippy **0** over the workspace; `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean; `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant at two tips, `determinism ok`, `thread_determinism ok (3 vs 1)`, `peak_rss_mib` 25.5; sweep **4 blocks / 24 cells / 118,400 games / 0 failures** at fresh seeds 1428..1435, `cap 0 / board 0 / stuck 0 / draw 26` — **FRONTIER 1436**. **Closing absolutes (SIXTEENTH box, `profiling-fast --no-default-features`, system allocator): fixed 567,018,948 / cube 1,473,886,815 / sealed 1,594,388,820; six rows sum to -0.892 / -1.419 / -1.129 % and the tip reads -0.882 / -1.214 / -1.030 % all-in against this run's second base.** ⚠ Four concurrent Commander commits landed on top of `(-367)` and are +0.016 / +0.003 / -0.009 % — re-take the base, do not predict it. ⚠ **Three more landed after the gate block above** (multiplayer robustness, a pod runner, a client picker); suite **19,632 / 0 / 6** and clippy **0** re-verified at that tip, but `--bench`, the sweep and the absolutes are the `1e21fa73` ones.
...
3. **The instrument that pays:** the pod run is where four bugs came from in one sitting. Run it before and after anything Commander-shaped; a stall rate that moves is a bug, not noise. **Tip reading: `--commander --games 400 --seed 43` → 400 games, 0 undecided, 41.83 turns/game, 1,969.0 actions/game, 321.53 games/s; `--seats 3 --games 200 --seed 97` → 200 / 0 / 30.06.** It was **15 % "no legal move"** before the CR 800.4a pending-decision fix. `CRAB_CAP_DIAG=1` names an undecided pod game's board. ⚠ Read **turns**/game, not actions/game, to tell "different games" from "fewer pointless actions" — CR 509.1a's block gate moved actions 2,096.6 → 1,969.0 with turns unchanged.
...
5. **Still ⏳, roughly in value order:** more planeswalker commanders into `sets::cmdr` (the flag is done, 32 of the 33 such cards are unimplemented); Eminence and commander ninjutsu (**no implemented card prints either**, so both are primitives without a consumer — implement a card first or skip); join forces; Friends forever / Doctor's companion (no card exists in the catalog). ⚠ Check a mechanic has a consumer *before* building it: "Spell commander" looked like one in the local cache and is not a real mechanic at all.
...
12. ⚠ **The local `.scryfall_cache.json` is not entirely trustworthy**: it holds fabricated entries ("Ransack, the Lab" / "Gather, the Townsfolk" printing a non-existent "Spell commander"). Every audit here keys on the catalog's names, which those are not, but confirm a mechanic against the live API before building for it.
