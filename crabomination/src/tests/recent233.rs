...
fn metamorphic_blast_shrinks_to_rabbit() {
    ...
    assert!(cp.subtypes.creature_types.contains(&crate::card::CreatureType::Rabbit), "a Rabbit");
}
...
fn metamorphic_blast_draws_two() {
    let mut g = two_player_game();
    for i in 0..3 {
        g.players[0].add_to_library_top(crate::card::CardId(9200 + i), catalog::mountain());
    }
    let modes = spree_modes(&catalog::metamorphic_blast());
