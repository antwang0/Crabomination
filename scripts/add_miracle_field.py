"""Insert `miracle: None,` into fully-specified CardDefinition literals
missing the new field. Parse cargo E0063 output for the sites, then patch
each by anchoring after the `saga_chapters:` field line."""
...
def main():
    ...
    for line in proc.stdout.splitlines():
        ...
        if code.get("code") != "E0063":
            continue
        if "missing field `miracle`" not in msg.get("message", ""):
            continue
    ...
    for path, lns in edits.items():
        ...
        for ln in sorted(lns, reverse=True):
            ...
            # `saga_chapters:` field line and insert after it.
            insert_at = None
            for k in range(ln - 1, min(ln + 200, len(lines))):
                if re.match(r"\s*saga_chapters:.*$", lines[k]):
                    insert_at = k + 1
                    break
            ...
            lines.insert(insert_at, indent + "miracle: None,\n")
