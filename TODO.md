...
2. **Gates at the closing tip:** suite **20,242 / 0 / 7** (`CRAB_ANSWER_LOG=strict`); clippy **0**; `release-fast` check clean; `--bench` **195,806 / 27.49 / 611.9 / 0 — byte-identical**, `determinism ok`, `thread_determinism ok (3 vs 1)`. **Pod, seed 9921, 1,000 games at each of 2..12 seats: 11,000 decided, every `undecided_by` zero, zero panics**; turns/game 19.26 / 32.00 / 45.54 / 56.84 / 64.58 / 77.46 / 92.30 / 99.34 / 123.68 / 137.91 / 145.02 (still ~12 a seat). **`--card-census` at 12 seats: every card of all twelve lists played, 566 distinct.** Two-player cube/sos/sealed at fresh seeds: 0 panics, one rules draw per cube seed (CR 104.4a — PERF's sweeps carry cube draws at other seeds too; not a stall).
3. 🔎🔎 **THE FIND: OFFICIAL PRECONS ARE REACHABLE, AND THE FIELD NOW HAS TWO.** MTGJSON's deck files say how a precon splits (the Scryfall cache can't), and `scripts/precon_scan.py` ranks all ~200 Commander precons by cards the catalog lacks. **Sultai Arisen (TDC, Teval) was 99/100 already; Mind Flayarrrs (CLB, N'ghathrod) 93/100.** Both are pod seats 11 and 12, card for card, appended so every committed reading below 11 seats is unchanged. ⚠ The scan's first draft undercounted by matching only `name:` literals; it now also matches factory slugs — **a card built by a helper (`land("…")`) has no `name:` field**.
4. ✅ **Seven primitives, each built for the class, not the card:** `SpendRestriction::SpellFromGraveyard` + `SpellKind::from_graveyard` on every graveyard-cast path (CR 106.6/601.2a — retrace, escape and aftermath used to pay with an EMPTY kind, so no restricted mana could fund them at all); `ControlledHaveAbilitiesOfExiledWithSource` (Steward); `GraveyardCardsHaveEscapeMatching.once_per_turn` (Kotis); `game::could_produce` (CR 106.7 — Fellwar/Orchard read only basic TYPES and made {C} from nothing); `Effect::ExileIfLeavesBattlefield` (CR 614 — fixed Geth, Whip of Erebos, Gruesome Encore, Greenwidow too); `AdditionalCastCost::OneOf` (CR 601.2b); `TriggerZone::WhileSuspended` (CR 702.62b); `Player::milled_ids_this_turn` (+ `PutIntoGraveyardFromLibraryThisTurn`, fixes the pod commander N'ghathrod, The Weaver King, Welcome the Dead).
5. 🔎 **Two latent engine bugs the precon cards exposed:** `SharesCreatureTypeWithSource` read only a *departed* source's LKI, so it was false for every living source; `DynamicPt::ChosenPlayerGraveyardMatching` never scaled toughness (a 0/0 Sewer Nemesis died on entry). **A new card is the cheapest probe of an old primitive** — both were found by cards that "only needed wiring".
