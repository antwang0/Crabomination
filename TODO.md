...
2. **Gates at the `d1e10986` tip, all re-run:** `--bench` **195,806 / 27.49 / 611.9 / 0 stalls / peak_rss 25.2** byte-identical to the committed invariant, `determinism ok`, `thread_determinism ok (3 vs 1)`; suite **19,589 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, 12 golden traces unmoved); clippy **0**; `cargo check --profile release-fast` clean. Sweep: **12 cells / 59,200 games / 0 failures** over seeds 1398..1401, `draw 2`, **FRONTIER 1402**. `audit_stubs` 0 of 21,797, `audit_panics` 0 bare.
3. 🔎🔎 **THE DEVICE THAT PAID THIS RUN IS THE SAME ONE AS LAST RUN'S, AIMED ONE LEVEL UP: DIFF THE WALKERS, THEN ASK WHO CALLS THE WEAKER ONE.** `evaluate_requirement_on_card` is documented as the library/hand walker — printed types, `false` for every battlefield-state leaf — and **61 production sites filter `self.battlefield` through it**. The arm diff (a 120-line `re` script over the two `match req` bodies: 165 vs 205 arms, 92 differing, 59 of them a deliberate `false`) named the 33 real ones; the call-site grep named the class. Fixed at the **entry**, not per site: the public entry gates on `battlefield_find` and routes a live permanent to `requirement_on_permanent`. The P3 audit's 14 known drifts all close and its allowlist is now **empty**.
4. ⚠ **AND THE SAME QUESTION IS STILL UNASKED OF THE OTHER TWO WALKERS.** `layers.rs::requirement_matches_card` reads printed types on purpose (it runs *inside* the layer pass — do not "fix" it without reading `layer_reads_are_printed`), but `mod.rs::blocker_matches_block_filter`'s `_ => false` is now machine-checked instead: one dead leaf out of 30 catalog block filters (**Temple Thief's `IsEnchanted`**, INCOMPLETE_CARDS carries the fix shape). ⚠ `false` there is **not** conservative in both directions — unblockable under `ExceptBy`, dead restriction under `By`.
5. ⚠⚠ **A RULES FIX CAN COST 0.6 % OF `cube` AND `--bench` WILL NOT SAY SO.** The gate read fixed +0.093 / cube +0.636 / sealed +0.146 % with `--bench` byte-identical and all 144 golden traces identical on all three pools. `(-343)` took two-thirds of `cube` back (**-0.279 %**) by hoisting the per-walk `PrintedGates` at 13 battlefield walks. 📐 **Price the ROUTING separately from the walk** — two-thirds of that cost was the entry (`battlefield_find` + `PrintedGates` per *ask*), not the evaluator. Closing tip: **+0.096 / +0.355 / +0.150 %**.
6. 📐 **`evaluate_requirement_on_card(f, c, …)` WHERE `c` CAME OUT OF `self.battlefield.iter()` IS A GREP-ABLE ANTI-PATTERN NOW.** The gate answers it right but pays a by-id re-find for a card the caller holds. ~40 such sites remain, all resolution-time `run_effect` arms or combat legality *guards* (which need restructuring to hold a `let`): **40 and 216 asks a `cube` run between them**, so they are churn, not a row. The one hot leftover is a `.filter(..).collect()` at 14,558 asks / +0.03 % — filed as the tail of PERF's candidates.
7. **Perf queue REPLENISHED off a fresh whole-profile read — read PERF's candidates head.** New: **(J) `resolve_combat_into` 3,674 Ir of self a call / 1.57 % — the third-largest body in the program and the only one of the top three never read** (`(-337)`/`(-338)` took the other two); (K) `do_untap` 3,032 Ir a call / 0.53 %; (L) `fingerprint_as` 0.48 %, a *watchdog*, with a behaviour-preserving split (a differing scalar head implies a differing digest and the common path never reads the value) — **census the head's separation rate first**. Still open from before: (A) `ManaCost::cmc`, (B) the `CardCold` `Arc::ptr_eq` gate, (C) `blocker_pair_block`'s pair count, (H) the other zones' by-id scans.
8. **Tip absolutes at `d1e10986` (`profiling-fast --no-default-features`, system allocator): fixed 580,220,836 / cube 1,515,632,261 / sealed 1,634,886,499.** ✅ The twelfth box's closing absolutes **reproduced to six significant figures** (+0.00001 / +0.00007 / -0.00002 %) — same toolchain, same 4-core/15 GB shape, same Xeon @ 2.10 GHz. **"Absolutes do not cross boxes" is about the CPU model, not the box.**
9. ⚠⚠ **THE MIMALLOC TRAP AND THE SUSPENDED-CONTAINER TRAP ARE BOTH STILL LIVE.** `cargo build --profile profiling-fast -p crabomination --bin bot_ladder --no-default-features` — **both flags load-bearing**; check `nm <bin> | grep -cE " (T|t) (_)?mi_"` = 0 before filing a number. And **background waits do not progress between turns**: run builds in the FOREGROUND, and when one overruns the 600 s tool timeout, **re-issue the same `cargo build`** — it blocks on the artifact lock and that block *is* the wait (the `release` build took three re-issues).
10. **The profile is FLAT and the candidates list is no longer thin — say so rather than re-deriving it.** Top self row `dispatch_triggers_for_events_slow` 4.97 %, line profile FLAT; `compute_permanent_pass` 3.57 %; the system-allocator family ~8 % (not what ships). Rank by **self per call**, not by self: that is what named (J), (K) and (L) in one `cg_calls.py` run.
11. 🔎 **Cheap instruments this run added, both reusable:** the walker arm-diff script shape is in item 3 (throwaway, 120 lines, `re` + brace matching — rewrite it, do not hunt for it), and `cgdiff.py` — an exact per-function self-Ir diff of two callgrind dumps, **name-compression aware**, which `cg_calls.py`'s top-N table silently is not (its truncation invented twenty "0 -> N" rows on the first read). If a per-function diff is ever needed again, write that, not a table diff.
12. **Cards & rules, leftover only:** `audit_dropped_may` 199; `audit_keyword_drift`'s last 4 (bestow / escalate / flashback / kicker) each need a PRIMITIVE; card skips `nocolors` 950 / `noback` 60 / `nosubtypes` 43 / `nokeywords` 29; `audit_incomplete` 858 documented approximations; **Temple Thief's `IsEnchanted`** (item 4).
