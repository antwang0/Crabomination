...
fn parse_profile(name: &str) -> Option<Pilot> {
    match name {
        ...
        // Round 65: the greedy attack filter judged against eligible
        // blockers. Gate as A against `dflt` (.ladder/run_r65_attack_guard.sh);
        // `mcts-guard-256` is the lobby's search on the same flag.
        "atk-guard" => Some(Pilot::Scored(EvalWeights::attack_blocker_guard_on())),
        // `stun-hold`: hold an X creature until it enters without stun
        // counters (Slumbering Trudge); `mcts-stunhold-256` is the lobby's
        // search on the same flag.
        "stun-hold" => Some(Pilot::Scored(EvalWeights::stun_x_hold_on())),
        // `xout`: X by outcome (round 76b) — every X finalist re-sized by
        // the pick's own evaluator; `mcts-xout-256` is the lobby's search
        // on the same flag.
        ...
        // Round 68: the attack sim's main-phase casts capped at 0 / 1 / 2;
        // `sim-cast1` is the default since the round adopted it, and
        // `sim-cast-off` is the uncapped control (`.ladder/run_r68_simcast.sh`).
        "sim-cast0" => Some(Pilot::Scored(EvalWeights::sim_main_cast_cap_at(0))),
        "sim-cast1" => Some(Pilot::Scored(EvalWeights::sim_main_cast_cap_at(1))),
        "sim-cast2" => Some(Pilot::Scored(EvalWeights::sim_main_cast_cap_at(2))),
        "sim-cast-off" => Some(Pilot::Scored(EvalWeights::sim_main_cast_off())),
        // Round 70: the attack chain skipped when the menu alone picked
        // greedy. Gate as A against `dflt` for no loss
        // (.ladder/run_r70_chainskip.sh).
        "chain-skipg" => Some(Pilot::Scored(EvalWeights::attack_chain_skip_greedy_on())),
        // Adopted on the default; `chain-skipg-off` is the round-68 default.
        "chain-skipg-off" => Some(Pilot::Scored(EvalWeights::attack_chain_skip_greedy_off())),
        // Round 71's three block-chain arms, each the default plus one flag.
        "bchain-skipg" => Some(Pilot::Scored(EvalWeights::block_chain_skip_greedy_on())),
        "bchain-empty" => Some(Pilot::Scored(EvalWeights::block_chain_empty_only_on())),
        "bchain-seed" => Some(Pilot::Scored(EvalWeights::block_chain_from_menu_on())),
        // 2026-09-06 targeting work: hostile player slots aimed at the
        // opponent (seat flag), the other player as a cast-time arm, both;
        // the own-graveyard pick plus the X=0 no-op prune; everything.
        "hostile-targets" => Some(Pilot::Scored(EvalWeights::hostile_player_targets_on())),
        "player-arms" => Some(Pilot::Scored(EvalWeights::player_target_arms_on())),
        "target-fixes" => Some(Pilot::Scored(EvalWeights::target_fixes_on())),
        "x0-skip" => Some(Pilot::Scored(EvalWeights::skip_noop_x0_on())),
        ...
        // Round 68: the net's leaf on the current default, scored and searched.
        "net67" => Some(Pilot::Scored(EvalWeights::net_on_default())),
        "mcts-net67-256" => Some(Pilot::Mcts(MctsConfig {
            iterations: 256,
            horizon_turns: 3,
            weights: EvalWeights::net_on_default(),
            ..MctsConfig::default()
        })),
        // Round 79: the search's depth curve on the NET leaf (the lobby's
        // and the client's search) — 512 vs the adopted 256.
        "mcts-net67-512" => Some(Pilot::Mcts(MctsConfig {
            iterations: 512,
            horizon_turns: 3,
            weights: EvalWeights::net_on_default(),
            ..MctsConfig::default()
        ...
        // Round 73: the same head on the SETTLED successor.
        "net67-pols" => Some(Pilot::Scored(EvalWeights::net_on_default_policy_settled())),
        ...
        // The lobby's exact pilot (`lobby.rs::default_bot`): `mcts-net-deep`
        // on `client_pilot()` — det1 + the saturation fallback (r54) + the
        // attack chain (r55) + the block chain (r56). Round 62 reads it
        // against `dflt`, the heuristic it has to add something on top of.
        "mcts-client" => Some(Pilot::Mcts(MctsConfig {
            iterations: 64,
            horizon_turns: 3,
            weights: EvalWeights::client_pilot(),
            ..MctsConfig::default()
        })),
        // The same search on the adopted default's weights with NO net: the
        // material eval is the rollout leaf. Round 62's control for
        ...
        "mcts-guard-256" => Some(Pilot::Mcts(MctsConfig {
            iterations: 256,
            horizon_turns: 3,
            weights: EvalWeights::attack_blocker_guard_on(),
            ..MctsConfig::default()
        })),
        "mcts-stunhold-256" => Some(Pilot::Mcts(MctsConfig {
            iterations: 256,
            horizon_turns: 3,
            weights: EvalWeights::stun_x_hold_on(),
            ..MctsConfig::default()
        })),
        "mcts-xout-256" => Some(Pilot::Mcts(MctsConfig {
            iterations: 256,
            horizon_turns: 3,
        ...
        "mcts-xoutoff-256" => Some(Pilot::Mcts(MctsConfig {
            iterations: 256,
            horizon_turns: 3,
            weights: EvalWeights::x_by_outcome_off(),
            ..MctsConfig::default()
        })),
        "mcts-gypick-256" => Some(Pilot::Mcts(MctsConfig {
            iterations: 256,
            horizon_turns: 3,
