...
fn venture_ctx(source: crabomination::card::CardId) -> crabomination::game::effects::EffectContext {
...
fn venture(g: &mut GameState, ctx: &crabomination::game::effects::EffectContext) {
...
fn tomb_of_annihilation_mints_the_atropal() {
    ...
    assert!(atropal.definition.keywords.contains(&crabomination::card::Keyword::Deathtouch));
}
...
fn cr_509_4_flash_foliage_blocks_the_attacker() {
    ...
    assert!(g.perform_action(GameAction::CastSpell {
        card_id: foliage, target: Some(crabomination::game::types::Target::Permanent(attacker)),
...
fn precipitous_drop_scales_with_completion() {
    ...
    g.players[0].mana_pool.add_colorless(2);
    g.perform_action(GameAction::CastSpell {
        card_id: drop, target: Some(crabomination::game::types::Target::Permanent(big)),
        additional_targets: vec![], mode: None, x_value: None,
...
fn bar_the_gate_counters_creature_spells() {
    ...
    g.priority.player_with_priority = 1;
    g.perform_action(GameAction::CastSpell {
        card_id: gate, target: Some(crabomination::game::types::Target::Permanent(bear)),
        additional_targets: vec![], mode: None, x_value: None,
...
fn fates_reversal_returns_and_ventures() {
    ...
    g.perform_action(GameAction::CastSpell {
        card_id: spell, target: Some(crabomination::game::types::Target::Permanent(dead)),
        additional_targets: vec![], mode: None, x_value: None,
    }).expect("cast");
    drain_stack(&mut g);
...
fn shessra_pays_two_life_to_draw_after_a_death() {
    ...
    g.resolve_effect(&crabomination::effect::Effect::Destroy {
...
fn ellywick_ventures_and_emblem_anthems() {
    ...
    g.battlefield_find_mut(elly).unwrap().add_counters(crabomination::card::CounterType::Loyalty, 8);
...
fn manticore_finishes_a_damaged_creature() {
    ...
    g.resolve_effect(&crabomination::effect::Effect::DealDamage {
        to: crabomination::effect::Selector::EachPermanent(
            crabomination::card::SelectionRequirement::HasCreatureType(crabomination::card::CreatureType::Wurm),
        ),
