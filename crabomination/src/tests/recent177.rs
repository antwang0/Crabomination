...
fn exemplar_of_light_counters_then_draws() {
    ...
    drain_stack(&mut g);
    let counters = *g.battlefield_find(ex).unwrap().counters.get(&crate::card::CounterType::PlusOnePlusOne).unwrap_or(&0);
...
fn bumbleflowers_sharepot_food_and_destroy() {
    ...
    drain_stack(&mut g);
    assert!(
        g.battlefield.iter().any(|c| c.definition.subtypes.artifact_subtypes.contains(&crate::card::ArtifactSubtype::Food)),
        "made a Food token"
    );
