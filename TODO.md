...
1. ⚠⚠ **TWO SESSIONS SHARE THIS BRANCH. FETCH BEFORE YOU START AND BEFORE EVERY PUSH.** Rebased five times in one session and twice in the other; every conflict was in a file both had appended to and every resolution was "keep both blocks" — **prefer a shape that APPENDS**, and grep for the house spelling before inventing an alias (`type Factory`). ⚠ **`pkill -f <pattern>` KILLS YOUR OWN SHELL** when the pattern is in the command line running it; kill by pid. ⚠ **DO NOT EDIT THE TREE WHILE AN OPTIMIZED BUILD RUNS** — cargo stamps the output newer than your edit and hands back a stale binary that looks fresh. ⚠ **DISK: `target/debug/incremental` regrows to 15-19 GB per suite run and a `release` build then dies as a bogus *compile error*** — `rm -rf` it BEFORE any optimized build. Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure"**.
2. **Gates at the closing tip:** suite **20,189 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, `--workspace --exclude crabomination_client`); clippy **0** over the same set; `--bench` (release-fast, `CRAB_THREAD_CHECK=1`) **195,806 / 27.49 / 611.9 / 0 stalls — byte-identical to the committed invariant**, `determinism ok (all pairs split)`, `thread_determinism ok (3 vs 1)`, `peak_rss_mib` 29.7; `audit_panics` **0 bare**; CR_COVERAGE **146/146**; every ratchet gate clean (`as_enters`, `enters_tapped`, `singular_fanout`, `player_static_target` 1-allowlisted, `doc_card_names` gate 0, **`another_trigger` 0 / 20-allowlisted**, and the two new ones below). **Pod smoke at TWO fresh seeds: 9112 × 500 × 2..9 seats = 4,000 and 9113 × 2,000 × 2..9 = 16,000 — every block 100 % decided, every `undecided_by` column zero, zero panics.** turns/game at 9113: 19.61 / 32.62 / 45.15 / 57.05 / 64.23 / 76.89 / 93.58 / **99.02**. **FRONTIER 9114 / 6003 — **re-taken at `80f0eeb7` on a `release` build and a fresh pod seed 9115: suite 20,175 / 0 / 6, clippy 0, `--bench` 195,806 / 27.49 / 611.9 / 0 stalls still byte-identical, 4,900 pod games over 2..8 seats 100 % decided with every `undecided_by` column zero. FRONTIER 9116**.** ⚠⚠ **QUOTE THE EXCLUSION WITH THE NUMBER**: `crabomination_client` holds exactly **200** `#[test]`s and is the only member the documented command excludes, so the same tree reads 200 higher on a box that can build Bevy — that gap read as a 200-test regression and cost a round trip of "corrections" in both directions. ⚠ Box: **2.10 GHz** Xeon, 4 cores; release-fast engine **8m16s**, `release` did not finish in 25 min — use release-fast for a gate.
3. ✅✅ **THE 614.12 CLASS IS CLOSED: 89 → 6**, and the six left each need more than a field move (a copy, a coin flip, a random pick, a reflexive attach, two documented approximations). ENGINE_BACKLOG's forty-eighth find. 📐📐 **THE WIRING WAS THE GAP, NOT THE PRIMITIVE:** `as_enters_effect` had shipped for forty cards; what had not was running it on every entry path. `game::as_enters::apply_as_enters_replacements` is the one funnel all four hops call now (cast / move / **land drop** / **token mint**). **A class audit has to ask "what shapes does the engine already have?" as well as "what shapes do the cards ship?"**
4. ⚠⚠ **THE LAND DROP IS THE ONE ENTRY PATH THAT CANNOT REPLAY.** Every other suspending action parks *before* anything is paid; here the land is on the battlefield and the drop is spent. `ResumeContext::LandEntry` is a **continuation** (`is_action_replay` = false): apply the answer, resume the parked effect, run `actions::finish_land_entry` exactly once. ⚠ And inside an entry, `Effect::ChooseMode` reads `ctx.mode` — the pick made when the object went on the stack — so it silently takes mode 0 and never asks; **`Effect::AsEntersChooseMode` is the one that works without a stack item.**
5. 🔎🔎 **FIND: THE FILTER LANGUAGE HAS NO IMPLICIT ZONE.** `legal_targets_for_filter` applies one `SelectionRequirement` to every zone an effect can reach, so a printed "target creature card **from your graveyard**" written as `target_filtered(R::Creature)` is satisfied by a creature on the **battlefield** — and the picker walks the battlefield first. 27 cards; `scripts/audit_target_zone.py` is the ratchet, ENGINE_BACKLOG's **fifty-second**. **Mortuary Mire, in the Judith pod deck, tucked live creatures; Cremate exiled a permanent for {B}.**
6. 📐📐 **AND HALF OF IT WAS INVISIBLE IN SELF-PLAY, WHICH IS WHAT MADE IT A CLASS.** `Effect::prefers_graveyard_target` sends the picker to a graveyard for a `Move` to your hand, your battlefield or exile — those cards did the right thing *for the wrong reason* and misfired only over an empty graveyard. Every `Move` to a **library**, and every bespoke effect, has no such classifier and misfired always. **When a heuristic stands in front of a defect, the census is the only thing that sees the half the heuristic covers.**
7. 🔎🔎 **FIND: THE PRINTED WORD "ANOTHER" BETWEEN TWO TARGET SLOTS HAD NO WAY TO BE SAID.** CR 601.2c lets two instances of "target" name one object; "another" forbids it, and the language could say that only about the *source*. 41 cards, `scripts/audit_another_target.py`, ENGINE_BACKLOG's **fifty-third**. **Fiend Hunter and Hostage Taker exiled *themselves*, which is leaving the battlefield, which returns the card, which triggers the exile again.** `SelectionRequirement::OtherThanTargetSlot(u8)` is the new atom, read out of `target_slots_scratch` like `SameControllerAsTargetSlot`.
8. ⚠⚠ **A NEW CROSS-SLOT ATOM NEEDS THREE WIRINGS AND THE THIRD IS THE ONE NOBODY LOOKS AT.** ① The **activation** path never stamped `target_slots_scratch` (only the cast path did). ② The **auto-picker** only *preferred* an unpicked permanent and its mandatory-slot fallback deliberately reused one, so it built sets the validator then rejected — a bot action that fails rather than a target it avoids. ③ `cross_slot_targets_ok` had no `Or` arm, so an atom under one was dropped silently. **Grep every walker that answers the same question before calling a primitive done.**
9. 📐 **THE NARROWING IS THE SAFE DIRECTION AND FOUR CARDS KEEP IT.** Atzocan Archer, Nessian Wilds Ravager, Domri's −2 and Ulvenwald Tracker print "another target creature" and are modelled "a creature you don't control". The faithful filter was tried: a `Fight` defender slot is hostile, so the picker ranks an own creature **last** — and last is still *picked* on a mandatory slot with nothing else legal. **A narrowing cannot make an illegal play; widening makes a legal bad one.**
10. ✅ **The ninth target deck is Yuriko, the Tiger's Shadow (UB) — the seat that pilots commander ninjutsu (CR 702.49d)**, the one route out of the command zone that is **not a cast**, so CR 903.8's tax never applies. `--seats 9` reaches it; `bot_ladder`'s clamp is `target_decks().len()` now. ⚠⚠ **The number is a finding about the clause: 30.0 % of nine-seat pods (2,000 games) against a seat's 11.1 %, second only to Edgar's 37.1 %, for the same structural reason — a payoff that reads the WHOLE TABLE grows with the table** where every other seat's clock is per-opponent. Zellix at 0.7 % is the other half of it. DECK_FEATURES carries both.
11. 🔎 **THE OTHER SESSION'S FIND, STILL THE BEST MULTIPLAYER ONE: a fan-out `PlayerRef` handed to an arm that resolves it SINGULARLY.** `resolve_player` answers with the **first seat of the set** and drops the rest — exact in a duel, wrong in a pod. **19 → 2**, `scripts/audit_singular_fanout.py`, the fifty-first find. 💡 The two survivors are the next work there, and `default_hostile_opponent` needs a *least*-hostile sibling for gift clauses (bot policy, not rules).
12. 📐📐 **THE AUDIT READER IS THE SHARED BUG SURFACE AND BOTH SESSIONS HIT IT.** Keying a site on the first string literal mis-files a factory ending in `..creature("Name", …)`; slicing to the next `pub fn` puts the next card's doc inside this one and hides any card behind a private helper — `_factory_body`, brace-matched, fixes both. This run added three more, all of which only *remove* false positives: same-file **helpers** are part of a body (the five Primordials), so is `..other_factory()` delegation (Junk Diver / Myr Retriever), and a Leyline's "you may begin the game with it on the battlefield" is CR 103.6, not a resolution choice. `audit_dropped_may` 198 → **178**. 💡 **The other ten readers are separate copies** — unifying them moves every ratchet at once.
13. ✅ **COMMANDER_BACKLOG is re-generated and the top-1000 slice moved 918 → 953** (top-250 **250/250**, top-500 497/500; **47 rows left in section 2**). Fourteen of them landed this run in `sets::decks::recent330`, two of them multiplayer-native — **Tempt with Discovery** (a real CR 207.2c tempting offer) and **Cut a Deal** (one card per opponent, so its test is a four-seat game). 💡 **The other censused queue: 23 of the 178 dropped-"may" rows are in the CUBE pool**, the 2-player simulator's own deck space, where a dropped choice is a live defect in the trained path — Restoration Angel, Liliana's −2, Grapple with the Past, Grisly Salvage, Malevolent Rumble, Beastmaster Ascension, Chrome Mox / Isochron Scepter, Jace the Mind Sculptor. ⚠ Deep-Cavern Bat and Elite Spellbinder may be `RESOLVER_OPTIONAL` exemptions instead — read their arms before wrapping. 🔎🔎 **And the fan-out family now has a FIFTH ratchet, whose four passes are worth more than its five cards: `audit_another_trigger.py`** — "whenever **another** creature you control dies" is an `EventSpec` scope, invisible to `audit_another_target`, and a wrong one fires the permanent off **itself**. It read 221 → 99 → 43 → 28 → 5 and **every cut was a false-positive class**: "X *or* another Y" widens rather than excludes; the needle was in the trigger's *effect*; a card's helper is part of its body and for scopes lives in **another file** (`on_other_dies` in `shortcut.rs`); and **four** encodings say "not me". ⚠ **A new ratchet's first number measures the ratchet, not the catalog** — and only 2 of the 5 fixes were observable, the rest fixed because "nothing can see it" is a property of today's effect. ENGINE_BACKLOG has the entry.
14. 💡 **The four exert cards are the one bucket that needs a rules feature, and it is CR 508.1g:** "you may exert this creature as it attacks" is an **optional cost to attack**, chosen during declaration and locked in, whose bonus is a *linked* "when you do" trigger (CR 701.43d / 607.2h). The engine auto-exerts, so Glorybringer skips its untap even when the bonus has no legal target. ⚠ The comments cited **702.137 / 702.83** and both are wrong — it is **701.43**. `Attack` is `Copy` and built at ~1,400 sites, so the shape is a new `GameAction` beside `DeclareAttackersBanded`, not a new field. Also unblocks Enlist (702.154b, same 508.1g). **Other standing leads:** `audit_incomplete` 858; COMMANDER_BACKLOG is **generated and stale** (network works — Scryfall answered 200); 32 of 33 "can be your commander" non-creatures; a tenth deck wants an identity the field has not run.
15. ⚠ **Gotchas that each cost a round.** ① `Effect::ChooseMode` never asks outside a stack item. ② `add_card_to_battlefield` skips every entry replacement — use `add_card_to_battlefield_entering`, and script the decider **first**. ③ A new `Effect` variant needs an arm in **`effect/query.rs`'s exhaustive match**; a new `SelectionRequirement` needs one in `evaluate_requirement_static_hinted` **and** `requirement_on_permanent`, because three of the four walkers end in a permissive `_ =>` and it compiles either way. ④ `target_filtered(f)` is **slot 0** — two of them in one effect is one target (Markov Retribution dealt its damage to itself for exactly that). ⑤ `Target` is not `Copy`. ⑥ **`add_card_to_battlefield` + `fire_self_etb_triggers` fires the *entering* card's own triggers and nothing else**, so a "whenever a creature you control enters" **watcher** sees nothing — cast the creature; and a death watcher needs `dispatch_triggers_for_events(&check_state_based_actions())`, because the SBA sweep returns its events rather than dispatching them. This cost three red tests. ⑥ `CR_COVERAGE.md` is **generated** — take either side of a conflict and re-run `scripts/cr_coverage.py`. ⚠ **The rules table is still DONE and that is still the main claim**: FEATURE_ROADMAP's Commander and 800-series sections are all ✅ bar CR 800.4i (unreachable) and an N-seat encoder (⛔ by design). What is left is **cards**.
