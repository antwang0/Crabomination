...
2. **Gates at the `1c7f0414` tip, all re-run:** `--bench` **195,806 / 27.49 / 611.9 / 0 stalls / peak_rss 25.0** byte-identical to the committed invariant, `determinism ok`, `thread_determinism ok (3 vs 1)`; suite **19,589 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, 12 golden traces unmoved); clippy **0**; `cargo check --profile release-fast` clean. Sweep: **18 cells / 88,800 games / 0 failures** over seeds 1398..1403, `stuck 0 / draw 16 / cap 2` (both `cube` MAX_BATTLEFIELD runaways on a saturated seat — the script's documented carve-out), **FRONTIER 1404**. `audit_stubs` 0 of 21,797, `audit_panics` 0 bare, `audit_doc_drift` 0/0/0.
3. 🔎🔎 **THE DEVICE THAT PAID THIS RUN IS LAST RUN'S AIMED ONE LEVEL UP: DIFF THE WALKERS, THEN GREP FOR WHO CALLS THE WEAKER ONE.** `evaluate_requirement_on_card` is documented as the library/hand walker — printed types, `false` for every battlefield-state leaf — and **61 of its 206 production call sites filter `self.battlefield` through it**. So "destroy each tapped creature" found nothing and an animated land survived a `Creature` sweep. The arm diff (165 vs 205 arms, 92 differing, 59 a deliberate `false`) named the 33 real divergences; **the caller grep named the class**, and the fix is at the entry, not per site. ENGINE_BACKLOG's thirty-fifth find has the recipe.
4. ⚠⚠ **AN ALLOWLIST DOCUMENTS A CONTRACT, NEVER THE CALLERS — and that sentence cost three passes.** The P3 audit's fourteen known drifts were read as deliberate twice and as "six call sites" once; all fourteen were live on the board and the six were six of sixty-one. The allowlist is **empty** now and the test guards the gate. **Go re-read the other allowlists in this repo with that question.**
5. ⚠ **THE THIRD WALKER IS MACHINE-CHECKED NOW, NOT FIXED.** `mod.rs::blocker_matches_block_filter`'s `_ => false` is **not** conservative in both directions — unblockable under `CantBeBlockedExceptBy`, dead restriction under `CantBeBlockedBy`. New audit over all 30 catalog block filters: **one dead leaf, Temple Thief's `IsEnchanted`**, with the fix shape and its price (~0.11 % of `cube` unless threaded once per blocker) in INCOMPLETE_CARDS. ⚠ `layers.rs::requirement_matches_card` reads printed types **on purpose** — it runs inside the layer pass; do not "fix" it without reading `layer_reads_are_printed`.
6. ⚠⚠ **A RULES FIX CAN COST 0.6 % OF `cube` AND `--bench` WILL NOT SAY SO.** The gate read cube **+0.636 %** with `--bench` and all 144 golden traces byte-identical. 📐 **Price the ROUTING separately from the walk:** two-thirds of that was the entry (`battlefield_find` + `PrintedGates` per *ask*), which `(-343)` took back (**cube -0.279 %**) with 13 mechanical hoists. Run total: **fixed +0.024 / cube +0.115 / sealed +0.078 %** for a 61-site class fix.
7. 📐 **`evaluate_requirement_on_card(f, c, …)` WHERE `c` CAME OUT OF `self.battlefield.iter()` IS A GREP-ABLE ANTI-PATTERN.** ~40 sites remain, all resolution-time `run_effect` arms or combat legality *guards* (which need restructuring to hold a `let`): **40 and 216 asks a `cube` run between them**, churn not a row. One hot leftover: a `.filter(..).collect()` at 14,558 asks / +0.03 %, filed at the tail of PERF's candidates.
8. ✅ **`(-344)` — the CR 104.4b loop watch settles off the digest's scalar head: fixed -0.072 / cube -0.240 / sealed -0.071 %.** 📐 **The census is the entry's point and it has TWO numbers**: how often the cheap branch is *reachable* (54.4 % of `cube` samples) and how often the cheap key *separates* on it (97.5 %). A census that measures only the first is `(-329)`'s mistake. The shortcut is **audited, not argued** — a `debug_assert_ne!` recomputes the skipped digest, so the suite and every sweep cell are the ratchet; 59,200 games, no fire.
9. ⚠ **`cow::tests::game_state_stays_small` IS 1,680 NOW (was 1,672) AND THE RAISE IS THE PRECEDENT, NOT THE NUMBER.** `(-344)`'s 8-byte field costs 529 k Ir a `cube` run (`GameState::clone` +380 k, `memcpy` +148 k) and the A/B that asked for it already carries that cost. **Raise it again only with a measurement of that shape.**
10. **Perf queue after this run: (J) and (K) CLOSED as diffuse, (L) taken.** `resolve_combat_into` (1.57 %, 276 lines, top line 0.09 %) and `do_untap` (0.53 %, 89 lines, top 0.04 %) are both "the body IS the cost", `(-319)`'s verdict. ⚠ **Both reads cost NOTHING and saved two builds:** `cg_lines.py` needs no `profiling-lines` build — one 16-second `--dump-instr=yes` callgrind line-profiles every body in the program at once. **Line-profile a candidate before building for it.** Still open: (A) `ManaCost::cmc`, (B) the `CardCold` `Arc::ptr_eq` gate, (C) `blocker_pair_block`'s pair count, (H) the other zones' by-id scans.
11. **THE PROFILE IS FLAT AND THE GLOBAL LINE TABLE NOW SAYS SO IN PERF'S CANDIDATES HEAD — do not re-derive it.** Largest single source line in the program **0.97 %** (`Arc::clone_from_ref_in`'s memcpy), then 0.80 / 0.56 / 0.41 %, and **12,537 lines hold 86.9 %**. Rank by **self per call**, not by self — that is what named (J), (K) and (L) in one `cg_calls.py` run, and two of the three were still flat.
12. **Tip absolutes at `1c7f0414` (`profiling-fast --no-default-features`, system allocator): fixed 579,804,110 / cube 1,511,999,354 / sealed 1,633,720,908.** ✅ The twelfth box's closing absolutes **reproduced to six significant figures** (+0.00001 / +0.00007 / -0.00002 %). **"Absolutes do not cross boxes" is about the CPU MODEL, not the box** — same shape and toolchain agree to noise.
13. ⚠⚠ **THE MIMALLOC TRAP AND THE SUSPENDED-CONTAINER TRAP ARE BOTH STILL LIVE.** `cargo build --profile profiling-fast -p crabomination --bin bot_ladder --no-default-features` — **both flags load-bearing**; `nm <bin> | grep -cE " (T|t) (_)?mi_"` = 0 before filing a number. **Background waits do not progress between turns**: run builds in the FOREGROUND and **re-issue the same `cargo build`** when one overruns the 600 s tool timeout — it blocks on the artifact lock and that block *is* the wait (the `release` build took three). ⚠ **Never overlap two sweep blocks on four cores: 3m55s becomes 29m13s.**
14. 🔎 **`cgdiff.py` (in the scratchpad, rewrite it — 40 lines) is an exact per-function self-Ir diff of two callgrind dumps, NAME-COMPRESSION AWARE.** `cg_calls.py`'s top-N table is not a diffable table: its truncation invented twenty "0 -> N" rows on the first read of `(-343)`, which read as game divergence and was not.
15. **Cards & rules, leftover only:** `audit_dropped_may` 199; `audit_keyword_drift`'s last 4 (bestow / escalate / flashback / kicker) each need a PRIMITIVE; card skips `nocolors` 950 / `noback` 60 / `nosubtypes` 43 / `nokeywords` 29; `audit_incomplete` 858 documented approximations; **Temple Thief's `IsEnchanted`** (item 5).
