...
4. **Official precons are seats 11-59, card for card** (per-seat notes and residuals: DECK_FEATURES; open residual rows: INCOMPLETE_CARDS' Commander routine table). **This run: Entropic Uprising (C16, Yidris) LANDED as seat 51, Endless Punishment (DSC, Valgavoth) LANDED as seat 59** (also Fae Dominion 45 and Invent Superiority 48 earlier).
5. **⚠ OPEN CLAIMS — don't duplicate:** Open Hostility (C16, Saskia, 16; `01XnuL2a`), Breed Lethality (C16, Atraxa, 17; `01546QdT`), Mirror Mastery (CMD, Riku, 16; `011A87o2`), Token Triumph (SCD, Emmara, 15; `01P5KQkY`). **Arcane Wizardry (C17, Inalla, Archmage Ritualist, 16; `01StaUuo`, 2026-09-24 run 5).** Claim in this line, pushed on its own, before starting a list.
