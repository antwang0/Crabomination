...
1. ⚠⚠ **TWO SESSIONS SHARE THIS BRANCH. FETCH BEFORE YOU START AND BEFORE EVERY PUSH, AND RE-RUN THE SUITE AFTER EVERY REBASE.** `cargo nextest` fail-fast hides the rest — use `--no-fail-fast`. Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure"**. ⚠ **An engine-only `release` rebuild is 12m08s; one that touches `crabomination_base` is ~55 min**, because the catalog sits between them. So batch a run's base-crate edits into ONE commit and take the bench once — and if the remaining work is engine-only, a re-bench is cheap enough to take per commit.
2. **Gates at the closing tip (engine `feb5714d`; the commits above it are markdown and scripts):** suite **19,763 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); clippy **0** over the workspace; `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean; `--bench` (release) **195,806 / 27.49 / 611.9 / 0 stalls** — **byte-identical to the committed invariant at all THREE of this session's tips**, `determinism ok`, `thread_determinism ok (3 vs 1)`, `peak_rss_mib` 25.7 / 25.4 / 25.4. Pod smoke: **15,200 games over 2/3/4/5 seats at fresh seeds 3101-3112, 100 % decided, 0 stalls, every `undecided_by` column zero**, no panic or error line in any run. **FRONTIER 3113.** ⚠ No Ir A/B taken and none is owed: every arm gains one `Option::is_some` on the non-suspending pass (`splice_after_suspend` and `rewrap_parked` both return on `None` before building anything), and the aggregate did not move. PERF's Baseline has the block, including the build economics that cost this session most of its wall clock.
3. **Where it stands:** FEATURE_ROADMAP's **"Commander status"** is the map and it is current — CR 903 core, the whole 800-series, the variant list, and now a **CR 101.4** row. **Read that table, don't re-audit.** DECK_FEATURES: 5/5 decks complete. What 800.4 still approximates, on purpose: an ask **already materialized** when its seat leaves is dropped, not re-seated; 800.4i's last-known-information clause is unmodelled (and near-unreachable — `resolve_players` filters dead seats out of every fan-out, so only an effect naming a *specific* departed seat could see it).
4. **The runner:** `crabomination::pod` + `bot_ladder --commander [--seats N] [--games N] [--seed N] [--threads N]`. `pod::tests` validates every list every run and gates the chunk split. ⚠ The Partner deck is **last** on purpose: `--seats 5` is what reaches it.
5. 🔎🔎 **THE FIND, AND IT IS A CLASS: A LOOP WHOSE BODY SUSPENDS PARKS ONLY THE BODY.** `suspend_signal` holds one continuation; the loop around a suspending body used to run on, overwrite it, and abandon everything it had not reached. Invisible in a duel (one iteration), **three quarters of a four-seat pod**. Twenty arms taken this run on one helper, `effects::splice_after_suspend`. ENGINE_BACKLOG's **forty-third find** has the whole thing; `scripts/audit_loop_splice.py` is the ratchet (18 sites / 18 allowlisted / **0 unexplained**, and deleting one call takes it to 1).
6. 📐📐 **THE TWO RULES THAT FIX ANY MEMBER OF IT.** ① **A parked continuation is resumed under the STACK ITEM's `EffectContext`**, never the sub-context the loop built — so a tail must name its seats (`PlayerRef::Seat(q)`) and its target slots (`Effect::BindTargetSlot`) *inside the effect*. ② **A wrapper that rebinds context must put its binding back around whatever the body parked** (`rewrap_parked`) — without it `ApplyToTargets` over two targets destroyed the first one twice, because the inner `Seq`'s own tail came back with `Target(0)` meaning the spell's first target.
7. ⚠⚠ **AND A CHECK THAT STRADDLES A SUSPEND IS A SECOND DEFECT THE SPLICE DOES NOT REACH.** Strongarm Tactics read the graveyard for "did they discard a creature" *before* the suspended discard had moved the card, so every `wants_ui` seat was punished whatever it pitched; Tariff's `MayPay` came back under the stack ctx where `SacrificeSource` no longer named that seat's creature. Fixes: make the per-seat unit one `Seq` (it carries its own tail) with a threshold taken *before*, or split the arm into an ask pass and a choiceless apply pass.
8. ✅ **THE `CardId` PRIMITIVE WAS THE BEST NEXT ONE AND IT IS TAKEN.** `Selector::ExactObjects(Vec<CardId>)` names entities a loop has already resolved; `Effect::BindTargetObjects { ids, body }` puts objects an arm picked itself back into `ctx.targets`. They closed `ForEach` over permanents, `TurnFaceUpFree`'s `if_cant` and `EyeOfTheStorm` — **nine open loops became six**. 💡 **What the remaining six want is one thing: a way to carry per-iteration state that lives on `self`** (`current_voter` for `Vote`'s `PerVote`, `last_die_roll` for `RollDie`) — i.e. a `BindScratch`-shaped wrapper, and that is the next primitive. The other three (`MayPayRepeatedly`, `SearchExileLinked`, `SacrificeAnyNumber`) each want their arm restructured, not a primitive.
9. ✅✅ **DECK BALANCE IS ANSWERED, AND THE ANSWER IS A METHOD, NOT A LIST.** Judith 6.2 → **14.6 %** of four-seat pods. ⚠⚠ The obvious repair made it WORSE: a real aristocrats engine read **6.3 %** at four seats, *below* the filler list it replaced. `pick_sacrifice_value` takes a sacrifice only when `eval_material` improves, and a one-ply material evaluator cannot price a value engine. **The evaluator already sums over every live hostile seat, so this is a horizon limit, not a seat-count bug**: pod lists must be built out of what a greedy evaluator can price — removal, card advantage, fat, per-opponent effects that resolve in one shot.
10. **Cards and decks, what is left:** ⏳ 32 of 33 "can be your commander" non-creatures unimplemented — *cards are chosen to complete decks*, so they wait on a sixth target deck wanting one. ⏳ `audit_each_opponent`'s residuals are **six**, and **none is in a target deck** (re-checked this run against `pod/decks.rs`). ✅ CARD_BACKLOG's "Target-deck card defects" table is EMPTY.
11. ⚠⚠ **POOL MEMBERSHIP IS A MEASUREMENT, NOT A VETO.** A cube/SOS card's behavior change *can* move the committed bench aggregate and a golden trace, so the rule is "change it, then run `--bench`". 📐 And the cheap pre-check that made this run's modal commit safe to predict: **read the trace decks and `archetypes()` first** — neither holds a modal spell, so `CR 700.2`'s change could not move either, and it did not.
12. ⚠⚠ **WHEN THE COMMITTED TABLE AND THE AGGREGATE DISAGREE, THE AGGREGATE WINS.** `cr_903_seeded_pod_outcomes_match_the_committed_table` is a **re-bless gate, not a measurement**: three games said the ranked auto-targeter lengthened pods and 32,000 said every configuration got **shorter**. Read **turns**/game, and read the **two-seat row as the control**.
13. 🔎 **RUN THE ASSERTIONS BUILD OVER PODS** — `RUSTFLAGS="-C debug-assertions=yes" CARGO_TARGET_DIR=target-audit cargo build --profile overflow …` then `--commander --seats N --games 1000`. ⚠⚠ **`rc=$?` AFTER A PIPE READS THE PIPE'S STATUS** — an abort (134) reads as 0 and a whole sweep looks clean. Redirect to a file, then grep.
14. 📐 **VERIFY EVERY CR NUMBER AGAINST `MagicCompRules_20260417.txt` BEFORE COMMITTING IT.** Three were wrong this run and two of them were in prose nobody would have re-checked: "tempting offer" is an **ability word** (CR 207.2c) with no rules entry — 701.60 is *Suspect*; "can't pay a cost you can't pay" is CR **118.3**, not 118.6 (unpayable costs); modal *resolution* is CR 700.2, not 601.2b (announcement). ⚠ And CR 702.120a pays escalate **as the spell is cast** — the engine asks at resolution, a standing approximation now noted in the arm.
15. ⚠ **Five gotchas that each cost a debugging round.** ① A `ScriptedDecider` answers in ASK ORDER — to answer one question and leave the rest, write a four-line `Decider` matching that `Decision` variant and deferring to `AutoDecider`, and note `AutoDecider` **declines every `OptionalTrigger`**. ② A "target player" slot **auto-picks an OPPONENT**. ③ `Selector::TriggerSource` binds the **damaged** player at resolution while a trigger's `filter` is evaluated against the **dealer** at gather time; not a bug class. ④ New Commander cards go in `catalog/src/sets/cmdr.rs`, and pool membership is an **explicit curated list** in `cube.rs`. ⑤ `add_card_to_battlefield` does **not** run `as_enters_effect`; cast it. ⑥ `resolve_answering` in `core_rules::multiplayer` is the test harness for any suspend/resume assertion — `drain_stack` passes priority, which a pending decision blocks.
