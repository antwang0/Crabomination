...
1. **FIRST:** `git fetch origin claude/modern_decks && git checkout -B claude/modern_decks origin/claude/modern_decks`. Rebase, never force; code before tracker prose; **fetch before every push AND before you start**. ⚠ A concurrent session landed three commits mid-run and one of them filed the same `(-353)` Log entry this run had already written — **when the rebase conflicts on a tracker file, check whether the other side did the work better and drop yours.** Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure" + "Standing rules"**, the 2026-09-12/13 detail in **ENGINE_BACKLOG's "moved verbatim"** block. NEXT is 15 lines and stays 15.
2. **Gates at the `e0ef3b92` tip:** suite **19,605 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); clippy **0** over the workspace (Round 77's `shortlist_tick` needed the tree's standing `#[allow(clippy::too_many_arguments)]`); `cargo check --profile release-fast --all-targets` clean; sweep **3 blocks / 18 cells / 88,800 games / 0 failures** at fresh seeds 1422..1427, `cap 0 / stuck 0 / draw 2` (a draw is a legal rules outcome) — **FRONTIER 1428**; `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant at all three tips measured, `determinism ok`, `thread_determinism ok (3 vs 1)`, `peak_rss_mib` 25.4. ⚠⚠ **A concurrent ML session landed Round 77/78 (the default's shortlist, `server/bot.rs`) after this run's last measurement, so EVERY ABSOLUTE IN PERF'S BASELINE IS STALE AT THE TIP — RE-TAKE THE BASE, DO NOT PREDICT IT.** A/B rows are unaffected. 📐 The `--bench` invariant survived it for the recorded reason: **it is a `gang` mirror and does not cover the `dflt`/search pilots.**
3. 🔎🔎 **THE FIND, AND IT IS A METHOD: WHEN A TABLE HAS BEEN "DIFFUSE" FOR ELEVEN PASSES, CHANGE THE AXIS YOU RANK IT ON.** Candidate (O) — the allocation table, ~756 k allocations — had been censused by **caller** (`cg_growth.py`: "~10,300 takeable re-growths, laborious") and left for eleven passes. Ranking the same dump by **source line** (`cg_alloc_sites.py` over the whole program instead of one function, which nobody had run) produced **four rows in one session**, and three of them are allocations that should not have existed rather than growths a `reserve` could move.
4. 📐📐 **AN ALLOCATION COSTS 222 Ir ALL-IN, AND THAT IS MEASURED BY DELETION, NOT SUMMED FROM SYMBOLS.** `(-355)` removed exactly 11,380 and saved 2,531,745 Ir; `(-357)` reads 202 on 5,596. So the program's allocations are **~11.2 % of `cube`**, above `(-90)`'s 9.9 % — a symbol sum omits `malloc_consolidate` and the allocator's own `memcpy`. **Quote 222 Ir when sizing an allocation lead.**
5. 🔎 **THE CLASS THAT PRODUCED FOUR OF THE FIVE ROWS: "a value cloned so a short-lived reader could OWN it."** Three were *deleted* — a disjoint-FIELD borrow (`self.battlefield` vs `self.players`), and twice a `&[T]` where both branches outlive the reader — and only the fourth had to be pooled, and the pooled one is the smallest of the five. **Ask whether the owner needs to own before asking how to make owning cheap.**
6. ⚠⚠ **TAKE THE `--dump-instr` DUMP ON MORE THAN ONE POOL.** `(-358)` was priced at 4,948 allocations off the `cube` dump and measured **-0.049 % on `cube` and -0.312 % on `sealed`** (16,208 there) — the run's largest row, under-ranked 5.6x by its own census. Both dumps are now filed in (O).
7. 💡💡 **THE TOP LEAD IS (O)'s SHORTLIST AND IT IS RANKED AND PRICED — pull from the top of it and nothing else.** Biggest pair: the two auto-tap scratch sites in **one** function, 12,880 / 14,480 allocations — `Box::new(OneColorDecider)` (pool it, `(-356)`'s shape) and the `events` buffer (an out-param through `auto_tap_for_cost{,_only,_filtered}`, a five-site signature change, which is why this run left it). Then `TokenDefinition::clone` (6,688, unread). ❌ **`GrantScan::equipment` is REFUTED as `(-360)`** — it removed its 4,638 allocations and cost +0.115 / +0.052 / +0.115 %.
8. ⚠⚠ **INLINE BUFFERS: TWO OF THE THREE TRIED HERE LOST, AND `(-165)`'s RULE NOW NEEDS THREE QUESTIONS — ① does it grow its owner? ② how is it FILLED and how often is the owner MOVED (`(-356)`: +393 k of memcpy on a 136-byte value through four frames; `cube` spilled 1,960 of 8,724 fills at 16 slots where `sealed` barely spilled)? ③ how often is it READ, and does the widening push a caller across LLVM's inline threshold?** ③ is `(-360)` and **cannot be answered by inspection**: 16 bytes on `GrantScan` took `bot::usable_abilities` from 0 to 30,596 calls (+1.84 M), more than twice the 4,638 allocations removed. 📐 **A row that APPEARS with a large call count means something stopped being inlined** — the mirror of this file's "a vanished row was inlined" rule. ⚠ Separately: `Vec<&'a T>` **cannot** become `SmallVec<[&'a T; N]>` in a borrow-carrying struct at all — `Vec`'s `Drop` is `unsafe impl<#[may_dangle] T>`, `SmallVec`'s is not, so the borrow is forced to the end of the scope (four `E0502`s on `DispatchScan`).
9. ⚠ **A THREAD-LOCAL FREE LIST KEEPS ABOUT HALF OF THE 222 Ir ON THIS PROFILE**: `LocalKey::with` + `FnOnce::call_once` read ~52 Ir a snapshot for the pair (`(-356)`). Both are symbols `release`'s thin LTO inlines, so the shipped row is ~3x the measured one — **a pool's Ir reading understates it; a deletion's does not.**
10. ⚠ **EVERY GATE THIS BRANCH LANDS CARRIES A RECOMPUTE-AND-COMPARE `debug_assert!` ON ITS CHEAP PATH**, and `(-354)` shows the shape to prefer: **one device with one assertion covering twenty-nine sites** beats twenty-nine hand-written gates, because any one of the twenty-nine could have been the assertion nobody wrote. The sweep is where these fire, not the suite.
11. **Perf queue: ✅ (B)'s open half TAKEN as `(-354)` (29 sites, one `cold_any`); (O) FOUR ROWS TAKEN as `(-355)`..`(-358)`.** `(Q)` is **closed as a hoist** — its remaining 76 % of `cube` rollbacks are `try_pay_after_snapshot_mode` failing, i.e. CR 601.2h, which cannot run before the take; the concurrent session turned it into a bot-**strength** lead (`CRAB_PAY_FAILS`, 28.5 % of `cube`'s payments fail). Open: **(H)** (refuted shape, price the clone side first), **(N)** (needs a 4-byte key), **(P)**. `scripts/cold_census.py` is new and committed — it re-ranks the `CardCold` readers in one command.
12. **THE GLOBAL LINE TABLE SAYS THE *Ir* PROFILE IS FLAT — do not re-derive it.** Largest single source line 0.97 %, 12,537 lines hold 86.9 %. ⚠ **But "flat" was never true of the ALLOCATION table**, which is the lesson of this run: rank by allocation count and the same dump has an order of magnitude of spread. ⚠ `Battlefield`'s lane word has exactly ONE two-bit field left (shift 62), with a `const` assertion in `zone.rs`.
13. **Closing tip absolutes at `512aea87` (`profiling-fast --no-default-features`, system allocator, FIFTEENTH box @ 2.80 GHz): fixed 571,716,921 / cube 1,495,505,814 / sealed 1,612,122,733; run total -0.568 / -0.470 / -0.630 % over five rows.** ⚠⚠ **THE BOX RULE LANDED FOR THE SECOND RUN RUNNING AND THE OFFSET HAS A SIZE:** the base missed its prediction by **-0.198 / -0.091 / -0.093 %**, which is this file's recorded twelfth-vs-eleventh step (+0.196 / +0.090 / +0.089 %) with the sign flipped. **A fresh container is a fresh box; re-take the base, and reach for a tree explanation only when the miss is a novel one.** ⚠ Build times are CLOSED as a lever.
14. ⚠⚠ **FIVE TRAPS, ALL LIVE.** `cargo build --profile profiling-fast -p crabomination --bin bot_ladder --no-default-features` — **both flags load-bearing**; `nm <bin> | grep -cE " (T|t) (_)?mi_"` = 0 before filing a number. ⚠ **DO NOT EDIT THE TREE WHILE A MEASUREMENT BINARY IS BUILDING** (this run serialized a `release` build against an edit for exactly this). ⚠ **Never overlap two sweep blocks on four cores.** ⚠ `cargo build --release` took **24m20s** here and `profiling-lines` 8m41s, `profiling-fast` 8m41s cold / 2m40s warm — start the long one before you need it. ⚠ **DISK: `target/debug/incremental` reached 5.6 GB and `rm -rf` returned all of it with nothing to rebuild.** ⚠ A single-run `games_per_s` is **not a reading**: this run read 283 with a build running and 487-492 on an idle box.
