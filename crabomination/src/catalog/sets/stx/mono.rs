...
/// target creature."
///
/// ✅ Push XLIII: 🟡 → ✅. The "additional sacrifice cost" is now paid
/// at *cast* time via push XXXIX's `CardDefinition.additional_sac_cost`
/// field — `cast_spell` rejects with `SelectionRequirementViolated` when
/// the controller has no other creature to sacrifice, then auto-picks
/// the lowest-value matching creature and sacrifices it before the
/// spell goes on the stack. The body now contains only the targeted
/// exile (no double-counted in-resolution sac).
...
/// and draw two cards."
///
/// ✅ Push XLIII: 🟡 → ✅. The "additional cost discard a card" is now
/// paid at *cast* time via the new `CardDefinition.
/// additional_discard_cost: Option<u32>` field — `cast_spell` rejects
/// with `SelectionRequirementViolated` when the controller has zero
/// cards in hand at cast time, then auto-picks the first card in hand
/// and discards it before the spell goes on the stack. The 2 life +
/// draw 2 halves resolve unconditionally afterwards. Sister cast-time
/// primitive to push XXXIX's `additional_sac_cost` (Daemogoth Woe-Eater,
/// Eyeblight Cullers).
