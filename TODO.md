...
8. ✅ **`CRAB_CAP_DIAG` NAMES THE LOOP NOW.** `ActionCensus` tallies an undecided pod game's performed actions by `(variant, card, ability index)` and prints the top twelve beside the board dump — `cap_diagnosis`'s own header says a loop whose card is in none of the four sampled zones is invisible, and the action the bot keeps taking never is. One `OnceLock` read a game when off. 🔎 **AND IT PAID ON ITS FIRST RUN.** The eight-seat sweep capped **1 game in 2,000** and the census named the loop in one line: `ActivateAbility#0 Eldrazi Spawn x7535` against a seat holding **pool 7533**, with **no Eldrazi Spawn in the board tally** — precisely the shape `cap_diagnosis`'s own header says it cannot see. A `sac_cost: true` ability accepted 7,535 times is 7,535 sacrifices of a token that should support **one**, so either the sacrifice is not charged or the token is re-created; the Broodscale that mints them (`mod_set/creatures.rs`) reads correct against its oracle, so **start at the activation, not the card.** Repro: `CRAB_CAP_DIAG=1 target/release-fast/bot_ladder --commander --seats 8 --games 2000 --threads 3 --seed 9101`, pod seed **18045724030442976829**, turn 59, stack 1. ⚠ The old six-seat repro (pod seed 11643393128411363082 = base 43 game 35 rotation 5, `seed = base + i · 0x9E3779B97F4A7C15`) is **stale** — it decides in 2,863 actions now. ⚠ The census records no **seat**; add one, because the pool and the activation were read off different players here and nothing in the line says whether that is real.
