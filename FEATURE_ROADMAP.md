...
| CR 800.4c a control effect ending with the default controller gone | ⚠ **missing** | The one 800-series row with no arm. A owns X, B takes it permanently, C takes it until end of turn, B leaves: 800.4a's revert only touches permanents B *currently* controls, so B's grant survives and C's expiry hands X back to a seat that is not in the game (CR says exile it). Needs the control-effect registry swept by granting seat in `objects_leave_with_player`. Unreachable in the pod decks — none of the five carries a control-theft effect — so it is filed rather than built |
