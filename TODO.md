...
2. **Gates at this run's tip (2026-09-25, session `018BnccM` run 5):** suite **21,914 / 0 / 7** (strict answer log; `target_walkers` caught two audit gaps in the Omo cards, fixed and re-run); workspace clippy **0**; `--bench` **195,806 / 0 stalls — byte-identical**, `determinism ok`; 4-seat pods ≥ 999/1,000 decided per new seat (open queue names the three undecided); strict debug pods 600/600 (4 and 6 seats); cube/sos/sealed 7,500 decided; every card of every seated list played in the census.
