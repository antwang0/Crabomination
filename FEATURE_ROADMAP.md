...
| A **melded** commander is still the commander (Gisela's ruling, CR 903.3 + 701.37) | ✅ *since 2026-09-24* | `GameState::commander_card_of` maps the melded permanent to its commander component: `is_commander`, `is_own_commander_object` ("if you control your commander") and the combat commander-damage tally read it, so Brisela's hits land on Gisela's card. Leaving splits `meld_parts` and only the commander card goes home. Test `cmdr_gisela::a_melded_commander_is_still_the_commander`; pod seat 33 plays it. ⚠ A melded commander bounced to hand/library: the CR 903.9b replacement is keyed on the component's id and the parts move one by one — untested |
