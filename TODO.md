...
9. **Seeds: FRONTIER 1366, and this run did NOT move it** — the three rows are behaviour-preserving by trace (48 cube games identical across every binary) and add no `debug_assert!`, so the sweep was traded for the third A/B. ⚠ **Take a block first next run** (`CRAB_ANSWER_LOG=strict scripts/fresh_seed_sweep.sh "cube all sealed" "1366 1367 1368 1369" 400`, `target-audit/overflow` with `-C debug-assertions=yes`) — `(-331)` changed a combat-legality *path*, not its answers, and nothing has swept it. Standing audits all 0 this run (`audit_panics` 0 bare / 70 sites, `audit_engine_stack` 0, `audit_stash_in_loop` 0 unexplained, `audit_seat_from_selector` 0 open, `audit_target_walkers --check` 0, `audit_doc_drift` 0/0/0).
10. **Bugs/robustness: nothing open.** Structural audits, determinism and the panic census are all clean; TODO's robustness section has no open entries. `audit_incomplete` / `audit_stubs` are Rust bins and were not re-run this session — they were 0 at the `(-329)` tip and no card changed.
11. **Best next moves, in order.** (a) **Finish line 4's device**: `cg_calls.py <dump> 200`, take every row whose callee is shared, and ask `--callers` of each — `ManaPool::is_empty`, `counter_count` (225,536 calls), `same_team` (87,764), `defender_for` (74,096), `event_kind_bits` (256,878) are all unexamined, subject to line 5's trap. (b) Queue item (C), the pair count in `pick_blocks_inner`. (c) The fresh-seed block from line 9, as the FIRST gate. (d) leftover context: `audit_dropped_may` 199; `audit_keyword_drift`'s last 4 each need a PRIMITIVE; card skips with a card behind them: `nocolors` 950, `noback` 60, `nosubtypes` 43, `nokeywords` 29.
