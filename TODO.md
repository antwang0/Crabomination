...
3. **Gates at the closing tip `3aca0ddb`** (the `--bench` and pod numbers taken at `7f9520ed`; the eight cards changed after it are in none of `archetypes()`, `golden_trace.rs` or `pod/decks.rs`, and the cross-process pod-outcome test passes unchanged)**:** suite **19,768 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); clippy **0** over the workspace; `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean; `--bench` (release) **195,806 / 27.49 / 611.9 / 0 stalls** — **byte-identical to the committed invariant**, `determinism ok`, `thread_determinism ok (3 vs 1)`, `peak_rss_mib` 25.9. Pod smoke: **12,000 games over 2/3/4/5 seats at fresh seeds 3201-3203, 100 % decided, 0 stalls, every `undecided_by` column zero**, no panic or error line. **FRONTIER 3204.**
...
9. ✅ **`Effect::BindScratch` closed the last two loop-splice rows that wanted a primitive** (`Vote`'s `PerVote`, `RollDie`). `scripts/audit_loop_splice.py` reads **13/13/0/0** and grew a **staleness** half (an `ALLOW` entry whose site is gone now fails — it found three dead ones immediately) and a `scope_at` that reads a multi-line arm pattern. Four rows stay open and **two of them are catalog judgements, not code** (`SacrificeAnyNumber`, `MayPayRepeatedly`, `FlipCoinsChooseCount` — every body is choiceless today).
10. **Cards and decks:** 5/5 target decks complete; the pod field reads 39.8 / 14.7 / 20.0 / 25.5 at four seats (seed 43, 3,000 games), inside the noise of the recorded 40.8 / 14.6 / 20.3 / 24.3. ⏳ DECK_FEATURES' open item is still **board wipes**: Sigarda, Hanna and Tatyova have none. ⏳ 32 of 33 "can be your commander" non-creatures wait on a sixth target deck.
11. ⚠ **POOL MEMBERSHIP IS A MEASUREMENT, NOT A VETO — and the cheap pre-check is two deck lists.** `--bench` plays `--decks fixed` = `bot_ladder::archetypes()`, 38 vanilla/burn/one-clause cards, and `golden_trace.rs` has two more. **Not one of the 126 changed cards is in either**, which is why this run could predict the bench before it built it.
12. ⚠ **WHEN THE COMMITTED POD TABLE AND THE AGGREGATE DISAGREE, THE AGGREGATE WINS.** `cr_903_seeded_pod_outcomes_match_the_committed_table` is a re-bless gate over **three** games; read turns/game over thousands, and read the two-seat row as the control.
