...
1. ⚠⚠ **TWO SESSIONS SHARE THIS BRANCH. FETCH BEFORE YOU START AND BEFORE EVERY PUSH, AND RE-RUN THE SUITE AFTER EVERY REBASE.** Both sessions independently shipped the Signet/Talisman swap and both independently fixed `apnap_skips_eliminated_seat_in_cycle`; one of them left the branch red for four commits because CR 800.4d (`0320e1cb`, correct) removed a departed player's triggers from the stack and that test still asserted the fourth one. ⚠ **`cargo nextest` fail-fast hides the rest** — the red was found only by `--no-fail-fast`. Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure"**.
2. **Where it stands:** FEATURE_ROADMAP's **"Commander status"** is the map and it is current — CR 903 core (now incl. **903.3's Vehicle/Spacecraft commanders** and **903.5e**), the 800-series (now incl. **800.4b/d/j/m**), the whole variant list. **Read that table, don't re-audit.** DECK_FEATURES: **5/5 decks complete**, each two-colour seat on its guild Signet + Talisman.
3. **The runner:** `crabomination::pod` + `bot_ladder --commander [--seats N] [--games N] [--seed N]`. `pod::tests` validates every list every run, so an illegal deck cannot land. ⚠ The Partner deck is **last** on purpose: `pod_field(4)` never draws it; `--seats 5` is what reaches it.
4. 🔎🔎 **THE FIND, AND IT IS A METHOD: THE SHIPPED CR TEXT IS IN THE REPO AND IT DISAGREES WITH WHAT THE CODE CITES.** `crabomination/MagicCompRules_20260417.txt`, read with `awk '/^903\./,/^904\./'`, is the authority — not memory, not the tracker. Reading 800.4 and 903 end to end turned up **four missing sub-rules that every ✅ row had hidden** (800.4d, 800.4j, 800.4m, 903.3's non-creature commanders) and **five citations pointing at renumbered rules** (903.4b→903.4c reminder text, 903.4c→**903.5d** basic land types, 903.4a→903.4/604.3, 903.3c→702.124 twice). **Read the rule, then the row.**
5. 🔎 **The bug shape that keeps recurring: anything that leaves the table waiting on a seat that cannot act.** A pending decision was the first (CR 800.4a, 15 % of four-seat games); **priority is the same gate one level down** (800.4a-closing + 800.4j); a delayed trigger registered by a departed seat (800.4d) and a duration keyed to a turn that never comes (800.4m) are the same family. When the stall rate moves, ask **which seat the table is waiting on** first.
6. **This run shipped:** CR 800.4a-closing/800.4j (`priority_recipient`); CR 800.4d (`push_pending_trigger` is the one funnel — step, event and delayed pushes all take the guard); CR 800.4m (`departed_seats_skipped_into_this_turn`, a seat mask reused by CR 615's damage locks); CR 800.4a/b's *player*-control half (Mindslaver); CR 903.3 Vehicle/Spacecraft commanders (`format::is_legal_commander`, `NotLegendaryCreature`→`IllegalCommander`) + the citation sweep; `default_hostile_opponent` (the bot's defender) and `first_alive_opponent_of` (the auto-targeter); and the **Commander free-spell cycle** finally getting the line that is its whole point — "if you control a commander, cast it without paying its mana cost" is an `AlternativeCost` of nothing with `condition: Some(ControlsOwnCommander)`, **no engine change needed**, and four shipped cards had been missing it.
7. ⚠⚠ **THE FORTY-SECOND FIND IS THE BEST LEAD IN THIS FILE AND IT IS HALF-DIAGNOSED FOR YOU.** On one four-seat board Haywire Mite's "{G}, Sacrifice this creature" **completed with every pool empty and no land tapped** — the sacrifice half of the cost paid, the ability on the stack, `pending=None`, `prio == ctrl`. ENGINE_BACKLOG has the probe dump and the one-line wiring that reaches the board (`targeting.rs`'s `opp` → `default_hostile_opponent`, seed `0xC0FFEE`, `bot_vs_bot_commander_demo_terminates`). **The control is written and passing**: `cr_602_2b_a_sac_ability_still_needs_its_mana` + its `wants_ui` twin refuse the same activation in a duel with an empty stack, so the defect is state-dependent and **`stack=3` is the difference to chase first.** ⚠ Three engine rebuilds bought those columns; do not re-derive them. ⚠ The ranked pick is worth taking for the auto-targeter once this closes.
8. ⚠⚠ **CORRECTION TO AN EARLIER NEXT: "no bot seat sets `wants_ui`" IS FALSE.** `pod::build_pod_template` sets it on **every** seat, so pod games do run the suspend/replay path the two-player ladder never reaches. That is a reason to trust the pod as an instrument — and it means a suspend-path defect is reachable from self-play the moment a card carrying one enters a pod list.
9. ⚠ **The JoinForces/TemptingOffer latent bug is a different shape than an older NEXT said, and the fix idiom is already in the tree.** `run_effect(body, …)` passes **`body`** as the effect an inner ask suspends with, so the parked continuation is the body alone. The defect is that a **loop** arm rebuilds no continuation: a suspend in iteration *k* silently drops *k+1..n*. Forbidden Ritual's arm (`effects/mod.rs` ~3965) and `SearchUpToN` show the splice. ⚠ What blocks copying it is that each remaining iteration needs a *different* bound player and no `Effect` pins one — that needs a wrapper variant, a `crabomination_base` change plus the walker family. `ForEachOpponent` has **one** catalog card; price it before building it.
10. **What CR 800.4 still approximates, on purpose:** 800.4f/g/h — a choice owed by a departed player is **dropped**, where the rule hands it to another player the object's controller picks. Dropping is what keeps the table moving; reassigning needs a decision-retargeting path that does not exist. 800.4i's last-known-information clause is unmodelled.
11. **Cards and decks, what is left:** ⏳ 32 of 33 "can be your commander" non-creatures are unimplemented; the flag and validator are done, so each is a plain card. ⏳ `scripts/audit_each_opponent.py`'s seven residuals are in INCOMPLETE_CARDS with what each needs. ⏳ **Obscuring Haze** is the free-spell cycle's missing member and CARD_BACKLOG names its blocker: an *all-damage* fog filtered by **source**, where the engine has only a combat-only one and a single-target one. ⚠ **Deck balance is the loudest number**: seed 43, 400 four-seat games reads Sigarda **45.5 %** / Tatyova 26.8 % / Hanna 22.2 % / **Judith 5.5 %** — the aristocrats list does not assemble. Board wipes are also one seat's (both Krark's).
12. ⚠⚠ **Golden traces are the Commander guardrail too, and the test is POOL MEMBERSHIP.** With no commander seated every Commander change must be byte-identical in a duel. `commander_identity_colors` keeps the any-colour answer for a seat with **no** commander (CR 903.4f says otherwise; deliberate, for the cube's fixing), and `is_rider()`'s auto-tap reaches no card in `cube::cube_pool_all()`. **Check the pool before assuming a mana-path change is duel-safe.** This run's engine changes are all gated on "somebody has left the game", which one elimination ends in a duel, or short-circuit to the single opponent before ranking.
13. ⚠⚠ **AND WHEN THE COMMITTED TABLE AND THE AGGREGATE DISAGREE, THE AGGREGATE WINS.** `cr_903_seeded_pod_outcomes_match_the_committed_table` is a **re-bless gate, not a measurement**: three games said the new defender heuristic lengthened pods (67→92, 46→79 turns) with a plausible story, and 1,700 games said every configuration got **shorter** (42.08→39.65 at four seats, 53.85→49.95 at five). **Tip reading at `06d60b26`: 1,700 games / six configs / 2-3-4-5 seats, 100 % decided, zero panics, zero stalls, `--threads 1` identical in every column; `--bench` invariant 195,806 / 27.49 / 611.9 / 0 stalls byte-identical, suite 19,704/19,704, clippy clean.** ⚠ Read **turns**/game, not actions/game. `CRAB_CAP_DIAG=1` names an undecided pod game's board.
14. 📐 **The N-seat audit method:** read every deck card's Scryfall text for "each opponent / each other player / their first / that player" and its body for a fan-out ref. Esper Sentinel modelled "their first noncreature spell each turn" as `once_per_turn` — exact at two seats, one fire per *table* in a pod; Adeline made one attacking token where "for each opponent … that player" wants one each. Both bugs are invisible at two players and to a per-card test written at two players. 📐 And **where a trigger functions is a property of the ABILITY, not the card** (Oloro's two upkeep triggers differ); a *static*'s is card-level here because `StaticAbility` is `{description, effect}` in 2,275 literals.
15. ⚠ **Four gotchas that each cost a debugging round.** ① A `ScriptedDecider` answers in ASK ORDER — to answer one question and leave the rest, write a four-line `Decider` matching that `Decision` variant and deferring to `AutoDecider` (`SayYesToOptional` is the pattern), and note `AutoDecider` **declines every `OptionalTrigger`**, so a `MayDo` body is dead under it. ② A "target player" slot **auto-picks an OPPONENT**, so a targeted gift goes to the wrong seat — `prefers_friendly_target` declares the shape. ③ `Selector::TriggerSource` binds the **damaged** player at resolution while a trigger's `filter` is evaluated against the **dealer** at gather time; twenty catalog cards rely on it and it is **not** a bug class. ④ New Commander cards live in `catalog/src/sets/cmdr.rs`, registered in `all_factories.rs` — free of the net vocab (`Vocab`/`encode.rs` read `cube::cube_pool_all()`), but **`cargo check --workspace --all-targets` is the gate that sees the bins**.
