...
1. **FIRST:** `git fetch origin claude/modern_decks && git checkout -B claude/modern_decks origin/claude/modern_decks`. Rebase, never force; code before tracker prose; **fetch before every push** — SIX sessions shared this branch on 2026-09-13. Gotchas in **CLAUDE.md**, measurement in **PERF's "Standing rules"**, and ⚠ **the 2026-09-12 AND 2026-09-13 detail is in ENGINE_BACKLOG's "The 2026-09-12/13 handoff detail, moved verbatim"** — ~340 lines now: the printed-body skip taxonomy, the whole `EventKind` fan-out census and its five defects, the modal census, the closed-by-measurement list, the collision log and the seed discipline. **Read it before you take a skip kind or a seed, and say in this line which you are taking.** NEXT is 15 lines and stays 15; findings go to PERF, ENGINE_BACKLOG or INCOMPLETE_CARDS.
2. **Gates at the tip, all re-run this run:** suite **19,546 / 0 / 5** (`CRAB_ANSWER_LOG=strict`), clippy **0** (`--workspace --all-targets`), golden_trace **12 / 12 unmoved**, `--bench` **195,806 / 27.49 / 611.9 / 0 stalls byte-identical to the committed invariant** with `determinism ok` and `thread_determinism ok (3 vs 1)`, `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean (the ONLY gate that sees `debug-assertions = false`), the 5 skipped all justified. ⚠⚠ **THIS BOX IS 4 CORES** — its `--bench` wall clock (439 games/s) compares with NO absolute in PERF, every one of which was taken on 24. **The counters are the invariant.**
3. **Perf — `(-294)` took the queue's top row and PERF's candidates head says what is left of it.** The lead was the fixed per-dispatch preamble and the shape of the answer was *how many times is `events` walked*, not *which statement is slow*: six passes, four answerable from the mask pass two builds, **-0.339 / -0.275 / -0.404 %** with the whole delta in two rows. The row is still #1 and still flat (no line above 0.40 % of the program), and **both open structural questions are about DISPATCH COUNT**: the 47 % that return at the empty-batch early-out, and whether `perform_action_inner`'s 162,026 batches can be merged. ✅ **`(-296)` then took a piece of the NEW #1**, which is a *caller*, not a self row, and no self table can see it: **`bot::cast_candidates` is 8.9 % of `sealed` inclusive against 1.25 % of self**, found with `cg_ratio.py` off the sealed/fixed join (top row `printed_requirement_impl`, **13.21x**). Two near-equal halves — affordability 3.49 % and target enumeration 3.61 %, the latter **26.6 requirement evaluations a call** — and PERF's candidates head sizes both. `(-296)` is the cost-static source filter: "nothing to iterate" -> "nothing to match", **sealed -0.318 / cube -0.093 / fixed +0.032 %** (the `fixed` residual is kept and stated; its boards have nothing to filter). ⚠ Its first reading was `fixed` **+0.153 %** and the fix was one `!is_empty() &&` short-circuit — **a predicate in a hot filter is priced by its fast path, not by its body.** ⚠ **The allocator row is READ and closed with a reason** — 217,546 growths over 156 callers, nothing above `(-103)`'s rule with a real call count; the 950 k fresh allocations are `GameState::clone` (71 k, the bot's probes), the CoW unshare family (~131 k) and `finish_grow` spread thin. Don't spend a build on it.
4. **Bugs — `all` 1274 is CLOSED and it was never a watch bug.** `(-295)`: `ab.sac_cost` is "sacrifice THIS permanent" and five mana-sink generators read it as "costs a permanent", so four of them took, unpriced, the exchanges `pick_sacrifice_value` had just refused — Blightsteel Colossus fed to a Thopter Foundry for 1 life and a 1/1, 2,884 times. `cap 2 -> cap 0`; zero incidence on both gating pools (72,000 paired games, every pair an exact mirror), 4 discordant pairs in 13,598 on `all`. **`all` 1281 is still open and its field is NAMED now: `ctr`**, the battlefield counter total, +1 every two turns with everything else period-2 — Sphere of Safety x2, Rest in Peace x2, a Beacon each. A counter is ordinarily real progress, so 1281 is the adjudication call ("no seat can win or lose"), not a field to drop.
5. **Seeds: frontier 1292 — THIS RUN TOOK 1282..1291: 30 cells / 148,000 games / 0 failures**, `cap 0 / board 0 / stuck 0 / draw 14`, on the `overflow` build with `-C debug-assertions=yes` at the `(-295)` tip. Say which block you are taking in this line *before* you take it; that is five same-seed collisions in two days and PERF's table is the only thing that stops it. ⚠ A sweep at this tip is not the same workload as one at the last: `(-295)` changes what the bots do on any board with a sacrifice-other outlet.
6. **Best next moves.** (a) **PERF's candidates head has the biggest sized, unstarted lead on the page: `cost_reduction_for_spell_full_over`'s TAIL, ~0.5 % of `sealed`.** `(-296)` took its source loop; everything after line 230 of its 640 reads the CARD being cast and runs in full on all 49,140 affordability checks — ~22 `if let Some(..)` field checks and **thirteen** separate `for sa in &card.definition.static_abilities` loops. Two devices, both priced there: merge the thirteen into one (~0.13 %, mechanical, check first that no two match the same variant), and a definition-level "carries any self-cost reduction" bit for the other ~0.35 % — ⚠ read `CardMemo`'s whole five-word layout before claiming a spare bit. Also open in the same tree: 26.6 printed-requirement evaluations per target enumeration, against a `first_legal_*` short-circuit that already exists in the module. (b) ⚠ **DO NOT re-take the rest of `(-295)`'s class as a defect fix — `(-297)` laddered it and it LOSES.** `bounce_other_filter` / `exile_other_filter` / `discard_cost` had no priced owner and 48 catalog abilities pair one with a sink shape, so it looked identical; giving them `pick_sacrifice_value` reads **pooled 49.58 % on sealed** over 4 seeds with real incidence (hundreds of discordant pairs a cell, two intervals wholly below 50). **A last-resort sink is not priced against doing nothing — it competes with doing nothing AND wasting the mana, which `eval_material` cannot see.** ⚠⚠ **The code is REVERTED, not parked** — `ability_sink_bits` is per (permanent, ability), so carrying the arm cost the shipped default **+0.034 / +0.015 / +0.022 %** (a seventh of `(-296)`'s win, for a flag that is off), even with a separate gate bit keeping the default's gate byte-identical. `(-297)` carries the whole design and `.ladder/run_r73_sinkcosts.sh` its thresholds. If taken again it is as a STRENGTH arm and what changes is the owner's BASELINE, not the predicate. (c) The follow-up named in code: `pick_removal_sacrifice`'s favourable-trade test reads the SOURCE's power, so a destroy-shaped sacrifice-other ability has no trade math and `pick_removal_destroy` keeps the narrow gate on purpose. (d) The skips with a card behind them: `nocolors` **950**, `nosubtypes` 43, `noback` 60, `nokeywords` 29 — say which you are taking. (e) `audit_keyword_drift`'s last 4 each need a PRIMITIVE. (f) `audit_variant_coverage` is at ONE dead primitive, closed with a reason (`GrantCastBackFromGraveyard`); do not go looking.
