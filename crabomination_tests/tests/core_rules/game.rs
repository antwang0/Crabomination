...
use crabomination::game::{cast, cast_at, drain_stack, two_player_game};
use crabomination::catalog;
use crabomination::card::StaticAbility;
use crabomination::decision::{DecisionAnswer, ScriptedDecider};
use crabomination::effect::{Effect, ManaPayload, PlayerRef, Selector, StaticEffect, Value};
...
fn annihilator_1_attack_forces_defender_sacrifice() {
    ...
    use crabomination::card::{CardType, CardDefinition, CreatureType, Keyword, Subtypes};
...
fn ancestral_recall_draws_three_cards() {
    ...
    cast_at(&mut g, id, crabomination::game::types::Target::Player(0));
...
fn ancestral_recall_can_target_an_opponent() {
    ...
    cast_at(&mut g, id, crabomination::game::types::Target::Player(1));
    assert_eq!(
...
fn each_mox_taps_for_its_color() {
    let cases: [(fn() -> crabomination::card::CardDefinition, Color); 4] = [
...
fn setup_attacker(g: &mut GameState, player: usize, def: impl Fn() -> crabomination::card::CardDefinition) -> CardId {
...
fn would_accept_dry_runs_without_mutating_caller_state() {
    ...
    assert!(!g.would_accept(GameAction::PlayLand(crabomination::card::CardId(999))));
    assert_eq!(g.battlefield.len(), battlefield_before);
...
fn becomes_blocked_trigger_resolves_against_attacker_not_blocker() {
    use crabomination::card::{CardDefinition, CardType, CounterType, Subtypes, TriggeredAbility};
...
fn one_u_spell() -> crabomination::card::CardDefinition {
...
fn tarmogoyf_pt_scales_with_card_types_in_graveyards() {
    ...
    g.players[1].graveyard.push(crabomination::card::CardInstance::new(
        crabomination::card::CardId(9002),
...
fn force_of_will_pitches_a_blue_card_to_counter_a_spell() {
    ...
    assert!(g.stack.iter().any(|si| matches!(
        si,
        crabomination::game::StackItem::Spell { card, .. } if card.id == fow
    )));
...
fn cr_614_2_doubled_trample_over_lethal() {
    ...
    let attacker = setup_attacker(&mut g, 0, || {
        vanilla_body("Trampler 3/3", 3, 3, vec![crabomination::card::Keyword::Trample])
    });
...
fn psychic_frog_discard_adds_plus_one_counter() {
    ...
    assert_eq!(card.counter_count(crabomination::card::CounterType::PlusOnePlusOne), 1,
...
fn force_of_negation_alt_cost_works_on_opponents_turn() {
    ...
    assert!(g.stack.iter().any(|si| matches!(
        si,
        crabomination::game::StackItem::Spell { card, .. } if card.id == fon
    )));
}
...
fn gemstone_mine_etb_with_three_charge_counters() {
    ...
    assert_eq!(card.counter_count(crabomination::card::CounterType::Charge), 3,
...
fn gemstone_mine_taps_three_times_then_sacrifices() {
    ...
    g.battlefield_find_mut(id).unwrap()
        .add_counters(crabomination::card::CounterType::Charge, 3);
    ...
    assert_eq!(g.battlefield_find(id).unwrap()
        .counter_count(crabomination::card::CounterType::Charge), 2);
    ...
    assert!(g.battlefield_find(id).is_some());
    assert_eq!(g.battlefield_find(id).unwrap()
        .counter_count(crabomination::card::CounterType::Charge), 1);
    g.battlefield_find_mut(id).unwrap().tapped = false;
...
fn inquisition_suspends_for_caster_ui_and_applies_chosen_discard() {
    ...
    let crabomination::decision::Decision::Discard { player, count, hand } = &pd.decision else {
...
fn symmetric_discard_reaches_every_ui_seat() {
    ...
    let crabomination::decision::Decision::Discard { player, .. } = &pd.decision else {
        panic!("expected Decision::Discard, got {:?}", pd.decision);
    };
    assert_eq!(*player, 0);
...
fn consign_to_memory_mode_one_counters_legendary_spell() {
    ...
    assert!(g.stack.iter().any(|si| matches!(
        si, crabomination::game::StackItem::Spell { card, .. } if card.id == thalia
...
fn gemstone_caverns_starts_in_play_with_luck_counter() {
    ...
    assert_eq!(
        caverns.counter_count(crabomination::card::CounterType::Charge),
        1,
...
fn serum_powder_exiles_hand_and_redraws() {
    ...
    assert!(matches!(
        ...
        Some(crabomination::decision::Decision::Mulligan { player: 0, .. }),
    ));
}
...
fn london_mulligan_repose_on_short_bottoming_answer() {
    ...
    assert!(matches!(pd.decision, crabomination::decision::Decision::PutOnLibrary { player: 0, count: 1, .. }));
    ...
    assert!(
        matches!(pd.decision, crabomination::decision::Decision::PutOnLibrary { player: 0, count: 1, .. }),
...
fn mulligan_decision_lists_serum_powders() {
    ...
    let crabomination::decision::Decision::Mulligan { serum_powders, .. } = &pd.decision else {
...
fn reveal_until_find_caps_at_n_when_no_match() {
    ...
    g.resolve_effect(
        &Effect::RevealUntilFind {
            ...
            cap: Value::Const(10),
            life_per_revealed: 1,
            miss_dest: crabomination::effect::RevealMissDest::Graveyard,
        },
        &ctx,
    ).unwrap();
...
fn damping_sphere_preserves_non_mana_activated_abilities() {
    ...
    use crabomination::effect::{Effect, ManaPayload, PlayerRef};
...
fn auto_target_prefers_opponent_permanent_for_hostile_effect() {
    ...
    assert_eq!(target, crabomination::game::Target::Permanent(opp_bear),
...
fn auto_target_deprioritizes_warded_permanents() {
    ...
    assert_eq!(target, crabomination::game::Target::Permanent(plain), "prefers the un-warded bear");
...
fn auto_target_prefers_friendly_permanent_for_buff_effect() {
    ...
    assert_eq!(target, crabomination::game::Target::Permanent(own_bear),
...
fn auto_target_disentomb_prefers_creature_in_your_graveyard() {
    ...
    assert_eq!(target, crabomination::game::Target::Permanent(dead_bear),
...
fn auto_target_skips_hexproof_opponent_creature() {
    ...
    assert_ne!(
        target,
        Some(crabomination::game::Target::Permanent(caryatid)),
    ...
    assert_eq!(target, Some(crabomination::game::Target::Permanent(own_bear)));
}
...
fn auto_target_regrowth_skips_player_in_favor_of_graveyard_card() {
    ...
    assert_eq!(
        target,
        crabomination::game::Target::Permanent(dead_card),
...
fn auto_target_mind_rot_picks_opponent_player() {
    ...
    assert_eq!(target, crabomination::game::Target::Player(1),
...
fn auto_target_player_side_effects_resolve_via_filter() {
    ...
    let discard = Effect::Discard {
        who: crabomination::effect::shortcut::target_filtered(SelectionRequirement::Player),
        amount: Value::Const(1),
        random: true,
    };
...
fn auto_target_cling_to_dust_picks_graveyard_card_not_player() {
    ...
    assert_eq!(target, crabomination::game::Target::Permanent(dead),
...
fn loyalty_negative_cost_returns_not_enough_loyalty() {
    ...
    if let Some(c) = g.battlefield_find_mut(teferi) {
        c.counters
            .insert(crabomination::card::CounterType::Loyalty, 2);
    }
    let err = g.perform_action(GameAction::ActivateLoyaltyAbility {
            x_value: None,
        card_id: teferi,
...
fn learn_ui_player_suspends_and_resumes_via_submit_decision() {
    ...
    use crabomination::card::CardInstance;
    use crabomination::decision::{Decision, LearnChoice};
    let mut g = two_player_game();
    g.players[0].wants_ui = true;
    let lesson = g.next_id();
    g.players[0]
...
fn inkling_summoning_creates_a_2_1_flying_inkling_token() {
    ...
    assert!(i.has_keyword(&crabomination::card::Keyword::Flying));
}
...
fn token_without_name_derives_name_from_creature_subtypes() {
    use crabomination::card::{ArtifactSubtype, CreatureType, Subtypes, TokenDefinition};
    use crabomination::game::effects::token_to_card_definition;
...
fn mill_caps_at_library_size_per_cr_701_17b() {
    ...
    let mill_def = crabomination::card::CardDefinition {
        name: "Test Mill 10",
        cost: crabomination::mana::ManaCost::default(),
        card_types: vec![CardType::Sorcery],
        subtypes: crabomination::card::Subtypes::default(),
        effect: Effect::Mill {
            who: crabomination::effect::Selector::Player(crabomination::effect::PlayerRef::Target(0)),
...
fn library_position_from_top_inserts_at_index() {
    ...
    use crabomination::effect::{LibraryPosition, ZoneDest};
    let mut g = two_player_game();
    ...
    let ctx = crabomination::game::effects::EffectContext::for_spell(
        0,
        Some(crabomination::game::types::Target::Permanent(target_card)),
        0,
        0,
    );
...
fn lifelink_combat_damage_gains_life() {
    ...
    std::sync::Arc::make_mut(g.battlefield_find_mut(lifelinker).unwrap().definition_mut()).keywords.push(
        crabomination::card::Keyword::Lifelink,
    );
    let life_before = g.players[0].life;
    g.step = TurnStep::DeclareAttackers;
    g.perform_action(GameAction::DeclareAttackers(vec![Attack {
...
fn cant_block_creature_can_still_attack() {
    ...
    let attacker = setup_attacker(&mut g, 0, || {
        ...
        def.keywords = vec![crabomination::card::Keyword::CantBlock];
        def
    });
    g.step = TurnStep::DeclareAttackers;
...
fn effect_untap_removes_stun_counter_instead_of_untapping() {
    ...
    g.battlefield_find_mut(bear).unwrap().add_counters(
        crabomination::card::CounterType::Stun, 1,
    );
    ...
    assert_eq!(card.counter_count(crabomination::card::CounterType::Stun), 0,
...
fn cr_121_2b_draw_cap_truncates_draws() {
    ...
    use crabomination::effect::PlayerStaticTarget;
    let mut g = two_player_game();
...
fn vanilla_body(name: &'static str, p: i32, t: i32, kws: Vec<crabomination::card::Keyword>) -> crabomination::card::CardDefinition {
    use crabomination::card::{CardDefinition, CardType};
    CardDefinition {
        name,
        card_types: vec![CardType::Creature],
...
fn cr_510_1c_trampler_assigns_lethal_to_each_blocker_then_tramples() {
    ...
    let mut g = two_player_game();
    let attacker = setup_attacker(&mut g, 0, || {
        vanilla_body("Trampler 5/5", 5, 5, vec![crabomination::card::Keyword::Trample])
    });
...
fn cr_510_1c_deathtouch_attacker_assigns_one_to_each_blocker() {
    ...
    let mut g = two_player_game();
    let attacker = setup_attacker(&mut g, 0, || {
        vanilla_body("DT 3/3", 3, 3, vec![crabomination::card::Keyword::Deathtouch])
    });
...
fn cr_510_1c_invalid_over_assignment_falls_back_to_default() {
    ...
    let mut g = two_player_game();
    let attacker = setup_attacker(&mut g, 0, || {
        vanilla_body("Trampler 4/4", 4, 4, vec![crabomination::card::Keyword::Trample])
    });
    let b1 = setup_attacker(&mut g, 1, || vanilla_body("Wall A", 2, 2, vec![]));
    let b2 = setup_attacker(&mut g, 1, || vanilla_body("Wall B", 2, 2, vec![]));
...
fn cr_702_6_equip_attaches_at_sorcery_speed_to_your_creature() {
    ...
    assert!(matches!(
        g.perform_action(crabomination::game::GameAction::Equip { equipment: boner, target: bear }),
        Err(crabomination::game::GameError::SorcerySpeedOnly)
    ));
...
fn cr_702_16f_protection_blocks_equip() {
    ...
    use crabomination::mana::{cost, generic, r, Color};
    let mut g = two_player_game();
    let mut bear_def = catalog::grizzly_bears();
    ...
    g.step = crabomination::game::TurnStep::PreCombatMain;
    assert!(matches!(
        g.perform_action(crabomination::game::GameAction::Equip { equipment: blade, target: bear }),
        Err(crabomination::game::GameError::TargetHasProtection(_))
...
fn cr_702_122_crew_requires_total_power_at_least_n() {
    ...
    g.perform_action(crabomination::game::GameAction::Crew {
...
fn coach_printed_power(g: &crabomination::game::GameState, id: crabomination::card::CardId) -> i32 {
...
fn cr_509_1b_unblockable_attacker_cannot_be_blocked() {
    ...
    g.priority.player_with_priority = 0;
    g.active_player_idx = 0;
    g.perform_action(crabomination::game::GameAction::DeclareAttackers(vec![crabomination::game::Attack {
        attacker: land,
        target: crabomination::game::AttackTarget::Player(1),
    }])).expect("tar pit attacks");
    g.step = crabomination::game::TurnStep::DeclareBlockers;
    g.priority.player_with_priority = 1;
    let err = g.perform_action(crabomination::game::GameAction::DeclareBlockers(vec![(blocker, land)]))
        .expect_err("unblockable can't be blocked");
    assert!(matches!(err, crabomination::game::GameError::CannotBlock(_)), "got {err:?}");
}
...
fn flanking_shrinks_nonflanking_blocker() {
    ...
    let attacker = g.add_card_to_battlefield(0, catalog::grizzly_bears());
    let blocker = g.add_card_to_battlefield(1, catalog::grizzly_bears());
    std::sync::Arc::make_mut(g.battlefield_find_mut(attacker).unwrap().definition_mut())
        .keywords
        .push(crabomination::card::Keyword::Flanking);
    g.clear_sickness(attacker);
    g.step = crabomination::game::TurnStep::DeclareAttackers;
    g.active_player_idx = 0;
    g.priority.player_with_priority = 0;
    ...
    g.step = crabomination::game::TurnStep::DeclareBlockers;
    g.priority.player_with_priority = 1;
    g.perform_action(crabomination::game::GameAction::DeclareBlockers(vec![(blocker, attacker)]))
        .unwrap();
...
fn bushido_pumps_attacker_when_blocked() {
    ...
    let attacker = g.add_card_to_battlefield(0, catalog::grizzly_bears());
    let blocker = g.add_card_to_battlefield(1, catalog::grizzly_bears());
    std::sync::Arc::make_mut(g.battlefield_find_mut(attacker).unwrap().definition_mut()).keywords
        .push(crabomination::card::Keyword::Bushido(2));
    g.clear_sickness(attacker);
    g.step = TurnStep::DeclareAttackers;
    g.active_player_idx = 0;
    g.priority.player_with_priority = 0;
...
fn rampage_pumps_attacker_per_extra_blocker() {
    ...
    std::sync::Arc::make_mut(g.battlefield_find_mut(attacker).unwrap().definition_mut()).keywords
        .push(crabomination::card::Keyword::Rampage(2));
    g.clear_sickness(attacker);
    g.step = TurnStep::DeclareAttackers;
    g.active_player_idx = 0;
    g.priority.player_with_priority = 0;
...
fn cr_122_7_nth_counter_threshold_trigger() {
    use crabomination::card::{CardType, CounterType, EventKind, EventScope, EventSpec, TriggeredAbility};
    use crabomination::effect::Predicate;
    let watcher = crabomination::card::CardDefinition {
...
fn cr_701_10b_double_power_is_plus_x_zero() {
    ...
    g.battlefield_find_mut(bear).unwrap().add_counters(
        crabomination::card::CounterType::PlusOnePlusOne, 1); // now 3/3
    ...
    let ctx = EffectContext::for_spell(0, Some(crabomination::game::Target::Permanent(bear)), 0, 0);
...
fn pathway_cycle_faces_tap_for_their_colors() {
    ...
    type PathwayCase = (fn() -> crabomination::card::CardDefinition, Color, &'static str, Color);
...
fn choose_n_suspends_for_wants_ui_and_runs_picked_modes() {
    ...
    let mut g = two_player_game();
    g.players[0].wants_ui = true;
    let spell = crabomination::card::CardDefinition {
        name: "Test Charm",
        card_types: vec![crabomination::card::CardType::Instant],
...
fn riot_trigger_mode_defers_to_resolution_for_wants_ui() {
    ...
    assert_eq!(
        goblin_card.counter_count(crabomination::card::CounterType::PlusOnePlusOne),
        1,
...
fn pay_life_amount_suspends_for_wants_ui() {
    ...
    let spell = crabomination::card::CardDefinition {
        name: "Test Plunge",
        card_types: vec![crabomination::card::CardType::Instant],
        effect: Effect::PayLifeLookTake { who: crabomination::effect::PlayerRef::You },
        ..Default::default()
    };
    let id = g.add_card_to_hand(0, spell);
    g.perform_action(GameAction::CastSpell {
...
fn cr_613_7_eot_grant_after_lose_all_abilities_survives() {
    ...
    use crabomination::effect::shortcut::target_filtered;
    let lose_all = || crabomination::card::CardDefinition {
        name: "Test Frogify",
        card_types: vec![CardType::Instant],
        effect: Effect::LoseAllAbilities {
            what: target_filtered(crabomination::card::SelectionRequirement::Creature),
            duration: crabomination::effect::Duration::EndOfTurn,
        },
        ..Default::default()
    };
    let grant_flying = || crabomination::card::CardDefinition {
        name: "Test Jump",
        card_types: vec![CardType::Instant],
...
fn tribute_question_routes_to_the_opponent() {
    ...
    assert_eq!(
        fan_bf.counters.get(&crabomination::card::CounterType::PlusOnePlusOne).copied().unwrap_or(0),
