...
2. **Gates at the tip, all re-run 2026-09-13 (second run):** suite **19,545 / 0 / 5** (`CRAB_ANSWER_LOG=strict`), clippy **0** (`--workspace --all-targets`), golden_trace **12 / 12 unmoved**, `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant (`games_per_s` 433 on this 4-core box, which compares with nothing in PERF — the COUNTERS are the invariant), `cargo check --profile release-fast` clean, **the 5 skipped are all justified** (1 expensive A/B ladder + 4 diagnostics, each with an `#[ignore = "..."]` reason), `audit_printed_body` **0 on all ten columns** over **21,798** factories with its accounting line all zeroes, `audit_panics` **0 bare**, `audit_doc_drift` **0**, `audit_seat_from_selector` **0 open**, `audit_stash_in_loop` 1/1/0, `audit_card_names` **0/0/0/0**, `audit_variant_coverage` **0 dead capability / 1 dead primitive** (was 2 — Nuclear Fallout is `AddRadCounters`' card; the survivor is closed with a reason in the audit's docstring and in SEVEN source comments), `audit_keyword_drift` **0 invented / 4 missing**, `audit_target_walkers` 132 wrappers, `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean (the ONLY gate that sees `debug-assertions = false`), and ⚠ **`robustness_grid.sh`'s FOUR legs re-run at the closing tip — ladder 30 cells / 33,120 games, pilots 45 cells, actor 3 cells, and the `--wide` ladder 52 cells / 301,600 games (`cap 0 / board 0 / stuck 0 / draw 10`), all 0 failures**, with `check_asserts` reading 8 assertion strings in the actor binary (0 would mean the flag never reached it and the leg was green for nothing). `stats.jsonl` carries `stalls_board` now, verified end to end on a 600-game sealed run: `"stalls":0,"stalls_capped":0,"stalls_board":0,"stalls_stuck":0`. ⚠⚠ **THIS BOX IS 4 CORES** — its `--bench` wall clock (321 games/s) compares with NO absolute in PERF, every one of which was taken on 24. The **counters** are the invariant.
