...
12. **Best next moves, in order.** (a) **Run `(-339)`'s grep on the OTHER zones** — `hand` 80 sites, `graveyard` 36, `exile` 32, `library` 12 hand-written `iter().find(|c| c.id == x)`, and none of those piles has a `find_by_id` at all. The question is whether any is hot enough to want one; a `CardPile` hint cache is the same `(-38)` device. **Grep first, then profile the survivors.** (b) Queue item (E), `pick_blocks_inner`'s body, which is the one hot function `(-319)`'s "do not spend another `profiling-lines` build" verdict does not cover. (c) A fresh-seed block from 1382 as the FIRST gate after any lane/memo change. (d) leftover context: `audit_dropped_may` 199; `audit_keyword_drift`'s last 4 each need a PRIMITIVE; card skips with a card behind them: `nocolors` 950, `noback` 60, `nosubtypes` 43, `nokeywords` 29.
