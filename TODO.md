...
4. **Pod smoke this run, every seat, fresh seeds:** release census 8-seat (seeds 31001+, 6,900 games), 6-seat (32001+, 9,300), 4-seat (33001+, 13,800), 5-seat (37001+, 11,100: two CR 104.4a simultaneous-loss draws) and 7-seat (38001+, 8,100) — **all decided after the fixes, zero panics**; a second round at the later tip (4/6/8 seats, seeds 54001+/56001+/58001+, 30,000 games) found one loop (below) and one CR 104.4a draw; strict debug census 4/6/8 seats (35001+ / 34001+ / 36001+, 1,104 / 744 / 368 games) all decided. `census.sh`-style loop: `--pod-decks` over groups of N, `--games 300`, one seed per group; a non-zero exit is a crash — bisect with `--first/--games`, then `gdb -batch -ex run -ex bt`.
