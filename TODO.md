...
2. **Gates at the closing tip:** suite **20,225 / 0 / 6** (`CRAB_ANSWER_LOG=strict`; +1 on the committed 20,224 is this run's new test, and the count still reads 20,0xx ↔ 20,4xx with the box because the Bevy client holds 200 more); clippy **0** over the workspace; `--bench` **195,806 / 27.49 / 611.9 / 0 stalls — byte-identical**, `determinism ok`, `thread_determinism ok (3 vs 1)`, `peak_rss_mib` 29.2. **Pod, `release-fast`, seed 9901: 1,000 games at each of 4/7/10 seats = 3,000, all decided, every `undecided_by` column zero, zero panics.**
...
5. 🔎 **THE TENTH LEFT EXACTLY ONE AND IT IS A REAL DEFECT — Yuriko's Agony Warp, never cast once in 2,000 games** (a 40-game run at another seed named the same single card, so not variance). The **card is correct**; the **bot cannot reach it**, through two sign-and-slot-blind folds: `trick_modes_combat_only` holds any instant with a temporary stat leaf out of every non-combat step **regardless of sign** (a shrink is removal, not a telegraphed pump), and `pick_combat_trick`'s `pump_amounts` **sums a `Seq` without reading which slot each clause names**, folding `-3/-0` + `-0/-3` into one `(-3,-3)` aimed at our own creature. ENGINE_BACKLOG's sixty-third find has the full diagnosis; INCOMPLETE_CARDS carries the 🟡.
6. 📐 **The general shape there, and the part worth fixing: the HOLD predicate is `any` and the PICK predicate is `all`.** `contains_temp_stat_leaf` matches a tree containing *any* temporary stat change; `all_temp_pumps` matches only one that is *entirely* constant pumps. An instant between the two is held out of every main phase by the first and refused by the second. ⚠ The real fix is **per-slot trick evaluation** and it touches an **adopted, A/B-measured** weight (+3.6 at round 66), so it needs its own A/B — not a drive-by. ✅ But it is **safe to scope**: Agony Warp is in no two-player pool, and the only other multi-slot-pump instant (Dissection Practice) never reaches `pump_amounts` at all, so a fix cannot move a golden trace.
...
10. 💡 **THE OPEN QUEUE.** ① Agony Warp's per-slot trick evaluation (item 6) — the one named defect in a target deck. ② `audit_dropped_may` **208** — the self-cost subset is mined out, so the next cut is a different question. ③ The 69-row non-Aura "invented target slot" census in INCOMPLETE_CARDS (the karoo shape), a divergence only under shroud or a response. ④ COMMANDER_BACKLOG's remaining top-1000 rows (954/1000 in catalog; 173 of the top-300 commanders missing). ⑤ Extend `--card-census` to the two-player ladder, which has no per-card coverage answer at all.
