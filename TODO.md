...
2. **Gates at the `(-334)` tip, all re-run this run:** suite **19,551 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, nextest — 19,549 plus the two new `card_instance` tests), clippy **0** (`--workspace --all-targets --exclude crabomination_client`, and again `--features trig-census`), `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean, `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant with `determinism ok` and `thread_determinism ok`, the 48 `CRAB_DUMP_TRACES` cube games identical across all four A/B binaries, and **24 fresh-seed cells / 118,400 games / 0 failures**. ⚠⚠ **EIGHTH BOX** — its `(-329)` base reads **-0.191 / -0.087 / -0.086 %** against the seventh box's filed absolutes, the same size and the opposite sign to the seventh's gap against the sixth: **deltas and counters cross boxes, absolutes do not.** Timings here, and they are a THIRD of what the seventh box's notes imply: cold `profiling-fast` 9m01s, a `crabomination_base` change 8m52s, an engine-only change **2m54s**, a three-pool callgrind **20 s** in parallel, the debug suite 90 s after a ~5 min build, workspace clippy 5m33s.
3. **Perf — FIVE ROWS TAKEN, cumulative fixed -0.505 / cube -1.230 / sealed -0.629 %.** `(-330)` `CardInstance::combat_keywords`, one pass over the four keyword sources for the eleven-unit-keyword family, replacing eight or nine `has_keyword` asks a candidate in both combat pickers (calls -66..-71 %); `(-331)` `protection_prevents_views`' whole-body guard hoisted out of the (blocker, attacker) pair loop as `GameState::view_has_protection` (calls 100,796 -> 16,820 on `cube`); `(-332)` `empty_mana_pools`' own gate moved into an `#[inline]` wrapper, `(-300)`'s split; `(-333)`+`(-334)` the whole block-legality pair check behind a per-side keyword gate (`blocker_pair_block` calls 72,918 -> 17,528 on `cube`, **-0.604 %** together). Per-row numbers in PERF's Log and Baseline.
4. **The device is a LADDER and all five rows are rungs of it; PERF's candidates has it written out.** It starts with `cg_calls.py` then `cg_edges.py --callers` of every shared-leaf row. `has_keyword` had had a 0.48-0.50 % self row on all three pools for the whole life of this file and nobody had asked whose it was — **87 % of it was two functions**. `protection_prevents_views` is the same shape one level up: a function whose *entire body* is behind one guard is a function whose guard the caller can hoist, and the ratio you get is the caller's loop nesting, not the guard's cost. **Neither was ever in a candidates list**; both were sitting in the self table as themselves, which is why the last session's checklist (still valid, PERF's candidates has it) walked past them.
5. ⚠⚠ **AND THE TRAP IN THE SAME DEVICE: a non-generic `crabomination_base` leaf's CALL is a `release-fast` ARTIFACT.** `release`'s thin LTO inlines it; the A/B profile structurally cannot. `ManaPool::is_empty` (0.31 / 0.17 / 0.23 %), `ManaCost::cmc` (0.28 / 0.24 / 0.31 %), `CardInstance::power` / `counter_count` / `can_block` / `toughness` are all in that class — **rank them by the BODY, never by the row**, or an `#[inline]` reads as a win here and buys the shipped binary nothing. `(-330)` is real because it removed *asks*. `profiling-lto` separates the two and costs a cold build.
6. **Perf — the sized queue is in PERF's "Perf candidates", top first:** (A) `ManaCost::cmc` — ✅ **the filed "needs a sixth `CardMemo` word" blocker is WRONG**, word 1 has bits 55-62 free, but only ~90 k of the 148 k calls want the *printed* cost (`can_afford_in_state_with`'s 42,988 are the relaxed one); (B) `clear_end_of_turn_effects` 0.48 / 0.28 / 0.27 %, already guarded, the 26-field guard IS the row, so the only device left is a dirty bit and that is a `CardData` write to price first; ✅ (C) `blocker_pair_block` is TAKEN as `(-333)`/`(-334)` — the pair count was the lever, exactly as filed; what is left is 4,874 / 17,528 / 14,524 calls at 123-136 Ir, i.e. the pairs a real restriction reaches; (D) the last keyword-ask ratio in `pick_attacks_inner`, **declined at ~0.017 %**, clarity only. Also still unread with the last session's checklist: `resolve_top_of_stack_inner` 0.79 %, `advance_step` 0.66 %, `cast_candidates` 1.04 %.
7. ❌ **Perf — refuted this run with NO BUILD SPENT, and the method is the point: `perform_action_inner`'s five `is_cast()` evaluations.** Reads like `(-322)` at the enum level, worth ~0.31 % if each ask is ~10 Ir — **`objdump` says each ask is THREE instructions** (`movabs` / `bt` / `jae`, a 64-bit discriminant bitmask), so the four redundant ones are 0.09 % and some are CSE'd. **A `matches!` over many variants is a bitmask test, not a walk; disassemble one before ranking a family.** (The same dump shows a **7.4 KB probed stack frame** on that function — no Ir, but it is where part of the 32 MB stack requirement lives.)
8. ❌ **Perf — still closed, do not re-open without a device none of the prior ones was:** `permanent_value_with`'s printed-shape gate (`(-320)`), the `ComputedPermanent` memo lane (`(-312)`), `available_mana` (`(-317)`), the target enumerator, `main_phase_action_with`'s `has_magecraft`/`has_opus`, the three `continuous_effects` duration scans in the SBA sweep, the dispatcher's event fold's `events.is_empty()` half, and the three structural rows `dispatch_triggers_for_events_slow` / `compute_permanent_pass` / `check_state_based_actions_into` (line-profile verdict: FLAT — `(-319)`'s entry says do not spend another `profiling-lines` build on any of them).
9. **Seeds: THREE blocks this run, 36 cells / 177,600 games / 0 failures, `cap 0 / board 0 / stuck 0` on all three** (`draw 4 / 8 / 4`; pools `cube all sealed`, seeds 1366..1369, 1370..1373 and 1374..1377, `target-audit/overflow` with `-C debug-assertions=yes` and `CRAB_ANSWER_LOG=strict`). The first two audit `(-330)`'s keyword mask and `(-331)`'s protection hoist; **the third is the RATCHET for `(-333)`/`(-334)`'s two hand-kept keyword families** — 59,200 dflt games asserting the gate against the full pair check on every pair it skips, on boards the suite never builds and the `gang` trace diff never reaches. **FRONTIER 1378 — read it from PERF's Baseline, not from this line.** A cell is 24-69 s here and a whole block ~10 min, cheaper than one `profiling-fast` build. Standing audits all 0 this run (`audit_panics` 0 bare / 70 sites, `audit_engine_stack` 0, `audit_stash_in_loop` 0 unexplained, `audit_seat_from_selector` 0 open, `audit_target_walkers --check` 0, `audit_doc_drift` 0/0/0).
10. **Bugs/robustness: nothing open, and one latent hole closed.** `KeywordCounters::insert(kw, 0)` stored a zero-valued entry, breaking the invariant `has_tag`'s doc comment and `(-330)`'s `combat_keywords` both rest on ("a stored entry is a granted keyword"); `insert` removes instead now. No behaviour change — `insert` has **no engine or server call site at all**, only tests — so it is the class closed before it bites, not a fix. Structural audits, determinism and the panic census are otherwise clean. `audit_incomplete` / `audit_stubs` are Rust bins and were not re-run — they were 0 at the `(-329)` tip and no card changed.
11. **Best next moves, in order.** (a) **Finish line 4's device**: `cg_calls.py <dump> 200`, take every row whose callee is shared, and ask `--callers` of each — `ManaPool::is_empty`, `counter_count` (225,536 calls), `same_team` (87,764), `defender_for` (74,096), `event_kind_bits` (256,878) are all unexamined, subject to line 5's trap. (b) **`pick_blocks_inner` itself — 0.47 / 1.00 / 0.54 % of SELF, 3,501 Ir a call, and never line-profiled.** Its callees are all gated now, so the body is the row; `(-319)`'s "do not spend another `profiling-lines` build" verdict names three OTHER functions and does not cover this one. Queue item (E). (c) A fresh-seed block from frontier 1374, as the FIRST gate after any lane/memo change. (d) leftover context: `audit_dropped_may` 199; `audit_keyword_drift`'s last 4 each need a PRIMITIVE; card skips with a card behind them: `nocolors` 950, `noback` 60, `nosubtypes` 43, `nokeywords` 29.
