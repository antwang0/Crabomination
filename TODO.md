...
1. **Where it stands:** every row in FEATURE_ROADMAP's **"Commander status"** is ✅ — CR 903 core, the 800-series, the whole variant-mechanic list including Doctor's companion. **Read that table, don't re-audit.** Per-deck completion is DECK_FEATURES' "Commander target decks": **5/5 complete**, the fifth being the Partner seat.
...
5. **This run shipped, in order:** CR 106.6 mana provenance (`SpendRestriction::is_rider` names the class; `spell_kind_for` fills `SpellKind.commander`; riders land in `game/effects/commander.rs`) + Path of Ancestry / Opal Palace + the bug that `cast_from_command_zone` paid with an empty `SpellKind` so **no** restricted mana could ever fund a commander cast; the four decks taking both staples; CR 702.124m Doctor's companion + The Third Doctor / Graham O'Brien; CR 106.6a's per-mana rider count; CR 903.4's colourless-identity case; the Krark/Rograkh Partner deck + CR 702.124b/d tests.
6. 💡💡 **THE TOP LEAD IS PER-OPPONENT TARGETING, AND IT IS PRICED: 8 implemented cards** (the five Primordials, Grasp of Fate, Omega Heartless Evolution, Tempted by the Oriq), all shaped "for each opponent, [verb] target X that player controls". `Effect::ForEachOpponent` cannot give it — the body would need its own target chosen per iteration as the spell is cast (CR 601.2c). ⚠ **The blocker is DYNAMIC SLOT ARITY, not the fan-out**: a target slot is declared statically by `Selector::TargetFiltered { slot, filter }` in the card literal, and "one slot per opponent" is a count only the game knows. `CapTargetsAt { amount }` + `ApplyToTargets` gives the right *count* and the wrong *distribution* (three targets, possibly all one opponent's). A correct version needs a per-slot "controlled by the Nth opponent" filter **and** a variable slot census, which touches the `requires_target` / `target_filter_for_slot` / `auto_target` / bot-candidate family — the exact walker family this file calls a bug class. Budget a whole run; do not start it with two hours left.
...
9. **Guardrail at the tip `51551a7f`:** `--bench` invariant **195,806 / 27.49 / 611.9 / 0 stalls**, `determinism ok`, `peak_rss_mib` 25.4 — byte-identical across five more Commander rules changes. Suite **19,683/19,683**, clippy clean over the workspace. ⚠ `games_per_s` read 470-505 over five builds on one box at one calib: that spread is the noise floor and the invariant carries the verdict. The pod's own determinism check is free: `--threads 1` vs the default, identical in every column.
