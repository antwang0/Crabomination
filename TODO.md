...
3. **The instrument that pays:** the pod run is where four bugs came from in one sitting. Run it before and after anything Commander-shaped; a stall rate that moves is a bug, not noise. **Tip reading: PODNUMBERS** It was **15 % "no legal move"** before the CR 800.4a pending-decision fix. `CRAB_CAP_DIAG=1` names an undecided pod game's board. ⚠ Read **turns**/game, not actions/game, to tell "different games" from "fewer pointless actions".
...
9. **Guardrail at the tip:** `--bench` invariant BENCHNUMBERS ⚠ games/s does **not** cross hosts (this box is a 2.10 GHz Xeon); PERF's Commander block says what to compare. The pod's own determinism check is free and is `--threads 1` vs the default.
...
15. **Where the new cards live:** `crabomination_catalog/src/sets/cmdr.rs` — cards whose text is *about* CR 903, not about their set. Register in `all_factories.rs`. ⚠ **Latent, shared with `TemptingOffer`:** a multi-seat ask arm clears the replay log before running its body, so a body that *itself* suspends (a `wants_ui` seat's search) replays the arm from the top with an empty log and re-asks — and, for `JoinForces`, re-pays. No bot seat sets `wants_ui`, so no self-play game can reach it; a UI pod can. Fixing it is one fix for both arms.
