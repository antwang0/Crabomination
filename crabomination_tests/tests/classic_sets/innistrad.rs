...
fn wanderlight_spirit_blocks_only_flying() {
    ...
    assert!(
        !crabomination::game::can_block_attacker_computed(binst, &bcomp, || false, None, &[], crabomination::mana::ColorSet::empty(), 2),
        "can't block a ground attacker"
    );
    assert!(
        crabomination::game::can_block_attacker_computed(binst, &bcomp, || false, None, &[Keyword::Flying], crabomination::mana::ColorSet::empty(), 2),
        "can block a flyer"
    );
}
