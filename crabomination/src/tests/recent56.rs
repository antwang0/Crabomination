...
fn angel_1_1() -> crate::card::CardDefinition {
...
fn youthful_valkyrie_grows_on_another_angel() {
    ...
    drain_stack(&mut g);
    assert_eq!(
        g.battlefield_find(val).unwrap().counter_count(crate::card::CounterType::PlusOnePlusOne),
        1,
...
fn twinblade_paladin_grows_and_gains_double_strike() {
    ...
    gain_life(&mut g, 0, 1);
    assert_eq!(
        g.battlefield_find(pal).unwrap().counter_count(crate::card::CounterType::PlusOnePlusOne),
        1,
...
fn celestial_unicorn_and_gideons_company_grow_on_life_gain() {
    ...
    gain_life(&mut g, 0, 1);
    let p1p1 = crate::card::CounterType::PlusOnePlusOne;
...
fn sunscorch_regent_grows_and_gains_on_opponent_spell() {
    ...
    drain_stack(&mut g);
    assert_eq!(
        g.battlefield_find(reg).unwrap().counter_count(crate::card::CounterType::PlusOnePlusOne),
        1,
