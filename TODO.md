...
1. ⚠⚠ **TWO SESSIONS SHARE THIS BRANCH. FETCH BEFORE YOU START AND BEFORE EVERY PUSH, AND RE-RUN THE SUITE AFTER EVERY REBASE.** `cargo nextest` fail-fast hides the rest — use `--no-fail-fast`. Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure"**.
2. **Gates at the closing tip:** suite **19,750 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); clippy **0** over the workspace; `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean; `--bench` **195,806 / 27.49 / 611.9 / 0 stalls**, `determinism ok` — byte-identical to the committed invariant at **five** separate tips across this run, including the two that changed six `cube`/SOS pool cards. Pod smoke at the tip: **3,000 games at 4 and 5 seats, fresh seed 2007, 100 % decided, 0 stalls** (41.51 / 52.80 turns a game). Assertions sweep (`overflow` + `-C debug-assertions=yes`): **8,000 pod games at 2/3/4/5 seats, fresh seeds 2005-2006, 0 assertions**, plus the two-player grid on fixed/cube/sos/all at seed 2005 (4,080 games, clean) — **FRONTIER 2008**. Ir guardrail, the clean single-commit A/B (`3964e1c5` → `a09f4704`): **fixed +0.025 / cube +0.030 / sealed +0.025 %** — all three together, i.e. a codegen row, which is what a widened key and a `bool` parameter are. PERF's Baseline has the block.
3. **Where it stands:** FEATURE_ROADMAP's **"Commander status"** is the map and it is current — CR 903 core, the 800-series (now incl. **800.4f/g**, **800.4a's effect-elimination half** and **800.4c**), the whole variant list. **Every 800-series row is now ✅** — the one remaining ⚠ in that table is a caveat *inside* the 800.4f/g/h row (an ask already materialized when its seat leaves is dropped, not re-seated), not a row with no arm. **Read that table, don't re-audit.** DECK_FEATURES: 5/5 decks complete; the Commander free-spell cycle is complete. **What 800.4 still approximates, on purpose:** an ask **already materialized** when its seat leaves is dropped, not re-seated — the legal set would have to be re-derived against the post-departure board, which is a different capability. Rare now that `route_ask` stops one being posed to a departed seat at all. 800.4i's last-known-information clause is unmodelled.
4. **The runner:** `crabomination::pod` + `bot_ladder --commander [--seats N] [--games N] [--seed N] [--threads N]`. `pod::tests` validates every list every run and now also gates the chunk split (`cr_903_a_pod_run_is_independent_of_how_it_is_chunked` — the pod's half of `thread_determinism`). ⚠ The Partner deck is **last** on purpose: `--seats 5` is what reaches it.
5. 🔎🔎 **THE METHOD THAT CLOSED THE FORTY-SECOND FIND: WHEN A DUMP AND AN AUTHORITY DISAGREE, CHECK WHAT THE DUMP IS NOT PRINTING.** "An activation completed with no mana anywhere" was **the estimate, not the engine** — the pool held a Path of Ancestry *rider* pip and `ManaPool::total`/`amount`, which the blame string prints, exclude the restricted bucket wholesale. Two regression tests saying "the engine rejects this in a duel" made it look state-dependent and pointed at `stack=3`; it was not. One `eprintln!` of the **whole** `ManaPool` at the payment site named it in the first run.
6. 🔎🔎 **AND THE METHOD THAT FOUND THREE MORE: RUN THE ASSERTIONS BUILD OVER PODS.** `RUSTFLAGS="-C debug-assertions=yes" CARGO_TARGET_DIR=target-audit cargo build --profile overflow -p crabomination --bin bot_ladder` (Cargo.toml's recipe, ~8 min) then `--commander --seats N --games 1000`. ⚠⚠ **`rc=$?` AFTER A PIPE READS THE PIPE'S STATUS, NOT THE BINARY'S** — an abort (134) reads as 0 and a whole sweep looks clean. **Redirect to a file, then grep.** It cost one sweep this run before anyone noticed, and it is how the layer-5 colour gate was found on the first 1,000 games.
7. **This run shipped (two sessions, one branch — theirs and mine interleaved):** the rider-mana estimate fix and the ranked auto-targeter; **CR 800.4f/g**, **800.4a**'s effect-elimination half, **CR 102.1/800.4a**, **800.4c**; **CR 613.5**'s colour gate; the per-player-loop suspend guards; **CR 615**'s filtered all-damage shield + **Obscuring Haze**; the pod chunk-determinism gate. And, this session: **CR 603.2c**'s subject-keyed damage batch (`combat::BatchSubject`), **30 cards** across its **four** clauses with **four ratchets** to hold them (and `CounterAdded` closed as a non-question — the event carries a count, so its eleven candidates were never gaps), **CR 800.4e/k** and **CR 903.10a**'s two persistence clauses getting the tests the roadmap claimed without one, and the **Judith retune**.
8. ✅✅ **DECK BALANCE IS ANSWERED, AND THE ANSWER IS A METHOD, NOT A LIST.** Judith 6.2 → **14.6 %** of four-seat pods (field 43.5/6.2/22.2/28.0 → **40.8/14.6/20.3/24.3**, 3,000 games a configuration at seed 43). ⚠⚠ **And the obvious repair made it WORSE, which is the reusable part:** a real aristocrats engine — free sac outlets, drain payoffs, recursive fodder, 35 of 72 nonbasics — read **6.3 %** at four seats and **16.8 %** head to head, *below the filler list it replaced*. `pick_sacrifice_value` takes a sacrifice only when `eval_material` improves, and a one-ply material evaluator cannot price a value engine: a body for a scry, or for one life off each opponent at 40, is material-negative every time it is asked. **The evaluator is NOT two-player-shaped** — it already sums over every live hostile seat — **so this is a horizon limit, not a seat-count bug, and pod lists must be built out of what a greedy evaluator can price**: removal, card advantage, fat, and per-opponent effects that resolve in one shot. **The number now is Krark/Rograkh at 8.1 %** of five-seat games.
9. 💡 **AND THE CARD LEAD IS NOW UNBLOCKED, WHICH IS ITSELF THE FIND.** The target-deck defect list was filed as "eight cards, all in the `cube` pool, take them as one batch with one bench reading" — **five were fixed and not one moved `--bench` or a golden trace** (195,806 / 27.49 / 0 stalls, byte-identical after every one). Pool *membership* is not pool *reach*: the bench plays `--decks fixed` and the traces are fixed-seed games, and a card has to be drawn and played in one to move a number. ⚠⚠ **And FOUR OF THE FIVE needed no engine change at all** — Wall of Roots' `CounterType::MinusZeroMinusOne` and Obstinate Baloth's `opponent_discard_deploys` both already existed and the card simply was not using them, and Sowing Mycospawn's nerf was one word. Spark Double is the one that did — a copy of a planeswalker was entering with **no** loyalty and dying to the first SBA sweep, because the entering counters are seeded off the printed line before the copy rewrites it. ⚠ **Two of the eight rows were stale besides** (Simian Spirit Guide and Arcane Signet had both shipped). **Read the definition before believing the row, and grep for the primitive before building one.**
10. **Cards and decks, what is left:** ⏳ 32 of 33 "can be your commander" non-creatures are unimplemented — *cards are chosen to complete decks*, so they wait on a sixth target deck wanting one. ⏳ `audit_each_opponent`'s residuals are down to **six** (Malcolm closed: the subject-keyed batch makes "a Treasure for each opponent dealt damage" exact); none of the six is in a target deck. ✅✅ **CARD_BACKLOG's "Target-deck card defects" table is EMPTY** — six fixed, two never broken, and **five of the six needed no new primitive**: the counter type, the discard-replacement field, the `non_legendary` flag, the plural exile selector, the keyword-counter effect and the base-P/T + add-type effects all existed and the card was not using them. `--bench` byte-identical after every one. ⚠⚠ **FOUR of the eight claims were wrong about the code, including a sizing this run wrote itself** — Ghost Vacuum was filed as "three missing pieces, the only real build" and needed none of them. **Read the definition before believing the row, and grep for the primitive before sizing the build.**
11. ⚠ **The per-player-loop suspend is now GUARDED, not fixed.** `ForEachOpponent` and `TemptingOffer` park `body` as the continuation, so a suspend in iteration *k* drops *k+1..n*; splicing needs each remaining iteration to pin a *different* player and that rides on `EffectContext`, which no `Effect` carries. Zero catalog bodies can suspend today, so the assertions are the price of the decline — the first card that can will hit them on the first pod game. Forbidden Ritual's `tail_after` (`effects/mod.rs` ~3965) is the splice to copy when one arrives.
12. ⚠⚠ **POOL MEMBERSHIP IS A MEASUREMENT, NOT A VETO — AND THAT CORRECTS THE PREVIOUS HANDOFF.** A cube/SOS card's behavior change *can* move the committed bench aggregate and a golden trace, so the rule is "change it, then run `--bench`", not "do not change it". Six pool cards changed this run across two commits and the aggregate came back **byte-identical** both times — several cards leaving one graveyard at once, or three unblocked attackers into one seat, are rare in those pools. The Commander-side argument is unchanged and still has to be made out loud in the commit: with no commander seated, a Commander change must be duel-identical.
13. ⚠⚠ **AND WHEN THE COMMITTED TABLE AND THE AGGREGATE DISAGREE, THE AGGREGATE WINS.** `cr_903_seeded_pod_outcomes_match_the_committed_table` is a **re-bless gate, not a measurement**: three games said the ranked auto-targeter lengthened pods and 32,000 said every configuration got **shorter**. Read **turns**/game, and read the **two-seat row as the control** — it is byte-identical when a change is duel-safe.
14. 📐 **The N-seat audit method:** read every deck card's Scryfall text for "each opponent / each other player / their first / that player" and its body for a fan-out ref. Esper Sentinel modelled "their first noncreature spell each turn" as `once_per_turn` — exact at two seats, one fire per *table* in a pod. 📐 And **where a trigger functions is a property of the ABILITY, not the card**.
15. ⚠ **Five gotchas that each cost a debugging round.** ① A `ScriptedDecider` answers in ASK ORDER — to answer one question and leave the rest, write a four-line `Decider` matching that `Decision` variant and deferring to `AutoDecider`, and note `AutoDecider` **declines every `OptionalTrigger`**. ② A "target player" slot **auto-picks an OPPONENT** (`prefers_friendly_target` declares the shape). ③ `Selector::TriggerSource` binds the **damaged** player at resolution while a trigger's `filter` is evaluated against the **dealer** at gather time; it is **not** a bug class. ④ New Commander cards go in `catalog/src/sets/cmdr.rs`, and pool membership is an **explicit curated list** in `cube.rs`, not the file a card lives in — a card added elsewhere and not listed there stays out of `Vocab`. ⑤ `add_card_to_battlefield` does **not** run `as_enters_effect`, so a dynamic-P/T creature placed that way is a 0/0 that deals no damage; cast it.
