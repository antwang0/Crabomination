...
2. **Gates at the `928ccbca` tip, all re-run:** `--bench` **195,806 / 27.49 / 611.9 / 0 stalls / peak_rss 25.1** byte-identical to the committed invariant across all six commits, `determinism ok`, `thread_determinism ok (3 vs 1)`; suite **19,592 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, 12 golden traces unmoved); clippy **0**; `cargo check --profile release-fast` clean. Sweep: **24 cells / 118,400 games / 0 failures** over seeds 1398..1405, `stuck 0 / draw 18 / cap 2` (both `cube` MAX_BATTLEFIELD runaways on a saturated seat — the script's carve-out), **FRONTIER 1406**. `audit_stubs` 0 of 21,797, `audit_panics` 0 bare, `audit_doc_drift` 0/0/0.
...
6. 📐 **`effective_power_on` / `effective_toughness_on` EXIST NOW AND THEY RE-PRICE THE THIRTY-FOURTH FIND.** `Value::PowerOf`'s fix cost `fixed +0.118 %` for ONE read because `effective_power` pays a by-id find per call. **Seventeen computed reads through the `_on` form cost +0.001 / +0.022 / +0.001 % between them.** Use `_on` whenever the caller is already walking the battlefield.
7. ⚠⚠ **A RULES FIX CAN COST 0.6 % OF `cube` AND `--bench` WILL NOT SAY SO.** The card-walker gate read cube **+0.636 %** with `--bench` and all 144 golden traces byte-identical. 📐 **Price the ROUTING separately from the walk:** two-thirds of that was the entry (`battlefield_find` + `PrintedGates` per *ask*), which `(-343)` took back (**cube -0.279 %**) with 13 mechanical hoists. Run total: **fixed +0.025 / cube +0.137 / sealed +0.080 %** for three class fixes.
...
13. **Tip absolutes at `928ccbca` (`profiling-fast --no-default-features`, system allocator): fixed 579,810,595 / cube 1,512,328,349 / sealed 1,633,742,099.** ✅ The twelfth box's closing absolutes **reproduced to six significant figures**. **"Absolutes do not cross boxes" is about the CPU MODEL, not the box** — same shape and toolchain agree to noise.
