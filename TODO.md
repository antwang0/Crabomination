...
1. ⚠⚠ **TWO SESSIONS SHARE THIS BRANCH AND ONE OF THEM LEFT IT RED FOR FOUR COMMITS.** `0320e1cb` (CR 800.4d, correct) removed a departed player's triggers from the stack and left `apnap_skips_eliminated_seat_in_cycle` asserting the behaviour it removed, comment and all. **Run the full suite after every rebase onto a concurrent commit, not only after your own**; a `cargo check` is not enough and neither is your own binary's tests. **Where it stands:** every row in FEATURE_ROADMAP's **"Commander status"** is ✅, per-opponent targeting included — CR 903 core, the 800-series, the whole variant-mechanic list including Doctor's companion. **Read that table, don't re-audit.** Per-deck completion is DECK_FEATURES' "Commander target decks": **5/5 complete**, the fifth being the Partner seat.
2. **The runner:** `crabomination::pod` + `bot_ladder --commander [--seats N] [--games N] [--seed N]`. Five legal 100-card decks in `pod/decks.rs` (Sigarda GW / Judith BR / Hanna UW / Tatyova GU / **Krark+Rograkh R, two commanders**); `pod::tests` runs each through `validate_commander_deck` every run, so an illegal list cannot land. ⚠ The Partner deck is **last** on purpose: `pod_field(4)` never draws it, so the committed outcome table is untouched by its existence. `--seats 5` is what reaches it.
3. **The instrument that pays:** the pod run is where six bugs have come from. Run it before and after anything Commander-shaped; a stall rate that moves is a bug, not noise. **Tip reading at `51551a7f`, 2,100 games / six configs / 2-3-4-5 seats, all 100 % decided, zero panics — the table is in PERF's third-Commander-session block.** `CRAB_CAP_DIAG=1` names an undecided pod game's board. ⚠ Read **turns**/game, not actions/game, to tell "different games" from "fewer pointless actions" — and read the **committed table**, not the aggregate: CR 106.6a moved seed 43's 400-game aggregate by 0.1 actions and moved none of the three committed seeds.
...
5. **This run shipped, in order:** CR 106.6 mana provenance (`SpendRestriction::is_rider` names the class; `spell_kind_for` fills `SpellKind.commander`; riders land in `game/effects/commander.rs`) + Path of Ancestry / Opal Palace + the bug that `cast_from_command_zone` paid with an empty `SpellKind` so **no** restricted mana could ever fund a commander cast; the four decks taking both staples; CR 702.124m Doctor's companion + The Third Doctor / Graham O'Brien; CR 106.6a's per-mana rider count; CR 903.4's colourless-identity case; the Krark/Rograkh Partner deck + CR 702.124b/d tests; **CR 601.2c per-opponent targeting** (item 6) across eight cards; two card bugs the new `audit_each_opponent.py` found (Aria of Flame gave *you* the 10 life its drawback gives each opponent; Pharika's Spawn was missing its escape edict); CR 903.5e (`validate_deck_refs` takes a flat list and so never saw a sideboard).
6. ✅ **PER-OPPONENT TARGETING SHIPPED, and the shape is the transferable part: when the RULE is about the chosen SET, say it as a constraint on the set, not as slots.** "For each opponent, … up to one target X that player controls" (8 cards) is not expressible as one target slot per opponent — a slot is declared statically by the card literal and the count is only known at run time. `Effect::ForEachOpponentTarget` is instead a transparent wrapper meaning "at most one target per controller, at most one per opponent", enforced in the same three places CR 115.3's distinct-*object* rule is (picker, cast path, resolution). ⚠ **A transparent wrapper is cheap to add *because* the walker family is policed**: `requires_target` is exhaustive (compiler), `for_each_inner` has `the_shared_recursion_names_every_effect_wrapper`, and `scripts/audit_target_walkers.py` reads the same before and after. Add one and run that script. ⚠ **And read your new walker for hot-path cost**: `per_opponent_targets` fell through to `for_each_inner` at first, which is a whole-tree walk **on every cast**.
7. **Cards, what is left:** ✅ the colourless staples are in all five decks and each two-colour deck runs its guild Signet + Talisman. ⏳ 32 of 33 "can be your commander" non-creatures are unimplemented; the flag is done, so each is a plain card. ⏳ **`scripts/audit_each_opponent.py` is the card-side sweep and it is down to seven survivors**, all named in INCOMPLETE_CARDS with what they need — the biggest being Malcolm's *batched* damage trigger (wrong at two seats too; needs a batch event kind) and Kaya's −2 (a fixed own-creature slot 0 beside per-opponent slots, which `ApplyToTargets` cannot express).
...
9. **Guardrail at the closing tip `9c6d5204`:** `--bench` invariant **195,806 / 27.49 / 611.9 / 0 stalls**, `determinism ok`, `peak_rss_mib` 25.6 — byte-identical across **every commit of this session**: six Commander rules changes, a targeting-core change, two deck edits, two card bug fixes. Suite **19,699/19,699**, clippy clean over the workspace, `cargo check --profile release-fast` clean (the debug-assertions-OFF gate), pod **100 % decided at 2/3/4/5 seats**, `--threads 1` identical. 🔎 **The run's one stall was a deck, not the engine, and the pod named it in a line**: a fresh 5-seat seed read 0.3 % board-cap undecided and `CRAB_CAP_DIAG=1` printed **1,486 Goblins** — Krenko, Mob Boss doubles every activation. Goblin King replaced it and 1,600 five-seat games over four seeds are clean. **Never put an unbounded doubler in a smoke-test deck.** 📐 **Judith (BR) wins 4-5 % of four-seat pods and the Signet/Talisman upgrade did not move it** — that deck's problem is its plan, not its mana; start there before adding fixing. ⚠ `games_per_s` read 470-505 over five builds on one box at one calib: that spread is the noise floor and the invariant carries the verdict. The pod's own determinism check is free: `--threads 1` vs the default, identical in every column.
10. ⚠⚠ **Golden traces are the Commander guardrail too, and the test is POOL MEMBERSHIP.** With no commander seated every Commander change must be byte-identical in a duel. Two mana-path changes this run could have broken that and did not, for the same reason: `commander_identity_colors` keeps the any-colour answer for a seat with **no** commander (the colourless-*commander* case is now correctly "no mana, not {C}" — different question, don't merge them again), and `is_rider()`'s auto-tap only reaches cards none of which are in `cube::cube_pool_all()`. **Check the pool before assuming a mana-path change is duel-safe.**
11. 📐 **The N-seat audit method:** read every deck card's Scryfall text for "each opponent / each other player / their first / that player" and its body for a fan-out ref. Esper Sentinel modelled "their first noncreature spell each turn" as `once_per_turn` — exact at two seats, one fire per *table* in a pod; Adeline made one attacking token where "for each opponent … that player" wants one each. Both bugs are invisible at two players and to a per-card test written at two players.
12. ⚠ **A `ScriptedDecider` answers in ASK ORDER, and the engine's ask order is not the test's business.** When a test needs one question answered and the rest left alone, write a four-line `Decider` that matches that `Decision` variant and defers to `AutoDecider` — `SayYesToOptional` in `core_rules/multiplayer.rs` is the pattern. ⚠ And the `AutoDecider` **declines every `OptionalTrigger`**, so a `MayDo` body is dead under it.
13. 📐 **Where a trigger functions is a property of the ABILITY, not the card** (Oloro's two upkeep triggers differ); where a *static* functions is card-level here, because `StaticAbility` is `{description, effect}` in 2,275 literals with no builder and no printed card splits them. A card that does split them gets the per-ability field.
14. ⚠ **Two binding gotchas, both cost a debugging round.** ① A "target player" slot **auto-picks an OPPONENT**, so a targeted gift (the Partner-with fetch) silently went to the wrong seat and looked like "nothing happened" — `prefers_friendly_target` declares the shape. ② `Selector::TriggerSource` binds the **damaged player** at resolution on a dealer-side damage kind, but a trigger's `filter` is evaluated against the **dealer** at gather time, so `with_filter(EntityMatches{TriggerSource,…})` and `dealt_by(…)` both gate correctly — twenty catalog cards use the first and it is **not** a bug class.
15. **Where the new cards live:** `crabomination_catalog/src/sets/cmdr.rs` — cards whose text is *about* CR 903. Register in `all_factories.rs`; ✅ that does **not** touch the net vocab (`Vocab`/`encode.rs` read `cube::cube_pool_all()`, a curated list, not the catalog registry), so a Commander card is free. ⚠ **`cargo check --all-targets` is the gate that sees the bins** — a new `Keyword` variant passed 19,676 tests and left `dump_cards`'s exhaustive match uncovered. ⚠ **Latent, shared by `JoinForces`/`TemptingOffer`:** a multi-seat ask arm clears the replay log before running its body, so a body that *itself* suspends replays the arm from the top and re-asks. No bot seat sets `wants_ui`, so no self-play game can reach it; a UI pod can. One fix for both arms.
