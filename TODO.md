...
11. **Perf queue: (J)/(K) CLOSED as diffuse, (L) = `(-344)`, (M) = `(-345)` + `(-346)` with its owed half refuted as `(-347)`, (A) = `(-348)`.** Open: **(B)** the `CardCold` `Arc::ptr_eq` gate (the one shape needing no write-site change, ~0.12 % of `cube`, and **price its hit rate first — the gate DEGRADES over a game**), (C) `blocker_pair_block`'s pair count, (H) the other zones' by-id scans. ❌ `call_mut` at 1.23 % of `cube` is **not** a lead: 98.6 % of it is std's own `Filter` fold and a no-LTO artifact — PERF's candidates head has the read. ⚠ **Both (J)/(K) reads cost NOTHING:** `cg_lines.py` needs no `profiling-lines` build — one 16-second `--dump-instr=yes` callgrind line-profiles every body at once. **Line-profile a candidate before building for it.**
