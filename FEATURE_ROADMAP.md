...
| Per-deck card coverage in a pod run | ✅ | `bot_ladder --commander --card-census` — totals the action census by card and names, per seated deck, what a run never cast, played or activated. First run (10 seats, 2,000 games): **nine of the ten target decks played every card in the list**; the tenth left one, Yuriko's Agony Warp, which the bot cannot reach (sixty-third find) |
