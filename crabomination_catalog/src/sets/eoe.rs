...
use crate::card::{
    ...
    SelectionRequirement, StaticAbility, StationBand, Subtypes, TokenDefinition, TriggeredAbility,
    WardCost,
};
...
pub fn biomechan_engineer() -> CardDefinition {
    CardDefinition {
        ...
        triggered_abilities: vec![etb(Effect::CreateToken {
            who: PlayerRef::You,
            count: Value::Const(1),
            definition: std::sync::Arc::new(lander_token()),
        })],
...
pub fn lumen_class_frigate() -> CardDefinition {
    ...
    CardDefinition {
        ...
        station: vec![
            StationBand {
                min: 2,
                statics: vec![StaticEffect::PumpPT {
                    applies_to: others,
                    power: 1,
                    toughness: 1,
                }],
        ...
        triggered_abilities: vec![
            ...
            TriggeredAbility {
                event: EventSpec::new(EventKind::CreatureDied, EventScope::YourControl)
                    .with_filter(Predicate::EntityMatches {
                        what: Selector::TriggerSource,
                        filter: SelectionRequirement::WithCounter(CounterType::PlusOnePlusOne).and(
    ...
    CardDefinition {
        ...
        triggered_abilities: vec![TriggeredAbility {
            ...
            effect: Effect::CreateToken {
                who: PlayerRef::You,
                count: Value::Const(1),
                definition: std::sync::Arc::new(robot_2_2()),
            },
        }],
        ..Default::default()
    }
    ...
    CardDefinition {
        ...
        effect: Effect::Seq(vec![
            Effect::Move {
                what: target_filtered(
                    SelectionRequirement::InYourGraveyard
                        .and(SelectionRequirement::Creature.or(
                            SelectionRequirement::HasArtifactSubtype(ArtifactSubtype::Spacecraft),
                        ))
...
pub fn hemosymbic_mite() -> CardDefinition {
    CardDefinition {
        ...
        card_types: vec![CardType::Creature],
        subtypes: Subtypes {
            creature_types: vec![CreatureType::Mite],
            ..Default::default()
        },
        power: 1,
        toughness: 1,
...
pub fn emissary_escort() -> CardDefinition {
    CardDefinition {
        ...
        power: 0,
        toughness: 4,
        dynamic_pt: Some(DynamicPt::BasePlusGreatestOtherArtifactMv {
            base_p: 0,
            base_t: 4,
        }),
        ..Default::default()
        ...
        effect: Effect::Seq(vec![
            Effect::If {
                ...
                then: Box::new(Effect::CreateToken {
                    who: PlayerRef::You,
                    count: Value::Const(1),
                    definition: std::sync::Arc::new(drone),
                }),
                else_: Box::new(Effect::Noop),
            },
            Effect::Move {
...
pub fn seedship_broodtender() -> CardDefinition {
    CardDefinition {
        ...
        activated_abilities: vec![ActivatedAbility {
            ...
            sorcery_speed: true,
            effect: Effect::Move {
                what: target_filtered(SelectionRequirement::InYourGraveyard.and(
...
fn eoe_planet(name: &'static str, color: crate::mana::Color) -> CardDefinition {
    CardDefinition {
        ...
        subtypes: Subtypes {
            land_types: vec![LandType::Planet],
            ..Default::default()
        },
...
pub fn evendo_waking_haven() -> CardDefinition {
    ...
    c.station = vec![planet_mana_band(
        cost(&[g()]),
        ...
        activated: vec![ActivatedAbility {
            ...
            effect: Effect::Seq(vec![
                Effect::CreateToken {
                    who: PlayerRef::You,
                    count: Value::Const(1),
                    definition: std::sync::Arc::new(robot_token()),
                },
                Effect::PumpPT {
                    what: team(),
                    power: Value::Const(1),
    ...
    CardDefinition {
        ...
        triggered_abilities: vec![etb(Effect::Search {
            ...
            filter: SelectionRequirement::IsBasicLand
                ...
                .or(SelectionRequirement::Creature.and(SelectionRequirement::ManaValueAtMost(1))),
            to: ZoneDest::Hand(PlayerRef::You),
        })],
        alternative_cost: Some(warp(cost(&[generic(1), w()]))),
        ..Default::default()
...
pub fn mutinous_massacre() -> CardDefinition {
    ...
    CardDefinition {
        ...
        effect: Effect::ChooseN {
            picks: vec![0],
            modes: vec![mode(true), mode(false)],
        },
        ..Default::default()
    }
}
    ...
    CardDefinition {
        ...
        activated_abilities: vec![ActivatedAbility {
            ...
            effect: Effect::CreateToken {
                who: PlayerRef::You,
                count: Value::Const(1),
                definition: std::sync::Arc::new(tapped_robot),
            },
            ..Default::default()
        }],
        ..Default::default()
    ...
    CardDefinition {
        ...
        triggered_abilities: vec![TriggeredAbility {
            ...
            effect: Effect::CreateToken {
                who: PlayerRef::You,
                count: Value::Const(1),
                definition: std::sync::Arc::new(munitions),
            },
        }],
        ..Default::default()
    }
...
pub fn temporal_intervention() -> CardDefinition {
    CardDefinition {
        ...
        static_abilities: vec![StaticAbility {
            ...
            effect: StaticEffect::SelfCostReducedIf {
                condition: Predicate::VoidActive {
                    who: PlayerRef::You,
                },
                amount: 2,
            },
    ...
    CardDefinition {
        ...
        effect: Effect::CounterUnlessPaid {
            ...
            if_paid: Some(Box::new(Effect::CreateToken {
                who: PlayerRef::You,
                count: Value::Const(1),
                definition: std::sync::Arc::new(crabomination_base::tokens::lander_token()),
            })),
        },
        ..Default::default()
    }
...
pub fn infinite_guideline_station() -> CardDefinition {
    ...
    CardDefinition {
        ...
        triggered_abilities: vec![etb(Effect::CreateToken {
            ...
            definition: std::sync::Arc::new(eoe_robot_token(true)),
        })],
        activated_abilities: vec![station()],
        station: vec![StationBand {
            min: 12,
