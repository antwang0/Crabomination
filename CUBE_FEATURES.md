...
|---|---|---|
| Equipment + equip-cost ability | 🟡 | `GameAction::Equip` + `equipped_bonus` (Shuko, Lavaspur Boots). Board-scaled bonus (`EquipBonus.scale` — Nettlecyst), equipment-granted triggers (`EquipBonus.triggered_abilities` — Sword of Body and Mind), Reconfigure (`Keyword::Reconfigure`, attach/unattach — Lion Sash), living weapon (Batterskull), and equip-triggers that charge the *Equipment* (`triggers_on_equipment` — Umezawa's Jitte, charges on combat damage to player and creature) all ship. |
