...
1. **FIRST:** `git fetch origin claude/modern_decks && git checkout -B claude/modern_decks origin/claude/modern_decks`. Rebase, never force; code before tracker prose; **fetch before every push AND before you start** — two sessions ran on this branch today and one of them filed the other's rows while it was still measuring. Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure" + "Standing rules"**, ⚠ the 2026-09-12/13 detail (printed-body skip taxonomy, the `EventKind` fan-out census, the modal census, the collision log, the seed discipline) is in **ENGINE_BACKLOG's "The 2026-09-12/13 handoff detail, moved verbatim"** — read it before taking a skip kind or a seed. NEXT is 15 lines and stays 15.
2. **Gates at the `(-329)` tip, all re-run this run:** suite **19,549 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, nextest), clippy **0** (`--workspace --all-targets --exclude crabomination_client`, and again `--features trig-census`), golden traces unmoved, `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean, `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant with `determinism ok` and `thread_determinism ok`. ⚠⚠ **TWO SESSIONS, TWO BOXES TODAY** — `(-320)`/`(-321)` on the sixth, `(-322)`..`(-329)` on the seventh, whose base reads **+0.19 / +0.08 / +0.08 %** against the filed `(-321)` absolutes while `blocker_self_block`'s `cube` row reproduces to the digit: the gap is the container, so **deltas and counters cross boxes and absolutes do not.** Timings this box: cold `release` ~12 min quiet, cold `profiling-fast` 9m30s / warm ~3m10s, `overflow` warm 2m33s, `release-fast --features trig-census` ~8 min, a three-pool callgrind ~2 min in parallel, the debug suite ~2 min after a ~4 min rebuild.
3. **Perf — EIGHT ROWS TAKEN, cumulative fixed -1.463 / cube -1.556 / sealed -1.148 %.** `(-322)` `blocker_pair_block`'s nine slice walks become two (row -44 %); `(-323)` `blocker_self_block` notes the block tax in the walk it already makes; `(-324)` `combat_participation_cap` behind a new `Battlefield` lane (row -87 %); `(-325)` the two `find_card_anywhere` walkers take `find_card_zone`'s back-to-front graveyard order; `(-326)` `statics_granted_dying_triggers` takes `trigger_grant_sources`' member-lane walker (row to zero); `(-327)` `apply_soulbond_pairing` joins the gate-keyword lane's union (row -74..-82 %); `(-328)` the two equip-trigger walks take that member lane too (`fire_step_triggers` self -22 / -16 / -13 % a call); `(-329)` the two command-zone tails go behind `command_zones_are_empty` (`dispatch_board_scan` self -10..-11 % a call). Per-row numbers in PERF's Log and Baseline.
4. **The device, and it is a CHECKLIST, not a table read.** For every function over ~0.3 % on any pool, open it and classify each walk: (a) a short slice asked N times (`has_kw` per family) — collapse to one `match`; (b) a whole-board walk with a definition-only predicate — a **lane** when the callers are outside a freeze scope, a **`PresenceGate` slot** when they are inside one, an **addition to an existing lane's union** when the predicate is printed-keyword-shaped; (c) a whole-board walk another function already memoizes — **grep before building**, `(-326)`/`(-328)` were already paid for; (d) a zone scan from the wrong end; (e) a `Filter`/`FlatMap` chain over a collection empty on every board the simulator plays — an emptiness test in front of it, and ⚠ it is worth ~9 Ir a call rather than ~2 because the chain is rebuilt per call and each `Filter` forwards its closure per element (a chain with NO filters, the emblem walk, is 0.014 % and declined). **None of the eight was in a self table as itself.** PERF's candidates section has the scriptable ranking for (b)/(c) and the unread-body list: `perform_action_inner` 1.70 %, `resolve_top_of_stack_inner` 0.79 %, `advance_step` 0.66 %, `cast_candidates` 1.04 %, `pick_attacks_inner` / `pick_blocks_inner`.
5. ❌❌ **Perf — TWO REFUTED AND REVERTED this run, and they are the SAME mistake twice: a memo's miss rate assumed instead of censused.** (i) `on_left_battlefield` reading its ten guards through the shared walker before taking `&mut` (+0.179 / +0.217 / +0.112 %) — `find_card_anywhere_mut` already locates with shared borrows (`make_mut_slow` under it is 6,168 calls on BOTH sides) and the guard is TRUE on 7,168 of 10,826 leaves. (ii) `resolve_combat_into`'s `must_block` clear reading before `iter_mut` so a no-op combat does not bump the zone's `writes` (-0.023 / -0.016 / +0.015 %, flat and mixed) — `sba_fold`'s 89 % miss is real per-card writes, censused at **2,332 / 21,798 hits (10.70 %)**. **Both were one `--callees` read or one census run away from being declined for free.**
6. **Perf — the sized queue is in PERF's "Perf candidates", top first:** the dispatcher's event fold (0.50-0.67 %; ⚠ **the `events.is_empty()` fast path is already dead** — `folded` clones ~9.4 events when it runs, so what is left needs two slices or a caller-owned buffer, ~8 call sites); `dispatch_board_scan`'s ungated command-zone tail (~0.07 %, free); `ManaCost::cmc` 0.23 % (needs a sixth `CardMemo` word — batch with another base change). ⚠ `CardDefinition::debug_flags` reads **1.4 % of `cube`** and is the `wants_converge` class — once per distinct card NAME per process, ~nothing in a gate run — **do not rank it**, and note it dilutes every percentage on the page.
7. ❌ **Perf — still closed, do not re-open without a device none of the prior ones was:** `permanent_value_with`'s printed-shape gate (`(-320)`), the `ComputedPermanent` memo lane (`(-312)`), `available_mana` (`(-317)`), the target enumerator, `main_phase_action_with`'s `has_magecraft`/`has_opus`, the three `continuous_effects` duration scans in the SBA sweep (~0.03 %), and the three structural rows `dispatch_triggers_for_events_slow` / `compute_permanent_pass` / `check_state_based_actions_into` (line-profile verdict: FLAT).
8. **Lane budget: shift 60 spent by `(-324)`, and 62 is THE LAST ONE.** Prefer widening an existing lane's union (`(-327)`) or reusing a member list (`(-326)`, `(-328)` — `for_each_board_scan_source(bit, f)` now serves any `BOARD_SCAN` bit) over spending it; `ContinuousEffects::fold` has bits 8..30 free if a duration or family word is ever wanted. ⚠ **And full-lane invalidation is NOT a lever**: the nine `for c in &mut self.battlefield` loops that were dropping every lane through `DerefMut` now take `iter_mut`, and the A/B reads 0.0002-0.0004 % — a shape fix, not a win.
9. **Seeds: THREE blocks this run, 36 cells / 177,600 games / 0 failures, `cap 0 / board 0 / stuck 0` on all three** (pools `cube all sealed`, seeds 1354..1357, 1358..1361 and 1362..1365, `target-audit/overflow` with `-C debug-assertions=yes` and `CRAB_ANSWER_LOG=strict`). They audit `(-324)`'s new lane assert, `(-323)`'s exact-tax assert, the nine loops that now keep their lanes across a write, and `(-326)`/`(-328)`'s shared member-lane walker. **FRONTIER 1366 — read it from PERF's Baseline, not from this line.** Standing audits all 0 (`audit_panics` 0 bare / 70 sites, `audit_engine_stack` 0, `audit_stash_in_loop` 0 unexplained, `audit_seat_from_selector` 0 open, `audit_target_walkers --check` 0, `audit_doc_drift` 0/0/0, `audit_incomplete` 0 structural, `audit_stubs` 0).
10. **Bugs/robustness: nothing open.** Structural audits, determinism and the panic census are all clean; TODO's robustness section has no open entries.
11. **Best next moves, in order.** (a) **Run line 4's checklist over the unread bodies it names**, with PERF's scriptable ranking as the pre-sort. (b) The dispatcher's event fold is the biggest sized row left and now needs a *design* (two slices or a caller-owned buffer), not a census. (c) The fresh-seed sweep as the FIRST gate after any lane/memo change — frontier 1366. (d) leftover context: `audit_dropped_may` 199 (`LookPickToHand` not safe at this granularity — put 73, exile 20, return 16, collect 12, begin 12); `audit_keyword_drift`'s last 4 each need a PRIMITIVE; card skips with a card behind them: `nocolors` 950, `noback` 60, `nosubtypes` 43, `nokeywords` 29.
