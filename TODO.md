...
1. **FIRST:** `git fetch origin claude/modern_decks && git checkout -B claude/modern_decks origin/claude/modern_decks`. Rebase, never force; code before tracker prose; **fetch before every push AND before you start**. ⚠⚠ **THIS RUN LOST TWO `profiling-fast` BUILDS AND A FINISHED A/B TO A DUPLICATE**: it took candidate (M), built and measured it, and found on its pre-push fetch that another session had landed `(-345)` for the same candidate forty minutes earlier. **Fetch, then pick, then build.** Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure" + "Standing rules"**, the 2026-09-12/13 detail is in **ENGINE_BACKLOG's "moved verbatim"** block. NEXT is 15 lines and stays 15.
2. **Gates at the `a878464f` tip** (a CONCURRENT session's `(-349)`/`(-350)` landed on top of `0bd3c37c`; its own sweep is 6 cells / 29,600 games at seeds 1416..1417, FRONTIER 1418). At `0bd3c37c`, all re-run: `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant, `determinism ok`, `thread_determinism ok (3 vs 1)`; suite **19,598 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); clippy **0**; `cargo check --profile release-fast` clean. Sweep: **12 cells / 59,200 games / 0 failures** over seeds 1412..1415, `cap 0 / board 0 / stuck 0 / draw 4`, **FRONTIER 1416**. ⚠ `peak_rss_mib` reads **25.2 / 27.2 / 25.2 across three runs of one binary** — arena variance, not a reading.
3. 🔎🔎 **THE FIND OF THIS RUN WAS READING THE PREVIOUS RUN'S COMMIT, AND THE BUG WAS IN THE `&&`.** `(-345)` gated its keyword-removal walk on `has_family(KEYWORD)`, and `modification_families` maps `RemoveKeyword` / `CantHaveKeyword` / `ReplaceColorWord` to **no family at all** — so "target creature loses flying until your next turn" left every `creature with flying` filter matching it. **`has_family(m) && any(p)` is a two-place premise with nothing connecting the places.** `(-346)`'s `ContinuousEffects::any_in_family` owns the pair and asserts it; there is one other caller and it was right.
4. 🔎 **The device that found it generalises: when a commit lands a gate, ask which ENUM ARM the gate's mask is derived from and go read that mapping.** The same question is unasked for `pt_reduction_in_scope`, `damage_scaling_in_scope` and the three type gates — all five read `has_family` and four of them read it *alone* (sound), but the fold's arms have never been diffed against the walks that consume them.
5. ❌📐 **`(-347)` REFUTES A MEMO AND `(-348)` TAKES ONE, AND BOTH TURN ON THE SAME ARITHMETIC: PRICE THE READ, NOT JUST THE WALK.** `(-348)`'s memo read is **14 Ir against the 24.7 Ir call-plus-body it replaces**, so candidate (A) came in at half its filed row (-0.101 / -0.074 / -0.123 %) — and part of even that is the un-inlined call `release` removes, so quote it as a `release-fast` number. `(-347)` is the same sum the other way: "`card_color_change_unscoped` costs 101,066 Ir a `fixed` run" is the **ceiling** on the win, not the win: the lane read cost more (+0.006 / +0.014 / +0.012 %, reverted). **A memo beats a walk only when the ask rate BETWEEN INVALIDATIONS is high** — every zone lane that paid off here is asked many times per board write, this one twice. Price the walk, then divide by the ask rate, before building.
6. ⚠⚠ **`iter().any(&pred)` COSTS +0.16 % ON ALL THREE POOLS AND IT IS NOT A ZONE-LANE-ONLY TRAP.** `&F: FnMut` goes through `core::ops::function::impls::call_mut`, which does not inline. **Any helper that forwards a predicate takes it `+ Copy` and passes it by value.** `Battlefield::lane`'s doc has said so since the lane memo; `any_in_family` learned it the expensive way and now says so too.
7. 📐 **A ROW WITH A CALL-COUNT DELTA OF ZERO AND A LARGE Ir DELTA IS AN INLINING MOVE, NOT WORK.** `computed_permanent_hinted` read +398,552 Ir at the identical 13,254 gathers on both sides of `(-346)`'s A/B, and `LocalKey::with` / `cp_pool::alloc` traded ±2.8 M. **Read the call column before ranking the Ir column** — `cg_calls.py` diffed between two dumps is the whole instrument.
8. 📐 **THE BOX PREDICTED ITSELF AND THAT IS NOW A CHECK, NOT A CURIOSITY.** This is the FOURTEENTH box, Xeon @ **2.80 GHz** = the *eleventh* box's CPU, and applying PERF's recorded eleventh-vs-twelfth offset to the filed tip predicted all three base absolutes **within 0.003 %**. **Predict the base from the CPU before you build it**: a base that misses the prediction is a *tree* problem (a binary built while the tree was edited, a stale worktree), not a box problem. It is what confirmed this run's base was genuine.
9. ✅ **`(-346)` — the keyword gates' stored leg, repaired: fixed -0.007 / cube +0.019 / sealed -0.001 %, i.e. a rules fix for nothing.** Three regression tests cover the three routes into `RemoveKeyword` (stored, aura `remove_keywords`, printed `LoseKeyword`), each with the control assertion that the INSTANCE still carries the keyword — which is what makes them fail on the old code rather than pass by accident.
10. ⚠ **EVERY GATE THIS BRANCH LANDS NOW CARRIES A RECOMPUTE-AND-COMPARE `debug_assert!` ON ITS CHEAP PATH**, so the suite and every sweep cell audit it (`(-303)` point 2). `(-345)` had one on `has_kw` and none on `has_kw_tag`; `(-346)` added it. **A gate without that ratchet is a silent wrong answer waiting for the board that reaches it** — and the sweep is where it fires, not the suite: 29,600 games carry boards 19,598 tests do not.
11. **Perf queue: (J)/(K) CLOSED as diffuse, (L) = `(-344)`, (M) = `(-345)` + `(-346)` with its owed half refuted as `(-347)`, (A) = `(-348)`.** ❌ **(N) was filed and CENSUSED the same day: same key on 12.0 / 14.3 / 20.7 % of SBA calls and the answer is `Skip` on 80.7 / 85.4 / 87.0 %, so one bit beside a stamp is the whole device — but the key needs BOTH write counters (a resolution can install a continuous effect without touching the battlefield), which is 8 bytes on the cloned `GameState` = ~530 k Ir on `cube` against ~850 k saved. Net ~0.02 %, inside the width noise floor.** Take it only if the key fits in four bytes or the memo moves off the cloned state. ✅ **(B) is TAKEN as `(-349)`/`(-350)` — and it took the contention objection filed here and changed its own shape for it**, a `bool` on `CardData` instead of the entry's process-wide `Arc` (which would have put every card's refcount on one cache line that callgrind cannot see). **A filed objection is worth as much as a filed lead.** Open: **(H)** the other zones' by-id scans. ❌ **(C) is CLOSED — its numbers were stale**: `(-333)` already took three quarters of `blocker_pair_block` (17,528 calls / 0.16 % now, against the 72,918 / 0.58 % the entry reasons from). ❌ `call_mut` at 1.23 % of `cube` is **not** a lead either: 98.6 % is std's own `Filter` fold, a no-LTO artifact. 📐 **ONE 16-SECOND `--dump-instr=yes` DUMP CLOSED FOUR MORE BODIES THIS RUN** (`blocker_self_block`, `simulate_attack_leaf`, `quality_band_assigner`, `dispatch_triggers_for_events_slow` — the program's largest self row at 5.01 %, diffuse over 258 lines). **Line-profile and re-size a candidate before building for it; both cost nothing.** ⚠ **Both (J)/(K) reads cost NOTHING:** `cg_lines.py` needs no `profiling-lines` build — one 16-second `--dump-instr=yes` callgrind line-profiles every body at once. **Line-profile a candidate before building for it.**
12. **THE PROFILE IS FLAT AND PERF'S CANDIDATES HEAD SAYS SO WITH THE GLOBAL LINE TABLE — do not re-derive it.** Largest single source line **0.97 %** (`Arc::clone_from_ref_in`'s memcpy), then 0.80 / 0.56 / 0.41 %, and **12,537 lines hold 86.9 %**. Rank by **self per call**, not by self. ⚠ **`Battlefield`'s lane word has exactly ONE two-bit field left** (shift 62) and `zone.rs` now carries a `const` assertion saying so; a thirty-third presence question needs a second word or a shared lane.
13. **Tip absolutes at `a878464f` (`profiling-fast --no-default-features`, system allocator, FOURTEENTH box @ 2.80 GHz): fixed 577,254,354 / cube 1,507,866,096 / sealed 1,628,320,869**; at `0bd3c37c`, this session's own closing tip, 577,507,476 / 1,508,692,832 / 1,628,908,517.** Run total **-0.101 / -0.077 / -0.123 %**, two rules-correctness fixes among them. 📐 **The noise floor on this instrument is ±0.001 %** — a value-identical refactor (`modification_families` made exhaustive) read -2,310 / +20,993 / -16,719 Ir across a full rebuild. ⚠ **Build times are CLOSED as a lever** — PERF's own measurement kills the file-split idea (a 138x file-size difference buys nothing, and a split of a hot body without `#[inline]` across the new seam COSTS ~0.1 %).
14. ⚠⚠ **THREE TRAPS, ALL LIVE.** `cargo build --profile profiling-fast -p crabomination --bin bot_ladder --no-default-features` — **both flags load-bearing**; `nm <bin> | grep -cE " (T|t) (_)?mi_"` = 0 before filing a number. ⚠ **DO NOT EDIT THE TREE WHILE A BASE BINARY IS BUILDING** — rustc parses early so it usually survives, but the only thing that *proved* it here was item 8's prediction. ⚠ **Never overlap two sweep blocks on four cores.** Disk: this run reached 83 %; `rm -rf target/debug/incremental` alone returned **9.4 GB**, and `target/profiling-fast` is another 1.4 GB.
15. **Cards & rules, leftover only:** `audit_dropped_may` 199; `audit_keyword_drift`'s last 4 (bestow / escalate / flashback / kicker) each need a PRIMITIVE; card skips `nocolors` 950 / `noback` 60 / `nosubtypes` 43 / `nokeywords` 29; `audit_incomplete` 858 documented approximations; **Temple Thief's `IsEnchanted`** (thirty-sixth find, machine-checked, fix shape in INCOMPLETE_CARDS). ⚠ 605 `definition.is_*()` reads remain and **that is NOT a grep to run to the end** — most are on cards in zones where the printed answer is the only one there is.
