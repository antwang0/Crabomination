...
2. **Gates at the closing tip (re-taken after a concurrent session's CR 603.2c batch landed under this run):** suite **19,742 / 0 / 6** (`CRAB_ANSWER_LOG=strict`); clippy **0** over the workspace; `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean; `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant, `determinism ok`; Ir guardrail **fixed +0.066 / cube +0.038 / sealed -0.050 %** over the run's twelve commits (PERF's Log, base `b7ba4cb8`); `audit_panics` **0 bare**. Assertions sweep (`overflow` + `-C debug-assertions=yes`) **16,000 pod games at 2/3/4/5 seats, fresh seeds 2001-2004, 100 % decided, 0 undecided, 0 assertions** plus the two-player grid on fixed/cube/sos/all at seed 2001 — **FRONTIER 2005**. ⚠ The sweep was taken one commit before the rebase; the release re-check after it (4,000 pod games at 4 and 5 seats, seed 2001) is still 100 % decided with 0 undecided, and `--bench` is unmoved.
...
7. **This run shipped, twelve commits:** the rider-mana estimate fix; the ranked auto-targeter it unblocked (`first_alive_opponent_of` deleted); **CR 800.4f/g** (`game/departed.rs` — `route_ask` + `seat_prompts`, replacing 102 hand-written `wants_ui` reads); **CR 800.4a** for a seat an *effect* eliminates (`Player::left_game`); **CR 102.1/800.4a** (`opponents_of` filters the eliminated); the combat-damage trigger's subject binding (Phage and Entropic Specter had **dead abilities**); **CR 613.5**'s colour gate on the printed fast path; the per-player-loop suspend guards; **CR 615**'s filtered all-damage shield + **Obscuring Haze**; the pod chunk-determinism gate; the PERF guardrail row.
8. ✅ **DECK BALANCE IS CLOSED and the method is the keeper: PRICE THE CARDS BEFORE REBUILDING THE LIST.** Judith read 7.8 % at four seats and 23.7 % in a *duel*, so never a multiplayer artifact. A card audit (no build spent) said `audit_incomplete` structural clean, two approximation notes of 73 cards and both non-issues, commander faithful — **nothing worse than printed** — and only then was the list retuned (28 of 72 nonbasics, Judith 6.2 → **14.6 %** at four seats, field 40.8/14.6/20.3/24.3). A deck losing three-to-one has two explanations and only one is a bug; the audit is an afternoon and the rebuild re-blesses the pod table.
...
10. **Cards and decks, what else is left:** ⏳ 32 of 33 "can be your commander" non-creatures are unimplemented; the flag and validator are done, so each is a plain card — but *cards are chosen to complete decks, not to raise a count*, so they wait on a sixth target deck wanting one. ⏳ `scripts/audit_each_opponent.py`'s **six** residuals each need a primitive (per-player protection, a cross-player permanent comparison, a per-opponent conditional, a per-opponent static restriction, a damaged-opponent count, a speed comparison); **none is in a target deck**, which is why none was taken. Kaya, Spirits' Justice is the cheapest-looking and is **not**: `ForEachOpponentTarget` caps `ctx.targets` one-per-controller *including slot 0*, so wrapping only the opponent half is off by one.
...
12. ⚠⚠ **Golden traces are the Commander guardrail too, and the test is POOL MEMBERSHIP.** With no commander seated every Commander change must be byte-identical in a duel. Every engine change this run is gated on "somebody has left the game" (which one elimination ends in a duel) or short-circuits to the single opponent — that is *why* the traces held, and the next one needs the same argument made out loud, in the commit message.
