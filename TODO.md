...
7. 💡 **THE OPEN QUEUE.** ① **Next list: Goblin Storm (SLD, 10 missing)**; Wretched Ranks (FDC, 9) releases 2026-10-02. Re-run `precon_scan.py` first — the ranking moves as cards land. ⚠ **A concurrent session is working DECK_FEATURES' residual list** (it took Myriad Landscape, Herald's Horn, Overcharged Amalgam and bot escape mid-run) — fetch before picking one. ② The two precons' residuals are listed in DECK_FEATURES (Colossal Grave-Reaver's pick, Grell Philosopher's grant, Psionic Ritual's replicate, Herald's Horn's upkeep, Overcharged Amalgam's ability counter, Myriad Landscape's shared type — this last needs a "shares a land type with the last-searched card" requirement). ③ `AdditionalCastCost::OneOf` picks for a human too — a cast-time prompt is the follow-up. ④ Sewer Nemesis always chooses an opponent. ⑤ The older queue: per-slot trick evaluation (needs an A/B), `audit_dropped_may`, the 69-row invented-target census, `--card-census` for the two-player ladder.
8. ⚠ **Mana-producing permanents with a real spend restriction are never auto-tapped** (engine-wide policy, `payload_produced_colors`), so Lord of the Forsaken's graveyard mana is a deliberate activation only — no bot ever takes it. Same for Titans' Nest, Cavern-style lands. A bot that floats restricted mana before a matching cast is the lever if it ever matters.
