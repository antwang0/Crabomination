...
2. **Gates at the `1bceaeb9` tip, all re-run:** `--bench` **195,806 / 27.49 / 611.9 / 0 stalls / peak_rss 25.2** byte-identical to the committed invariant across all ten commits, `determinism ok`, `thread_determinism ok (3 vs 1)`; suite **19,595 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, 12 golden traces unmoved); clippy **0**; `cargo check --profile release-fast` clean. Sweep: **36 cells / 177,600 games / 0 failures** over seeds 1398..1409, `stuck 0 / draw 26 / cap 2` (both `cube` MAX_BATTLEFIELD runaways on a saturated seat — the script's carve-out), **FRONTIER 1410**. `audit_stubs` 0 of 21,797, `audit_panics` 0 bare, `audit_doc_drift` 0/0/0.
...
7. ⚠⚠ **A RULES FIX CAN COST 0.6 % OF A POOL AND `--bench` WILL NOT SAY SO** — it and all 144 golden traces were byte-identical across every commit of this run. 📐 **Price the ROUTING separately from the walk:** two-thirds of the card-walker gate's cost was the entry (`battlefield_find` + `PrintedGates` per *ask*), which `(-343)` took back (**cube -0.279 %**). Run total: **fixed +0.168 / cube +0.159 / sealed +0.121 %** for five class fixes.
8. ⚠⚠ **THE RUN'S ONE REAL PRICE IS THE KEYWORD/COLOUR LEAVES AT `fixed +0.144 %`, AND IT IS 370 EXTRA GATHERS** (`gather_continuous_effects_inner` 13,254 -> 13,624). **A computed read on an UNFROZEN state gathers once per distinct state**; inside a freeze scope it is nearly free. 📐 **And the pool pattern inverts — `fixed` pays most because it is the pool a presence gate would serve entirely.** That gate is **PERF candidate (M)** and the soundness is the whole job: nothing covers `LoseKeyword` / `CantHaveKeyword` / `EquipBonus::remove_keywords`, so the obvious two-leg gate is silently wrong. **Do not ship it in a hurry; the colour half is already sound.**
9. ✅ **`(-344)` — the CR 104.4b loop watch settles off the digest's scalar head: fixed -0.072 / cube -0.240 / sealed -0.071 %.** 📐 **A memo census has TWO numbers**: how often the cheap branch is *reachable* (54.4 % of `cube` samples) and how often the cheap key *separates* on it (97.5 %). Measuring only the first is `(-329)`'s mistake. The shortcut is **audited, not argued** — a `debug_assert_ne!` recomputes the skipped digest, so the suite and every sweep cell are the ratchet; 59,200 games, no fire.
...
11. **Perf queue: (J) and (K) CLOSED as diffuse, (L) taken as `(-344)`, (M) new and specified.** `resolve_combat_into` (1.57 %, 276 lines, top line 0.09 %) and `do_untap` (0.53 %, 89 lines, top 0.04 %) are both "the body IS the cost". ⚠ **Both reads cost NOTHING and saved two builds:** `cg_lines.py` needs no `profiling-lines` build — one 16-second `--dump-instr=yes` callgrind line-profiles every body at once. **Line-profile a candidate before building for it.** Also open: (A) `ManaCost::cmc`, (B) the `CardCold` `Arc::ptr_eq` gate, (C) `blocker_pair_block`'s pair count, (H) the other zones' by-id scans.
...
13. **Tip absolutes at `1bceaeb9` (`profiling-fast --no-default-features`, system allocator): fixed 580,639,840 / cube 1,512,666,565 / sealed 1,634,413,695.** ✅ The twelfth box's closing absolutes **reproduced to six significant figures**. **"Absolutes do not cross boxes" is about the CPU MODEL, not the box.** ⚠ **Build times are CLOSED as a lever** — PERF's own measurement kills the file-split idea (a 138x file-size difference buys nothing, and a split of a hot body without `#[inline]` across the new seam COSTS ~0.1 %).
