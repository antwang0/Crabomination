...
7. 💡💡 **THE TOP LEAD IS (O)'s SHORTLIST AND IT IS RANKED AND PRICED — pull from the top of it and nothing else.** Biggest pair: the two auto-tap scratch sites in **one** function, 12,880 / 14,480 allocations — `Box::new(OneColorDecider)` (pool it, `(-356)`'s shape) and the `events` buffer (an out-param through `auto_tap_for_cost{,_only,_filtered}`, a five-site signature change, which is why this run left it). Then `GrantScan::equipment` (4,638, **compiles as a `SmallVec` — checked**), `TokenDefinition::clone` (6,688, unread).
8. ⚠⚠ **TWO INLINE-BUFFER TRAPS, BOTH NEW AND BOTH COST A BUILD HERE.** ① `Vec<&'a T>` **cannot** become `SmallVec<[&'a T; N]>` in a borrow-carrying struct: `Vec`'s `Drop` is `unsafe impl<#[may_dangle] T>` so dropck knows it never reads the references, `SmallVec`'s is not, and the borrow is forced to the end of the scope (four `E0502`s on `DispatchScan`). ② `(-165)`'s "does it grow its owner?" is **necessary and not sufficient**: also ask how often the owner is MOVED (+393 k of memcpy on a 136-byte value through four frames) and what the spill rate is (`cube` spilled 1,960 of 8,724 fills at 16 slots where `sealed` barely spilled — the three pools disagreed and the widest-board one decided it).
