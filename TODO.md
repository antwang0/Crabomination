...
4. **Official precons are seats 11-66, card for card** (per-seat notes and residuals: DECK_FEATURES; open residual rows: INCOMPLETE_CARDS' Commander routine table). **This run: Entropic Uprising (C16, Yidris) LANDED as seat 51, Endless Punishment (DSC, Valgavoth) LANDED as seat 59, Arcane Wizardry (C17, Inalla) LANDED as seat 66** (also Fae Dominion 45 and Invent Superiority 48 earlier). **Session `01XnuL2a` landed Open Hostility (C16, Saskia) as seat 64** (seats 42, 46, 49, 53, 55 earlier). **Session `01546QdT` landed Seize Control (38), Rebellion Rising (41), Feline Ferocity (52), Primal Genesis (56) and Breed Lethality (C16, Atraxa) as seat 61** (its census found the bot looping two Enduring Scalelords — fixed, CR 732.2). **Session `011A87o2` landed Devour for Power (CMD, The Mimeoplasm) as seat 58, Mirror Mastery (CMD, Riku) as seat 62 and First Flight (SCD, Isperia — Gideon Jura's CR 508.1d lure) as seat 65.** **Session `01P5KQkY` landed seats 32-34, 36-37, 40, 44, 47, 50, 54, 57, 60 and 63 (Raining Cats and Dogs, SLD, Rin and Seri — 4 residuals; primitives `StaticEffect::TokensMayBecome`, `Value::LandCardsRevealedThisEffect`)**; its bot finds: pod seats cycle, choose-two/three spells are cast, attacking-or-blocking instants ride the combat window.
5. **⚠ OPEN CLAIMS — don't duplicate:** **Enduring Enchantments (CMM, Anikthea, Hand of Erebos, 17; `01546QdT`, 2026-09-24 12:05).** **Growing Threat (MOC, Brimaz, Blight of Oreskos, 17; `01P5KQkY`, 2026-09-24 run 2).** **Phantom Premonition (KHC, Ranar the Ever-Watchful, 18; `01XnuL2a`, 2026-09-24 run 4).** **Undead Unleashed (MIC, Wilhelt, the Rotcleaver, 18; `011A87o2`, 2026-09-24 13:00).** **Urza's Iron Alliance (BRC, Urza, Chief Artificer, 16; `01StaUuo`, 2026-09-24 run 5).** **Hatsune Miku (SLD, Trostani, Selesnya's Voice, 17; `015cKV6u`, 2026-09-24 12:25).** Claim in this line, pushed on its own, before starting a list.
6. 🔎 **This run's engine finds** (ENGINE_BACKLOG): a mana source paying its own {1} overflowed the stack (CR 601.2g, found by a 1,000-game pod — `pod::tests` replays it); SpellCast triggers never named their caster (CR 603.2 — "another target player" hit the caster or nobody); no bot path suspended a no-cost card (Wheel of Fate), and the census didn't count suspend/foretell as plays. ⚠ **Postcombat-main triggers never fired in real games** (CR 505.1a — `advance_step` skipped `StepBegins(PostCombatMain)`; Survival, Florian, Tymna were dead, their tests fired the step by hand). Arcane Wizardry: **The Abyss destroyed every nonartifact creature** (it is one, of their choice); "tap N untapped [X] you control" never let the source pay; **both death funnels filled only a trigger's first target slot**; and the extra-slot filler only ever offered the *first* opponent — a two-seat assumption.
