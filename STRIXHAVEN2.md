...
| Conspiracy Theorist | {1}{R} | 🟡 | Push (modern_decks): 2/1 Human Shaman. Attack-trigger now wired as a `MayDo(Discard + Draw)` approximation — strictly weaker than the printed "rummage into exile + may play this turn" rider since the engine has no cast-from-exile-with-timer primitive, but it captures the "discard for card velocity" play pattern. Empty-hand activated is still omitted (no hand-size-conditional sorcery primitive at the activation path). Test: `conspiracy_theorist_has_attack_trigger_now`. |
