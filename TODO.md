...
3. **The instrument that pays:** the pod run is where four bugs came from in one sitting. Run it before and after anything Commander-shaped; a stall rate that moves is a bug, not noise. **Tip reading: `--commander --games 400 --seed 43` → 400 games, 0 undecided, 41.83 turns/game, 315.77 games/s.** It was **15 % "no legal move"** before the CR 800.4a pending-decision fix. `CRAB_CAP_DIAG=1` names an undecided pod game's board.
...
9. **Guardrail at the tip:** `--bench` invariant **195,806 / 27.49 / 611.9 / 0 stalls**, `determinism ok` — byte-identical to the committed one, so every Commander change so far is behaviour-preserving in a duel. ⚠ games/s does **not** cross hosts (this box is a 2.10 GHz Xeon); PERF's Commander block says what to compare.
