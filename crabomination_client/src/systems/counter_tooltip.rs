...
pub(crate) fn keyword_reminder(kw: &crabomination::card::Keyword) -> Option<&'static str> {
    ...
    Some(match kw {
        ...
        K::BandsWithOther(_) => "Bands with matching creatures; the defending player assigns the combat damage of a creature two of them block.",
        ...
        K::Partner => "You can have two commanders if both have partner.",
        K::PartnerWith(_) => "You can have two commanders if they partner with each other.",
        K::ChooseABackground => "You can have a Background as a second commander.",
        ...
        K::ProtectionFromOwnColors => "Can't be blocked, targeted, or damaged by sources sharing any of this permanent's own colors.",
        K::ProtectionFromColorsOutsideCommanderIdentity => "Can't be blocked, targeted, or damaged by sources of any color outside its controller's commander color identity.",
        ...
        K::Flashback(_) | K::FlashbackTap { .. } => "You may cast it from your graveyard for its flashback cost, then exile it.",
        ...
        K::CantBlockPowerAtMost(_) => "Can't block creatures with power that low or less.",
        K::CantBlockMatching(_) => "Can't block creatures matching this restriction.",
...
pub(crate) fn keyword_label(kw: &crabomination::card::Keyword) -> String {
    ...
    match kw {
        ...
        K::Ward(wc) => match wc {
            ...
            crabomination::card::WardCost::ReturnMatchingToHand(_, n) => {
                format!("Ward—Return {n} matching permanent(s) to their owner's hand")
            }
            ...
            crabomination::card::WardCost::BottomFromGraveyard(n) => {
        ...
        K::BandsWithOther(_) => "Bands with other".into(),
        ...
        K::CantBlockPowerAtMost(n) => format!("Can't block power {n} or less"),
        ...
        K::FlashbackTap { count, .. } => format!("Flashback—Tap {count} creatures"),
        ...
        K::ProtectionFromOwnColors => "Protection from its colors".into(),
        K::ProtectionFromColorsOutsideCommanderIdentity => {
            "Protection from each color outside your commander's color identity".into()
        }
        ...
        K::Partner => "Partner".into(),
        K::PartnerWith(name) => format!("Partner with {name}"),
        K::ChooseABackground => "Choose a Background".into(),
...
fn counter_reminder(kind: CounterType) -> Option<&'static str> {
    Some(match kind {
        ...
        CounterType::Impostor => "Impostor mark — a marked creature that dies is exiled instead (Illicit Masquerade).",
        CounterType::Bloodstain => "Bloodstain tally — the fifth one sacrifices the enchantment (Blood Spatter Analysis).",
        CounterType::Petal => "Lotus Blossom tally — sacrifice it for that many mana of one colour.",
        CounterType::Scream => "All Hallow's Eve's fuse — one comes off each of your upkeeps; the last one cashes it in.",
        CounterType::Doom => "Armageddon Clock's tally — it burns every player for that much each draw step.",
        CounterType::Page => "Book tally — the host's ability scales with (or is discounted by) the page count.",
        CounterType::Plot => "Shared plot tally — any player may add one; the host pops at its printed threshold.",
        ...
        CounterType::Rev => "Chainsaw tally — the equipped creature gets +1/+0 per rev counter.",
        ...
        CounterType::Shell => "Shed one each upkeep; the creature grows up once the last comes off.",
        ...
        CounterType::Pressure => "Magma Mine's charge tally — sacrifice it to deal this much damage to any target.",
        ...
        CounterType::Slime => "Sludge Monster strips non-Horrors carrying one; Toxrill shrinks its victims -1/-1 per counter.",
        CounterType::Descent => "Descent into Avernus gains two a turn; the tally is the Treasure each player makes and the damage each player takes.",
        CounterType::Corpse => "From the Catacombs marks what it reanimated.",
        CounterType::Rally => "Aligned Heart's flurry tally — one more Monk for each counter.",
        CounterType::Enlightened => "The Book of Exalted Deeds' memory aid; the Angel keeps its ability without it.",
        CounterType::Hoofprint => "Hoofprints of the Stag's draw tally — four make a 4/4 flying Elemental on your turn.",
        CounterType::Bribery => "Gwafa Hazid's mark — a creature carrying one can't attack or block while Gwafa is out.",
        CounterType::Spite => "Curse of Vengeance's tally of the cursed player's spells — its controller gains that much life and draws that many if the player leaves.",
        CounterType::Soul => "Séance Board's morbid tally — it taps for that much mana of one color, for instants, sorceries, Demons and Spirits.",
        CounterType::Cage => "Mairsil's cage — Mairsil has the activated abilities of each card its controller owns in exile with one.",
        CounterType::Blessing => "Boon of the Spirit Realm's tally — creatures you control get +1/+1 for each.",
        CounterType::Component => "Component Pouch's store — remove one to add two mana of different colors.",
        CounterType::Landmark => "Treasure Map's tally — at three it transforms and makes three Treasures.",
        CounterType::Plan => "Glorious Purpose's tally — the sixth sacrifices it and casts from the top four.",
        CounterType::Takeover => "The Master, Formed Anew may enter as a copy of this exiled creature card.",
        CounterType::Necrodermis => "The War in Heaven's mark — no rules meaning of its own.",
        CounterType::Ingredient => "Communal Brewing's tally — each creature spell you cast enters with that many extra +1/+1 counters.",
        CounterType::Vow => "Promise of Loyalty's mark — this creature can't attack the vow's caster or their planeswalkers.",
        CounterType::Duty => "Immortal Obligation's mark — goaded, and can't attack the caster or block their creatures while it's here.",
        CounterType::Slumber => "Arixmethes's sleep — it's a land, not a creature, while it has one; your spells may remove them.",
        CounterType::Story => "Staff of the Storyteller's tally: remove one to draw a card.",
        CounterType::Strife => "Crescendo of War's tally — attackers, and its controller's blockers, get +1/+0 per counter.",
        CounterType::Night => "Replicating Ring's tally — at eight it removes them all and makes eight Replicated Rings.",
        CounterType::Fury => "Charging Cinderhorn's tally — at an end step with no attackers it gains one, then deals that much damage to the active player.",
        CounterType::Corruption => "Geyadrone Dihada's mark — you have protection from each permanent carrying one, and her -7 takes control of them all.",
