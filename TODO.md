...
3. **Gates at this run's tip (`fb937aec5` + trackers):** suite **22,927 / 0 / 7** (strict); clippy 0; `--bench` **200,190 byte-identical**; cube / sos / sealed 300 games each decided.
4. **Pod smoke this run, every seat, fresh seeds:** release census 8-seat (seeds 31001+, 6,900 games), 6-seat (32001+, 9,300), 4-seat (33001+, 13,800) — **all decided after the fixes, zero panics**; strict debug census 4/6/8 seats (35001+ / 34001+ / 36001+, 1,104 / 744 / 368 games) all decided. `census.sh`-style loop: `--pod-decks` over groups of N, `--games 300`, one seed per group; a non-zero exit is a crash — bisect with `--first/--games`, then `gdb -batch -ex run -ex bt`.
5. **This run's finds (all fixed, tests cite the rule):** ⚠ **a stack overflow** — two Scalelord Reckoners' target-trigger chain recursed in `push_pending_trigger` (CR 603.3b; now queued past depth 8); ⚠ **a mana-pool u32 overflow** — two Leyline Tyrants re-added kept red once per keeper, and spent firebending red was re-minted every step (CR 702.189a); CR 704.3 — the CR 903.9a commander return ran inside resolutions and during pending decisions (Danse Macabre); the bot aimed attacks at a seat Mystic Barrier bars (947 creatures never attacked); seven "from the battlefield" graveyard triggers fired on discards (Unquenchable Fury's cleanup loop); Imposing Grandeur / Stinging Study counted a commander in any zone.
