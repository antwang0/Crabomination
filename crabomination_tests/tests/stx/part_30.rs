...
fn jadzi_magecraft_plays_land_or_casts_top_for_one() {
    ...
    crabomination::game::cast_at(&mut g, s1, Target::Player(1));
    drain_stack(&mut g);
    ...
    crabomination::game::cast_at(&mut g, s2, Target::Player(1));
    drain_stack(&mut g);
...
fn journey_to_the_oracle_puts_hand_lands_onto_battlefield() {
    ...
    crabomination::game::cast(&mut g, journey);
    drain_stack(&mut g);
    for l in [h1, h2, h3] {
...
fn revel_in_silence_locks_opponents_and_exiles_itself() {
    ...
    g.step = TurnStep::PreCombatMain;
    g.priority.player_with_priority = 0;
    crabomination::game::cast(&mut g, revel);
    drain_stack(&mut g);
