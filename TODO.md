...
2. **Gates at the `(-327)` tip, all re-run this run:** suite **19,549 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, nextest), clippy **0** (`--workspace --all-targets --exclude crabomination_client`, and again `--features trig-census`), golden traces unmoved, `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean, `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant with `determinism ok` and `thread_determinism ok`. ⚠⚠ **TWO SESSIONS RAN ON THIS BRANCH TODAY AND ON DIFFERENT BOXES** — `(-320)`/`(-321)` on the sixth, `(-322)`..`(-327)` on the seventh, and each row on both sides is a same-box A/B. **SEVENTH BOX, 4 cores, rustc 1.95.0, valgrind 3.22.0 — the base here reads +0.19 / +0.08 / +0.08 % against the filed `(-321)` absolutes** while `blocker_self_block`'s `cube` row reproduces to the digit, so the gap is the container, not the engine: **absolutes do not cross boxes, deltas and counters do.** Timings this box: cold `release` ~12 min quiet / ~70 min contended, cold `profiling-fast` 9m30s and warm ~3m10s, `overflow` warm 2m33s, a three-pool callgrind ~2 min in parallel, the debug suite ~2 min after a ~4 min rebuild.
3. **Perf — SIX ROWS TAKEN, cumulative fixed -0.927 / cube -1.309 / sealed -0.851 %.** `(-322)` `blocker_pair_block`'s nine slice walks become two (-0.163 / -0.448 / -0.215, the row -44 % on all three); `(-323)` `blocker_self_block` notes the block tax in the walk it already makes (-0.080 / -0.086 / -0.081); `(-324)` `combat_participation_cap` goes behind a new `Battlefield` lane (-0.308 / -0.194 / -0.213, the row -87 %); `(-325)` the two `find_card_anywhere` walkers take `find_card_zone`'s back-to-front graveyard order (-0.039 / -0.069 / -0.018); `(-326)` `statics_granted_dying_triggers` takes `trigger_grant_sources`' member-lane walker (-0.233 / -0.363 / -0.218, the row to zero); `(-327)` `apply_soulbond_pairing` joins the gate-keyword lane's union (-0.108 / -0.156 / -0.110, the row -74..-82 %).
4. **The device, and it is a CHECKLIST, not a table read.** For every function over ~0.3 % on any pool, open it and classify each walk: (a) a short slice asked N times (`has_kw` per family) — collapse to one `match`; (b) a whole-board walk with a definition-only predicate — a **lane** when the callers are outside a freeze scope, a **`PresenceGate` slot** when they are inside one, and an **addition to an existing lane's union** when the predicate is printed-keyword-shaped; (c) a whole-board walk another function already memoizes — **grep before building a lane**, `(-326)` was already paid for; (d) a zone scan from the wrong end. **None of the six was in a self table as itself.** PERF's candidates section carries the unread list: `perform_action_inner` 1.70 %, `resolve_combat_into` 1.52 %, `fire_step_triggers` 0.87 %, `cast_candidates` 1.04 %, `pick_attacks_inner`, `pick_blocks_inner`.
5. ❌ **Perf — REFUTED AND REVERTED this run: `on_left_battlefield` reading its ten guards through the shared walker before taking `&mut` (+0.179 / +0.217 / +0.112 %).** `find_card_anywhere_mut` already locates with shared borrows (`make_mut_slow` under it is 6,168 calls on BOTH sides), and the guard is TRUE on 7,168 of 10,826 leaves, so the second walk runs two thirds of the time for nothing. **The `(-280)` CoW device is priced by how often the guard is FALSE — census the `--callees` edge before building it.**
6. **Perf — the sized queue is in PERF's "Perf candidates", top first:** the dispatcher's event fold (`SpecFromIterNested` under `dispatch_triggers_for_events_slow`, 0.50-0.67 %, **census which of its two collects owns it before building**); `dispatch_board_scan`'s ungated command-zone tail (~0.07 %, free); `ManaCost::cmc` 0.23 % (needs a sixth `CardMemo` word — batch with another base change). ⚠ `CardDefinition::debug_flags` reads **1.4 % of `cube`** and is the `wants_converge` class: once per distinct card NAME per process, so it is ~nothing in a gate run — **do not rank it**, and note it dilutes every percentage on the page.
7. ❌ **Perf — still closed, do not re-open without a device none of the prior ones was:** `permanent_value_with`'s printed-shape gate (`(-320)`, built and refuted — the population is already creatures), the `ComputedPermanent` memo lane (`(-312)`), `available_mana` (`(-317)`), the target enumerator, `main_phase_action_with`'s `has_magecraft`/`has_opus`, and the three structural rows `dispatch_triggers_for_events_slow` / `compute_permanent_pass` / `check_state_based_actions_into` (line-profile verdict: FLAT, no line to take).
8. **Lane budget: shift 60 spent by `(-324)`, and 62 is THE LAST ONE.** Prefer widening an existing lane's union (`(-327)`) or reusing a member list (`(-326)`) over spending it; `ContinuousEffects::fold` has bits 8..30 free if a duration or family word is ever wanted.
9. **Seeds: one block this run, 12 cells / 59,200 games / 0 failures, `cap 0 / board 0 / stuck 0 / draw 16`** (pools `cube all sealed`, seeds 1354..1357, `target-audit/overflow` with `-C debug-assertions=yes` and `CRAB_ANSWER_LOG=strict`). It is what audits `(-324)`'s new lane `debug_assert!` and `(-323)`'s exact-tax assert on dflt-pilot boards. **FRONTIER 1358 — read it from PERF's Baseline, not from this line.** Standing audits all 0 (`audit_panics` 0 bare / 70 sites, `audit_engine_stack` 0, `audit_stash_in_loop` 0 unexplained, `audit_seat_from_selector` 0 open, `audit_target_walkers --check` 0, `audit_doc_drift` 0/0/0, `audit_incomplete` 0 structural, `audit_stubs` 0).
...
11. **Best next moves, in order.** (a) **Run line 4's checklist over the unread bodies it names** — `perform_action_inner` and `resolve_combat_into` are the two biggest, and `(-326)` says to grep for `battlefield.iter().filter(...dispatch_scan_bits...)` before building anything. (b) The dispatcher's event fold (line 6) needs **one census build**, not a design. (c) The fresh-seed sweep as the FIRST gate after any lane/memo change — frontier 1358. (d) leftover context: `audit_dropped_may` 199 (`LookPickToHand` not safe at this granularity — put 73, exile 20, return 16, collect 12, begin 12); `audit_keyword_drift`'s last 4 each need a PRIMITIVE; card skips with a card behind them: `nocolors` 950, `noback` 60, `nosubtypes` 43, `nokeywords` 29.
