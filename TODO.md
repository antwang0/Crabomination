...
2. **Gates at the closing tip `5a4bc0d6`:** suite **20,273 / 0 / 6** — and **20,285 / 0 / 6** re-taken at `00d0f08b` after the concurrent 614.12 commits merged in — ⚠ **this line was "corrected" to 20,085 and 20,285 is the measured number**; the reasoning behind the correction was sound about *which commit the count belongs to* (a one-line tracker edit cannot move it, and nothing `include_str!`s a tracker) and simply had the wrong digits. The arithmetic settles it: the very next commit adds **two** tests and the suite reads **20,287**. ⚠⚠ **A count is evidence and an inference about a count is not** — re-run the suite before rewriting someone's number — (`CRAB_ANSWER_LOG=strict`); clippy **0** over the workspace; `--bench` **195,806 / 27.49 / 611.9 / 0 stalls — byte-identical to the committed invariant**, `determinism ok (all pairs split)`, `thread_determinism ok (3 vs 1)`, `peak_rss_mib` 31.7; `audit_panics` **0 bare**; every mirror column 0 (`seat_walks` 0, `retarget` 0, `each_player` 1-allowlisted, `target_opponent` 1 known, the three invented-* columns 0, `loop_splice` 13/13/0/0) and `audit_as_enters --gate` clean. CR_COVERAGE **146/146**. **Pod smoke at a FRESH seed 9102: 2,000 games at each of 2..8 seats = 14,000 games, 14,000 DECIDED, every `undecided_by` column zero, zero panics.** turns/game 19.37 / 32.35 / 45.30 / 56.88 / 64.74 / 77.85 / 93.34. **FRONTIER 9103.**
