...
2. **Gates at the `a560f8b0` tip, all re-run:** `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant, `determinism ok`, `thread_determinism ok (3 vs 1)`, `peak_rss 25.1`; suite **19,579 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, 12 golden traces unmoved); clippy **0** both feature sets; `cargo check --profile release-fast` clean. Sweep: **12 cells / 59,200 games / 0 failures**, seeds 1386..1389, **FRONTIER 1390**. `audit_stubs` 0, `audit_panics` 0 bare.
3. ⚠⚠ **THE MIMALLOC TRAP COST THIS RUN A 7m29s BUILD AND IT LOOKS LIKE A WIN, NOT A FAILURE.** `cargo build --profile profiling-fast --bin bot_ladder` — **no `-p`** — keeps mimalloc and reads **-5.9 / -6.7 / -6.9 %** against the system allocator. Both `-p crabomination` AND `--no-default-features` are load-bearing. Check `nm <bin> | grep -cE " (T|t) (_)?mi_"` = 0 **before** you file a number. PERF's twelfth-box Baseline block has it.
4. **Tip absolutes at `012e0e21`, twelfth box (`profiling-fast`, system allocator): fixed 579,179,120 / cube 1,509,871,383 / sealed 1,631,936,758.** They reproduce the eleventh box's to +0.09..0.20 % on a different CPU — second box running. Re-take them fresh, but do not discard a filed absolute unread. 🔎 **And `cg_lines.py` needs NO `profiling-lines` build**: `.debug_line` survives `split-debuginfo = "unpacked"`, so the `(-342)` source-line grouping device is one `--dump-instr=yes` callgrind (**15 s**) plus 7 s of script. Hand it the binary the dump was taken from.
5. **Perf — TWO CANDIDATES BUILT, MEASURED AND REVERTED THIS RUN. Read both refutations in PERF's candidates head before planning perf work; they close whole families.** (a) `(-303)`'s cross-scope key applied to `LayerFreezeState::perms` (the per-card layer memo): sound, and **+0.70 % uncapped / +1.13 % capped on `cube`**. The carried hit rate is **3.1 %** — a freeze scope opens on an unmoved board once in thirty-two — against the cross *gather* memo's 23.7 %, because a gather is asked once a scope and a pass once a permanent. **`(-303)`'s hit rate does not transfer to a finer key.** And `cp_pool` is worth **~40,000 allocations a six-game `cube` run** (5.3 % of 757,012): its feed IS the scope-end release, so the two devices are mutually exclusive by construction. (b) `(-129)`'s outlining applied to `affected_includes_gated`, the largest row `cg_frames.py` has left: **+0.026 %**, prologue unchanged at five registers. **`out/call ≈ 0` is necessary but not sufficient** — the surviving `All`/`AllOpponents` arms are six- and seven-term `&&` chains that want the registers themselves. `(-129)` paid because `event_matches_spec`'s hot arms were one compare each. Read the hot arms' disassembly before spending a build.
6. **The profile is FLAT and the candidates list is thin — say so rather than re-deriving it.** Whole-program line profile at this tip: top single row **0.97 %** (`Arc::clone_from_ref_in`), and the `(-342)` grouping by source line is all slice-iteration and pointer machinery (`iter/macros.rs:?` 4.67 %, `ptr/non_null.rs:444` 3.15 %, …). `card.rs:13` (`CardId`'s `PartialEq`) is down to **17.18 M / 1.14 %** from 22.21 M — `(-339)`..`(-342)` took 5 M off it. Allocation is **757,012 a `cube` run and diffuse** (`finish_grow`'s largest single site is 7 % of regrowths). Open queue in PERF: (A) `ManaCost::cmc`, (B) the `CardCold` `Arc::ptr_eq` gate (~0.12 %, a base change at ~7 min a build here), (C) `blocker_pair_block`'s pair count, (H) the other zones' by-id scans (censused, 138 sites, the obvious fix is wrong).
7. ⚠ **THE CONTAINER IS SUSPENDED BETWEEN TURNS — background waits DO NOT PROGRESS.** Run builds and A/Bs in the FOREGROUND with the tool's 600 s timeout; when one overruns and is backgrounded, **re-issue the same `cargo build` in the foreground** — it blocks on the artifact-directory lock and that block *is* the wait. ⚠⚠ **DISK: `target` reached 1.3 GB free mid-run.** `rm -rf target/debug/incremental && find target/debug/deps -maxdepth 1 -type f -size +50M -mmin +60 -delete` returned **15 GB** and rebuilt nothing. Do it before a `release` build, not after it dies.
8. **Box timings (twelfth box, 4 cores, 15 GB — roughly 2.5x the eleventh):** cold `profiling-fast` 7m29s, **engine-only 2m13s**, a `crabomination_base` change 7m20s, cold `release` 7m51s, `overflow` 7m02s, full suite 2m34s warm, workspace clippy 3m51s, `cargo check --profile release-fast` 1m04s, three-pool callgrind ~35 s in parallel, one `cube` callgrind 15 s.
9. **Bugs — THREE FIXED THIS RUN, ALL ONE CLASS, AND THE CLASS WAS FOUND BY READING WALKERS RATHER THAN BY AN AUDIT.** (a) `has_toxic`/`has_modular` read printed + `granted_keywords_eot` and stopped; closed as a class with `CardInstance::has_keyword_tag`, `has_keyword` with the value ignored. (b) **`layers::requirement_matches_card` — the THIRD requirement walker, the one the layer pass itself runs — answered `HasKeyword`/`HasToxic`/`HasModular` off `definition.keywords` alone**, so Crosswinds ("creatures with flying get -2/-0") missed a creature granted flying until end of turn or off a keyword counter and still shrank one whose flying had been stripped; `eval.rs`'s walker answered the same filter with all four sources, so the two disagreed depending on which path evaluated it. The fix is not circular: the grant, the counters and the two removal lists are *inputs* to the layer pass, like the `Tapped`/`WithCounter` leaves beside them. (c) Six hand-written copies of the morph keyword set (plus cycling ×2, flashback ×2, disturb ×3) unified onto `CardDefinition::has_morph_ability` & co.
10. 🔎 **THE VEIN IS NOT EXHAUSTED AND IT IS THE BEST LEAD ON THIS PAGE. There are FOUR parallel requirement walkers** and they were never diffed: `eval.rs`'s `evaluate_requirement_static_hinted` (battlefield, 160 arms), `eval.rs`'s `evaluate_requirement_on_card` (off-battlefield, 138), `layers.rs`'s `requirement_matches_card` (the layer pass), `mod.rs:28878`'s computed-view one. **116 arms are shared between the first two and 50 of them differ textually** — most legitimately (computed vs printed), but that is 50 hand-reads nobody has done, and the keyword family was one of them. Extract the arm bodies with a small `re` script over the four ranges and read the diff.
11. **Cards & rules, leftover only:** `audit_dropped_may` 199; `audit_keyword_drift`'s last 4 (bestow / escalate / flashback / kicker) each need a PRIMITIVE; card skips `nocolors` 950 / `noback` 60 / `nosubtypes` 43 / `nokeywords` 29; `audit_incomplete` 858 documented approximations.
