...
gate    --bench on a **`release`** binary (cgu 1 + thin LTO, not the `release-fast` the older rows use — the counters are
        ...
        columns; audit_printed_body_injections **29 / 29**; audit_card_names **0 / 0 / 0 / 0** with an EMPTY
...
suite2  after the `run_each_unless_pays` split: **19,417 / 0 / 5** under `CRAB_ANSWER_LOG=strict`, golden_trace 10 / 10 still
        ...
        rebase over the other session's CR 608.2b fix is a tree neither of us had tested: **19,418 / 0 / 5, clippy 0, and
        `--bench` 195,806 / 27.49 / 611.9 / 0 stalls with determinism + thread_determinism ok — the counters cross the merge
        unchanged, which is the whole reason they and not the wall clock are the gate**
...
actor   callgrind selfplay_train --actors 1 --games 60 --steps 1 --seed 7 (profiling-fast -p crabomination_ml --no-default-features, system allocator,
        CRAB_NO_JITTER=1), the run's base b7bd9250 built in a worktree against the shared target dir (sources touched, md5 differs) vs the (-287) tip:
        2,483,983,953 -> 2,436,453,201 Ir (-1.913 %), 60 games / 6,063 rows / 0 stalls both sides — the eight legs on the training path, one number
