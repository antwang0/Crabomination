...
9. **Guardrail at the tip `04742c60`:** `--bench` invariant **195,806 / 27.49 / 611.9 / 0 stalls**, `determinism ok`, `peak_rss_mib` 25.5 — byte-identical to the committed one across six Commander rules features, two multiplayer card fixes and a concurrent session's mana-provenance commit, so all of it is behaviour-preserving in a duel. ⚠ `games_per_s` read 474-505 over four builds on one box at one calib: that spread is the noise floor, and the invariant is what carries the verdict. ⚠ games/s does **not** cross hosts (this box is a 2.10 GHz Xeon); PERF's Commander block says what to compare. The pod's own determinism check is free and is `--threads 1` vs the default.
