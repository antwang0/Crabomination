...
7. ✅✅ **`CRAB_CAP_DIAG` NAMES THE LOOP NOW, AND IT CLOSED ONE END TO END IN THREE READS.** `ActionCensus` tallies an undecided pod game's actions by `(seat, variant, card, ability index)`; `cap_diagnosis`'s board is **per seat and untruncated**. The eight-seat sweep capped 1 game in 2,000 and the three reads were: ① `ActivateAbility#0 Eldrazi Spawn x7535` (the census — `cap_diagnosis`'s four zone lines showed nothing, which is the blindness its own header warns about); ② adding the **seat** to the key: `p5` takes all 7,535 *and* holds all 7,533 floating mana, so one loop, not two; ③ the per-seat board: `p5 board: … Basking Broodscale x1, Cordial Vampire x1, Eldrazi Spawn x1 …`, which the old twelve-row whole-battlefield tally had hidden behind seven Vampires. **Put the actor in the key of any census whose consumers are per-seat, and never let an inventory line truncate.**
8. 🔎🔎 **AND THE ANSWER IS THAT NOTHING IS BROKEN — IT IS A REAL INFINITE, AND THAT IS THE FINDING.** Verified by reading, not guessing: the sacrifice **is** charged (`activate_ability_inner`'s battlefield branch ends at `bf_pos.ok_or(CardNotOnBattlefield)?`), Basking Broodscale matches its oracle, `shortcut::graft()` no-ops when empty, and **Cordial Vampire is exactly its print** — "whenever this creature **or another creature** dies", i.e. `CreatureDied` / `AnyPlayer`, no Vampire filter, correctly. The loop: p5 stole p3's Broodscale with **New Blood** (which text-changes it into a Vampire), so — sac the Spawn for {C} → a creature died → Cordial Vampire puts a +1/+1 counter on **each Vampire you control**, Broodscale included → Broodscale's "whenever one or more +1/+1 counters are put on this creature, you may create an Eldrazi Spawn" → repeat. Every step is correct Magic. **The bot loops because every iteration is materially positive** — a dozen Vampires each gain +1/+1 — so `pick_sacrifice_value`'s `ev > baseline` is true forever and it never converts to a win. ⚠ Neither backstop can see it: the battlefield never grows (one Spawn at a time) so `MAX_BATTLEFIELD` is untouched, and the counters move every iteration so CR 104.4's fingerprint never repeats — the diag printed `repeats 0/12` and that is *correct*. **The fix is a bot one — a real player takes CR 720's shortcut and names a number — and `bot.rs` is the `--bench` path, so it needs the throughput gate re-taken, not just the suite.** A guard keyed on "same (card, ability) N times this turn with no cast between" is provably bench-neutral for N large (no archetype activates anything 64x a turn) but needs per-turn state the bot does not carry. Repro: `CRAB_CAP_DIAG=1 target/release-fast/bot_ladder --commander --seats 8 --games 2000 --threads 3 --seed 9101`, pod seed **18045724030442976829**, turn 59. Rate 1/2,000 at eight seats, 0/12,000 at 2..7.
