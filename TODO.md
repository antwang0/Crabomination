...
5. **Seeds: frontier 1292 — THIS RUN IS TAKING 1282..1291 (claimed before it was run; the verdict is in PERF's Baseline entry for the run).** Say which block you are taking in this line *before* you take it; that is five same-seed collisions in two days and PERF's table is the only thing that stops it. ⚠ A sweep at this tip is not the same workload as one at the last: `(-295)` changes what the bots do on any board with a sacrifice-other outlet.
6. **Best next moves.** (a) **The class `(-295)` opened is only half taken**: `sac_other_filter` now reaches its priced owner, but `bounce_other_filter`, `exile_other_filter` and `discard_cost` have **no priced owner at all** — `ability_sink_bits` does not set `AB_SAC` for them and `pick_sacrifice_value` does not consider them. (b) The follow-up named in code: `pick_removal_sacrifice`'s favourable-trade test reads the SOURCE's power, so a destroy-shaped sacrifice-other ability has no trade math and `pick_removal_destroy` keeps the narrow gate on purpose. (c) The skips with a card behind them: `nocolors` **950**, `nosubtypes` 43, `noback` 60, `nokeywords` 29 — say which you are taking. (d) `audit_keyword_drift`'s last 4 each need a PRIMITIVE. (e) `audit_variant_coverage` is at ONE dead primitive, closed with a reason (`GrantCastBackFromGraveyard`); do not go looking.
