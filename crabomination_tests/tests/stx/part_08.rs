...
fn witherbloom_vitalist_grows_on_lifegain() {
    ...
    g.players[0].mana_pool.add(Color::White, 1);
    g.perform_action(GameAction::CastSpell {
        card_id: life, target: Some(crabomination::game::types::Target::Player(0)),
        additional_targets: vec![], mode: None, x_value: None,
...
fn witherbloom_toxinkeeper_etb_shrinks_target() {
    ...
    g.players[0].mana_pool.add_colorless(2);
    g.perform_action(GameAction::CastSpell {
        card_id: id, target: Some(crabomination::game::types::Target::Permanent(opp_bear)),
        additional_targets: vec![], mode: None, x_value: None,
...
fn witherbloom_pesthatch_mints_pest_and_pumps() {
    ...
    g.players[0].mana_pool.add_colorless(1);
    g.perform_action(GameAction::CastSpell {
        card_id: id, target: Some(crabomination::game::types::Target::Permanent(friend)),
        additional_targets: vec![], mode: None, x_value: None,
...
fn lorehold_grave_crusader_etb_exiles_target_graveyard_card() {
    ...
    g.players[0].mana_pool.add_colorless(2);
    g.perform_action(GameAction::CastSpell {
        card_id: id, target: Some(crabomination::game::types::Target::Permanent(bolt)),
        additional_targets: vec![], mode: None, x_value: None,
...
fn lorehold_pyrescholar_grows_on_card_leave_gy() {
    ...
    g.players[0].mana_pool.add_colorless(1);
    g.perform_action(GameAction::CastSpell {
        card_id: acolyte, target: Some(crabomination::game::types::Target::Permanent(gy_card)),
        additional_targets: vec![], mode: None, x_value: None,
...
fn lorehold_pyromaster_taps_for_three_damage() {
    ...
    let opp_before = g.players[1].life;
    g.perform_action(GameAction::ActivateAbility {
        card_id: id, ability_index: 0, target: Some(crabomination::game::types::Target::Player(1)), additional_targets: Vec::new(), x_value: None }).expect("Pyromaster activated");
    drain_stack(&mut g);
...
fn inkling_quillbearer_magecraft_shrinks_target() {
    ...
    g.players[0].mana_pool.add(Color::Red, 1);
    g.perform_action(GameAction::CastSpell {
        card_id: bolt, target: Some(crabomination::game::types::Target::Permanent(opp_bear)),
        additional_targets: vec![], mode: None, x_value: None,
    }).expect("Bolt castable");
    drain_stack(&mut g);
...
fn reduce_to_ashes_only_damages_a_tall_creature() {
    ...
    g.players[0].mana_pool.add_colorless(20);
    g.perform_action(GameAction::CastSpell {
        card_id: id, target: Some(crabomination::game::types::Target::Permanent(hulk)),
        additional_targets: vec![], mode: None, x_value: None,
    }).expect("castable");
    drain_stack(&mut g);
...
fn mercurial_transformation_strips_keywords_from_target() {
    ...
    g.perform_action(GameAction::CastSpell {
        card_id: id,
        target: Some(crabomination::game::types::Target::Permanent(dragon)),
        additional_targets: vec![], mode: None, x_value: None,
...
fn mercurial_transformation_strips_etb_triggers_from_target() {
    ...
    g.perform_action(GameAction::CastSpell {
        card_id: merc,
        target: Some(crabomination::game::types::Target::Permanent(witch)),
        additional_targets: vec![], mode: None, x_value: None,
