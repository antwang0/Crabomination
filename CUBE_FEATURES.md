...
| Master of Cruelties | 🟡 | {2}{B}{R} 1/4 First Strike Deathtouch Demon. "Attacks only alone" via `Keyword::AttacksAlone`; the life-set rides `AttacksAndIsntBlocked` (fires only when unblocked). Only the "deals no combat damage this turn" rider remains dropped — its first-strike deathtouch ping finishes the kill. |
