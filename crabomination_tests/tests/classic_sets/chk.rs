...
fn soilshaper_spiritcraft_animates_a_land() {
    ...
    assert!(cp.card_types.contains(&crabomination::card::CardType::Creature), "land is now a creature");
...
fn kamigawa_vanilla_keywords() {
    assert!(catalog::sokenzan_bruiser().keywords
        .contains(&Keyword::Landwalk(crabomination::card::LandType::Mountain)));
...
fn honden_cleansing_fire_scales_with_shrines() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(honden, 0, None, 0);
    g.resolve_effect(&trig, &ctx).unwrap();
...
fn crescent_moon_extra_draw() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(kami, 0, None, 0);
    g.resolve_effect(&trig, &ctx).unwrap();
...
fn akki_underminer_forces_sacrifice() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(akki, 0, Some(Target::Player(1)), 0);
    g.resolve_effect(&trig, &ctx).unwrap();
    drain_stack(&mut g);
...
fn ronin_cliffrider_pings_defenders() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(ronin, 0, Some(Target::Player(1)), 0);
    g.resolve_effect(&trig, &ctx).unwrap();
    g.check_state_based_actions();
...
fn time_of_need_tutors_legend_to_hand() {
    ...
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Search(Some(konda)),
    ]));
...
fn kami_of_the_painted_road_gains_protection_on_spiritcraft() {
    ...
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Color(crabomination::mana::Color::Red),
    ]));
    ...
    assert!(c.keywords.contains(&Keyword::Protection(crabomination::mana::Color::Red)),
...
fn orochi_leafcaller_filters_mana() {
    ...
    assert_eq!(g.players[0].mana_pool.amount(crabomination::mana::Color::Blue), 1, "produced blue");
}
...
fn kitsune_healer_prevents_next_1_damage() {
    ...
    g.deal_damage_to_from(crabomination::game::effects::EntityRef::Permanent(bear), 1, None, &mut events);
...
fn akki_rockspeaker_adds_red_on_etb() {
    ...
    drain_stack(&mut g);
    assert_eq!(g.players[0].mana_pool.amount(crabomination::mana::Color::Red), 1, "ETB added one red mana");
}
...
fn mistblade_shinobi_bounces_on_combat_damage() {
    ...
    advance_to(&mut g, crabomination::game::TurnStep::DeclareAttackers);
    g.perform_action(GameAction::DeclareAttackers(vec![Attack {
        attacker: ninja, target: AttackTarget::Player(1),
    }])).expect("attack");
    drain_stack(&mut g);
    advance_to(&mut g, crabomination::game::TurnStep::CombatDamage);
    drain_stack(&mut g);
...
fn otherworldly_journey_blinks_with_counter() {
    ...
    advance_to(&mut g, crabomination::game::TurnStep::End);
    drain_stack(&mut g);
...
fn kumanos_pupils_exiles_what_it_kills() {
    ...
    drain_stack(&mut g);
    advance_to(&mut g, crabomination::game::TurnStep::DeclareBlockers);
...
fn heartbeat_of_spring_doubles_land_mana() {
    ...
    assert_eq!(g.players[0].mana_pool.amount(crabomination::mana::Color::Green), 2, "{{G}} becomes {{G}}{{G}}");
    ...
    assert_eq!(g.players[1].mana_pool.amount(crabomination::mana::Color::Green), 2, "opponent's {{G}} doubles too");
}
...
fn gutwrencher_oni_upkeep_discard_unless_ogre() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(oni, 0, None, 0);
    // No Ogre → discard one.
    let before = g.players[0].hand.len();
    g.resolve_effect(&trig, &ctx).unwrap();
    drain_stack(&mut g);
...
fn long_forgotten_gohei_anthems_and_reduces_arcane_cost() {
    ...
    g.players[0].mana_pool.add(crabomination::mana::Color::Red, 1); // only {R}, no generic
    let bear = g.add_card_to_battlefield(1, catalog::grizzly_bears());
    g.perform_action(GameAction::CastSpell {
...
fn marrow_gnawer_lords_rats_and_mints_tokens() {
    ...
    let rats = g.battlefield.iter()
        .filter(|c| c.controller == 0
            && c.definition.subtypes.creature_types.contains(&crabomination::card::CreatureType::Rat))
        .count();
...
fn kuro_pitlord_upkeep_sacrifice_and_pay_life_shrink() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(kuro, 0, None, 0);
    g.resolve_effect(&trig, &ctx).unwrap();
    drain_stack(&mut g);
...
fn seizan_drains_and_draws_for_active_player() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(seizan, 0, None, 0);
    g.resolve_effect(&trig, &ctx).unwrap();
    drain_stack(&mut g);
...
fn eerie_procession_finds_an_arcane_card() {
    ...
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Search(Some(ray)),
    ]));
    g.players[0].mana_pool.add(crabomination::mana::Color::Blue, 1);
    g.players[0].mana_pool.add_colorless(2);
...
fn ethereal_haze_fogs_combat() {
    ...
    drain_stack(&mut g);
    advance_to(&mut g, crabomination::game::TurnStep::PostCombatMain);
...
fn hands_carry_protection_and_bushido() {
    ...
    assert!(cruelty.keywords.contains(&Keyword::Protection(crabomination::mana::Color::White)));
    assert!(cruelty.keywords.contains(&Keyword::Bushido(1)));
    let honor = catalog::hand_of_honor();
    assert!(honor.keywords.contains(&Keyword::Protection(crabomination::mana::Color::Black)));
}
...
fn nezumi_shadow_watcher_destroys_a_ninja() {
    ...
    let ninja = g.add_card_to_battlefield(1, {
        let mut d = catalog::grizzly_bears();
        d.name = "Ninja"; d.subtypes.creature_types = vec![crabomination::card::CreatureType::Ninja]; d
    });
...
fn akki_lavarunner_flips_on_combat_damage_to_player() {
    ...
    assert!(tok.definition.keywords.contains(&Keyword::Protection(crabomination::mana::Color::Red)));
}
...
fn tok_tok_adds_one_to_red_damage_to_players() {
    ...
    red.name = "Red Ogre";
    red.cost = crabomination::mana::cost(&[crabomination::mana::generic(1), crabomination::mana::r()]); // red source
...
fn blessed_breath_grants_protection() {
    ...
    g.players[0].mana_pool.add(crabomination::mana::Color::White, 1);
    g.decider = Box::new(ScriptedDecider::new([DecisionAnswer::Color(crabomination::mana::Color::Red)]));
    g.perform_action(GameAction::CastSpell {
        card_id: spell, target: Some(Target::Permanent(bear)), additional_targets: vec![], mode: None, x_value: None,
    }).expect("cast Blessed Breath");
    drain_stack(&mut g);
    let cp = g.computed_permanent(bear).unwrap();
    assert!(cp.keywords.contains(&Keyword::Protection(crabomination::mana::Color::Red)), "protection from red");
}
