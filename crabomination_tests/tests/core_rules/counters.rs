...
fn arcbound_ravager_sacs_artifact_for_a_counter() {
    ...
    g.perform_action(GameAction::ActivateAbility {
        card_id: ravager, ability_index: 0, target: None, additional_targets: Vec::new(), x_value: None, mode: None,
...
fn melee_pumps_the_attacker() {
    ...
    let def = CardDefinition {
        ...
        power: 2,
        toughness: 2,
        triggered_abilities: vec![crabomination::effect::shortcut::melee()],
        ..Default::default()
    };
...
fn vigean_graftmage_untaps_a_countered_creature() {
    ...
    g.players[0].mana_pool.add_colorless(1);
    g.perform_action(GameAction::ActivateAbility {
        card_id: mage, ability_index: 0, target: Some(Target::Permanent(target)), additional_targets: Vec::new(), x_value: None, mode: None,
...
fn helium_squirter_grants_flying_to_a_countered_creature() {
    ...
    g.players[0].mana_pool.add_colorless(1);
    g.perform_action(GameAction::ActivateAbility {
        card_id: squirter, ability_index: 0, target: Some(Target::Permanent(target)), additional_targets: Vec::new(), x_value: None, mode: None,
...
fn amasser(n: i32) -> crabomination::card::CardDefinition {
    use crabomination::card::{CardType, CardDefinition};
