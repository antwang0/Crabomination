...
8. 💡 **NINE LOOPS ARE STILL OPEN AND THE ALLOWLIST NAMES THE PRIMITIVE EACH WAITS ON** — chiefly a `Selector` that names a specific `CardId` (`ForEach` over permanents wants it, and so do `TurnFaceUpFree`/`EyeOfTheStorm`), and a way to carry per-iteration state that lives on `self` (`current_voter`, `last_die_roll`). **The `CardId` selector unblocks three of the nine and is the single best next engine primitive here.**
