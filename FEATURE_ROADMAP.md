...
| CR 702.174a Gift promises the gift to ONE chosen opponent | ✅ *since 2026-09-26* | `actions.rs` stamps `chosen_player` at cast via `choose_player.rs::choose_gift_recipient` (a ballot, headless pick the opponent with the fewest creatures); gift bodies read `PlayerRef::ChosenPlayerOfSource`, a resolving gift spell through `chosen_opponent_scratch`. 21 gift cards had given every opponent the gift. Tests `multiplayer::cr_702_174a_*` |
