...
fn resolve_spell_mode(
    ...
    let mut ctx = crabomination::game::effects::EffectContext::for_spell(0, None, mode, 0);
    ctx.targets = targets;
...
fn constable_exile_until_leaves() {
        ...
        .resolve_effect(
            &crabomination::effect::Effect::AddCounter {
                what: crabomination::effect::Selector::EachPermanent(
                    crabomination::card::SelectionRequirement::HasCreatureType(
                        crabomination::card::CreatureType::Giant,
                    ),
                ),
    ...
    let mut ctx2 = crabomination::game::effects::EffectContext::for_spell(0, None, 0, 0);
