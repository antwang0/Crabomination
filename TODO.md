...
10. **Cards and decks, what is left:** ⏳ 32 of 33 "can be your commander" non-creatures are unimplemented; the flag and validator are done, so each is a plain card. ⏳ `scripts/audit_each_opponent.py`'s seven residuals are in INCOMPLETE_CARDS with what each needs. ✅ Obscuring Haze shipped, so the free-spell cycle is complete (the primitive, `Effect::PreventAllDamageByMatchingThisTurn`, landed in its own commit ahead of it).
