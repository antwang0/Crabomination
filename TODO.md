...
2. ⚠⚠ **READ PERF'S `(-339)` ENTRY BEFORE PLANNING ANY PERF WORK. The session's largest row was found by `grep`, not by a dump**, and it had survived every pass this file records: 53 hand-written `battlefield.iter().find(|c| c.id == x)` scans that never took `find_by_id`'s hint cache. **A hand-written `iter().find()` inlines into its caller and never becomes a symbol**, so the self table, `cg_calls.py`, `cg_edges.py` and the line profile are ALL structurally blind to it — the candidates list's entire method is "rank the symbols". Queue item (G) had sized ONE of those sites at ~0.07 % and nearly declined it; the class was **-0.814 % on `cube`**. **When a hot-path helper exists, grep for its hand-written spelling before profiling for it.**
