...
6. **Best next moves.** (a) ✅ **`notyped` is 0 and is a GATE now** — the seven were read by hand against the oracle first (zero defects, so the yield was always the gate), and the two shapes were a nested literal as the base and a spread that is not the start of its line; +8 cards on the type line, +7 subtypes, +7 keywords, +5 P/T, 0 wrong anywhere, and `nocolors` 945 -> 950 is those cards reaching a column they then skip at. Injections **54**. (b) The skips with a card behind them: `nocolors` **950**, `nosubtypes` 43, `noback` 60, `nokeywords` 29 — say which you are taking before you take it. (c) `audit_keyword_drift`'s last 4 each need a PRIMITIVE. (d) `audit_variant_coverage` is down to ONE dead primitive and it is CLOSED WITH A REASON — `GrantCastBackFromGraveyard` is the one-shot grant nothing prints (the printed shape is Disturb, a keyword); do not go looking for the Pestilent Cauldron sacrifice its doc used to name. (e) `audit_target_walkers`' sixth column (`target_filter_for_slot_in_mode_kicked`, 50 unnamed of 132) is LATENT with **zero current yield** — the catalog gate passes — so it is a surface, not a queue. (f) **The perf queue has been at floor for eight runs and every perf leg came off a bug fix**; PERF's "Perf candidates" head carries the closed leads. (g) ⚠ **THE REST OF THE FAN-OUT LIST HAS NEVER BEEN AUDITED and it is the cheapest bug-class pull on this page.** `event_kind_fans_out` is 30 kinds; censusing every `EventSpec::new(EventKind::X, EventScope::Y)` in the catalog against it, the kinds a single batch can carry several of and that are **absent** are, by trigger count: ~~`EntersBattlefield` **341**~~ **TAKEN this run, and it was real** (item 3c); what is left is `DealsCombatDamageToPlayer` **52** (every attacker connects in one batch) — ⚠ **TAKEN 2026-09-13 07:20 UTC by the session on `claude/cool-lovelace-nqvucq`'s schedule, claimed here BEFORE the work per item 5; if this line is still unresolved at your tip the claim lapsed and it is yours** — then `PutIntoGraveyard` **12**, `TokenCreated` 5, `Explored` 4, `LandPutIntoGraveyard` 3, all four UNCLAIMED. `SpellCast` (288) and `StepBegins` (396) are safe — `SpellCast` is hardcoded out of the batch walk and a step begins once. **Probe each before believing it**: `CardMilled` looked like a five-card bug from the census and was a three-card one, because two of the five print the plural wording.
