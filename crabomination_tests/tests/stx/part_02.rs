...
fn shaile_counters_only_creatures_that_entered_this_turn() {
    ...
    g.perform_action(GameAction::ActivateAbility {
        card_id: shaile, ability_index: 0, target: None, additional_targets: Vec::new(), x_value: None, mode: None,
...
fn witherbloom_pest_eater_etb_creates_pest_token() {
    ...
    let pest = g.battlefield.iter().find(|c|
        c.definition.has_creature_type(crabomination::card::CreatureType::Pest)
            && c.id != id
...
fn forked_bolt_rejects_duplicate_target() {
    use crabomination::game::GameError;
    let mut g = two_player_game();
    let bear = g.add_card_to_battlefield(1, catalog::grizzly_bears());
    let id = g.add_card_to_hand(0, catalog::forked_bolt());
    g.players[0].mana_pool.add(Color::Red, 1);
...
fn bone_to_ash_counters_creature_spell_and_cantrips() {
    ...
    let bone_to_ash_target = match g.stack.last().expect("a spell is on the stack") {
        crabomination::game::types::StackItem::Spell { card, .. } => card.id,
...
fn strixhaven_stadium_mana_ability_adds_point_counter() {
    ...
    g.perform_action(GameAction::ActivateAbility {
        card_id: stadium, ability_index: 0, target: None, additional_targets: Vec::new(), x_value: None, mode: None,
    }).expect("tap for {C}");
    drain_stack(&mut g);
...
fn soothing_hush_counters_creature_spell() {
    ...
    g.perform_action(GameAction::CastSpell {
        card_id: hush,
        target: Some(crabomination::game::types::Target::Permanent(bears)),
        additional_targets: vec![],
        mode: None, x_value: None,
...
fn pillage_destroys_target_land_or_artifact() {
    ...
    g.perform_action(GameAction::CastSpell {
        card_id: id,
        target: Some(crabomination::game::types::Target::Permanent(opp_land)),
        additional_targets: vec![], mode: None, x_value: None,
    ...
    g.perform_action(GameAction::CastSpell {
        card_id: id,
        target: Some(crabomination::game::types::Target::Permanent(opp_artifact)),
        additional_targets: vec![], mode: None, x_value: None,
...
fn hindering_light_cant_counter_spell_aimed_elsewhere() {
    ...
    g.perform_action(GameAction::CastSpell {
        card_id: bolt,
        target: Some(crabomination::game::types::Target::Permanent(own_bear)),
        additional_targets: vec![], mode: None, x_value: None,
    }).expect("Bolt castable");
