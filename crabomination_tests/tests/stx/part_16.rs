...
fn shortcut_etb_mint_token_and_drain_expands_to_seq_create_then_drain() {
    ...
    let ta = crabomination::effect::shortcut::etb_mint_token_and_drain(
        crabomination::catalog::stx_pest_token(), 2,
    );
    ...
    if let crabomination::effect::Effect::Seq(steps) = &ta.effect {
        ...
        assert!(matches!(steps[1], crabomination::effect::Effect::Drain { .. }));
    } else {
...
fn shortcut_etb_drain_and_draw_drains_and_draws() {
    use crabomination::effect::shortcut::etb_drain_and_draw;
    use crabomination::card::{Effect, EventKind, EventScope};
