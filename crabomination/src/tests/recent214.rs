...
fn sphinx_makes_your_spells_uncounterable() {
    ...
    let mine = crate::card::CardInstance::new(g.next_id(), catalog::lightning_bolt(), 0);
    let theirs = crate::card::CardInstance::new(g.next_id(), catalog::lightning_bolt(), 1);
...
fn drogskol_draws_on_lifegain() {
    ...
    let evs = g.resolve_effect(&Effect::GainLife { who: Selector::You, amount: Value::Const(1) },
        &crate::game::effects::EffectContext::for_ability(reaver, 0, None)).unwrap();
