...
fn enigmatic_incarnation_fetches_on_end_step() {
    ...
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Bool(true),
        crabomination::decision::DecisionAnswer::Search(Some(hill)),
    ]));
    g.fire_step_triggers(TurnStep::End);
    drain_stack(&mut g);
...
fn kunoros_locks_graveyards_only() {
    ...
    assert!(g.cast_from_zone_blocked(
        ...
        crabomination::card::Zone::Graveyard
    ));
    // Library casts are NOT locked (unlike Grafdigger's Cage).
    assert!(!g.cast_from_zone_blocked(
        0,
        &catalog::lightning_bolt(),
        crabomination::card::Zone::Library
    ));
}
...
fn moss_viper_has_deathtouch() {
    ...
    assert!(g.battlefield_find(v).unwrap().has_keyword(&crabomination::card::Keyword::Deathtouch));
}
...
fn hyrax_tower_scout_untaps() {
    ...
    g.players[0].mana_pool.add_colorless(2);
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Target(Target::Permanent(dork)),
    ]));
    g.perform_action(GameAction::CastSpell {
...
fn captivating_unicorn_constellation_taps() {
    ...
    let ench = g.add_card_to_battlefield(0, catalog::escape_protocol());
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Target(Target::Permanent(opp)),
    ]));
    g.dispatch_triggers_for_events(&[GameEvent::PermanentEntered { card_id: ench }]);
    drain_stack(&mut g);
...
fn nyxborn_vanillas_are_enchantment_creatures() {
    for (def, pt) in [
        ...
        assert!(def.card_types.contains(&crabomination::card::CardType::Enchantment), "{}", def.name);
        assert!(def.card_types.contains(&crabomination::card::CardType::Creature), "{}", def.name);
...
fn leonin_dies_exiles_opponent_graveyard() {
    ...
    g.players[0].mana_pool.add(Color::Red, 1);
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Target(Target::Permanent(gy)),
    ]));
    g.perform_action(GameAction::CastSpell {
...
fn scavenging_harpy_exiles_from_opponent_graveyard() {
    ...
    g.players[0].mana_pool.add_colorless(2);
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Target(Target::Permanent(dead)),
    ]));
    g.perform_action(GameAction::CastSpell {
...
fn warbriar_blessing_fights() {
    ...
    g.players[0].mana_pool.add_colorless(1);
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Target(Target::Permanent(enemy)),
    ]));
    g.perform_action(GameAction::CastSpell {
...
fn chainweb_aracnir_pings_a_flyer() {
    ...
    g.players[0].mana_pool.add(Color::Green, 1);
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Target(Target::Permanent(hawk)),
    ]));
    g.perform_action(GameAction::CastSpell {
...
fn archon_of_falling_stars_returns_enchantment() {
    ...
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Bool(true),
        crabomination::decision::DecisionAnswer::Target(Target::Permanent(ench)),
    ]));
...
fn shimmerwing_chimera_bounces_own_enchantment_at_upkeep() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(chimera, 0, Some(Target::Permanent(other)), 0);
    g.resolve_effect(&trig, &ctx).unwrap();
...
fn inevitable_end_sacrifices_at_upkeep() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(victim, 0, None, 0);
...
fn kiora_makes_kraken() {
    ...
    assert!(kraken.definition.keywords.contains(&crabomination::card::Keyword::Hexproof));
}
...
fn nightmare_shepherd_copies_the_dead() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(doomed, 0, None, 0);
    g.resolve_effect(&eff, &ctx).unwrap();
    ...
    assert!(copy.definition.subtypes.creature_types.contains(&crabomination::card::CreatureType::Nightmare));
}
...
fn lagonna_band_storyteller_recurs_enchantment() {
    ...
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Bool(true),
        crabomination::decision::DecisionAnswer::Target(Target::Permanent(aura)),
    ]));
    let life_before = g.players[0].life;
    let eff = catalog::lagonna_band_storyteller().triggered_abilities[0].effect.clone();
    let ctx = crabomination::game::effects::EffectContext::for_trigger(teller, 0, Some(Target::Permanent(aura)), 0);
    g.resolve_effect(&eff, &ctx).unwrap();
...
fn triumph_of_anax_fight() {
    ...
    let mut ctx = crabomination::game::effects::EffectContext::for_trigger(saga, 0, Some(Target::Permanent(mine)), 0);
...
fn triumph_of_anax_pumps_by_lore() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(saga, 0, Some(Target::Permanent(bear)), 0);
    g.resolve_effect(&eff, &ctx).unwrap();
    let cp = g.computed_permanent(bear).unwrap();
...
fn elspeth_suns_nemesis_makes_soldiers() {
    ...
    let soldiers = g.battlefield.iter()
        .filter(|c| c.controller == 0 && c.definition.subtypes.creature_types.contains(&crabomination::card::CreatureType::Soldier))
        .count();
...
fn gravebreaker_lamia_discounts_graveyard_casts() {
    ...
    use crabomination::game::actions::cost_reduction_for_spell_zoned;
...
fn ashiok_nightmare_muse_makes_token() {
    ...
    let token = g.battlefield.iter().find(|c|
        c.controller == 0 && c.definition.subtypes.creature_types.contains(&crabomination::card::CreatureType::Nightmare)).unwrap();
...
fn elspeths_devotee_tutors_elspeth() {
    ...
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Search(Some(walker)),
    ]));
...
fn deathbellow_war_cry_tutors_minotaurs() {
    ...
    g.players[0].mana_pool.add_colorless(5);
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Search(Some(m1)),
        crabomination::decision::DecisionAnswer::Search(Some(m2)),
...
fn siona_finds_an_aura() {
    ...
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Search(Some(aura)),
    ]));
...
fn view_surfaces_aura_characteristic_override() {
    ...
    assert!(pv.creature_subtypes.contains(&crabomination::card::CreatureType::Fish), "shown as a Fish");
...
fn dreamshaper_shaman_digs_to_a_permanent() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(shaman, 0, None, 0);
    g.resolve_effect(&eff, &ctx).unwrap();
    drain_stack(&mut g);
...
fn athreos_reclaims_coin_countered_creature() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(athreos, 0, Some(Target::Permanent(victim)), 0);
    g.resolve_effect(&eff, &ctx).unwrap();
...
fn medomais_prophecy_chapter_iii_draws_on_named_cast() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(saga, 0, None, 0);
...
fn atris_splits_top_three_into_hand_and_graveyard() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(atris, 0, None, 0);
    let hand_before = g.players[0].hand.len();
    let gy_before = g.players[0].graveyard.len();
    g.resolve_effect(&eff, &ctx).unwrap();
...
fn storm_herald_returns_auras_then_exiles_eot() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(herald, 0, None, 0);
    g.resolve_effect(&eff, &ctx).unwrap();
