#!/usr/bin/env bash
# Round 20: cosine lr decay (base 1e-4 -> 1e-5 floor over 60k steps —
# the horizon matches the champion regime's observed early-stop lengths,
# 46-64k) on the champion config. Control: r17b arm A (constant 1e-4,
# AUC 0.8126/0.8069, pooled gates 51.4/52.9%).
set -eu
TRAIN=./target/release/selfplay_train
LADDER=./target/release-fast/bot_ladder

for tseed in 43 97; do
  out="nets_r20_s$tseed"
  $TRAIN --attn --lambda 0.7 --seed "$tseed" --games 250000 --steps 200000 \
      --window 500000 --lr 1e-4 --lr-cosine 60000 \
      --relabel-mode new --stop-after-stale 12 \
      --out "$out" > ".ladder/r20_train_s$tseed.log" 2>&1
  echo "train seed $tseed done"
  $TRAIN --calibrate 500 --attn --use-best "$out/best.safetensors" \
      > ".ladder/r20_calib_s$tseed.txt" 2>&1
done

for tseed in 43 97; do
  export CRAB_NET="nets_r20_s$tseed/best.safetensors"
  for gseed in 43 97; do
    $LADDER --a net --b gang --decks sealed --games 100 --seed "$gseed" \
        > ".ladder/r20_s${tseed}_net_gang_s$gseed.txt" 2>&1
    $LADDER --a net --b atk-sim --decks sealed --games 100 --seed "$gseed" \
        > ".ladder/r20_s${tseed}_net_atksim_s$gseed.txt" 2>&1
  done
  echo "gates seed $tseed done"
done
echo "round-20 complete"
