...
2. **Gates at the `a4fc16ed` tip, all re-run:** `--bench` **195,806 / 27.49 / 611.9 / 0 stalls / peak_rss 25.3** byte-identical to the committed invariant across all eight commits, `determinism ok`, `thread_determinism ok (3 vs 1)`; suite **19,593 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, 12 golden traces unmoved); clippy **0**; `cargo check --profile release-fast` clean. Sweep: **30 cells / 148,000 games / 0 failures** over seeds 1398..1407, `stuck 0 / draw 24 / cap 2` (both `cube` MAX_BATTLEFIELD runaways on a saturated seat — the script's carve-out), **FRONTIER 1408**. `audit_stubs` 0 of 21,797, `audit_panics` 0 bare, `audit_doc_drift` 0/0/0.
3. 🔎🔎 **THE METHOD, AND IT IS NOT EXHAUSTED: DIFF THE WALKERS' ARMS, THEN GREP FOR EVERY OTHER PLACE THAT ANSWERS THE SAME QUESTION.** The arm diff found 33 divergences (one commit). **The caller grep found 61 sites and a bug class**; the `\.power()` grep found two more families. Four commits, three CR 613 classes, twelve finds total for the method. ENGINE_BACKLOG's thirty-fifth, thirty-seventh and thirty-eighth finds have the recipes.
4. ⚠⚠ **AN ALLOWLIST DOCUMENTS A CONTRACT, NEVER THE CALLERS — and that sentence cost three passes.** `evaluate_requirement_on_card`'s "battlefield-state predicates can't be evaluated for library cards" is true of the *function* and was read as true of its 206 call sites; **61 of them iterate the battlefield**. The P3 audit's fourteen "deliberate" drifts were all live on the board. The allowlist is empty now and the test guards the gate. **Go re-read the other allowlists in this repo with that question.**
5. ⚠⚠ **AND THE MIRROR-IMAGE LESSON FROM THE SAME RUN: A RAW `c.power()` IS NOT AUTOMATICALLY A DEFECT.** Of ~26 remaining reads on a battlefield permanent, **only eleven are rules reads**; the rest are auto-decider orderings (a *strength* change with no gate — PERF's `primary_target_filter` note) and one is a per-card pump measurement that `effective_power` would actively break by folding in other permanents' anthems. **Ask whose power the card is asking about before changing a read.**
6. 📐 **`effective_power_on` / `effective_toughness_on` / `computed_is_creature` EXIST NOW AND THEY RE-PRICE THE THIRTY-FOURTH FIND.** `Value::PowerOf`'s fix cost `fixed +0.118 %` for ONE read because `effective_power` pays a by-id find per call. **Seventeen computed reads through the `_on` form cost +0.001 / +0.022 / +0.001 % between them, and `computed_is_creature` reads NEGATIVE** — a gated computed read can be cheaper than the printed one it replaces. Use `_on` whenever the caller is already walking the battlefield; gate a type read the way `printed_requirement_impl` gates its own.
7. ⚠⚠ **A RULES FIX CAN COST 0.6 % OF `cube` AND `--bench` WILL NOT SAY SO.** The card-walker gate read cube **+0.636 %** with `--bench` and all 144 golden traces byte-identical. 📐 **Price the ROUTING separately from the walk:** two-thirds of that was the entry (`battlefield_find` + `PrintedGates` per *ask*), which `(-343)` took back (**cube -0.279 %**) with 13 mechanical hoists. Run total: **fixed +0.024 / cube +0.136 / sealed +0.079 %** for four class fixes.
8. 📐 **`evaluate_requirement_on_card(f, c, …)` WHERE `c` CAME OUT OF `self.battlefield.iter()` IS A GREP-ABLE ANTI-PATTERN.** ~40 sites remain, all resolution-time `run_effect` arms or combat legality *guards* (which need restructuring to hold a `let`): **40 and 216 asks a `cube` run between them**, churn not a row. One hot leftover: a `.filter(..).collect()` at 14,558 asks / +0.03 %, filed at the tail of PERF's candidates.
...
10. ⚠ **`cow::tests::game_state_stays_small` IS 1,680 NOW (was 1,672) AND THE RAISE IS THE PRECEDENT, NOT THE NUMBER.** `(-344)`'s 8-byte field costs 529 k Ir a `cube` run (`GameState::clone` +380 k, `memcpy` +148 k) and the A/B that asked for it already carries that cost. **Raise it again only with a measurement of that shape.**
11. **Perf queue: (J) and (K) CLOSED as diffuse, (L) taken as `(-344)`.** `resolve_combat_into` (1.57 %, 276 lines, top line 0.09 %) and `do_untap` (0.53 %, 89 lines, top 0.04 %) are both "the body IS the cost". ⚠ **Both reads cost NOTHING and saved two builds:** `cg_lines.py` needs no `profiling-lines` build — one 16-second `--dump-instr=yes` callgrind line-profiles every body in the program at once. **Line-profile a candidate before building for it.** Still open: (A) `ManaCost::cmc`, (B) the `CardCold` `Arc::ptr_eq` gate, (C) `blocker_pair_block`'s pair count, (H) the other zones' by-id scans.
12. **THE PROFILE IS FLAT AND PERF'S CANDIDATES HEAD NOW SAYS SO WITH THE GLOBAL LINE TABLE — do not re-derive it.** Largest single source line **0.97 %** (`Arc::clone_from_ref_in`'s memcpy), then 0.80 / 0.56 / 0.41 %, and **12,537 lines hold 86.9 %**. Rank by **self per call**, not by self — that named (J), (K) and (L) in one `cg_calls.py` run, and two of the three were still flat.
13. **Tip absolutes at `a4fc16ed` (`profiling-fast --no-default-features`, system allocator): fixed 579,804,578 / cube 1,512,323,627 / sealed 1,633,737,757.** ✅ The twelfth box's closing absolutes **reproduced to six significant figures**. **"Absolutes do not cross boxes" is about the CPU MODEL, not the box** — same shape and toolchain agree to noise.
14. ⚠⚠ **THREE TRAPS, ALL LIVE.** `cargo build --profile profiling-fast -p crabomination --bin bot_ladder --no-default-features` — **both flags load-bearing**; `nm <bin> | grep -cE " (T|t) (_)?mi_"` = 0 before filing a number. **Background waits do not progress between turns**: run builds in the FOREGROUND and **re-issue the same `cargo build`** when one overruns the 600 s tool timeout — it blocks on the artifact lock and that block *is* the wait. ⚠ **Never overlap two sweep blocks on four cores: 3m55s becomes 29m13s.** Disk: a run of this size reaches 83 %; `find target/debug/deps -maxdepth 1 -type f -size +50M -delete` plus `rm -rf target/*/incremental` returns ~17 GB.
15. **Cards & rules, leftover only:** `audit_dropped_may` 199; `audit_keyword_drift`'s last 4 (bestow / escalate / flashback / kicker) each need a PRIMITIVE; card skips `nocolors` 950 / `noback` 60 / `nosubtypes` 43 / `nokeywords` 29; `audit_incomplete` 858 documented approximations; **Temple Thief's `IsEnchanted`** (ENGINE_BACKLOG's thirty-sixth find, machine-checked, fix shape in INCOMPLETE_CARDS).
