...
10. **Perf queue: ✅ (O)'s `TokenDefinition` row TAKEN as `(-364)` (one level up), its `lethals` row as `(-365)` and four short-lived lists as `(-366)`, `GameState::clone`'s two lines split ✅ `(-362)` / ❌ `(-363)`.** ❌ **The auto-tap `events` buffer is DECLINED, censused by consumer, no build spent**: 15 of its 21 consumers *move* the buffer into their own accumulator, so an out-param buys at most ~29 % of the row for a five-site signature change. ❌ **`PrintedList::push` is DECLINED as (T)** — 18,534 allocations, but the `Vec` form either reallocs at `into_boxed_slice` or puts +8 bytes on `ComputedPermanent` (226,734 constructions), and the rate is 0.082 pushes a pass. Open on the shortlist: `affected_from_requirement` `types` (3,684), `computed_permanent_hinted` (5,852 sealed), `mana_source_table`'s `out` (7,610, a pool question), `compute_permanents`' `pass` buffer (20,484, ditto). Also open: **(H)** (price the clone side first), **(N)**, **(P)**.
