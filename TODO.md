...
7. **Cards, what is left and what it costs:** the colourless staples are all in all five decks. ⏳ the ten **Signets** and ten **Talismans** are implemented and unused by the lists (the generator ranked ramp by mana value and the one-mana dorks won) — a `pod/decks.rs` list edit plus a re-bless of the outcome table, no engine work, and the cheapest remaining improvement to the decks' fixing. ⏳ 32 of 33 "can be your commander" non-creatures are unimplemented; the flag is done, so each is a plain card.
...
9. **Guardrail at the tip `8ba03e94`:** `--bench` invariant **195,806 / 27.49 / 611.9 / 0 stalls**, `determinism ok`, `peak_rss_mib` 25.3 — byte-identical across six Commander rules changes *and* a targeting-core change. Suite **19,688/19,688**, clippy clean over the workspace, pod 2,300 games over seven configs at 2/3/4/5 seats all decided. ⚠ `games_per_s` read 470-505 over five builds on one box at one calib: that spread is the noise floor and the invariant carries the verdict. The pod's own determinism check is free: `--threads 1` vs the default, identical in every column.
