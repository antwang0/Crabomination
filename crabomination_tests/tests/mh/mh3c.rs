...
fn cast(g: &mut GameState, id: crabomination::card::CardId, target: Option<crabomination::game::types::Target>,
        extra: Vec<crabomination::game::types::Target>) {
...
fn signature_slam_modified_creatures_deal_damage() {
    ...
    g.perform_action(GameAction::CastSpell {
        ...
        additional_targets: vec![crabomination::game::types::Target::Permanent(enemy)],
...
fn waterlogged_teachings_tutors_instant() {
    ...
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Search(Some(bolt)),
    ]));
    cast(&mut g, id, None, vec![]);
...
fn drowner_of_truth_colorless_spent_makes_spawn() {
    ...
    let mut g = two_player_game();
    g.players[0].mana_pool.add(crabomination::mana::Color::Green, 6);
...
fn drowner_of_truth_no_colorless_no_spawn() {
    ...
    let mut g = two_player_game();
    g.players[0].mana_pool.add(crabomination::mana::Color::Green, 7);
    g.players[0].mana_pool.add(crabomination::mana::Color::Blue, 1);
    let id = g.add_card_to_hand(0, catalog::drowner_of_truth());
    g.perform_action(GameAction::CastSpell {
        card_id: id, target: None, additional_targets: vec![], mode: None, x_value: None,
...
fn wumpus_aberration_no_colorless_opponent_free_drops() {
    use crabomination::decision::{DecisionAnswer, ScriptedDecider};
    let mut g = two_player_game();
    g.players[0].mana_pool.add(crabomination::mana::Color::Green, 4); // all colored, no {C}
...
fn cursed_wombat_amplifies_first_counter_each_turn() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_spell(
        0, Some(crabomination::game::types::Target::Permanent(bear)), 0, 0);
    let add = crabomination::effect::Effect::AddCounter {
    ...
    assert_eq!(g.battlefield_find(bear).unwrap().counter_count(crabomination::card::CounterType::PlusOnePlusOne), 2,
