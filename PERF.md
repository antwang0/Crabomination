...
8.97 % inclusive. `CardData` is **148 fields, 32 of them heap-owning**;
...
already hold (`finish_cleanup`, 2.62 %, gone). **The standing filter is a
`for … in &mut` over a zone in a function that runs on a turn or step
boundary.** What is already known and does not need re-deriving:
