...
fn project_for_inner(state: &GameState, viewer: Option<usize>) -> ClientView {
    ...
    ClientView {
        ...
        vote_controller: state.vote_controller_this_turn,
        ...
        spliceable_hand: affordances.spliceable,
        pitchable_hand: affordances.pitchable,
        kickable_hand: affordances.kickable,
        kicker_option_sets: affordances.kicker_option_sets,
        ...
        may_play_castable: affordances.may_play_castable,
        may_play_lands: affordances.may_play_lands,
...
fn trigger_event_label(event: &crate::card::EventSpec) -> &'static str {
    ...
    match (&event.kind, event.scope) {
        ...
        (EventKind::BlocksNOrMore(_), EventScope::SelfSource) => "Blocks 2+",
        (EventKind::BecomesBlockedByNOrMore(_), EventScope::SelfSource) => "Blocked by 2+",
        ...
        (EventKind::YourInstantOrSorceryDealtDamageToPlayer, _) => "Your spell hits a player",
        ...
        (EventKind::SpellCountered, EventScope::YourControl) => "You counter",
        (EventKind::SpellCountered, EventScope::OpponentControl) => "Opp counters",
        (EventKind::SpellCountered, EventScope::AnyPlayer) => "Any counter",
        (EventKind::LifeGained, EventScope::FromYourGraveyard) => "Lifegain (gy)",
        ...
        (EventKind::DealsDamageToPlayer, _) => "Damages a player",
...
#[doc(hidden)]
pub fn ability_effect_label(effect: &Effect) -> &'static str {
    match effect {
        ...
        Effect::BecomeMonarch { .. } => "Become the monarch",
        Effect::Vote { .. } => "Vote",
        Effect::ControlVotesThisTurn { .. } => "Choose how each player votes",
        Effect::AddCountersUpTo { .. } => "Put up to N counters",
        ...
        Effect::PlayerChoosesToDestroy { .. } => "That player picks a creature to destroy",
        ...
        Effect::CastWithoutPayingImmediate { reduce_generic: 0,
                                pay_own_cost: false, .. } => "Cast free",
        Effect::CastWithoutPayingImmediate { .. } => "Cast at a discount",
        ...
        Effect::EachPlayerRevealTopKeepIfNamed { .. } => "Reveal top; keep if named",
        Effect::ReturnSelfAttachedToChoiceOf { .. } => "Return attached to their choice",
