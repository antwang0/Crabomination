...
11. **Best next moves, in order.** (a) **Finish line 4's device**: `cg_calls.py <dump> 200`, take every row whose callee is shared, and ask `--callers` of each — `ManaPool::is_empty`, `counter_count` (225,536 calls), `same_team` (87,764), `defender_for` (74,096), `event_kind_bits` (256,878) are all unexamined, subject to line 5's trap. (b) `pick_blocks_inner` is still 1.0 % self on `cube` with the pair check now gated — read what is left of its body (the `attacker_info` collect is 28.6 M INCLUSIVE, 6,527 Ir a call) with line 4's ladder. (c) A fresh-seed block from frontier 1374, as the FIRST gate after any lane/memo change. (d) leftover context: `audit_dropped_may` 199; `audit_keyword_drift`'s last 4 each need a PRIMITIVE; card skips with a card behind them: `nocolors` 950, `noback` 60, `nosubtypes` 43, `nokeywords` 29.
