...
2. **Gates at the tip, all re-run this run:** suite **19,549 / 0 / 5** (`CRAB_ANSWER_LOG=strict`), clippy **0** (`--workspace --all-targets --exclude crabomination_client`), golden traces unmoved, `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant with `determinism ok` and `thread_determinism ok (3 vs 1)`. ⚠⚠ **FOURTH BOX: 4 cores, 479 games/s where the last 4-core box read 635-678 on its own binary. No absolute in PERF is comparable across boxes; the counters are the invariant.** A warm `profiling-fast` engine rebuild is **2m45s** here and a three-pool callgrind A/B is ~90 s, so an A/B cycle is five minutes — spend them.
3. **Perf — the run took -0.609 / -0.588 / -0.667 % (fixed / cube / sealed) in two commits.** `(-306)` gave `Battlefield` a **per-object `writes: u32`** bumped at its six `&mut` chokepoints (all six already called `note_battlefield_reach()`), which is the first sound key any memo over the zone's *instance* fields can use — the eight lanes beside it are `definition_epoch`-keyed and survive an element write. First consumer is the SBA board fold: **-0.208 / -0.240 / -0.382 %**, scan self -13/-15/-22 %. `(-308)` took `compute_permanent_pass`' eight identity `saturating_add`s behind one OR chain: **-0.348 / -0.341 / -0.245 %**, the largest single line in that function by three times.
4. **Perf — the two transferable rules this run leaves.** (a) **A hot arithmetic chain whose operands are usually the identity is a grep target**, and `cg_lines.py --in <fn>` over a `profiling-lines` dump is how you find it — that is all `(-308)` was. ⚠ Fold it to one `i64` sum and you change the function: saturation is per step. (b) ⚠⚠ **A clone-carried memo is priced by how often the PARENT is re-asked without writing, not by how often the clone writes** — `(-306)`'s per-object counter was built expecting to beat `sba_census`' global `NO_REACH` and reads *below* it (10.6/10.7/18.9 % vs 11.6/11.4/19.0 %), because a probe's sweeps are its own, on a board it has just mutated. **Anything keyed on "this board has not been written" is capped at ~19 %.**
5. **Perf — two whole lanes CLOSED this run, do not re-open either.** `(-307)`: `fire_combat_damage_triggers` is 1.97 % of `cube` self and `(-68)`'s fused whole-board walk is **3.6 %** of it by line — the other 90 % is per-trigger work (`by_kind`, `slot()`'s `EventKind` equality per pair, the `Effect`/`SelectionRequirement` clones). **A whole-board walk inside a hot function is not automatically that function's cost.** `(-309)`: the `CardId`-equality + `find`-loop family is 1.8 % of `cube` over twenty sites and **both** memos built against it lose (MTF on `perms` +0.146 %, four hint slots in `declare_blockers` +0.083 %) — the scans are two-to-eight entries; `(-38)`'s break-even is a 23-entry zone.
6. **Perf — the head's #1 is still the gather memo and its prerequisite is now PRICED, which is the finding.** `(-303)` says 95-99.6 % of gathers repeat and the no-reach half is worth ~1.8-2.5 % of every pool. ⚠ **`players_mut()` is not a mechanical refactor**: `pub players: Vec<Player>` has **~2,400 mutation-shaped sites** (917 engine / 1,457 suite / 13 client) and a `Deref`-only newtype makes every one a compile error. **Do not open it as a refactor.** The battlefield half of the key exists now (`(-306)`'s `writes`); the players half needs a cheap sound spelling — a narrow digest over the scalar tallies plus per-`CardPile` epochs is the shape to price next, and the four `evaluate_*` entries keep the input set open either way.
7. **Bugs — a printed "you may draw a card" is a CHOICE, 26 cards fixed and the class gated.** Eighteen came off `audit_dropped_may` (a mandatory draw can deck its controller, CR 104.3c, and the bot loses a decision); **the new gate found eight more the script could not reach** — `core_rules::structural_audit::every_printed_you_may_draw_is_a_choice` joins the committed Scryfall cache against each definition's `{:?}` rendering (`wants_converge`'s single-oracle device), vacuity guard at 30 of 50. **The device generalises and was pushed twice more the same run: 45 cards in total across "you may draw a card" / "you may gain" / "you may put a +1/+1 counter" / "you may destroy" / "you may create" / "you may move" / "you may shuffle."** Two refinements made the wide phrases usable — **parenthesised reminder text is stripped** (myriad's "you may create a token copy" was a false positive on Dalek Squadron) and **`OPTIONAL` carries the shapes as well as the names** (`up_to`, `min_targets: 0`, `AnyNumber`), because "you may shuffle up to four target cards" is a pick the engine already asks for. ⚠ **The phrases NOT in the table were measured, not guessed**: "you may pay" 308 unmodelled of 519 (kicker/ward reminder), "you may search" CR 701.19c, "you may sacrifice"/"exile"/"return"/"mill"/"discard"/"tap" are additional and alternative costs. The buckets still worth a look are **"you may attach" (6)**, **"you may remove" (8)**, **"you may copy" (9)** and **"you may add" (2)** — each needs per-card verification because each mixes costs with choices.
8. **Seeds: frontier 1308 — THIS RUN TOOK 1300..1307 IN TWO BLOCKS: 24 cells / 118,400 games / 0 failures**, `cap 0 / board 0 / stuck 0 / draw 10`, clean, no 50,000-action re-run needed. ⚠ **The second block ran at the FINAL tip on purpose**: each of the 45 `MayDo` wrappers adds a decision and a resume path to a trigger, which is what `CRAB_ANSWER_LOG=strict` exists to audit, and a card commit landing after the sweep is a card commit the sweep did not cover. Say which block you are taking in this line *before* you take it. All standing audits 0: `audit_panics` 0 bare, `audit_stash_in_loop` 0 unexplained, `audit_seat_from_selector` 0 open, `audit_target_walkers --check` 0, `audit_doc_drift` 0 body-wrong / 0 doc rot / 0 stale notes, `audit_variant_coverage` 0 dead capability.
9. **Best next moves.** (a) **Re-run `(-308)`'s device, but only on lines whose FUNCTION is unambiguous.** ⚠⚠ **Three consecutive line-profile-derived leads were refuted at this tip (`(-309)` twice, `(-310)`)** and the third is the rule: `addr2line` resolves an address to its innermost *inlined* frame, so a hot loop interleaved with a one-instruction callee carries that callee's line — `sync/atomic.rs:3875` read as 7.32 M of `find_by_id` hint misses and a rolling cursor that is strictly cheaper than a scan-per-id lost **+0.19..+0.34 %**, because the hint was hitting all along. **A large self cost on a std line under an engine caller names where the optimizer put the line number, not what the code does.** `(-308)` won because its line was `i32::saturating_add` with eight named call sites in the source; that is the only shape to act on. (b) The biggest untaken engine row is still `dispatch_triggers_for_events_slow` (**4.5 % cube / 6.0 % sealed self, ~750-990 Ir a call, 105 k calls**) and its line profile is FLAT — 267 lines under 0.37 % each — so it needs the structural half (**merging `perform_action_inner`'s batches, a CR 603.3b ORDERING question before a perf one**), not another line. (c) `audit_dropped_may` reads **240**; the next buckets are "put a creature/land card" (30), "begin the game" (12, companion/Leyline, genuinely unmodelled), "collect evidence" (11), "put a quest counter" (10) — and the gate in line 7 is the better instrument for any of them. (d) `audit_keyword_drift`'s last 4 each need a PRIMITIVE (bestow / escalate / flashback / kicker, one card each). (e) The skips with a card behind them: `nocolors` **950**, `nosubtypes` 43, `noback` 60, `nokeywords` 29.
