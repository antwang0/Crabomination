...
1. **FIRST:** `git fetch origin claude/modern_decks && git checkout -B claude/modern_decks origin/claude/modern_decks`. Rebase, never force; code before tracker prose; **fetch before every push**. Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure" + "Standing rules"**, ⚠ the 2026-09-12/13 detail (printed-body skip taxonomy, the `EventKind` fan-out census, the modal census, the collision log, the seed discipline) is in **ENGINE_BACKLOG's "The 2026-09-12/13 handoff detail, moved verbatim"** — read it before taking a skip kind or a seed, and say in this line which you are taking. NEXT is 15 lines and stays 15.
2. **Gates at the tip, all re-run this run:** suite **19,549 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, nextest), clippy **0** (`--workspace --all-targets --exclude crabomination_client`), golden traces unmoved, `cargo check --profile release-fast` clean, `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant with `determinism ok` and `thread_determinism ok (3 vs 1)`. ⚠⚠ **FIFTH BOX, 4 cores, rustc 1.95.0 — no absolute in PERF is comparable across boxes; the counters and the Ir are.** ⚠ **The A/B base was NOT re-taken and did not need to be: a fresh clone reproduced the record to six significant figures** (618,446,275 / 1,632,319,194 / 1,723,819,585 against 618,446,650 / 1,632,318,919 / 1,723,820,812). A `profiling-fast` engine rebuild is **2m53s warm**; the cold catalog+engine is **34 min**; a three-pool callgrind ~6 min; the debug test build dominates the suite gate. ⚠ **A FIFTH SESSION extended this run after a container restart and re-ran every gate at its own tip**: suite **19,549 / 0 / 5** with golden traces unmoved, clippy **0** both with and without `--features trig-census`, `cargo check --profile release-fast` clean. It shipped **no engine change** — both censuses are `trig-census`-gated — and that is measured rather than asserted: feature-off `profiling-fast` reads **-2,450 Ir on `fixed` / +911 on `cube`**, 0.0001-0.0004 %, code layout under a changed source-file hash. ⚠⚠ **`md5sum` is NOT the check for a claim like that** — `profiling-fast` embeds the source hash in debuginfo, so identical code gives different digests; the counter is the check. **It took no seed block** (frontier stays **1334**): with no shipped change, a sweep would re-test the code block 1332..1333 already covered.
3. **Perf — `(-313)` TOOK THE AFFORDABILITY READ'S QUESTION ORDER: -0.294 / -0.334 / -0.549 % (fixed / cube / sealed), and the reduction derivation is 13,768 -> 0 / 30,682 -> 698 / 49,140 -> 132 calls.** The row itself is -9.7 / -11.8 / -16.9 %. ⚠⚠ **NEITHER GATE IS A MEMO, and that is why they land where `(-304)`, `(-309)` and `(-312)` did not.** Gate 1: every channel between the printed cost and the read's `mana value + extra > total` test EXCEPT the reduction only raises the value, so with all six reduction channels empty the relaxed printed mana value already is the answer (fires on 40.4 / 42.5 / 58.9 % of calls). Gate 2: `reduce_generic` drains `Generic` symbols only, so the colour half of the payment test is reduction-independent and the total half needs the number only when it is already short.
4. **Perf — the transferable rule, and it is cheaper than a key.** Before building a witness over a hot derivation, ask **which of its inputs can only move the answer ONE WAY**. A monotone input needs no witness at all: it is skipped by asking the question in the other order. `(-313)` is two such arguments and cost no key, no epoch and no lifetime reasoning; the three refuted memos in front of `compute_permanent_pass` each cost all three. Both gates carry a `debug_assert!` that re-runs the full derivation and compares, so the argument is ratcheted rather than trusted, and `cost_reduction_audited` keeps `(-296)`'s narrowed-filter audit alive on a value the hot path no longer computes.
5. ❌ **Perf — `available_mana` IS CLOSED. Both questions the last handoff asked are answered and neither opens a device; do not spend a build re-asking.** `mana_census` (new, `trig-census` + `CRAB_MANA_CENSUS=1`, printed by `bot_ladder`) and one dump at the `(-314)` tip. (a) **The walk is the majority, not the callee tree** — self 16.57 M / **58 %** of the row on `cube`, 18.49 M / **68 %** on `sealed`, against 11.9 M / 8.6 M for `grant_scan` + `tap_ability_summoning_sick` + `grants_nothing_slow` + `board_has_mana_static` together — so more gating of the `(-314)` shape is capped at a third of the row even if it took all of it. (b) ⚠⚠ **The `(-87)` member list that would cut the walk IS THE BOARD**: the lane's predicate is definition-only and sound, and **11.5 of 17.7 permanents a call are in it on `cube`, 13.6 of 16.8 on `fixed`, 10.1 of 14.3 on `sealed` — 65-81 %**, because lands are most of a board and every land is a mana source. **A member list is priced by how much of the collection it EXCLUDES**, and the fifth-to-a-third it would skip are the cheap iterations (`controller != seat`, one load and a branch). (c) `relax_reachable` is clear on **100 %** of calls in all three pools, so `(-314)`'s gate is fully effective and nothing is left on that half. ⚠ The plumbing story is smaller than it read too: the `OnceCell` already amortises (48,596 `sealed` affordability checks collapse to 15,884 `available_mana` inits + 15,140 gathers) and there is no within-tick duplicate — `main_phase_action_with` runs once a tick and `sim_spell_action_inner`'s half is over simulated states. What is left is the work: 4.4-6.3 permanents a call reach `granted_abilities_of` on a board of 14-18, already gated.
6. ❌ **Perf — the `ComputedPermanent` memo lane is CLOSED (`(-312)`, +1.1 % on all three pools); do not open a fourth.** `(-309)` (id-compare family) and `(-307)` (combat dispatch) are CLOSED too. The two sized-but-untaken structural rows are unchanged: **`dispatch_triggers_for_events_slow`** (4.64 % of `cube` / 6.09 % of `sealed` self, line profile FLAT over 267 lines — it needs the structural half, merging `perform_action_inner`'s batches, which is a CR 603.3b ORDERING question before a perf one) and **`compute_permanent_pass`** itself (238 Ir of self per permanent; the next line out of it wants a `profiling-lines` dump, cold, and ⚠ `(-310)`'s rule about inlined frames).
7. **Perf — what `(-313)` did NOT find, so nobody re-spends it.** The affordability read's five per-card callees are all small now (0.5-1.5 M each on `cube` after the gates); `cast_candidates`' other half, **`auto_targets_for_effect_all_slots_kicked` at 3.61 % of `sealed` / 26.6 requirement evaluations a call**, is untouched and is the larger of the two halves at this tip — its question is still the one PERF's candidates names: does the generation pass need an *enumeration* or a *witness*, and do its two callers need the same thing.
8. **Seeds: frontier 1334 — THIS RUN TOOK 1328..1333 IN TWO BLOCKS: 18 cells / 88,800 games / 0 failures.** 1328..1331 at the `(-313)` tip (12 cells / 59,200 games, `cap 0 / board 2 / stuck 0 / draw 0`) and 1332..1333 at the CLOSING `(-314)` tip (6 cells / 29,600 games, `cap 0 / board 0 / stuck 0 / draw 2`). ⚠ **The two board caps are a THIRD Scute Swarm runaway**: `cube` 1328, 962 Swarms on a 990-permanent board at turn 68 with 574 of its own triggers on the stack, and the cell costs **344 s against 40-46 s for the seeds either side** — `MAX_BATTLEFIELD` working, the re-run correctly skipped, and a standing cost with three worked examples and no fix. **The block ran with `-C debug-assertions=yes`, so `(-313)`'s two new asserts ran on every one of those boards — which is the coverage a monotonicity argument cannot get from the suite**.** All standing audits re-run and 0: `audit_panics` **0 bare** (70 sites, 59 guarded, 11 lock-poison), `audit_stash_in_loop` **0 unexplained**, `audit_seat_from_selector` **0 open**, `audit_target_walkers --check` **0**, `audit_doc_drift` **0 body-wrong / 0 doc rot / 0 stale notes**, `audit_variant_coverage` 0 dead capability (1 dead primitive, `GrantCastBackFromGraveyard`, unchanged), `audit_keyword_drift` **0 invented / 4 missing** (bestow / escalate / flashback / kicker, one card each, each needing a primitive), `audit_dropped_may` **199** with the `PutFromHandOntoBattlefield` exemption re-checked against the engine.
9. **Best next moves.** (a) ⚠⚠ **NOT `available_mana` and NOT the target enumerator — this run closed BOTH, and both closures are censused rather than argued (`mana_census`, `call_site_census`, both in the tree and re-runnable in one run).** The enumerator is an *enumeration*, not a witness — the target is consumed into the `GameAction` — and **58.8 % of `sealed` / 87.7 % of `cube` of its calls come from the plain-cast block, which already has `(-313)`'s ordering** (`can_afford_in_state_with` runs before it). Its 31.9 % Spree site has no duplicate to remove (measured: the hoist is -0.035 / -0.021 / +0.036 %, reverted) and its only lever is an affordability pre-gate, which is a **behaviour change** needing a ladder run, because the bot's mana estimate is biased downward on purpose and the exact oracle clones the state. **The two structural rows in line 6 are what is left.** (b) **`auto_targets_for_effect_all_slots_kicked` (line 7)**, now the bigger half of `cast_candidates`. (c) `dispatch_triggers_for_events_slow`'s structural half (line 6). (d) ⚠ **Re-run the fresh-seed sweep as the FIRST gate after any memo or epoch change, not the last** — it is the only gate that reached `(-311)`'s collision, and the suite is not a substitute. (e) **`audit_dropped_may` is 199 and `LookPickToHand` is NOT safe at this granularity, measured** — its `take == 1` path rides `Decision::SearchLibrary` (11 findings) but "you may put" on those same cards also matches the *rest* disposition (Bucolic Ranch, Break Out), a real choice; it needs the span matched to the pick, not to the variant. Buckets: put 73, exile 20, return 16, collect 12, begin 12. (f) `audit_keyword_drift`'s last 4 each need a PRIMITIVE. (g) The skips with a card behind them: `nocolors` **950**, `nosubtypes` 43, `noback` 60, `nokeywords` 29.
