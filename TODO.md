...
1. **Where it stands:** CR 903 core ✅ and the CR 113.6b command-zone axis ✅ (both halves). The **map is FEATURE_ROADMAP's "Commander status"** — read it, don't re-audit. Per-deck completion is **DECK_FEATURES' "Commander target decks"** (4/4 complete).
2. **The runner:** `crabomination::pod` + `bot_ladder --commander [--seats N] [--games N] [--seed N]`. Four legal 100-card decks in `pod/decks.rs` (Sigarda GW / Judith BR / Hanna UW / Tatyova GU); `pod::tests` validates each through `validate_commander_deck` every run, so an illegal list cannot land.
3. **The instrument that pays, and it paid again:** the pod run is where four bugs came from in one sitting, and this run's fifth — `move_card_to` learning to scan command zones was too general (CR 400.7: a card there is a new object), and the pod read 41.74 turns/game against the committed 41.83 while every test stayed green. Run it before and after anything Commander-shaped; a stall rate that moves is a bug, not noise. **Tip reading at `8a4d78b1`: `--commander --games 400 --seed 43` → 400 / 0 undecided / 41.83 turns / 1,969.0 actions / 348.44 games/s; `--seats 3 --games 200 --seed 97` → 200 / 0 / 30.06 / 1,096.5; fresh seeds 4242 (600 games), 9091 (500) and 4243 (200, 2 seats) also 100 % decided — 1,900 games, zero panics.** It was **15 % "no legal move"** before the CR 800.4a pending-decision fix. `CRAB_CAP_DIAG=1` names an undecided pod game's board. ⚠ Read **turns**/game, not actions/game, to tell "different games" from "fewer pointless actions".
...
5. **This run shipped, in order:** CR 601.2f alt-cost casting from the command zone (`cast_spell_alternative_from` over `AltCastZone`, tax included, bot offers both casts); CR 113.6b triggers (`EventSpec.zone`) and statics (`CardDefinition.statics_in_command_zone`) that function from the command zone; CR 702.49d commander ninjutsu plus CR 602.2b's auto-tap and the bot's first-ever `GameAction::Ninjutsu` candidate; CR 207.2c join forces (`Effect::JoinForces` over `ask_seat_amount`); CR 702.124i partner—[text] as one keyword; CR 101.4 `Effect::ForEachOpponent`; a committed pod outcome table; **ten cards** (Edgar Markov, Oloro, Arahbo, Yuriko, The Ur-Dragon, Minds Aglow, Collective Voyage, Mana-Charged Dragon, Elmar, Sophina) and two multiplayer card fixes (Esper Sentinel, Adeline).
6. **Still ⏳, roughly in value order:** mana provenance (item 7 — the two remaining staples and nothing else blocks on it); more planeswalker commanders into `sets::cmdr` (the flag is done, 32 of 33 such cards unimplemented); Doctor's companion (CR 702.124m wants a legendary Time Lord **Doctor** on the other side and none is implemented — a card, not a rule). ⚠ Check a mechanic has a consumer *before* building it: "Friends forever" looked like its own mechanic and CR 702.124i says it is one of four labels of one keyword.
7. **Cards:** the two staples are ✅ — a concurrent session landed CR 106.6 mana provenance and both (`sets/cmdr.rs`, tests in `core_rules/commander_mana.rs`). **Neither is in a deck list yet**, and nor are the ten Signets and ten Talismans the picker passed over: adding them is a `pod/decks.rs` list edit and a re-bless of the pod outcome table, not engine work, and it is the cheapest improvement to the four decks' fixing.
...
9. **Guardrail at the tip:** `--bench` invariant **195,806 / 27.49 / 611.9 / 0 stalls**, `determinism ok`, `peak_rss_mib` 25.5 — byte-identical to the committed one, so every Commander change this run is behaviour-preserving in a duel. ⚠ games/s does **not** cross hosts (this box is a 2.10 GHz Xeon); PERF's Commander block says what to compare. The pod's own determinism check is free and is `--threads 1` vs the default.
10. ⚠ **Golden traces are the Commander guardrail too**: with no commander seated, every Commander change must be byte-identical in two-player games. `AnyColorInCommanderIdentity` keeps the old any-color behaviour when the identity set is empty *for exactly this reason*, and CR 509.1a's block gate keeps the duel branch verbatim; don't "fix" either without re-reading its comment.
11. 📐 **The N-seat audit method, and it found two:** read every deck card's Scryfall text for "each opponent / each other player / their first / that player" and its body for a fan-out ref. **Esper Sentinel** modelled "their first noncreature spell each turn" as `once_per_turn` — exact at two seats, one fire per *table* in a pod; **Adeline** made one attacking token where "for each opponent … that player" wants one each. Both bugs are invisible at two players and invisible to a per-card test written at two players. ⚠ The reverse smell (a singular opponent ref) is **clean** across the pod decks and the `is_fan_out` `debug_assert` polices the arms.
12. ⚠ **A `ScriptedDecider` answers in ASK ORDER, and the engine's ask order is not the test's business.** Three `Bool(true)`s put nothing onto the battlefield for The Ur-Dragon because the put's own "which permanent" ask ate the second one. When a test needs one question answered and the rest left alone, write a four-line `Decider` that matches that `Decision` variant and defers to `AutoDecider` — `SayYesToOptional` in `core_rules/multiplayer.rs` is the pattern. ⚠ And the `AutoDecider` **declines every `OptionalTrigger`**, so a `MayDo` body is dead under it.
13. 📐 **Where a trigger functions is a property of the ABILITY, not the card** (Oloro's two upkeep triggers differ); where a *static* functions is card-level here, because `StaticAbility` is `{description, effect}` in 2,275 literals with no builder and no printed card splits them. The doc on `statics_in_command_zone` says so — a card that does split them gets the per-ability field.
14. ⚠ **Two binding gotchas, both cost a debugging round.** ① A "target player" slot **auto-picks an OPPONENT**, so a targeted gift (the Partner-with fetch) silently went to the wrong seat and looked like "nothing happened" — `prefers_friendly_target` declares the shape, and a test asserting only "something moved" cannot see it. ② `Selector::TriggerSource` binds the **damaged player** at resolution on a dealer-side damage kind, but a trigger's `filter` is evaluated against the **dealer** at gather time, so `with_filter(EntityMatches{TriggerSource,…})` and `dealt_by(…)` both gate correctly — twenty catalog cards use the first and it is **not** a bug class (verified by flipping Yuriko's filter to a type she does not have and watching it stop firing).
15. **Where the new cards live:** `crabomination_catalog/src/sets/cmdr.rs` — cards whose text is *about* CR 903, not about their set. Register in `all_factories.rs`. ✅ **That registration does NOT touch the net vocab**, so a Commander card is free: `Vocab`/`encode.rs` read `cube::cube_pool_all()`, a *curated* list of explicit factories in `cube.rs`, not the catalog registry. Only editing `cube.rs`'s pools invalidates a trained net. ⚠ **Latent, shared with `TemptingOffer`:** a multi-seat ask arm clears the replay log before running its body, so a body that *itself* suspends (a `wants_ui` seat's search) replays the arm from the top with an empty log and re-asks — and, for `JoinForces`, re-pays. No bot seat sets `wants_ui`, so no self-play game can reach it; a UI pod can. Fixing it is one fix for both arms.
