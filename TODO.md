...
9. **Seeds: TWO blocks this run, 24 cells / 118,400 games / 0 failures, `cap 0 / board 0 / stuck 0` on both** (`draw 4 / 8`; pools `cube all sealed`, seeds 1366..1369 and 1370..1373, `target-audit/overflow` with `-C debug-assertions=yes` and `CRAB_ANSWER_LOG=strict`). They audit `(-330)`'s one-pass keyword mask and `(-331)`'s hoisted protection gate on dflt-pilot combats the suite never builds — both changed a combat-legality *path*, and the trace diff only covers `gang`. **FRONTIER 1374 — read it from PERF's Baseline, not from this line.** A cell is 24-69 s here and a whole block ~10 min, cheaper than one `profiling-fast` build. Standing audits all 0 this run (`audit_panics` 0 bare / 70 sites, `audit_engine_stack` 0, `audit_stash_in_loop` 0 unexplained, `audit_seat_from_selector` 0 open, `audit_target_walkers --check` 0, `audit_doc_drift` 0/0/0).
...
11. **Best next moves, in order.** (a) **Finish line 4's device**: `cg_calls.py <dump> 200`, take every row whose callee is shared, and ask `--callers` of each — `ManaPool::is_empty`, `counter_count` (225,536 calls), `same_team` (87,764), `defender_for` (74,096), `event_kind_bits` (256,878) are all unexamined, subject to line 5's trap. (b) Queue item (C), the pair count in `pick_blocks_inner`. (c) A fresh-seed block from frontier 1374, as the FIRST gate after any lane/memo change. (d) leftover context: `audit_dropped_may` 199; `audit_keyword_drift`'s last 4 each need a PRIMITIVE; card skips with a card behind them: `nocolors` 950, `noback` 60, `nosubtypes` 43, `nokeywords` 29.
