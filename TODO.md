...
2. **Gates at the closing tip `dd3fd7fc`:** suite **19,632 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); clippy **0** over the workspace; `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean; `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant at **three** tips, `determinism ok`, `thread_determinism ok (3 vs 1)`, `peak_rss_mib` 25.4; sweep **5 blocks / 30 cells / 148,000 games / 0 failures** at fresh seeds 1428..1437, `cap 0 / board 0 / stuck 0 / draw 26` — **FRONTIER 1438**. **Closing absolutes (SIXTEENTH box, `profiling-fast --no-default-features`, system allocator): fixed 567,182,951 / cube 1,475,488,276 / sealed 1,596,484,155; seven rows sum to -0.915 / -1.469 / -1.155 % and the tip reads -0.853 / -1.107 / -0.900 % all-in against this run's second base.** ⚠ **Seven concurrent Commander/multiplayer commits landed on top of `(-367)`** — that gap is their work, not drift; re-take the base, do not predict it. ⚠ **Re-verified at the closing tip after the last rebase:** suite 19,745/0/6, clippy 0, release-fast typecheck clean, `--bench` 195,806 / 27.49 / 611.9 / 0 stalls with `determinism ok` and `peak_rss_mib` 29.0, `audit_panics` 0 bare, and a fresh pod smoke at **seed 2005** — 12,000 games over 2/3/4/5 seats, 100 % decided, every `undecided_by` column zero. **FRONTIER 2006.** ⚠ **Closing re-verification, everything below taken at the final tip:** suite **19,750 / 0 / 6**, clippy 0, release-fast typecheck clean, `--bench` 195,806 / 27.49 / 611.9 / 0 stalls with `determinism ok`, `audit_panics` **0 bare**, and an assertions sweep (`overflow` + `-C debug-assertions=yes`) of **12,000 pod games** over 2/3/4/5 seats at seeds 3001-3002 plus the two-player grid on fixed/cube/sos/all at 3001 — 100 % decided, every `undecided_by` column zero, no assertion, no panic. **FRONTIER 3003.**
...
1. ⚠⚠ **TWO SESSIONS SHARE THIS BRANCH. FETCH BEFORE YOU START AND BEFORE EVERY PUSH.** Rebased **twice** mid-run — two commits landed while four of mine were local, then five more (including a `CR_COVERAGE.md` conflict, which is a **generated** file: take either side and re-run `scripts/cr_coverage.py`, never merge it by hand). `cargo nextest` fail-fast hides the rest — use `--no-fail-fast`. **A tracker bullet is a claim ticket**: grep `git log origin/<branch> --oneline -20` for its words BEFORE the first edit, and prefer a shape that APPENDS to one that rewrites. Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure"**. ⚠⚠ **DO NOT EDIT THE TREE WHILE A BUILD IS RUNNING — it cost a whole `nextest` build and a whole `release-fast` build this run.** rustc reads a crate's sources when it reaches that crate, so a mid-build edit gives a binary from neither tree and **cargo will not tell you**: the second build reported "Finished in 7m36s" over a tree that had changed under it, and only a re-run revealed `crabomination_base` was stale. Batch the edits, then build. ⚠ **DISK: `target/debug/incremental` reached 16 GB on its own**; `rm -rf` it before any optimized build, not after one dies. `find target/debug/deps -maxdepth 1 -type f -size +50M ! -newer <ref>` frees another 3.5 GB and rebuilds nothing.
2. **Gates at the closing tip `2104c659`:** suite **20,045 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean; **`--bench` 195,806 / 27.49 / 611.9 / 0 stalls — byte-identical to the committed invariant**, `determinism ok (all pairs split)`, `thread_determinism ok (3 vs 1)`, `peak_rss_mib` 29.5; `audit_panics` **0 bare**; every mirror column 0 (`invented_rider` / `invented_trigger` / `invented_may`, `loop_splice` 13/13/0/0, `each_player` 1-allowlisted, `target_opponent` 1 known) and two **new** ones at 0 — `audit_seat_walks`, `audit_retarget`. CR_COVERAGE **146/146**. **Pod smoke, seed 9101, 2,000 games at each of 2..8 seats = 14,000 games:** 13,999 decided before the bot fix, and the one holdout at eight seats is **fixed** — that block re-reads **2,000/2,000 decided, every `undecided_by` column zero**, with `--bench` byte-identical across the change. turns/game at the closing tip **19.33 / 32.62 / 45.15 / 57.21 / 64.11 / 77.71 / 92.84** (before the bot fix: 19.30 / 32.59 / 45.30 / 57.20 / 64.50 / 77.57 / 92.84 — that is the whole aggregate cost of it). clippy **0** over the workspace, re-taken at the final tip after the optimized build had the box to itself. **FRONTIER 9102.** ⚠ **The closing suite is 20,225 / 0 / 6** (`CRAB_ANSWER_LOG=strict`), with `cr_903_seeded_pod_outcomes_match_the_committed_table` re-blessed in the fix's own commit (item 8). ⚠⚠ **UNEXPLAINED AND LEFT OPEN: that count is 179 HIGHER than the same source tip reported an hour earlier (20,046), and no commit in between adds a test.** The only difference is that this run's `target/debug` was rebuilt from scratch after the disk cleanup, so the earlier number was very likely taken over a stale test binary — but that was **not** confirmed, and confirming it costs a full rebuild at the old tip. **Treat a suite COUNT as a gate in its own right**: if it moves without a commit that adds tests, distrust the earlier number, not this one.

3. 🔎🔎 **FIND ONE, A CLASS OF SIX: A SEAT-INDEX WALK IS THE SAME LIST AS A PLAYER WALK IN A DUEL AND IN A POD THAT HAS LOST NOBODY.** `resolve_players` filters `is_alive()`; six hand-written arms did not — four `for seat in 0..players.len()`, two `(controller + i) % n`. **`Effect::Vote` counted a departed seat's ballot** — CR 800.4g re-seated its ask onto a live opponent, who answered, and the answer was tallied as the dead seat's vote, so a four-seat pod down to two decided a will-of-the-council 4-0 and **nothing ever stalled**. Grenzo's Rebuttal aimed "the player to their left" at a board CR 800.4a had emptied. ENGINE_BACKLOG's **forty-seventh find**.
4. 📐 **ITS FIX IS ONE HELPER AND ITS RATCHET IS A SHAPE, NOT A GREP.** `GameState::seats_in_turn_order_from` is the live seats in turn order from a seat — "starting with you, each player …" — and `next_alive_seat` is "the player to their left". **Not `apnap_sort`**: APNAP starts at the *active* player, these cards at the spell's controller. `scripts/audit_seat_walks.py` counts a walk only when its body **asks** or **accumulates one entry per seat**; a walk that clears per-player state is not a finding, because a departed seat's state still has to be cleared. That filter is what took it from 25 raw hits to 6.
5. 🔎🔎 **FIND TWO, AND IT IS THE SAME LESSON FROM THE CARD SIDE: DEFLECTING SWAT DOES NOT COUNTER ANYTHING.** Printed body: "You may choose new targets for target spell or ability" (CR 115.7d). Shipped: `Effect::CounterSpell` — with a doc comment quoting a "Counter target spell" the card does not carry, a tracker row repeating it, and `deflecting_swat_counters_a_spell` **asserting the wrong rule**. EDHREC top-100 and the red member of the CR 118.9 free-spell cycle this branch built on purpose. Perplexing Chimera's "if you do, you may choose new targets" was dropped the same way. `scripts/audit_retarget.py` is the ratchet, **2 → 0**, and no primitive was needed: every sibling already used one of `ChooseNewTargetsForSpell` / `ChangeSpellTarget` / `ChangeTargetOfAbility` / `RedirectSpellTargetToSelf`.
6. 📐 **THE PRECISION GATE THAT MADE COLUMN SIX WORK IS WORTH COPYING.** A retarget clause reaches a print two ways and only one is a queue: "copy target …, **you may choose new targets for the copy**" is answered by any `Copy*`, because the engine repoints a copy's slots inside `repoint_copy_slot` rather than in an effect of its own. Splitting on whether the print says "copy" took 67 rows to 2. ⚠ The copy column's nine unread rows are **reader misses** — a private helper (`copy_cast_spell`, `copy_low_mv_is_ability`) the name-keyed reader cannot open, the limitation every script in `scripts/` shares.
7. ✅✅ **`CRAB_CAP_DIAG` NAMES THE LOOP NOW, AND IT CLOSED ONE END TO END IN THREE READS.** `ActionCensus` tallies an undecided pod game's actions by `(seat, variant, card, ability index)`; `cap_diagnosis`'s board is **per seat and untruncated**. The eight-seat sweep capped 1 game in 2,000 and the three reads were: ① `ActivateAbility#0 Eldrazi Spawn x7535` (the census — `cap_diagnosis`'s four zone lines showed nothing, which is the blindness its own header warns about); ② adding the **seat** to the key: `p5` takes all 7,535 *and* holds all 7,533 floating mana, so one loop, not two; ③ the per-seat board: `p5 board: … Basking Broodscale x1, Cordial Vampire x1, Eldrazi Spawn x1 …`, which the old twelve-row whole-battlefield tally had hidden behind seven Vampires. **Put the actor in the key of any census whose consumers are per-seat, and never let an inventory line truncate.** 📐 **AND THE POOL PRE-CHECK IS WHY THAT WAS SAFE TO TRY AT ALL, TAKEN BEFORE THE EDIT.** `pick_sacrifice_value` is gated on `sink::AB_SAC`, and **not one card in `bot_ladder::archetypes()` has a sacrifice cost** — the two that grep for one spell it `sac_cost: false`. `--bench` plays `--decks fixed` and `golden_trace.rs` runs the same two archetype decks, so neither gate could reach the arm, which the empirical re-take then confirmed. **A `bot.rs` change is not automatically a bench risk; ask which sink bit gates it and whether the pool can light that bit.**
8. 🔎🔎 **AND THE ANSWER WAS THAT NOTHING WAS BROKEN — IT WAS A REAL INFINITE, AND THE BOT'S TO STOP. FIXED.** Verified by reading, not guessing: the sacrifice **is** charged (`activate_ability_inner` ends its battlefield branch at `bf_pos.ok_or(CardNotOnBattlefield)?`), Basking Broodscale matches its oracle, `shortcut::graft()` no-ops when empty, and **Cordial Vampire is exactly its print** — "whenever this creature **or another creature** dies", no Vampire filter, correctly. The loop: p5 stole p3's Broodscale with **New Blood** (which text-changes it into a Vampire), so sac the Spawn for {C} → a creature died → a +1/+1 counter on each Vampire, Broodscale included → Broodscale mints the next Spawn → repeat. Every step is correct Magic. **The bot looped because every iteration is materially positive** — a dozen Vampires each gain +1/+1 — so `pick_sacrifice_value`'s `ev > baseline` held 7,535 times and the seat never converted. ⚠ Neither backstop could see it: the battlefield never grows (one Spawn at a time) and the counters keep CR 104.4's fingerprint moving, so `repeats 0/12` was *correct*. **Fix: `ability_is_pure_mana` — `pick_sacrifice_value` skips an ability whose whole body is "add mana", because the bot does not pre-tap and the engine's auto-tapper pays costs when a cast needs them (CR 106.4 empties the pool at the next step anyway).** Seed 9101 at eight seats: **2,000/2,000 decided**, and `--bench` **byte-identical** (195,806 / 27.49 / 611.9 / 0 stalls, both determinism checks ok). ⚠ **It re-blessed `cr_903_seeded_pod_outcomes_match_the_committed_table`, and the DIRECTION is the tell:** all three seeded games got **shorter** (55→47 / 101→67 / 57→49 turns, 2360→1909 / 3969→2831 / 2480→2116 actions), two winners held and seed 4242's moved 3→2 — while the 14,000-game aggregate barely moved (19.33 / 32.62 / 45.15 / 57.21 / 64.11 / 77.71 / 92.84 against 19.30 / 32.59 / 45.30 / 57.20 / 64.50 / 77.57 / 92.84, every block 2,000/2,000 decided on both sides). **A three-game gate moves far more than the aggregate when the fix needs a board state to be PRESENT** — the outlet has to be out with something to feed it, and when it is the loop was thousands of actions long. Item 12's rule, read from the fix side.
9. **Cards and decks: 8/8 target decks complete.** ✅ **An eighth deck: Zellix, Sanity Flayer + Passionate Archaeologist** — the pod's only **Choose a Background** pair (CR 702.124k) and its first Izzet identity, appended after `pod_field(7)`; `--seats 8` reaches it. It is the one seat whose second commander is a legendary *enchantment*: `is_legal_commander` rejects it alone, CR 702.124c combines {U} with {R}, and CR 702.124d's second tally is zero all game because CR 903.10a counts combat damage. ⚠ **"Two commanders" stopped being a unique shape** — the Partner test asks for two *creature* commanders now (CR 702.124h) and the planeswalker test for exactly *one* non-creature commander.
10. ✅ **The bond cycle is one body and ten cards.** "Enters tapped unless you have two or more opponents" shipped written **three times** over seven of the ten; it is `sets::cmdr::crowd_land` now, plus the three that were missing — **Sea of Clouds, Bountiful Promenade, Training Center** (EDHREC 164/210/154, the highest-ranked Commander cards the catalog lacked that are not a new mechanic). One table over all ten in `core_rules::multiplayer` asks what neither three-card test did: CR 800.4a, a pod worn down to two taps the next one.
11. ⚠ **VERIFY EVERY CR NUMBER AGAINST `MagicCompRules_20260417.txt`.** Four corrected this run: "Choose a Background" is **702.124k** (702.124j is "Partner with [name]" — five citations, one field from the one that was right), and "choose new targets" is **115.7d** (115.7c is "change any targets" — three citations).
12. ⚠ **POOL MEMBERSHIP IS A MEASUREMENT, NOT A VETO — the cheap pre-check is two deck lists.** `--bench` plays `--decks fixed` = `bot_ladder::archetypes()` (38 cards) and `golden_trace.rs` has two more. Goblin Game, Mind Bomb, Deflecting Swat and Perplexing Chimera are in neither, which is why this run could change Goblin Game's ask *order* to CR 101.4's APNAP and rewrite Deflecting Swat's whole body without a re-bless. ⚠⚠ **When the committed pod table and the aggregate disagree, the aggregate wins** — `cr_903_seeded_pod_outcomes_match_the_committed_table` is three games.
13. ✅ **THE ASSERTIONS BUILD IS THE CHEAP GATE.** `RUSTFLAGS="-C debug-assertions=yes" CARGO_TARGET_DIR=target-audit cargo build --profile overflow -p crabomination --bin bot_ladder`, then `RUST_MIN_STACK=33554432 …`. ⚠⚠ `rc=$?` **AFTER A PIPE READS THE PIPE'S STATUS** — an abort (134) reads as 0. Redirect to a file, then grep.
14. 💡 **Standing leads:** ① Eminence is worth ~three seats' share (Edgar 52.1 % of six-seat pods; two rounds of nerfing proved it is the *commander*, so **do not tune Edgar's 99 again**). ② 32 of 33 "can be your commander" non-creatures are unimplemented; an eighth deck no longer needs one, so they are ordinary card work. ③ COMMANDER_BACKLOG's top-1000 gap is now almost entirely **land cycles** — filter lands (`ManaPayload::OfColors` already fits), tri-lands, triomes, slow lands (`slow_land` exists) — same-shape batches with every primitive in the tree. ④ `audit_dropped_may` 199 and `audit_incomplete` 858 are the two big unmined columns.
15. ⚠ **Five gotchas that each cost a debugging round.** ① A `ScriptedDecider` answers in ASK ORDER — prefer a four-line `Decider` matching one `Decision` variant and deferring to `AutoDecider`, which **declines every `OptionalTrigger`**. ② `Selector::TriggerSource` binds the **damaged** player at resolution while a trigger's `filter` reads the **dealer** at gather time. ③ New Commander cards go in `catalog/src/sets/cmdr.rs`; pool membership is an **explicit curated list** in `cube.rs`; a new factory needs `python3 scripts/gen_factory_lists.py`. ④ `add_card_to_battlefield` does **not** run `as_enters_effect`; cast it. ⑤ `resolve_answering` in `core_rules::multiplayer` is the harness for any suspend/resume assertion — `drain_stack` passes priority, which a pending decision blocks.
