...
8. 💡💡 **THE BEST OPEN LEAD IS DECK BALANCE, AND IT IS NOW A NARROWED QUESTION.** Seed 43, 4,000 four-seat games: Sigarda 42.6 % / Tatyova 26.3 % / Hanna 23.3 % / **Judith 7.8 %**; in a *duel* Judith reads **23.7 %** against Sigarda, so it is not a multiplayer artifact. ✅ Ruled out this run with no build spent: `audit_incomplete`'s structural pass is clean, and of Judith's 73 distinct cards exactly **two** carry an approximation note (Arcane Signet's missing identity gate, which is a *buff*, and Chain Lightning's copy half). **So it is not defective cards — it is the list or the pilot.** Next: instrument which of its cards are ever cast, or price a list rebuild (⚠ that re-blesses the pod table).
9. **What CR 800.4 still approximates, on purpose:** an ask **already materialized** when its seat leaves is dropped, not re-seated — the legal set would have to be re-derived against the post-departure board, which is a different capability. Rare now that `route_ask` stops one being posed to a departed seat at all. 800.4i's last-known-information clause is unmodelled.
10. **Cards and decks, what is left:** ⏳ 32 of 33 "can be your commander" non-creatures are unimplemented; the flag and validator are done, so each is a plain card — but *cards are chosen to complete decks, not to raise a count*, so they wait on a sixth target deck wanting one. ⏳ `scripts/audit_each_opponent.py`'s **six** residuals each need a primitive (per-player protection, a cross-player permanent comparison, a per-opponent conditional, a per-opponent static restriction, a damaged-opponent count, a speed comparison); **none is in a target deck**, which is why none was taken. Kaya, Spirits' Justice is the cheapest-looking and is **not**: `ForEachOpponentTarget` caps `ctx.targets` one-per-controller *including slot 0*, so wrapping only the opponent half is off by one.
