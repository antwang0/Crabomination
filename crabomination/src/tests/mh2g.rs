...
fn steel_dromedary_counter_lock() {
    ...
    for _ in 0..2 {
        let eff = crate::effect::Effect::MoveCounters {
            from: crate::effect::Selector::This,
...
fn cr_702_62e_suspend_grants_suspend() {
    ...
    let ctx = {
        let mut c = crate::game::effects::EffectContext::for_spell(0, None, 0, 0);
...
fn yusri_flip_mix() {
    ...
    let ctx = crate::game::effects::EffectContext::for_trigger(yusri, 0, None, 0);
