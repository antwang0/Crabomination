...
1. **FIRST:** `git fetch origin claude/modern_decks && git checkout -B claude/modern_decks origin/claude/modern_decks`. Rebase, never force; code before tracker prose; **fetch before every push** — SEVEN sessions shared this branch on 2026-09-13. Gotchas in **CLAUDE.md**, measurement in **PERF's "Standing rules"**, and ⚠ **the 2026-09-12/13 detail is in ENGINE_BACKLOG's "The 2026-09-12/13 handoff detail, moved verbatim"** — the printed-body skip taxonomy, the whole `EventKind` fan-out census, the modal census, the closed-by-measurement list, the collision log and the seed discipline. **Read it before you take a skip kind or a seed, and say in this line which you are taking.** NEXT is 15 lines and stays 15; findings go to PERF, ENGINE_BACKLOG or INCOMPLETE_CARDS.
2. **Gates at the tip, all re-run this run:** suite **19,547 / 0 / 5** (`CRAB_ANSWER_LOG=strict`), clippy **0** (`--workspace --all-targets --exclude crabomination_client` — the client needs four apt packages this image lacks — and **0** on `-p crabomination --features trig-census --bins`, which is the only build that compiles `gather_census`), golden_trace **12 / 12 unmoved**, `--bench` **195,806 / 27.49 / 611.9 / 0 stalls byte-identical to the committed invariant** with `determinism ok` and `thread_determinism ok (3 vs 1)`. ⚠ `profiling-fast` inherits `release-fast`, so every A/B build already saw `debug-assertions = false` — that gate rides along with the measurement builds. ⚠⚠ **THIS BOX IS 4 CORES** (418 games/s); no absolute in PERF was taken on 4. **The counters are the invariant.**
3. **Perf — the run took -0.627 / -0.559 / -0.736 % (fixed / cube / sealed) in five commits, and ALL THREE of PERF's sized leads are now spent.** `(-298)`+`(-299)` finished the reduction walk's tail: nineteen `static_abilities` walks merged into one, then the whole tail behind `CardData::has_self_cost_reduction` (a one-bit definition memo on **word 1, bits 53-54** — word 0 is full). The row is **181.9 -> 71.0 Ir/call, -61 %**. `(-300)` took the first of `(-294)`'s two dispatch-count questions: **94,072 of 199,186 dispatches (47.2 %) are empty**, gated at the call site with `#[inline(always)]` (plain `#[inline]` leaves a 180,198-call stub and is worth 0.015-0.021 points less). ⚠ **`(-301)` is a REFUTATION to read before touching `printed_requirement_impl`**: its 40 %-self-recursion is NOT a lead — a spine loop loses +0.458 % keeping the order and **+1.410 %** not, because `.and()` nests LEFT and the innermost clause is the broad type test that short-circuits. **An order-free fold is not an order-free walk.**
4. **Perf — the target half is STARTED and the obvious next step on it is REFUTED.** `(-302)` put a printed card-type pre-test in front of the enumerator's board walk (**-0.126 % sealed, -97,138 walker calls**), off the filter's top-level `And` spine — sound because every conjunct must hold, so a partial pre-*test* is sound where a partial evaluation is not. ⚠⚠ **Do not widen it**: carrying `Nonland`/`Noncreature` and the two controller conjuncts was built TWICE, in structurally different shapes, and both read **fixed +0.369 / sealed +0.259..0.271 %** for 6,158 more rejections. **A pre-test is priced by the candidates it does NOT reject.** What is left of the row is the ~23 evaluations the pre-test keeps. The other open dispatch question — merging `perform_action_inner`'s batches — is now about the **105,114** that do work. ⚠ The allocator row and the `(-92)` flatness verdict are both still READ AND CLOSED; do not spend a build on either.
5. **Bugs — a target-walker CLASS closed, and it was latent.** `player_ref_selector` is the one list for "which `PlayerRef`s read a player out of a selector"; three of the five (`EachPlayerExceptControllerOf`, `CombatDamagerController`, `LastDamagerControllerOf`) were in none of the three walkers, so a `Selector::Target` under them read as untargeted. Only Fractured Identity ships one and its body targets too, which is why `audit_target_fields` saw it (**1 aimed -> 0**) and 19,546 tests did not. ENGINE_BACKLOG rule **5c**. **`all` 1281 is still open and its field is `ctr`** — the battlefield counter total, +1 every two turns, everything else period-2; a counter is ordinarily real progress, so 1281 is the adjudication call ("no seat can win or lose"), not a field to drop.
6. **Seeds: frontier 1296 — THIS RUN TOOK 1292..1295: 12 cells / 59,200 games / 0 failures**, `cap 2 (both slow-not-stuck) / board 0 / stuck 0 / draw 0`, on the `overflow` build with `-C debug-assertions=yes` and `CRAB_ANSWER_LOG=strict` at the `(-300)` tip. Both caps are `cube` 1292's Beacon board at `repeats 11/12` — one sample short at 6,000 actions, `cap 0 / draw 2` at 50,000. Say which block you are taking in this line *before* you take it.
7. **Best next moves.** (a) **THE RE-PROFILE IS DONE AND PERF'S CANDIDATES HEAD HAS A NEW #1: `GameState::compute_permanents`, 7.25 % of `cube` inclusive** (4.61 fixed / 3.96 sealed) — bigger than the dispatcher's self row on that pool — **and 47-56 % of its calls do a whole-game continuous-effect gather**. No repetition inside a call; the repetition is across the bot's 6,244 `declare_attackers_banded` and 4,708 `declare_blockers` simulations on a board that has not moved. ⚠⚠ **The obvious fix is already done — do not re-take it**: both combat declarers already wrap their compute in `with_frozen_layers` and the bot holds a scope at sixteen sites, so every gather left is a scope's FIRST one, and `GameState::clone` starts each probe unfrozen by construction. **The device is a memo whose lifetime spans a mutation** — a state-level gathered-effect memo cleared at the write chokepoints, the shape `Battlefield`'s lanes use one level down — **and `(-303)`'s census, taken this run, is the strongest number on the page: 95-99.6 % of gathers return the answer the previous gather returned and 50-52 % follow NO `&mut` battlefield reach** (`gather_census`, `--features trig-census` + `CRAB_GATHER_CENSUS=1`, byte-identical shipped binary). The row is exact: the gather's whole caller tree is **26.9 M / 4.25 % fixed, 82.7 M / 4.91 % cube, 63.8 M / 3.63 % sealed**, so serving only the `NO_REACH` half is worth **~1.8-2.5 % of the program on every pool** — larger than anything landed in one pass since `(-278)`. ⚠ `(-303)` lists the three things to establish first, and the first is that the gather's inputs are not only the battlefield. The cheap second device is routing `compute_permanents` through the scope's `perms` memo (it calls `apply_layers_one_gated` directly today, so it and `computed_permanent_hinted` memoize nothing for each other). ⚠ `cg_ratio.py` at this tip has nothing new — its 35.2x top row is callgrind's recursion context of the requirement walker, which `(-301)` closed. (b) ⚠ **The three refutations this run are the cheapest thing on the page to read before spending a build**: `(-301)` (the walker's recursion is not the cost, its order is worth 1.4 %) and `(-302)`'s wide arm: the queue's top two are spent and `cg_ratio.py` off a fresh `sealed`/`fixed` join is what found the current #1 in the first place. (c) ⚠ **DO NOT re-take `(-295)`'s class as a defect fix — `(-297)` laddered it and it LOSES** (pooled 49.58 % on sealed); if taken again it is a STRENGTH arm and what changes is the owner's BASELINE, not the predicate. (d) The skips with a card behind them: `nocolors` **950**, `nosubtypes` 43, `noback` 60, `nokeywords` 29 — say which you are taking. (e) `audit_keyword_drift`'s last 4 each need a PRIMITIVE (bestow / escalate / flashback / kicker, one card each). (f) `audit_variant_coverage` is at ONE dead primitive, closed with a reason (`GrantCastBackFromGraveyard`); do not go looking.
