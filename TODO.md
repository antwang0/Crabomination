...
1. ⚠⚠ **TWO SESSIONS SHARE THIS BRANCH. FETCH BEFORE YOU START AND BEFORE EVERY PUSH, AND RE-RUN THE SUITE AFTER EVERY REBASE.** `cargo nextest` fail-fast hides the rest — use `--no-fail-fast`. Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure"**. ⚠ **A `release` build here is ~55 min cold and `crabomination_base` is under the catalog**, so *any* edit to `effect.rs` costs the whole chain: batch the base-crate edits of a run into one commit and take the bench once, at the end.
2. **Gates at the closing tip (engine `4c4afec6`; the commits above it are markdown and scripts):** suite **19,762 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); clippy **0** over the workspace; `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean; `--bench` (release) **195,806 / 27.49 / 611.9 / 0 stalls** — **byte-identical to the committed invariant at both of this session's tips**, `determinism ok`, `thread_determinism ok (3 vs 1)`, `peak_rss_mib` 25.7 / 25.4. Pod smoke: **10,800 games over 2/3/4/5 seats at fresh seeds 3101-3108, 100 % decided, 0 stalls, every `undecided_by` column zero**, no panic or error line in any run. **FRONTIER 3109.** ⚠ No Ir A/B taken and none is owed: every arm gains one `Option::is_some` on the non-suspending pass (`splice_after_suspend` and `rewrap_parked` both return on `None` before building anything), and the aggregate did not move. PERF's Baseline has the block, including the build economics that cost this session most of its wall clock.
