...
9. **Best next moves.** (a) **Re-run `(-308)`'s device** — `cg_lines.py --in <fn>` over the top ten self rows, looking for identity-operand arithmetic and unguarded no-ops; it is the only device this run found that paid above 0.3 %. (b) The biggest untaken engine row is still `dispatch_triggers_for_events_slow` (**4.5 % cube / 6.0 % sealed self, ~750-990 Ir a call, 105 k calls**) and its line profile is FLAT — 267 lines under 0.37 % each — so it needs the structural half (**merging `perform_action_inner`'s batches, a CR 603.3b ORDERING question before a perf one**), not another line. (c) `audit_dropped_may` reads **240**; the next buckets are "put a creature/land card" (30), "begin the game" (12, companion/Leyline, genuinely unmodelled), "collect evidence" (11), "put a quest counter" (10) — and the gate in line 7 is the better instrument for any of them. (d) `audit_keyword_drift`'s last 4 each need a PRIMITIVE (bestow / escalate / flashback / kicker, one card each). (e) The skips with a card behind them: `nocolors` **950**, `nosubtypes` 43, `noback` 60, `nokeywords` 29.
