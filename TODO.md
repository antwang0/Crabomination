...
6. **Perf — what is left on that row, and it is still the biggest thing on the page.** `compute_permanents` is 7.25 % of `cube` inclusive and `(-311)` took only the *gather* half. Still untaken: the **layer passes** (75.6 M on `cube`) and `frozen_effects` (19.4 M). ✅ **The device for them now exists**: `GatherKey` includes `battlefield.writes`, so **`(GatherKey, CardId)` is exactly as sound a key for a `ComputedPermanent` as it is for the gather** — any instance write bumps the board counter. That makes `LayerFreezeState::perms` promotable from scope-scoped to key-scoped (drop the `end_of_scope` drain, clear on a key change instead). ⚠ Price the `GameState::clone` cost first: `perms` is eight inline slots and `(-311)` shows a 128-byte copy per clone is ~1-3 M Ir; **consider NOT carrying it across the clone** (the gather memo's carry was worth only 0.24/0.22/0.14 points).
