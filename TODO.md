...
2. **Gates at the closing tip `1a49709b`:** suite **19,761 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); clippy **0** over the workspace; `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean; `--bench` (release) **195,806 / 27.49 / 611.9 / 0 stalls** — **byte-identical to the committed invariant**, `determinism ok`, `thread_determinism ok (3 vs 1)`, `peak_rss_mib` 25.7. Pod smoke at the tip: **5,400 games over 2/3/4/5 seats at fresh seeds 3101-3104, 100 % decided, 0 stalls, every `undecided_by` column zero** (18.23 / 30.54 / 41.49 / 51.80 turns a game). **FRONTIER 3105.** ⚠ No Ir A/B taken and none is owed: the change adds one `Option::is_some` per loop iteration on the non-suspending pass (`splice_after_suspend` builds its tail in a closure), and the aggregate did not move. PERF's Baseline has the block, including the build economics that cost this session most of its wall clock.
