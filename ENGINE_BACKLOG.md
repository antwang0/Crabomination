...
| Bugs & robustness | [FIXED 2026-09-20 (the sixty-third find) — a target-deck card CASTABLE BY NO PATH, and the multi-target pump the trick picker folded onto one creature](#fixed-2026-09-20-the-sixty-third-find--a-target-deck-card-castable-by-no-path-and-the-multi-target-pump-the-trick-picker-folded-onto-one-creature) | 106 |
...
| Bugs & robustness | [The Commander NEXT as it stood on 2026-09-26, moved verbatim](#the-commander-next-as-it-stood-on-2026-09-26-moved-verbatim-when-session-01leharn-rewrote-it-to-15-lines) | 11 |
...
2026-09-23 (pod cards: From the Catacombs, Woe Strider, Kotis, Psionic
Ritual, Haze of Rage; granted replicate is still not offered; casualty and
bargain spend only a token). None of the
...
activated. **10-seat pods, 2,000 games, seed 9902: nine of the ten target decks
played every single card in the list.** The tenth left exactly one — **Yuriko's
Agony Warp, never cast once.** Not variance; a 40-game run at another seed
named the same single card.

The card is modelled correctly (`Seq[PumpPT{-3/-0, slot 0}, PumpPT{-0/-3,
slot 1}]`, both slots declared). It was **castable by no path at all**, which
takes two halves and both are in `bot.rs`:

1. The hand sweep drops `is_combat_trick(&c.definition)` **unconditionally**,
   with no step check — so a "pure temp-pump instant" never reaches the
   candidate menu in any step, and `pick_combat_trick` is its only route onto
   the stack. (The `trick_modes_combat_only` gate further down *does* carry a
   step check; it is not what closes this door, and an earlier draft of this
   entry said it was.)
...
Fix: a multi-slot pump is not a trick. `is_combat_trick` now also requires
every pump clause to name the **same** slot, so the seven-card "another target
creature" family falls through to the ordinary menu and is cast as the removal
it is. Before/after on the identical seed: **434 distinct cards played, Agony
Warp unplayed → 435, all ten decks complete.**

⚠⚠ **The blast radius was SEVEN cards and reasoning said one.** A source census
keyed on the literal `slot:` spelling found two candidates and concluded "one
card can move". The catalog-wide test written to *measure* the same claim
returned seven: Agony Warp, Consume Strength, Leeching Bite, Schismotivate,
Seeds of Strength, Skulduggery, Steal Strength. **A blast radius is measured
over the catalog, never derived from a grep** — this is the same lesson as the
shared-reader find, one level up.

🔎 **And the sixth card is why this is a WRONG action and not only a missed
one.** The mixed-sign cycle (Consume Strength, Leeching Bite, Schismotivate,
Skulduggery, Steal Strength) folds to `(0,0)`, so those were merely never
picked. **Seeds of Strength is three separate `+1/+1` pumps on three
creatures**, which `pump_amounts` folded into **`+3/+3` on one** — a 3x
over-valuation the picker would act on. A fold that loses a sign is a missed
play; a fold that adds across targets is a misplay.

✅ **Measured, not argued: none of the seven is in a two-player pool.** Six
appear nowhere in `crabomination/src` at all and Agony Warp only in
`pod/decks.rs`, so none is in `cube` / `sos` / `sealed` / `fixed`. `--bench`
came back byte-identical (195,806 / 27.49 / 611.9 / 0 stalls, `determinism ok`,
`thread_determinism ok`) and 10,000 two-player games over the three pools the
bench cannot reach were all decided with zero panics.
`the_multi_slot_exclusion_moves_only_cards_outside_the_two_player_pools` pins
the seven names so an eighth has to arrive with its own bench reading.

📐 **The shape left behind, still open and now harmless: the HOLD predicate is
`any` and the PICK predicate is `all`.** `contains_temp_stat_leaf` matches a
tree containing *any* temporary stat change; `all_temp_pumps` matches only one
that is *entirely* constant pumps. An instant between the two is held out of
the main phase by `trick_modes_combat_only` and refused by the trick picker.
Per-slot trick evaluation (each slot aimed at its own creature, a shrink scored
as removal) would let these cards be *tricks* rather than merely castable, and
it touches an adopted A/B-measured weight, so it needs its own A/B.
...
## The 2026-09-12/13 handoff detail, moved verbatim from TODO's NEXT

### The 2026-09-13 (second day) detail, moved verbatim at the sixth pass of the day

`TODO.md`'s NEXT had grown back to seven paragraphs against its own 15-line
bound — the same failure the block below records, one day later. Items **2b, 3,
4, 5 and 6** of that section are moved here **verbatim and not summarized**: the
turn-watch reading, the whole `EventKind` fan-out census and its five defects,
the closed-by-measurement list, the same-seed collision log and the seed
discipline, and the "best next moves" inventory. NEXT points here.

⚠ **One item below is overtaken and the correction is in PERF, not here** (this
block is verbatim by design): item 5's `all` **1274** is **CLOSED** — it was a
bot defect, not a watch bug (PERF `(-295)`, Baseline `1274`), and the digest
redesign it calls "the next run's highest-value pull" is **not needed for that
cell**. `all` 1281 is still open and its aperiodic field is named now (`ctr`).

2b. **This run — a watch that does not fire is a question about its INPUTS, and three runs reasoned about its constants instead.** `all` 1159, the sweep's only ever cap to survive a 50,000-action re-run, is `cap 0` (item 4a). Two tests came out of it and the first matters more than the fix: **`no_progress_step`** is the turn watch's state machine as a pure function, so the stream shapes that decide whether it fires are testable at all — none is reachable from a board (1159 needed 2,270 turns and a whole deck pool to make one) — and it pins the cliff: an excursion one sample past `NO_PROGRESS_MAX_PERIOD`, repeated once a cycle, means the watch **never** draws, however many turns it is given. **`progress_fingerprint`** makes the field list testable AS a field list. Negative control in the direction that can go wrong (taking a field out can only ADD draws): 20,000 replayed games, no verdict moved.
3. **The fan-out census, its two exposed defects, and the rules fix underneath them.** (a) **A `PlayerRef` resolved singularly drops every seat but the first.** `resolve_player` answers `EachPlayer` with seat 0 and says nothing; exact while the ref names one player (`EachOpponent` at two seats, which ~100 cards rely on), a silent under-application when it names two. **Seven shipped cards** — New Frontiers, Jace's −8, Krenko's Buzzcrusher, Case the Joint, Aether Rift, Mnemonic Nexus, Ill-Gotten Gains (which discarded *seat 0's hand size* from everyone). The gate is a `debug_assert!` in `resolve_player` reading the seats the ref ACTUALLY names — **dead in release** (`strings` the two binaries), so no training run can panic on it — and it found the two that no `who:` census could reach. Details in PERF's Baseline head. (b) **`EventKind::CardMilled` was NOT in `event_kind_fans_out`**, so a mill of N minted **one** trigger whatever N was — right for the plural "whenever **one or more** cards are put into your graveyard from your library", wrong for the singular "whenever **a** land card / **a** creature card is". **Three shipped cards**: Pedantic Learning offered its {1} once for a batch of lands; Dreadhound drained once for a batch of creature cards, disagreeing with its own `CreatureDied` half, which already fanned out; Hedge Shredder, whose body moves its own `TriggerSource`, deployed **one** land of a mill printing "put **them** onto the battlefield tapped". One line in the fan-out list plus `.once_per_batch()` on Devourer of Memory (the only genuinely plural one); Hedge Shredder stays fanned out **deliberately** — per-land firing is what moves every land, and the residual is the trigger count on the stack, not the board. ⚠ **Devourer's comment asserted the exact opposite** ("the engine has no per-event batching") and would have sent the next reader to break the one correct card. Five gates, all five proven able to fail by injection. **(c) The census that fix opened found a bigger one in the same list: `EventKind::EntersBattlefield` was absent too.** A `CreateToken { count: 3 }` pushes three `PermanentEntered` into ONE batch, so **Soul Warden gained 1 life off Spectral Procession's three tokens** — and **332 catalog factories** carry a non-`SelfSource` ETB trigger while the whole 19,523-test suite passed on either side of it, because no test had ever put two permanents into one batch past a watcher. Of the 332, **12 print the plural "one or more … enter"**: 6 already carried `once_per_turn` (which the dispatcher already excludes from fan-out), 4 needed `.once_per_batch()` (Satoru, Valley Questcaller, Builder's Talent, Spiritcall Enthusiast), and **2 must stay fanned out** — Woodland Champion, whose "put **that many** counters" IS one counter per entry, and Frantic Scapegoat, whose intervening `if` fails for every entry after the one that accepts. ⚠ **Three of those cards' doc comments already claimed per-entry firing** ("modeled as one counter per entering token", "the batched 'one or more' trigger is modeled per entering creature") — they described an intent the engine never delivered, which is why the card authors did not notice. Four more gates, all four proven able to fail. **(d) The census then ran to the end of the list, and five more kinds were missing.** Each probed against a real batch first, each reading 1 where the printed card wants N: `PutIntoGraveyard` (The Haunt of Hightower, 1 counter for a five-card mill), `LandPutIntoGraveyard` (Slogurk, 1 for five lands), `TokenCreated` (Mirkwood Bats, 1 life for three tokens), `Explored` (Wildgrowth Walker, 1 for two), `PlayerDamaged` (Aurification, 1 gold counter for two attackers), `CardExiled` (Soulherder, 1 for two). Pins: Moonshadow, The Gitrog Monster, Kambal's second half. ⚠ **`PutIntoGraveyard` needed a second fix the fan-out exposed**: one milled card puts TWO matching records in the batch (`CardPutIntoGraveyard` AND `CardMilled`, both matched by that kind), so the Haunt read **10** for a five-card mill until the dispatcher deduped on subject. Neither record can be dropped at the emission sites — the dredge mill pushes only `CardMilled`. ⚠⚠ **DO NOT add the combat-damage kinds** (`DealsCombatDamageToPlayer` & co.): `combat.rs`'s `fire_combat_damage_triggers` already pushes them once per damaging source, outside the dispatcher — that is why Larceny reads 2 for two attackers while Aurification read 1 in the same batch. Seven more gates, all seven proven able to fail. **(e) The recalibration of `cr_614_13_token_doubling_fires_per_doubled_token` was half right and the other half was a MISSING RULE.** Under per-token firing Voldaren triggers twice — that part is right, and the test asserts the COUNT now. But it does not transform twice: **CR 701.27f** says "if an activated or triggered ability of a permanent … tries to transform it, the permanent does so only if it hasn't transformed or converted since the ability was put onto the stack", so the second trigger's transform is **ignored** and the face stays on the back. The engine had no such rule, so any fanned-out kind whose trigger transforms `Selector::This` flip-flopped on an even-sized batch: **Daring Sleuth** read *untransformed* off a two-Clue sacrifice (`PermanentSacrificed` has fanned out since long before any of this), and six more shipped cards sit on the same shape. Implemented as `source_transformed_since_push` on `StackItem::Trigger`, set by `transform_permanent` on the items ALREADY on the stack — which is exactly the rule's anchor, since anything pushed later keeps the builder's `false`. It lives in `scratch`, not on `GameState`: the state is at its 1,600-byte cap and one `bool` cost 24. **Four** gates (`core_rules/cr_recent104`), the two-Clue one proven able to fail; the third card, Legion's Landing, is a gate too because its existing test attacks with THREE and got the face right by parity. **(f) The fresh-seed sweep found a bug for the first time in a while, and the STANDING GUARD is what caught it.** `cube` / `all` seed 1254 aborted `rc 134` on `perform_action_uncheckpointed`'s partial-mutation assert — **its first firing ever**. The Dawning Archaic attacks into a **Grafdigger's Cage**; the Cage refuses the free cast CORRECTLY and `cast_card_from_zone_spending` reports the refusal as `CardNotInHand`, which is the same error it raises for the engine invariant, so the effect propagated it — and an `Err` out of a resolution leaves the stack item `resolve_top_of_stack_inner` has already popped. ⚠ **THE ARM THE SWEEP LANDED ON IS NOT WHERE THE FIX MOSTLY GOES.** `cast_card_for_free` has thirteen effect-resolution call sites and **nine took that `Err` with `?`**: `Cascade`, `Discover`, `Ripple`, `KnowledgePool`, `PossibilityStorm`, `CastAnyOrderWithoutPaying`, `CastFromHandWithoutPaying`, `CastFreeParadigmCopy`, `ExileTopMayPayEnergyToCast`. **Cascade into an opponent's Drannith Magistrate is the same abort on a board people actually assemble** — far likelier than the 10-drop the sweep happened to find — so the check sits in the helper (CR 608.2: an instruction a player cannot follow is ignored) and the player-action path keeps its `Err`, where a refusal IS the rejection of an illegal action. Two regression tests, both proven able to fail by injection. ⚠ **The guard fired and named NOTHING** — `CardNotInHand(CardId(28))`, and 22 functions raise that — so it now prints the stack top by card name and the first diverging byte of the state; that is what named the card on the first re-run. **A guard that aborts is paying for a diagnosis and should collect one.** **(g) AND THE GRAVEYARD WALK DID NOT GET (d)'s DEDUPE** — `EventKind::PutIntoGraveyard` is a `is_graveyard_self_source_kind`, so every "when this card is put into a graveyard from anywhere" trigger fires from the OTHER walk (ten catalog factories), and **Ichor Wellspring drew two cards for a one-card mill** for the hour the fix was half-landed. The rule is `events::fanout_dedupes_on_subject` now and both walks read it. ⚠⚠ **THE TWO WALKS ARE THE STANDING HAZARD**: the fan-out decision, the once-per-turn budget, the intervening filter, the replaced-death skip and the subject dedupe are each written twice, and the graveyard one has silently missed a rule TWICE. **Unifying them is the cheapest structural pull on this page.** ⚠ **PARTLY TAKEN, AND "unify the walks" IS THE WRONG SHAPE FOR THE REST.** The two loops are NOT symmetric — the battlefield one carries the batch bit-mask and the measured guard ordering, the graveyard one has neither — so merging them puts a branch in the hot path to buy tidiness. What actually drifted is the verbatim duplication, and that is extracted: `EffectContext::for_intervening_filter` (the CR 603.4 filter context, **three** hand-written copies of nineteen identical fields) and `events::subject_already_fired` (the CR 701.15b dedupe, two byte-identical copies — the pair whose absence from the graveyard walk WAS the Ichor Wellspring bug). -87 lines net, 115 out of `game/mod.rs`; counters byte-identical. Still hand-written twice: the fan-out decision, the once-per-turn budget and the replaced-death skip — each 2-4 lines over shared helpers, so the remaining drift surface is small and the next reader should price it before merging loops. **(h) The census that fix opened found a bigger bug beside it: `None` from `pick_trigger_mode` does not mean "not modal" downstream — it means `mode.unwrap_or(0)`.** **Nine shipped cards** resolved a printed "choose one" at **mode 0 forever, without ever asking anyone.** Two holes: **`governing_modal` did not descend `If` or `HauntCreature`** (Prophetic Titan, Depth Defiler, Tizerus Charger, Wardens of the Cycle hide the modal in one `If` branch — the other is the delirium/kicked "choose both" or a `Noop`; Orzhov Pontiff carries it as its haunt body, so its famous -1/-1 mode was dead while its ETB half, the SAME `ChooseMode` at top level, worked); and **a nested modal whose mode TARGETS was refused by the deferral gate and fell through to `None`** rather than to the synchronous decider the top-level path has always used for that case (Merrow Reejerey, Stinging Lionfish, Component Collector, Atraxa's Skitterfang all print "tap **or untap**" and only ever tapped). ⚠ **The function's own doc asserted the opposite** — "the printed Magic cards in scope today all have a top-level `ChooseMode` so the simple walk is sufficient" — the same shape as Devourer of Memory's "the engine has no per-event batching", a claim the catalog outgrew left where the next reader would trust it. Three gates, all three proven able to fail, and the structural one NAMES the five cards when it fails. One existing test moved and says why: `stinging_lionfish_taps_on_first_off_turn_spell` had its scripted answers in the order the bug produced; CR 700.2b picks the mode when the ability goes ON THE STACK, so `Mode(0)` now precedes the `MayDo`'s yes. Same assertion, different asker. ⚠ **And the same census found a THIRD route to a resolution-time `Err`, through the ACTION rather than the catalog: `GameAction::CastSpell { mode }` is range-checked NOWHERE** (only the activated path clamps), so `mode: Some(99)` on any modal spell returns `Ok` from the action and `ModeOutOfBounds` from the resolution with the stack item already popped — an abort any client can trigger. Measured `Ok(1)` / `Err(ModeOutOfBounds(99))` / `stack left = 0`. The `ChooseMode` arm clamps now, as its three deferred paths always did. **The lesson is the one that keeps repeating: a legality question answered at RESOLUTION time is answered in the one place an `Err` cannot be recovered.**
4. **Closed by measurement this run; do not re-take.** (a) ✅ **`all` 1159 IS CLOSED** — `cap 0 / draw 10` — and the route was not a constant. `NO_PROGRESS_MAX_PERIOD` is **NOT** what held it: `since 0/8` in both dumps, and at `CRAB_MAX_ACTIONS=200000` the same cell runs to turn **9,099** and reads `repeats 6/12`, *lower* than 50,000's 10/12. The board is aperiodic because the **TAPPED SET** moves every turn — and the step this reading stopped one short of, which the concurrent session took (`08841a3e`), is the useful generalisation: **a periodicity detector cannot be tuned into seeing an aperiodic stream, so ask which FIELD is making it aperiodic and whether that field is progress.** A tap the untap step takes back is not, so `tapped` left the turn digest and the stream went constant. Both readings and the numbers are on the constant's own doc. ⚠ **`CRAB_PROGRESS_WATCH=<turn>` (`12ed0fcc`) is the instrument that answered it** — every field the digest reads, one line a turn, printed from the self-play driver rather than from the watch (which runs on every bot probe's clone). ⚠ **The sweep's `[SATURATED LIFE]` carve-out went with the cell** (`2be06a35`): a survived cap is a defect, saturated or not, because an unwinnable board has an ending now and a survived one means the watch has a NEW hole. (b) `Effect::Search`'s `to:` is not a defect class: a plain search resolves `controller: You` against the SEARCHED player by design. (c) The **ML side of a saturated feature IS guarded** — `server::encode`'s `scaled` / `scaled_u` plus three tests including `assert_globals_bounded` over every global and object feature; what is open is the training-distribution question, not a code gap. (d) **`audit_variant_coverage` is at ONE dead primitive** — `AddRadCounters` got Nuclear Fallout (`decks::recent329`) plus `sets::pip`'s Contaminated Drink / Glowing One / Feral Ghoul, and **no encoding moved** (new factories reach only `all_known_factories`, a name lookup; the cube and sealed pools are explicit lists). The survivor's fabricated Pestilent Cauldron attribution was in **seven** doc comments — not the three the first sweep found — and in a ✅ INCOMPLETE_CARDS row sitting directly above the 🟡 row saying the rider does not exist; all eight corrected. **The grep that proves a triage reached the code is the card NAME, not the variant name.**
5. ⚠⚠ **THE SEVENTH AND EIGHTH COLLISIONS HAPPENED TODAY AND CLAIMING EARLIER CANNOT FIX THEM** — this session ran the whole `EventKind` fan-out census independently (`EntersBattlefield`, the seven kinds, the three `once_per_batch` pins, the graveyard double-record) and discarded its own copy TWICE, each time minutes after the other session pushed the same finding; both sessions were in flight before either claimed. **What produced unique work both times was reading the other session's LANDED COMMIT adversarially**: CR 701.27f came out of disagreeing with its recalibrated test, and the graveyard walk came out of asking whether its one-walk fix covered the other walk. On a shared branch the highest-yield minute is on what just landed, not on the backlog. **Seed frontier 1272** — this session then took **1252..1261** and it was the block that paid: **30 cells / 138,000 games / 2 failures**, both seed 1254 (`cube` and `all`; `sealed` clean), both the one defect in item 3(f), and the same block re-read **30 cells / 148,000 games / 0 failures** `cap 0 / board 0 / stuck 0 / draw 16` after the fix. **1262..1271 TAKEN and clean: 30 cells / 148,000 games / 0 failures**, `cap 0 / board 0 / stuck 0 / draw 12`, swept at the opening tip and RE-swept at the closing one because item 3(h) changes what the bots are asked and so which games they play. **1272..1281 TAKEN and it FAILED: 30 cells / 148,000 games / 4 failures**, all `cap`, all surviving the script's own 50,000-action re-run — `all` **1274** (unlabelled) and `all` **1281** (`[SATURATED LIFE]`, turn 2001). **1274 is fully diagnosed and deliberately NOT fixed; the diagnosis is in PERF's `1274` entry and it is the next run's highest-value pull.** Two Thopter Foundries on 30 lands: sacrifice a Thopter, gain 1 life, make a Thopter. `CRAB_PROGRESS_WATCH=1900` over 56 turns reads EVERY digest field as period-2 except two — **`life` is monotone** (+1 per own turn; 946 at turn 1927, 3831 at turn 7696) and **`ids` never repeats** (the remade Thopter is a new `CardId` though the board is identical). ⚠ **Removing either one alone fixes NOTHING** — the digest is a single `u64`, so one aperiodic field suffices and there are two. At 200,000 actions: turn 7696, `repeats 0/12`, **`since 8/8`**, re-anchoring forever. What is left is a DESIGN call (is a token remade each turn 'a permanent entering and leaving'? is mutual unbounded life gain progress?), and the `tapped` change that closed 1159 was the same kind of call landed with a 20,000-game negative control — so it needs a run, not a corner of one. ⚠ **TEST THE CHEAPER HYPOTHESIS FIRST — it may not be a WATCH bug:** p0 sits on **`poison9`, one counter from losing**, for all 5,769 turns, and 2 Thopters against 946 life is no clock either, so this may be a board the bots fail to WIN rather than one that cannot be won — a `dflt` quality question, not a digest question. **Frontier 1282; 1282..1291 is untaken.** Before that, this session took **1232..1241** (pre-merge tip, `cap 0 / board 0 / stuck 0 / draw 14`, the first block under the tightened verdict) and **1242..1251** (merged tip, `-C debug-assertions=yes`, `draw 12`), 60 cells / 296,000 games / 0 failures between them. ⚠⚠ **THE FOURTH SAME-SEED COLLISION: BOTH sessions swept 1222..1231 on 2026-09-13**, independently, and both read **30 cells / 148,000 games / 0 failures / `cap 0 board 0 stuck 0 draw 4`** — byte-identical verdicts off two different builds, which is a determinism cross-check nobody asked for and an hour each nobody needed to spend. This one ran under `-C debug-assertions=yes`, so the new `resolve_player` fan-out gate ran on every decision of all 148,000 games; theirs did not. **TAKE THE NEXT SEEDS FROM PERF'S TABLE, NOT FROM THE TOTAL, AND SAY IN THIS LINE WHICH BLOCK YOU ARE TAKING BEFORE YOU TAKE IT** — that is now FIVE collisions in two days — the fifth was the dead-primitive item, where both sessions independently picked **Nuclear Fallout** off the same `audit_variant_coverage` row and one copy was dropped at the rebase — and the table is the only thing that stops it.
6. **Best next moves.** (a) ✅ **`notyped` is 0 and is a GATE now** — the seven were read by hand against the oracle first (zero defects, so the yield was always the gate), and the two shapes were a nested literal as the base and a spread that is not the start of its line; +8 cards on the type line, +7 subtypes, +7 keywords, +5 P/T, 0 wrong anywhere, and `nocolors` 945 -> 950 is those cards reaching a column they then skip at. Injections **54**. (b) The skips with a card behind them: `nocolors` **950**, `nosubtypes` 43, `noback` 60, `nokeywords` 29 — say which you are taking before you take it. (c) `audit_keyword_drift`'s last 4 each need a PRIMITIVE. (d) `audit_variant_coverage` is down to ONE dead primitive and it is CLOSED WITH A REASON — `GrantCastBackFromGraveyard` is the one-shot grant nothing prints (the printed shape is Disturb, a keyword); do not go looking for the Pestilent Cauldron sacrifice its doc used to name. (e) `audit_target_walkers`' sixth column (`target_filter_for_slot_in_mode_kicked`, 50 unnamed of 132) is LATENT with **zero current yield** — the catalog gate passes — so it is a surface, not a queue. (f) ✅ **THE PERF QUEUE IS RE-SEEDED** (2026-09-13, `1ae70cfd`) after eight runs at floor: PERF's "Perf candidates" head is now a fresh whole-profile read at `d457bcaa` with one clear top row on all three pools — **`dispatch_triggers_for_events` at 6.15 / 4.86 / 6.43 %**, nearly double the #2 on two of them, 199,220 calls at 575 Ir of self, callers and callees sized — **and then READ with `CRAB_TRIG_CENSUS`, which is already in the tree and had never been pointed at it: the row is PREAMBLE-BOUND, not walk-bound.** 105,132 working dispatches on sealed (47 % of its 199,220 calls return at the empty-batch early-out) at **3.54 permanent visits and 3.82 matcher calls each for 1,089 Ir of self**; the lane already hits on 99 % and cuts the board walk to 22.8 %. So a "group the batch's events by kind" device is REFUTED — attack the fixed per-dispatch preamble (the per-event stamping loop, the synthesis collects, the `Vec` builds) instead. The one walk-side number left: **72.6 % of pairs entering the loop can match no event in the batch** (61 % of calls) — price a scope-aware pre-check against `(-88)`'s rule first, the census's own deadness test IS the work. ⚠ **That row GREW this day** (seven kinds joined `event_kind_fans_out`, and fan-out removes the `break` that ended the inner event loop at the first match), so it does not compare with a pre-2026-09-13 reading and the cheapest win-shape is fewer (trigger, event) pairs entered, not a faster body. `(-293)` is the run's perf leg and it is a REFUTATION: the dispatcher's last unguarded `&mut scratch` store read 0.24 % of sealed in the caller table and guarding it bought 0.010 %, because four fifths of the unshares came back one frame down — **a CoW guard pays only where it is the LAST unguarded write to the group in the scope**. The head also carries the closed leads, and ⚠ **the Scute Swarm entry's open half is now closed too — the per-permanent floor is NOT superlinear.** Timing `next_action_settled` at 50..800 permanents: a vanilla bear and a bear with a triggered ability both have a per-permanent cost that FALLS (9.7 -> 4.7 and 10.5 -> 5.0), an anthem static is a constant ~4x and still falls, and only Scute Swarm's rises (45.0 -> 118.0) — because landfall copies every Swarm, so a probe that plays a land does O(N) token creation. The 20x on `cube` 1215 is the card, not the machinery; there is no algorithmic win waiting there. Also measured and closed: **merging the eight integration-test binaries buys nothing** (they link four at a time, so the seven extra cost 6.6 s, not 7x4.26) — CLAUDE.md says so at the convention now. (g) ⚠ **THE REST OF THE FAN-OUT LIST HAS NEVER BEEN AUDITED and it is the cheapest bug-class pull on this page.** `event_kind_fans_out` is 30 kinds; censusing every `EventSpec::new(EventKind::X, EventScope::Y)` in the catalog against it, the kinds a single batch can carry several of and that are **absent** are, by trigger count: **THE WHOLE CENSUS IS TAKEN** (items 3c/3d) — seven kinds added, three cards pinned, one duplicate-event dedup. It read `DealsCombatDamageToPlayer` **52** (every attacker connects in one batch), `PutIntoGraveyard` **12**, `TokenCreated` 5, `Explored` 4, `LandPutIntoGraveyard` 3. `SpellCast` (288) and `StepBegins` (396) are safe — `SpellCast` is hardcoded out of the batch walk and a step begins once. **And the fan-out audit's real dividend was the rule it exposed, not the kinds it added** (item 3e — CR 701.27f). **The method is the transferable half**: census `EventSpec::new(EventKind::X, EventScope::Y)` against the list, probe every row against a REAL batch before believing it, then join the survivors' factory names to the oracle cache and split on "one or more". `CardMilled` looked like a five-card bug and was a three-card one; `DealsCombatDamageToPlayer` looked like a 52-trigger bug and was none at all. What is left unexamined is the OTHER direction — a kind that fans out and should not. ✅ **Its DOUBLE-RECORD half is CLOSED, zero new defects.** `event_matches_spec` lets one `EventKind` match several `GameEvent` variants — that is how `PutIntoGraveyard` matched both `CardPutIntoGraveyard` and `CardMilled` and read 10 for a five-card mill — so the checkable question is which OTHER kinds in the list match more than one variant. **Of the 37, exactly TWO do**, and the second is `PermanentLeavesBattlefield` (`CreatureDied` + `PermanentLeftBattlefield`), which is NOT the same bug: the two routes are disjoint by construction (`note_left_without_dying` runs on every NON-graveyard exit, the death path collects its trigger before removal), so one exit writes one record whichever way it goes. Pinned in both directions by `cr_603_6_one_exit_is_one_leaves_battlefield_trigger`, because *disjoint by construction* is exactly the claim a later refactor breaks silently. What is STILL unexamined is the other half — a kind whose watchers are mostly PLURAL-worded and that fans out anyway — and `CardExiledFromPlayOrGraveyard` (Kaya, Ketramose), deliberately left off because both its cards print the plural. ⚠ **A concurrent session's NEXT claimed `DealsCombatDamageToPlayer` off this census at `bb4235b0` — it is NOT a bug and must NOT be added**: `combat.rs`'s `fire_combat_damage_triggers` already pushes that kind (and `DealsDamageToPlayer` / `DealsCombatDamage` / `DealsDamage`) once per damaging source, outside the dispatcher. Larceny reads 2 for two attackers today; adding it would double-fire.



`TODO.md`'s NEXT is the three-hour handoff and its header asks for **<= 15
lines**. It had reached **163** — ten times the bound — because every run
appended its findings to it rather than to the file that holds findings. The
numbered items below are the 2026-09-12 and 2026-09-13 detail, **moved here
verbatim and not summarized**: the printed-body skip taxonomy, the four shapes
in the `cap` bucket, the sweep-table discipline, the closed-with-a-reason
leads, and the per-session sentences. NEXT now points here.
...
### The Commander NEXT as it stood on 2026-09-26, moved verbatim when session `01LEHArn` rewrote it to 15 lines
...
2. **Session `01HFhNih` (Necron Dynasties as seat 177):** suite 22,812 / 0 / 7 before the upstream Saga-ratchet fix; workspace clippy 0; `--bench` **200,190 byte-identical** with the unearth fix and the shrink reclass; 4-seat (seed 17701, seats 177/176/175/174, `--card-census`: nothing unplayed) 999/1,000 decided (1 draw), 6-seat (17702, 177-172) 998/1,000 (2 draws); strict debug pods 17703-17706 (4 and 6 seats with 178) all decided. 2. **Gates at this run's tip (2026-09-25, session `018BnccM` run 5, after Timey-Wimey as seat 183):** suite **22,870 / 0 / 7** (strict answer log); workspace clippy **0**; `--bench` **200,190 decisions — byte-identical to the invariant**, `determinism ok`; 4-seat (seed 10560, seats 183/182/181/180, `--card-census`: nothing unplayed) and 6-seat (10561, seats 183-178) pods 1,000/1,000 decided; cube / sos / sealed 300 games each decided. `precon_scan.py --top` now lists no official precon with a missing card. **Session `01VVD5mW` (2026-09-26, residual sweep):** suite 21,811 / 0 / 1 (crabomination_tests) + lib 799 / 0 / 6; workspace clippy 0; `--bench` **200,190 — byte-identical**, `determinism ok`; pods at fresh seeds 9201 (2-8 seats, 2,000 each), 9310/9420 (every seat in six-seat groups, 150-200 games each) and 10590/10591 (WHO seats): 25,000+ games, zero panics, only legal draws. Tip after the second sweep: suite **23,143 / 0 / 7**; clippy 0; `--bench` 200,190 byte-identical; every seat in six-seat groups (seed 4001+, 1,860 games) and eight-seat groups (9501, 2,200 games) all decided, zero panics.
3. **Instruments:** `scripts/precon_scan.py --deck <MTGJSON name>` (missing cards; `--top 45` ranks every precon), `bot_ladder --commander --pod-decks I,J,.. --card-census` (what a list never plays), `--first N --games 1 --card-census` (replay one game; the census prints its five most frequent action lines, and an answer's line names the ask and its source — a capped game's loop is the top line), and debug pods `CRAB_ANSWER_LOG=strict target/debug/bot_ladder --commander --seats 21 --games 24 --seed N` — `debug_assert!` guards are where pod bugs are. **A new card is the cheapest probe of an old primitive**; a 1,000-game pod is the cheapest probe of the bot.
4. **Official precons are seats 11-183, card for card** (per-seat notes and residuals: DECK_FEATURES; open residual rows: INCOMPLETE_CARDS' Commander routine table). **Session `011z4qQX` landed Blast from the Past (WHO, The Fourth Doctor + Sarah Jane Smith) as seat 182** (granted read ahead, a non-legendary spell copy, the Seventh Doctor's guess against a Value, `AddOneOfAChosenCounterToEach`, `CounterAllOtherSpells`) **and Turtle Power! (TMC, Heroes in a Half Shell) as seat 180** (artifact-mana provenance — Coin of Mastery; `DoubleControllerDrawTriggers` — Krang; `CopiesAttackEachOtherOpponent` — Shredder; Vigor's damage-to-counters; the cap diagnostic now names a looping ability's effect and costs, which found Trazyn + Commander's Sphere + Out of the Tombs independently of `018BnccM`'s fix) **and Counter Blitz (FIC, Tidus) as seat 165 and The Fantastic Four (MSC, Invisible Woman — identity from rules text, CR 903.4) as seat 171 and Avengers Assemble (MSC, Captain America — Jocasta's commander-attack return) as seat 174** (`MustAttackPlayerFor`, `HighestLifeOpponent`, `OwnersGainControlOf`; ⚠ **counters now record who put them** — `CounterAdded.placer`, `EventScope::YouPutCounters`, CR 122.6: every "whenever you put" card fired on anyone's counters; ⚠ a reflexive trigger targeted without its source, CR 603.7d; ⚠ an ETB "up to X targets" read X as 0; `BecomeCopyKeepingName`, `RemoveAllCountersFromAnyNumber`). **Session `01RyGJHK` landed Mystic Intellect (C19, Sevinne) as seat 153 and Elven Empire (KHC, Lathril) as seat 156 and Food and Fellowship (LTC, Frodo + Sam) as seat 162** (`Effect::MayPayXTimes`, Numa's {X}{X}; ⚠ **as-enters asks inside a mass reanimation leaked their answer** — `resolve_as_enters_driven`, CR 614.12) (CR 724.2 `Effect::EndTheCombatPhase`; `R::OnTopOfLibrary`; bot: graveyard flashback/aftermath copies of its own spell, a Refuse punish response, idle-main retrieval — `server/spell_response.rs`; ⚠ Increasing Vengeance copied any spell). **Session `016DJoKA` landed Coven Counters (MIC, Leinore) as seat 120, Cavalry Charge (MOC, Sidar Jabari) as seat 121, Eternal Bargain (C13, Oloro) as seat 126 and Power Hungry (C13, Prossh) as seat 130 and Living Energy (DRC, Saheeli, Radiant Creator) as seat 138 and Bedecked Brokers (NCC, Perrie) as seat 146 and Merciless Rage (C19, Anje) as seat 152 and Limit Break (FIC, Cloud) as seat 157 and Revival Trance (FIC, Terra) as seat 160 and Wakanda Forever (MSC, T'Challa) as seat 167** (⚠ a spell was never "a source an opponent controls" to a damage reduction, CR 615.10; gated attack restrictions honored, CR 508.1c; `game/effects/revival.rs` — random modes, opponent-chosen reanimation, free-cast riders and owner tolls; blitz from the graveyard; `Predicate::SpellWasCastFromExile` for foretold riders, `Effect::DestroyOnePerOpponent`; ⚠ a targeted madness spell was never cast — the madness cast passed no target, CR 702.35b; "madness cost paid" and madness {X}; Aeon Engine's reversed turn order; CR 122.1b kinds of counters, keyword counters included — `game/effects/counter_kinds.rs`; ⚠ a copied removal spell's copies all aimed at the original's target — CR 707.10c, each now defaults past the others; CR 118.9 energy as an alternative cost, Stridehangar's artifact-token rider, doubling a player's counters; ⚠ the bot looped Whirler Virtuoso under Decoction Module + Panharmonicon to 980 Thopters — `board_is_saturated` stops a token sink once the board is overkill; ⚠ Eminence "whenever you attack" now fires from the command zone, CR 113.6b; ⚠ cards drawn "this turn" reset for every player; the bot's any-turn graveyard-grant gate; `Effect::ExchangePower` CR 701.10g, `EachPlayerTakesCreatureOfNext` — Order of Succession's pass around the table; ⚠ the bot looped Tooth and Claw under Primal Vigor 3,936 times — `renews_its_own_fodder`) **Session `018BnccM` landed Blame Game (MKC, Nelly Borca) as seat 93, Temur Roar (TDC, Eshki) as seat 97, Adaptive Enchantment (C18, Estrid) as seat 102 and Aura of Courage (AFC, Galea) as seat 106 Ruthless Regiment (C20, Jirina) as seat 111 and Most Wanted (OTC, Olivia) as seat 116 Tricky Terrain (M3C, Omo) as seat 122 Everyone's Invited! (SLD, Morophon) as seat 131 and Planar Portal (AFC, Prosper) as seat 141 and Dance of the Elements (ECC, Ashling) as seat 148 and Veloci-Ramp-Tor (LCC, Pantlaza) as seat 155 and Multiverse Reforged (FRC, Jace) as seat 161 and Hail, Caesar (PIP, Caesar) as seat 170 and Doom Prevails (MSC, Doctor Doom) as seat 176 and The Ruinous Powers (40K, Abaddon) as seat 179 and Masters of Evil (WHO, Davros) as seat 181 and Timey-Wimey (WHO, The Tenth Doctor + Rose Tyler) as seat 183** (granted evoke `GrantEvokeToSpells`, CR 702.74; `Predicate::CastForetold`, CR 702.143; Wayta's creature-damage trigger doubler, CR 603.2d; ⚠ `R::ExiledWithSource` never read an until-leaves exile; planeswalker tokens and `Effect::EmpowerJace`, a per-subtype planeswalker attack ban; squad's non-mana cost, a most-votes council, granted escape; `EventKind::Connived`, CR 701.50; Khârn's damage-to-control swap, Seeker of Slaanesh's forced attack (CR 508.1d), Lucius's leaves-the-battlefield return; face-down 2/2 Cybermen (CR 708.2), The Valeyard's doubled villainous choices, Ashad's artifact casualty, Don't Blink; ⚠ the bot's SAC_EXTRA cast gate missed Ashad's grant; suspend with mana-value counters, As Foretold's {0} cast, Killer, Face of Boe; ⚠ a printed-suspend card that gained suspend ticked twice an upkeep; ⚠ every linked phase-out put time counters on its source; ⚠ a departed seat's suspended ask outlived it (CR 800.4a); ⚠ the bot sacrificed to draw from an empty library (Trazyn + Out of the Tombs, 5,736 activations); ⚠ a resumed trigger that fizzled leaked its stashed answer, and a `Reflexive` body's runtime target is lost across a prompting seat's resume — keep an asked-of-an-opponent target on the trigger itself) (⚠ `EntersAsCopy.legendary` was never read; "until your next turn" copies reverted at this turn's cleanup; ⚠ a turn-granted trigger's "you control" filter read every seat's creatures as yours — Predators' Hour, Arm with Aether; `SacrificeAndRemember` skipped the resolution's sacrifice bookkeeping) (⚠ a pod seat's as-enters creature-type naming always named Demon — off-stack asks went to the headless decider; a dying token never matched `TriggerObjectIsChosenType`). **Session `01SSG31v` landed Upgrades Unleashed (NEC, Chishiro) as seat 109 Mardu Surge (TDC, Zurgo) as seat 113 and Miracle Worker (DSC, Aminatou, Veil Piercer) as seat 118 and Timeless Wisdom (C20, Gavi) as seat 128** (⚠ Glimmerpoint Stag's blink returned the card to its controller with a counter; the pod bot copied Extravagant Replication tokens into a board cap) (⚠ Mobilize now sacrifices at the next end step, CR 702.181a — it was end of combat; the bot never picks an X for Gix's "Discard X cards:") (the printed deck ships two Mossfire Valleys — CR 903.5b; the seat runs a thirteenth Mountain). Its finds: ⚠ **death snapshots dropped static/attachment P/T** (CR 603.10 — `GameState::lki_clone`, every `died_card_snapshots` site); `R::IsModified` looks back at `auras_at_death`; `ApplyToTargets` read hostile to the auto-targeter, so no "up to N target creatures you control" friendly slot was ever filled (Silkguard uncast in 1,800 pods); the bot played a karoo as its only land every turn (a no-progress draw). **This run: Entropic Uprising (C16, Yidris) LANDED as seat 51, Endless Punishment (DSC, Valgavoth) LANDED as seat 59, Arcane Wizardry (C17, Inalla) LANDED as seat 66, Urza's Iron Alliance (BRC, Urza) LANDED as seat 69, Exquisite Invention (C18, Saheeli) LANDED as seat 76, Mishra's Burnished Banner (BRC, Mishra) LANDED as seat 83, Tinker Time (MOC, Gimbal) LANDED as seat 84, Legends' Legacy (DMC, Dihada) LANDED as seat 85, Planeswalker Party (CMM, Commodore Guff) LANDED as seat 91, Reign of Dragons (FDC, Lathliss) LANDED as seat 95, Eldrazi Incursion (M3C, Ulalek) LANDED as seat 100, Eldrazi Unbound (CMM, Zhulodok) LANDED as seat 108, Subjective Reality (C18, Aminatou) LANDED as seat 114, Spirit Squadron (VOC, Millicent) LANDED as seat 124, Jump Scare! (DSC, Zimone) LANDED as seat 133, Obscura Operation (NCC, Kamiz) LANDED as seat 143, Elven Council (LTC, Galadriel) LANDED as seat 150, Ahoy Mateys (LCC, Admiral Brass) LANDED as seat 158, Scions & Spellcraft (FIC, Y'shtola) LANDED as seat 163, From Cute to Brute (SLD, Esika) LANDED as seat 168 (⚠ CR 712.2 — a transforming DFC's back face was castable from hand for free) — both past `MAX_SEATS` (64), so `--pod-decks` only** (also Fae Dominion 45 and Invent Superiority 48 earlier). **Session `01XnuL2a` landed Open Hostility (C16, Saskia) as seat 64 and Phantom Premonition (KHC, Ranar) as seat 70 Squirreled Away (BLC, Hazel) as seat 75, Evasive Maneuvers (C13, Derevi) as seat 81 and Draconic Destruction (SCD, Atarka) as seat 87 Family Matters (BLC, Zinnia) as seat 94 Virtue and Valor (WOC, Ellivere) as seat 96 — Auras put onto the battlefield attached, CR 303.4f — Divine Convocation (MOC, Kasla) as seat 101 (⚠ convoke taps used to fire no "becomes tapped" trigger, CR 702.51) Symbiotic Swarm (C20, Kathril) as seat 103 (⚠ an up-to-N graveyard slot named one card twice, CR 115.3) and Painbow (DMC, Jared) as seat 110 (⚠ cast triggers read X as 0; multicolored anthems dropped) Arcane Maelstrom (C20, Kalamax) as seat 117 (⚠ the bot never cast a pure retarget spell — Deflecting Swat), Enhanced Evolution (C20, Otrimi) as seat 119, Riveteers Rampage (NCC, Henzie — granted blitz; Weathered Sentinels' per-defender Defender waiver) as seat 125, Grand Larceny (OTC, Gonti — `game/effects/theft.rs`; spells-you-don't-own mana) as seat 129 and Party Time (CLB, Nalia — ⚠ a once-each-turn trigger was spent by an event that failed its condition) as seat 136, Faceless Menace (C19, Kadena — CR 702.37 first-face-down discount, Rayami's any-player death exile) as seat 142** (the bot now casts a power-doubling trick, Unleash Fury; ⚠ a flickered creature no longer re-reads as kicked, CR 400.7) (seats 42, 46, 49, 53, 55 earlier); ⚠ **bots never foretold** (no bot path foretold or cast a foretold card — now both), and CR 702.143a: foretelling is legal any time you have priority on your turn. **Session `01546QdT` landed Seize Control (38), Rebellion Rising (41), Feline Ferocity (52), Primal Genesis (56) and Breed Lethality (C16, Atraxa) as seat 61** (its census found the bot looping two Enduring Scalelords — fixed, CR 732.2) **and Enduring Enchantments (CMM, Anikthea) as seat 68** (Demon of Fate's Design's pay-life cast, Narci's Saga final-chapter rider; the bot now casts through board-granted alt costs — Kentaro and Fist of Suns were invisible to it too) **, Exit from Exile (CLB, Faldorn) as seat 72** (foretold / adventure / plotted casts and lands played from exile now count as "from exile") **, Death Toll (DSC, Winter) as seat 77** (batch card-type counts, a sacrifice-to-cast-from-top static) **, Mind Seize (C13, Jeleva) as seat 89** (protection from a chosen player, kicked split second) **, 20 Ways to Win (SLD, Go-Shintai) as seat 92** (alternate wins; Tragic Arrogance's caster now chooses) **, Cabaretti Cacophony (NCC, Kitt Kanto) as seat 98** (`Effect::EachOpponentChooses`) **and Draconic Rage (AFC, Vrondiss) as seat 104** (dice: ignore-lowest, two results assigned, each player rolls; ⚠ attaching to nothing detached the Aura, CR 701.3b) **and Draconic Dissent (CLB, Firkraag) as seat 107** (board-wide goad statics read by `goaders`; goaded creatures that can't block) **and Prismari Performance (C21, Zaffai) as seat 112** (Pyromancer's Goggles' copying mana rider; ⚠ an emblem's graveyard flashback grant was never read, CR 114.4; Radiant Performer's entry trigger had no spell target). **Session `011A87o2` landed Devour for Power (CMD, The Mimeoplasm) as seat 58, Mirror Mastery (CMD, Riku) as seat 62 First Flight (SCD, Isperia — Gideon Jura's CR 508.1d lure) as seat 65 Undead Unleashed (MIC, Wilhelt) as seat 73 and Nature's Vengeance (C18, Lord Windgrace) as seat 80. ⚠ The field is past `MAX_SEATS` (64, the `u64` seat masks): `--seats` stops there, later lists need `--pod-decks`.** **Session `01P5KQkY` landed seats 32-34, 36-37, 40, 44, 47, 50, 54, 57, 60, 63 (Raining Cats and Dogs, SLD, Rin and Seri), 67 (Growing Threat, MOC, Brimaz — planar die as an effect, `game/effects/planar.rs`), 74 (Call for Backup, MOC, Bright-Palm — CR 700.2 per-mode target checks, LookPick's X) 79 (World Shaper, EOC, Hearthhull — CR 611.3a attacked-this-turn statics) 82 (Peace Offering, BLC, Ms. Bumbleflower — CR 603.3d cast-trigger target slots, emblem free casting, named-token replacement) and 88 (Nature of the Beast, C13, Marath — Mystic Barrier's CR 508.1a direction lock; the bot spends remove-X-counters abilities)**; its bot finds: pod seats cycle, choose-two/three spells are cast, attacking-or-blocking instants ride the combat window, **Incubators are transformed** (`server/transform_sink.rs` — Brimaz 1.7 → 12 %). **Session `015cKV6u` landed Hatsune Miku (SLD, Trostani) as seat 71** (`--pod-decks` only): ⚠ the bot stacked its own Aetherflux shots forever (CR 117.3c — fixed, `stacks_onto_own_activation`); `ColorCountOf` missed the stack. ⚠ **Stalls, fixed:** one token batch mints at most `MAX_BATTLEFIELD + 1` (a bot probe of Storm Herd at 15,197 life minted 15,197 — 22 s a decision); a token-making mana sink waits for the second main phase (Emmara tapped 900 Saprolings for Selesnya Guildmage instead of attacking). **Fixed later:** Storm Herd cast at 1,000+ life board-capped 2 / 4,000 — the bot now holds a token spell that would carry the board past `MAX_BATTLEFIELD` (`cast_candidates`' board-bound gate). **It also landed Witherbloom Witchcraft (C21, Willowdusk) as seat 78**: cast creatures never reached `creatures_entered_this_turn` (fixed in the central `PermanentEntered` funnel); `PowerOf`/`ToughnessOf` missed library and stack; `CastSpellMatches` answered source atoms false. And **Witherbloom Pestilence (SOC, Dina) as seat 86**: the bot now casts a sacrifice-X spell (Immoral Bargain) and no prompting seat is asked for a target past an X cap. And **Silverquill Statement (C21, Breena) as seat 90**: CR 508.1's "whenever a player attacks one of your opponents" (`EventScope::OpponentOfYoursAttacked`); the pinger skips indestructible creatures and goes face with a paid repeat shot. And **Draconic Domination (C17, The Ur-Dragon) as seat 99**: `EntityMatches` on a player now reads the requirement; a became-target trigger's "that player" is its actor; Niv-Mizzet, Dracogenius drew on damage taken. **Silverquill Influence (SOC, Killian) as seat 105**: `R::EnchantedByYourAura`; `RevealTopMayCastOneFree` takes a card filter. **Prismari Artistry (SOC, Rootha) as seat 115**: ⚠ CR 202.3e — a cast trigger read an X spell's mana value without X; ⚠ `R::HasName` wasn't card-only (name-scoped statics were dropped); board-bound gates for cast-trigger fan-out (Leitmotif Composer), attack triggers (Redoubled Stormsinger × Harmonic Prodigy) and free copies (Surge to Victory); the bot declines Orzhov Advokist's peace offer from the last opponent standing. Then more board-bound gates (8-seat seeds 11100/11101 1,000/1,000): suspend free casts, Mirrorwing Dragon's copies capped at the room, stack-pending tokens counted, all stopping at `recommend::BOARD_GATE` (an eighth short of `MAX_BATTLEFIELD`). (Its Miracle Worker build was dropped: `01SSG31v` landed the deck first as seat 118.) **Desert Bloom (OTC, Yuma) as seat 123** — Its Scute Swarm pod caps are fixed later: a self-copying token mints nothing past `BOARD_GATE` (pods 1,000/1,000; 6-seat wall +50 %). **Lorehold Spirit (SOC, Quintorius) as seat 127**: `ExileFromGraveyard` now records what it exiled this resolution. **Paradox Power (WHO, The Thirteenth Doctor + Yasmin Khan) as seat 178** (session `01TfrB1i`): paradox counts, draw-from-bottom (River Song), a turn-gated graveyard flashback grant (Return the Past), a permanent exiled foretold (CR 702.143), a granted demonstrate (CR 702.150); the Saga ratchet reads chapter VI (The Flux). **Tyranid Swarm (40K, The Swarmlord) as seat 175** (session `01TfrB1i`): Biophagus's creature-cast counter rider (CR 106.6a), a per-creature own-counter pump (Clamavus), a remove-counters discount cost (Hierophant Bio-Titan); ⚠ a damage trigger's intervening-if read the damage amount as 0 (Ghyrson Starn). **Scrappy Survivors (PIP, Dogmeat) as seat 173** (session `01TfrB1i`): CR 603.10a what rode a dying creature (Gunner Conscript, Cass — its Aura cards return attached and its Equipment moves), Aura copies entering attached (Three Dog, CR 303.4f), a host-targeted equip / Aura discount (Strong Back), Aura-or-Equipment-only mana (Codsworth); ⚠ the bot took every optional token on an overkill board (980 Elf Warriors off Flourishing Defenses — declined now). **Science! (PIP, Dr. Madison Li) as seat 169** (session `01TfrB1i`): a counter-capped graveyard cast (Arcade Gannon), any owner's brain-countered abilities (Rex), `Selector::ExiledForCost` (Curie — ⚠ an "exile another" cost could pick its own source), a secret-number match, a per-artifact attack tax; Madison wins only 2.3 % of six-seat pods (an open lead). **Mutant Menace (PIP, The Wise Mothman) as seat 166** (session `01TfrB1i`): CR 728 healing radiation (Strong), CR 615 damage traded for rad counters (Bloatfly Swarm), a per-cast radiation mark (Nuka-Nuke Launcher), `EventSpec::batch_counts_subjects`, a card returning as its Aura back face (Harold and Bob). **Blight Curse (ECC, Auntie Ool) as seat 164** (session `01TfrB1i`): blight as an optional cost (CR 701.68), a clash that repeats while won, a player's life / toughness exchange; ⚠ the bot put a forced -1/-1 counter on its own 1/1 token (a Blowfly + Hapatra loop, 1,857 turns); ⚠ Hapatra + Marauding Raptor under Everlasting Torment is a genuine mandatory loop (CR 104.4b) the engine only ends at the action cap. **Revenant Recon (MKC, Mirko) as seat 159** (session `01TfrB1i`): ⚠ a surveiled card was never "put into a graveyard" — it now feeds the from-library family (Narcomoeba), and the literal "mills" wording is `EventKind::Milled` (CR 701.13); `Scried` / `Surveilled` split; CR 500.8 extra beginning phase; Lazav's keep-its-name copy; the bot casts a counterspell under `WithX` (Counterpoint). **Buckle Up (NEC, Kotori) as seat 151** (session `01TfrB1i`): ⚠ no bot attacked with an animated permanent (crewed Vehicle, manland, stationed Spacecraft — Kotori 5 → 25 %); granted crew counts; a copy can be a Vehicle. **Explorers of the Deep (LCC, Hakbal) as seat 145** (session `01TfrB1i`): explore-twice replacement, filtered retrace grants, a targeted-ability tax (Kopala). **Counter Intelligence (EOC, Inspirit) as seat 139** (session `01TfrB1i`): ⚠ CR 613.8 — a card-type-filtered set now reads layer-4 type changes (an anthem reached no animated land); `SpellGainsSunburst`, `PutCountersOf`; ⚠ no bot ever stationed a Spacecraft (CR 721 — fixed: the self-counter sink stations after combat); Inspirit still wins 3-8 %. **Deep Clue Sea (MKC, Morska) as seat 134** (session `01TfrB1i`): ⚠ permanent ascend is a static checked on every entry / control change (`game/ascend.rs`, CR 702.131b — six cards re-checked it at upkeep); `Effect::Ungoad`, a silence until your next turn, Tezzeret's first-artifact discount; Thirst for Knowledge kept its artifact discard. **Dungeons of Death (AFC, Sefris) as seat 132**: `PlayerRef::PlayerToYourRight`, doubled dungeon room abilities (Hama Pashar), Rod of Absorption's exile-instead static and total-mana-value free casts. **Deadly Disguise (MKC, Kaust) as seat 135**: `R::TurnedFaceUpThisTurn`, return face down, the next-face-down discount, turned-face-up trigger doubling, `turned_face_up_counters`. **Quandrix Unlimited (SOC, Zimone) as seat 137**: filtered counter adders, a per-counter first-X-spell discount, next-matching-spell triggers, X doubling, all-spell flash; ⚠ `HasXInCost` inside `CastSpellMatches` is false on the stack (CR 202.3e). **Maestros Massacre (NCC, Anhelo) as seat 140**: granted casualty (`casualty_for`), Audacious Swap's shuffle-and-replace. **Creative Energy (M3C, Satya) as seat 144**: energy doubling, `Value::EnergyOf` / `EnergySpentThisTurn`, `R::PowerExactlyXFromCost`, a kept tapped-attacking copy. ⚠ Fixed: `BecomeCopyOf` kept-ability accumulation (an OOM), the bot's proactive energy self-blink loop. **Riders of Rohan (LTC, Éowyn) as seat 147**: composed from existing parts (monarch, sagas, dash, keyword counters). **Abzan Armor (TDC, Felothar) as seat 149**: keep-total-power sweep, life/toughness exchange; ⚠ a granted `AttacksAsThoughNoDefender` was ignored at declaration (CR 508.1a). **The Hosts of Mordor (LTC, Sauron) as seat 154**: `R::IsRingBearer`; `MatchingAmong` now reads a card filter's X (CR 107.3). ⚠ Found in the suite: `blocker_matches_block_filter` answered `HasSupertype` false (Midnight Pathlighter's attackers were unblockable), and a target inside a dice branch is never surfaced at cast (Grave Endeavor).
5. **No open claims: every official precon is a seat** (`precon_scan.py --top` lists none missing; Timey-Wimey is `018BnccM`'s seat 183). The work now is the ~560 residual rows (INCOMPLETE_CARDS' Commander table — batch them by the primitive they share) and bot quality. Claim a residual *batch* in this line, pushed on its own, if it will take a run. **Claimed (session `015BCEt5`, 2026-09-26):** the player-choice batch — "choose a player / an opponent", "an opponent of your choice", "target player" read as you, and "that player" per-attacked-player targets (`player_ballot`, `YouAttackedPlayer`).
6. 🔎 **This run's engine finds** (ENGINE_BACKLOG): **Session `01VVD5mW`:** ⚠ vanishing's sacrifice is a last-counter trigger (CR 702.63a — `game/vanishing.rs`; vanishing tokens and copies entered with no counters); ⚠ Idris had no abilities (the imprint link vs. `exiled_by`); ⚠ a granted Changeling never met a type lord; ⚠ "until your next turn" coded as this turn (Jace, Gideon, Arlinn — an audit lists 16 more candidates, see ENGINE_BACKLOG); cast-only exile permissions; ⚠ Dancer's Chakrams' commander pump never applied (`SourceIsEquipped` read the Equipment as the equipped one); a per-creature lure, a per-source damage tally, `BecomeLegendary`, `MayPayXOfColor`; 40 residual rows closed. **Session `018BnccM` (strict random-pod sweeps, 120 combos):** ⚠ **three spell-cost reductions escaped the bot's cost-static filter** (Zimone, Infinite Analyst; Advanced Reconstruction; Gonti, Canny Acquisitor — the affordability read skipped them; a structural audit now reads the walks' source); ⚠ **Cairn Wanderer read dead creatures' last-known keywords** (CR 400.7 — and the death snapshots' clearing moved a count the gather memo does not key on); ⚠ a departed seat's suspended ask outlived it (CR 800.4a); the bot sacrificed to draw from an empty library (Trazyn + Out of the Tombs); a `pay_own_cost` free cast whose life payment knocked its controller out of the pod (CR 800.4a) errored after mutating (Emet-Selch, seed 25048 — the zone is re-checked after paying). 💡 Lead: effects that run state-based actions *inside* a resolution (Sacrifice, AddPoison) let a sacrificed commander go home (CR 903.9a) before the same spell's later step reads it — Danse Macabre can't reanimate a commander (CR 704.3 says SBAs wait for the spell); the resumed arm now drops its moot answer (seed 24058). **Session `01HFhNih`:** ⚠ **unearth never exiled a creature that left the battlefield** (CR 702.84a — the shortcut had only the end-step exile; now `ExileIfLeavesBattlefield`); ⚠ **Defile and five other per-count shrinks were never cast** (classed as combat tricks, which only pump your own creature — now removal); ⚠ **a draw-replacement ask suspended mid-draw and decked its seat** (Out of the Tombs; the CR 800.4 audit caught it; the pick is automatic now). 💡 Lead: constant shrinks (Disfigure) are still combat tricks — census whether they are ever cast before reclassing (it moves 2-player traces). **Session `01RyGJHK`:** ⚠ **an as-enters ask inside a mass reanimation leaked its answer** (Feasting Hobbit under Living Death devoured nothing — `resolve_as_enters_driven` answers every as-enters ask in place, CR 614.12); ⚠ **Ultimate Magic: Meteor's and Rejoin the Fight's per-opponent loops asked on past a suspend** (7,741 answers in one action-capped 8-seat pod — they return on the suspend now, and `scripts/audit_stash_in_loop.py` flags the shape: run it after adding a per-seat ask loop); ⚠ **the bot re-fought an indestructible Zetalpa with Apex Altisaur 7,538 times** (an optional fight now takes only a creature it kills — `server/fight_pick.rs`); CR 724.2 end the combat phase (Mandate of Peace); ⚠ `GameState` is size-capped at 1,680 bytes (`cow::tests::game_state_stays_small`) — a new flag goes in an existing byte (`end_requested`) or a cold group; the precon pod test runs a worker per core (630 → 171 s). **Session `016DJoKA`:** ⚠ **a per-seat pick that suspends must `return`** — Meteor and Rejoin the Fight treated `ask_seat_target_logged`'s `None` as "no pick" and re-asked forever (7,655 answers, an action cap; fixed upstream in `70e5dc676` / `60749500f`); ⚠ **no targeted madness spell was ever cast** (the madness cast passed no target — CR 702.35b); ⚠ **a copied removal spell's copies all aimed at the original's target** (each now defaults past the others, CR 707.10c); the bot's overkill-board guard stops a self-refilling token sink (Whirler Virtuoso under Decoction Module: 980 Thopters); a new `GameState` flag broke its 1,680-byte cap — cold flags go in `ColdState`. **⚠ the bot shuffled Belt of Giant Strength between two 11/11s 11,365 times** (a base-P/T setter lowers a big host, so "equip the biggest" flipped every tick; an action-capped 12-seat pod) — `pick_equip` now needs a strictly stronger new host. **Session `01XnuL2a`:** convoke taps now fire "becomes tapped" (CR 702.51); a flickered creature no longer reads as kicked (CR 400.7); an up-to-N graveyard slot names a card once (CR 115.3); cast triggers read the spell's X; multicolored anthems route to the layers; the bot's mana-value gate sees next-spell affinity / turn discounts; Unleash Fury-style self-power pumps are combat tricks. The bot answers a spell aimed at its side with a held retarget instant (Deflecting Swat, Redirect — `pick_retarget_shield`). ⚠ **A once-each-turn trigger was spent by an event failing its condition** (CR 603.3d — marked at gather, before the filter; Deep Gnome Terramancer, Folk Hero), and a static-granted trigger could not be once-each-turn at all. `R::DamagedAPlayerThisTurn` reads a dying creature's snapshot (Wave of Rats). **⚠ in pods a bot never cast an "up to N targets" spell it under-filled** — every pod seat is `wants_ui`, the cast suspended into the extra-slot prompt and `would_accept` read that as a rejection (Desecrate Reality 0 for 1,000); the bot now fills extra slots with `auto_extra_targets_for` on prompting seats, and the prompt honors `ForEachOpponentTarget`. `ManaSymbol::ColorlessHybrid` ({C/W}) exists now. a resolving permanent spell and `fire_self_etb_triggers` gather self-ETBs in two walkers — a per-instance rider belongs in `statics_granted_triggers_for`, which both read (Galea); **CR 903.9: a commander going home now emits `CommanderPutIntoCommandZone`** (both the SBA and the replacement route; Myth Unbound); two Scalelord Reckoners stacked 3,905 triggers retargeting each other's Dragon — the bot's hostile pick now skips a permanent its own stack items already aim at; **a battlefield cast trigger picked its target before the cast spell's mana value was in scope** (Skyfire Kirin, Hammerhead Tyrant — `ManaValueEqualsTriggerAmount` never matched at targeting); **goad has one reader, `GameState::goaders`** (`game/goad.rs`: resolved + held CR 611.2b + attached `AttachedIsGoaded`) — read goad through it, never `goaded_by`; five Auras/Equipment that re-goaded on triggers are statics now. Boros Reckoner's bare `Target(0)` on a `DealtDamage` trigger bound itself and looped forever under indestructible; the bot's `ChooseTarget` fallback hit its own board before an opponent's face. a mana source paying its own {1} overflowed the stack (CR 601.2g, found by a 1,000-game pod — `pod::tests` replays it); SpellCast triggers never named their caster (CR 603.2 — "another target player" hit the caster or nobody); no bot path suspended a no-cost card (Wheel of Fate), and the census didn't count suspend/foretell as plays. ⚠ **Chatterfang's token rider re-applied on every resume of a suspended resolution** (CR 614.13 — the board doubled per answer and a bot lookahead hung a pod). ⚠ **Postcombat-main triggers never fired in real games** (CR 505.1a — `advance_step` skipped `StepBegins(PostCombatMain)`; Survival, Florian, Tymna were dead, their tests fired the step by hand). Arcane Wizardry: **The Abyss destroyed every nonartifact creature** (it is one, of their choice); "tap N untapped [X] you control" never let the source pay; **both death funnels filled only a trigger's first target slot**; and the extra-slot filler only ever offered the *first* opponent — a two-seat assumption. Urza's Iron Alliance: an any-kind "remove a counter" cost never saw **keyword counters** (CR 122.1b), and a `WhileCondition`-gated "prevent all damage to this" never applied.
7. 💡 **Open queue:** **Y'shtola (163) wins 56 % of six-seat pods** (17302) while Dogmeat (173) wins 1.6 % and Madison (169) 2.3 % — the next bot lead is what Y'shtola's drain does that the Aura/Equipment and energy decks' attacks don't (census the six-seat pod, compare damage sources). (The seed-9405 8-seat caps decide as of `01VVD5mW`: a 2,200-game 8-seat sweep, seed 9501, seats 11-183, had none, and Observed Stasis is cast.) ⚠ **A Scute Swarm board below the gate is slow, not capped** (measured as `--pod-decks 128,127,126,125,124,123 --seed 11107 --first 259` with Sefris at 128, now 132: 814 Swarms, 8,171 no-op landfall triggers each cycling six seats of priority — 104 s for one decided game); ✅ **the Scute Swarm (seed 10521) and Extravagant Replication (seed 10511) board caps no longer reproduce** (1,000 / 1,000 decided on both seeds, session `011z4qQX`); ✅ **census over all 182 seats (six-seat batches, 300 games, seeds 21001+) decided every game but one CR 104.4b draw** after four fixes (session `011z4qQX`): Fireball cast single-target with the declined extra slot read as acceptance; token makers gated at 150 permanents (Whirler Thopters under Legion Loyalty, seed 21139); a Pariah'd player ranked last as a damage target; ⚠ **the loop digest reads damage clamped to printed toughness** (a Pariah'd Brash Taunter's mandatory loop climbed its damage forever, seed 21091 — now a draw). An **8-seat all-seat census** (groups of eight, seeds 23001+, 300 games each, 7,200 games) decided all but one board cap: `--pod-decks 81,82,83,84,85,86,87,88 --seed 23081 --first 50` — Woe Strider sacrifices (229) under Mazirek counter up Mycoloth, whose mandatory upkeep trigger mints a Saproling per counter (722 Saprolings + 225 Pests); the next lead is a sacrifice-for-value gate that prices a Mycoloth-style counter-to-token engine, or clutter measured before the upkeep burst. ✅ The 8-seat `--pod-decks 122,121,120,119,118,117,116,115 --seed 9405 --first 379` cap was **Dualcaster Mage + Replication Technique** (991 Mages; the copy's "choose new targets" is answered inline by the engine's `AutoDecider`, never by the bot): past 150 permanents a self-aimed copy now defaults off its target (`repoint_copy_slot`), 500/500. The two other old repros (seeds 10411, 10420) now decide; 10423 / 9405 #368 end in no-progress draws. Klauth's Will now cast (an "up to X" slot behind If / ChooseN was read as mandatory). A 600-game all-seat census (seeds 22001+, 18,300 six-seat games) decided every game but one no-progress draw (`--pod-decks 31..36 --seed 22031 --first 16`), and Clever Concealment was cast there; the only card never cast is Sudden Substitution (Kadena — needs an opposing noncreature spell on the stack and a bot that steals spells); ✅ **a sacrifice outlet fed by triggers is gated at 150 permanents** (Woe Strider + Eloise + Chatterfang, 968 Clues, `--pod-decks 73,74,75,76,77,78 --seed 20073 --first 184` — `renewal_guard::board_is_cluttered`); X burn at "any target" auto-targets the opponent's face first and the bot burns face only for lethal, so Fireball goes uncast (Nature of the Beast census); Suspend X (Aeon Chronicler); a random-opponent damage split to planeswalkers (Vial Smasher); life-lost counting for "each player who lost life" (Blood Tyrant); "up to one target" choosing none (Star Athlete, Key to the City); `Effect::ExileFromHand` for a prompting seat; bots never emit 6 cast variants; per-slot trick evaluation, `audit_dropped_may`, the invented-target census. Condemn (Nahiri) was unplayed at 31-32 seats. A Chishiro / Nelly two-seat endgame drew on no progress (`--pod-decks 116,93,106,109 --seed 10411 --first 37`; Chishiro answered 447 decisions). Again beside Otrimi (`--pod-decks 122,109,113,119 --seed 10423 --first 442`, 432 decisions). A Scute Swarm board still reaches the board cap through lands put onto the battlefield by effects (landfall fires on any land entry; the bot's gate covers only its hand land drop — `--pod-decks 122,116,111,106 --seed 10420 --first 422`).
...
9. **The rules table is DONE** (FEATURE_ROADMAP's Commander and 800-series sections, bar CR 800.4i and the N-seat encoder). What is left is the residuals the precons surfaced. `CR_COVERAGE.md` is generated: `scripts/cr_coverage.py`.

### Commander NEXT items 3-5, replaced 2026-09-26 by session `01XSSbzm` (verbatim)

3. **Gates at this run's tip (`fb937aec5` + trackers):** suite **22,927 / 0 / 7** (strict); clippy 0; `--bench` **200,190 byte-identical** at that tip (line 10: the invariant has since moved); cube / sos / sealed 300 games each decided.
4. **Pod smoke this run, every seat, fresh seeds:** release census 8-seat (seeds 31001+, 6,900 games), 6-seat (32001+, 9,300), 4-seat (33001+, 13,800), 5-seat (37001+, 11,100: two CR 104.4a simultaneous-loss draws) and 7-seat (38001+, 8,100) — **all decided after the fixes, zero panics**; a second round at the later tip (4/6/8 seats, seeds 54001+/56001+/58001+, 30,000 games) found one loop (below) and one CR 104.4a draw; a third round at the latest tip (5/7 seats, seeds 75001+/77001+, 19,200 games) found only two CR 104.4a all-seats-dead draws, `--bench` 197,136 unchanged, workspace clippy 0; strict debug census 4/6/8 seats (35001+ / 34001+ / 36001+, 1,104 / 744 / 368 games) all decided. `census.sh`-style loop: `--pod-decks` over groups of N, `--games 300`, one seed per group; a non-zero exit is a crash — bisect with `--first/--games`, then `gdb -batch -ex run -ex bt`.
5. **This run's finds (all fixed, tests cite the rule):** ⚠ **a stack overflow** — two Scalelord Reckoners' target-trigger chain recursed in `push_pending_trigger` (CR 603.3b; now queued past depth 8); ⚠ **a mana-pool u32 overflow** — two Leyline Tyrants re-added kept red once per keeper, and spent firebending red was re-minted every step (CR 702.189a); CR 704.3 — the CR 903.9a commander return ran inside resolutions and during pending decisions (Danse Macabre); the bot aimed attacks at a seat Mystic Barrier bars (947 creatures never attacked); seven "from the battlefield" graveyard triggers fired on discards (Unquenchable Fury's cleanup loop); Imposing Grandeur / Stinging Study counted a commander in any zone; Trove Warden returned on any leave; Sidar Kondo's restriction sat on your attackers, not opponents' blockers; Port Razer now can't attack a player twice a turn (`Keyword::CantAttackPlayerAttackedThisTurn`); ⚠ **Missy beside a Missy copy looped 404 times** — the second trigger "moved" the already-returned card battlefield to battlefield, killing it (CR 400.7: a Cyberman return reads only graveyard / exile / library, and — the class — `move_card_to` never moves a card already on the battlefield onto it; that was a blink); `Effect::ForEachPlayerTarget` ("for each player" includes you — five seated cards); Relic of Progenitus's "exile all graveyards"; Phyrexian Obliterator's "that source's controller".
