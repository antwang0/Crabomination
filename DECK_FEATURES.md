...
| The Ring tempts you (CR 701.54) | ✅ | `Effect::RingTempts` + `Player.{ring_temptations,ring_bearer}`; the four cumulative emblem abilities ride the level (can't-be-blocked-by-greater-power, attack-loot, blocked-creature `Effect::SacrificeAtEndOfCombat`, combat-damage drain). `EventKind::RingTempted` powers "choose a Ring-bearer" payoffs. `decks::ltr` (20 cards: Birthday Escape, Call of the Ring, Bilbo, Rohirrim Lancer, Took Reaper, Slip On the Ring, …). Bearer auto-picked (highest power); per-player UI choice is a TODO.md follow-up. |
