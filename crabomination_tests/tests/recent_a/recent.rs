...
fn vaultborn_tyrant_dies_into_a_copy() {
    ...
    assert!(copies[0].definition.card_types.contains(&crabomination::card::CardType::Artifact));
...
fn militia_rallier_attacks_with_partner_and_untaps() {
    ...
    let mut ctx = crabomination::game::effects::EffectContext::for_trigger(mr, 0, None, 0);
