...
pub fn big_score() -> CardDefinition {
    CardDefinition {
        ...
        toughness: 0,
        keywords: vec![],
        // Push XLIII: in-resolution Discard moved to `additional_discard_cost`
        // (cast-time primitive). The cast is illegal without another card
        // in hand; the body now mints 2 Treasures + draws 2.
        effect: Effect::Seq(vec![
            Effect::CreateToken {
                who: PlayerRef::You,
                count: Value::Const(2),
