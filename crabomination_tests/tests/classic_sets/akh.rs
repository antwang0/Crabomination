...
/// with the original's P/T; Eternalize mints a 4/4 instead.
#[test]
fn embalm_and_eternalize_mint_token_copies() {
    use crabomination::card::CreatureType;
    // Embalm: Sacred Cat → 1/1 Zombie Cat token copy, original exiled.
    {
    ...
    // Eternalize: Adorned Pouncer → 4/4 token copy that keeps Double Strike.
    {
        let mut g = two_player_game();
        ...
        assert!(tok.definition.keywords.contains(&crabomination::card::Keyword::DoubleStrike),
...
/// Batch: simple ETB triggers and self-pumps across the AKH pack.
#[test]
fn akh_etb_and_pump_batch() {
    ...
    // Anointer Priest gains 1 life when a creature token you control enters.
    {
        ...
        let servo = crabomination::card::TokenDefinition {
...
/// Batch: AKH removal spells (Open Fire, Gideon's Reproach, Cast Out).
#[test]
fn akh_removal_spells_batch() {
...
fn akh_embalm_bodies_have_graveyard_ability() {
    type Case = (fn() -> crabomination::card::CardDefinition, i32, i32);
    let cases: &[Case] = &[
