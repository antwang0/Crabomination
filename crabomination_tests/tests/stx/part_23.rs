...
fn cr_105_2_hybrid_pip_contributes_both_colors() {
    use crabomination::mana::hybrid;
    let c = crabomination::mana::cost(&[crabomination::mana::generic(1), hybrid(Color::White, Color::Black)]);
    let colors = c.colors();
...
fn lorehold_smite_b200_destroys_tapped_creature() {
    ...
    {
        let c: &mut crabomination::card::CardInstance = g.battlefield.iter_mut().find(|c| c.id == bear).unwrap();
        c.tapped = true;
    }
    let _ = CardId(0);
...
fn witherbloom_connectdrain_b201_drains_on_combat_damage_to_player() {
    ...
    {
        let c: &mut crabomination::card::CardInstance = g.battlefield.iter_mut().find(|c| c.id == cd).unwrap();
