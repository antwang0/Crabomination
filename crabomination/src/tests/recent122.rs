...
fn cephalid_inkmage_threshold_unblockable() {
    ...
    assert!(!g.computed_permanent(mage).unwrap().keywords.contains(&crate::card::Keyword::Unblockable),
...
fn curator_of_destinies_fact_or_fiction() {
    ...
    assert!(cp.keywords.contains(&crate::card::Keyword::Flying), "has flying");
    assert!(cp.keywords.contains(&crate::card::Keyword::CantBeCountered), "uncounterable");
