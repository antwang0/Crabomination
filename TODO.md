...
1. **FIRST:** `git fetch origin claude/modern_decks && git checkout -B claude/modern_decks origin/claude/modern_decks`. Rebase, never force; code before tracker prose; **fetch before every push AND before you start** — a concurrent session pushed three commits into the middle of the 2026-09-15 run. Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure" + "Standing rules"**, ⚠ the 2026-09-12/13 detail (printed-body skip taxonomy, the `EventKind` fan-out census, the modal census, the collision log, the seed discipline) is in **ENGINE_BACKLOG's "The 2026-09-12/13 handoff detail, moved verbatim"** — read it before taking a skip kind or a seed, and say in this line which you are taking. NEXT is 15 lines and stays 15.
2. **Gates at the tip, all re-run this run (and again after the mid-run rebase):** suite **19,549 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, nextest), clippy **0** (`--workspace --all-targets --exclude crabomination_client`, and again with `--features trig-census`), golden traces unmoved, `cargo check --profile release-fast` clean, `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant on every binary built this run, with `determinism ok` and `thread_determinism ok (3 vs 1)`. ⚠⚠ **SIXTH BOX, 4 cores, rustc 1.95.0 — no absolute in PERF is comparable across boxes; the counters and the Ir are.** ⚠ **The A/B base was re-taken here and reproduces the record to six significant figures** (616,268,574 / 1,626,125,678 / 1,713,378,250 against the filed 616,268,430 / 1,626,125,608 / 1,713,381,435). Timings: cold catalog+engine `profiling-fast` **~70 min**, warm **engine-only 2m10s-2m35s**, `target-audit/overflow` cold 10m53s / warm **2m03s**, `cargo check --profile release-fast` 20 s-2m06s, a three-pool callgrind ~3 min, the debug suite **91 s to run**. ⚠⚠ **`md5sum` is NOT the check for a "byte-identical" claim** — `profiling-fast` embeds the source hash in debuginfo; the counters and the Ir are.
3. **Perf — FIVE ROWS TAKEN. `(-317)` is the big one: `available_mana` reads its per-permanent answer off the definition, fixed -0.842 / cube -0.496 / sealed -0.540 %, the row -43.9 / -35.6 / -36.8 %.** For an untapped, non-sick permanent granted nothing, the loop's whole job is a pure function of the printed ability list, so `mana_summary_of` packs it into the word's last free bits — `AM_COLORS` (51-55), `AM_COLORLESS` (56), `AM_BEST_ONE` (57), `AM_EXACT` (58); `AM_EXACT` clear means "run the walk", so the pack can only **decline**, never mislead. **`(-318)`** put `fire_combat_damage_triggers`' opening fold behind a `writes`-keyed presence gate (fixed -0.061 / cube -0.417 / sealed -0.195 %, row -5.7 / -20.7 / -14.9 %). **`(-319)`** named that fold for the whole `attached_to` family (`GameState::attachment_in_scope`, the audit inside it) and took its one other hot site (-0.016 / -0.041 / -0.041 %). **`(-315)`** gated `grant_scan`'s one ungated pass (-0.188 / -0.113 / -0.203 %, row -36.4 / -29.5 / -47.1 %) and **`(-316)`** the CR 602.5g haste walk (-0.002 / -0.044 / -0.028 %, row halved). The branch moved **fixed -1.09 / cube -1.10 / sealed -0.99 %** between the `(-314)` tip (616,268,574 / 1,626,125,678 / 1,713,378,250) and the `(-319)` tip (609,523,889 / 1,608,296,018 / 1,696,358,147).
4. **Perf — six transferable rules, four of them new this run.** **(i) Size a presence gate by what the loop body does on a MISS, not by the loop** (`(-314)` gated an empty `Vec` probe, 0.05 %; `(-315)` gated two `Option` discriminants out of a big `CardDefinition`, thirty times that). **(ii) ~40 % of what a lane takes off a row comes back** as the lane read plus `walk_and_store`. **(iii) Ask for the WEAKEST precondition that makes a packed answer sound, and ask for it ONCE** — `(-317)` gated on `grants_nothing` twice over (-0.69 / -0.36 / -0.42 %); the walk only needs `granted.is_empty()`, worth a fifth of the win. **(iv) A mechanical file move is NOT free under codegen-units 16** — four predicates out of `bot.rs` read +0.179 / +0.097 / +0.100 % until each got `#[inline]`. **(v) A presence gate belongs in its OWN short-circuiting `any`, not folded into the loop it guards** — the fused `(-318)` taxed every miss and still paid the dear walk on an empty board (+0.018 % on `fixed`), and could only skip on a fold HIT: 28.8 / 52.9 / 50.9 % against the split form's 61.9 / 78.6 / 100.0 %. **(vi) `sba_fold`'s 10.6-18.9 % is the price of that CALL SITE, not of the write counter** — the same key reads 50-68 % under `fire_combat_damage_triggers`. Census a `writes`-keyed memo at the site that will use it.
5. ❌ **Perf — `available_mana` IS CLOSED, on all three devices now.** `e8308d53`'s census closed the two that cut the **iteration**: more `(-314)`-shape gating (capped at a third of the row, and `(-315)`/`(-316)` then took most of that third), and the `(-87)` member list (it is the board — 65-81 % of permanents satisfy any sound definition-only predicate, and **a member list is priced by how much it EXCLUDES**). `(-317)` took the third, the **body**, which the same census had measured at 58-68 % of the row. What is left is self 5.2 M / 10.7 M / 11.7 M (0.85 / 0.66 / 0.69 %) with a callee tree of `grant_scan` 3.89 M, `tap_ability_summoning_sick` 2.56 M, `grants_nothing_slow` 1.75 M on `cube` — each already behind the gate its own Log entry built. **Do not re-open it without a device that is none of the three.**
6. ❌ **Perf — THE `profiling-lines` DUMP IS TAKEN AND ITS VERDICT IS "STRUCTURAL"; DO NOT SPEND ANOTHER ON THESE THREE.** What `(-318)` leaves of `fire_combat_damage_triggers` (6.52 M / 25.92 M / 19.29 M) is **FLAT**: 112 lines / 27.3 M with a 0.22 % top row, and that row is `ptr/non_null.rs:444`, iterator machinery. `check_state_based_actions_into` reads the same way in the same dump — 309 lines / 55.7 M, 0.22 % top row — and `dispatch_triggers_for_events_slow`'s flatness was already filed. ⚠ **And the line table over-reads**: it put 0.17 % of `cube` on `(-319)`'s site against a measured 0.041 %, because a line's cost is the cost of the addresses `addr2line` maps there, not of the source construct. Cold `profiling-lines` build here is **7m08s**; the instrumented dump is ~4 min.

7. ❌ **Perf — sized and DECLINED, so nobody spends a build:** `main_phase_action_with`'s `has_magecraft` (bot.rs:7662) and `has_opus` (bot.rs:7706) are ungated whole-battlefield walks with definition-only predicates, but once *per tick* and with a `controller` compare plus a length load for a body — `is_opus_trigger::branches` is **60,168 Ir on `sealed` (0.0035 %)**, 2,210 on `cube`. 0.02-0.05 % for the pair. ⚠ **`GameState`'s size guard is 1,672 now** (`(-318)` raised it 8 bytes, with the growth already in the reading; the free alternative — stealing bits from `sba_fold`'s word under a shared stamp and a per-half computed flag — is written down beside the guard). ⚠ **And the mana word has NO free bits left**: 43-50 are `PLAIN_TAP`/`PLAIN_LAND`/`LAND_MANA_REPLACER`, 51-58 `(-317)`'s, 59-62 the marker bits, 63 the memo's — **a sixth per-definition family needs a sixth `CardMemo` word**, which is a `crabomination_base` change and a ~70-minute cold rebuild, so batch it. Only lane shifts **60 and 62** are left on the battlefield word. The `ComputedPermanent` lane stays CLOSED (`(-312)`), as do `(-309)`, `(-307)` and the **target enumerator**.
8. **Perf — the two structural rows are what is left above line 6.** `dispatch_triggers_for_events_slow` **4.65 % of `cube` / 6.15 % of `sealed`** self, line profile FLAT over 267 lines — it needs the structural half (merging `perform_action_inner`'s batches), which is a CR 603.3b ORDERING question before a perf one. `compute_permanent_pass` **3.80 % / 2.92 %**, 238 Ir of self per permanent — the next line out of it wants a `profiling-lines` dump, cold, and ⚠ `(-310)`'s rule about inlined frames.
9. **Seeds: frontier 1342 — THIS RUN TOOK 1334..1341 IN TWO BLOCKS: 24 cells / 118,400 games / 0 failures.** 1334..1337 at the `(-316)` tip (`cap 0 / board 0 / stuck 0 / draw 16`) and 1338..1341 at the CLOSING `(-317)` tip (`cap 0 / board 0 / stuck 0 / draw 0`), `target-audit/overflow` with `-C debug-assertions=yes` and `CRAB_ANSWER_LOG=strict` — so `Battlefield::lane`'s per-read audit AND `(-317)`'s `debug_assert_eq!` (which re-runs the whole walk against every packed answer) ran on every one of those boards. That is the coverage a new memo cannot get from the suite. No Scute Swarm cell in either block. All standing audits re-run and 0: `audit_panics` **0 bare** (70 sites, 59 guarded, 11 lock-poison), `audit_stash_in_loop` **0 unexplained**, `audit_seat_from_selector` **0 open**, `audit_target_walkers --check` **0**, `audit_doc_drift` **0 body-wrong / 0 doc rot / 0 stale notes**. 🆕 **`audit_engine_stack.py` is NEW and found a real one**: three production threads ran a whole match on the 2 MB default stack (the lobby's match driver, `crabomination_server`'s bot-match and pair-match), where every other binary gives 32 MB — and **a stack overflow is a `SIGSEGV`, not a panic**, so nothing prints and `panic = "abort"`'s exit 134 does not apply. Fixed, one `server::ENGINE_STACK_BYTES`, and the auditor is verified against the pre-fix tree (reports exactly those three, 0 after).
10. **Best next moves.** (a) ⚠ **The three biggest self rows on `cube` are all measured FLAT now (line 6), so the next win is not a line — it is a structure or a count.** The two named structural questions are `dispatch_triggers_for_events_slow`'s batch merge (a CR 603.3b ORDERING question first) and `compute_permanent_pass`. ❌ **And the allocation census IS taken at this tip and is flat too**: 759,943 `__rust_alloc` calls on a six-game `cube` run, top caller `finish_grow` at 150,029, whose own top caller is **7.0 % of the growths** with ninety-two more behind it. There is no `with_capacity` to add that moves a pool, and the 8.9 % Ir is the system allocator under valgrind rather than the shipped mimalloc. ❌ **Build time is closed by measurement too** (PERF's "the file-size lever is dead"), and `effects/mod.rs`'s 38.2 k lines are **69 methods** — a split along method seams moves ~1 k lines and cannot touch the compile cost; only breaking up a single giant `match` would, which is not a mechanical move. ⚠ And a split now has a price: four small functions out of `bot.rs` read **+0.18 / +0.10 / +0.10 %** until each got `#[inline]` (cgu 16, no LTO). (b) **The checklist nobody has run to the end: 285 `battlefield.iter().any(` sites, and the instrument that found this run's three is the CALLEE table, not the self table** — none of them had a symbol, all inlined into their parent. Take the callee tree of anything over ~0.5 % and ask of each callee *which of its passes is gated*, and of each remaining walk *what its body does on a miss*. (c) The two structural rows (line 8). (d) ⚠ **Re-run the fresh-seed sweep as the FIRST gate after any memo or epoch change, not the last** — it is the only gate that reached `(-311)`'s collision. (e) **`audit_dropped_may` is 199 and `LookPickToHand` is NOT safe at this granularity, measured** — buckets: put 73, exile 20, return 16, collect 12, begin 12. (f) `audit_keyword_drift`'s last 4 each need a PRIMITIVE. (g) The skips with a card behind them: `nocolors` **950**, `nosubtypes` 43, `noback` 60, `nokeywords` 29.
