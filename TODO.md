...
1. ⚠⚠ **TWO SESSIONS SHARE THIS BRANCH. FETCH BEFORE YOU START AND BEFORE EVERY PUSH.** Rebase, never force; code before tracker prose. ⚠ `pkill -f <pattern>` **KILLS YOUR OWN SHELL** when the pattern is in the command line running it — kill by pid. ⚠ **DO NOT EDIT THE TREE WHILE AN OPTIMIZED BUILD RUNS**: cargo stamps the output newer than your edit and hands back a stale binary that looks fresh. ⚠ **DISK: `rm -rf target/debug/incremental` before any optimized build.** Box: 2.10 GHz Xeon, 4 cores; release-fast engine ~9 min, `release` does not finish in 25. Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure"**.
2. **Gates at the closing tip:** suite **20,199 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, `--workspace --exclude crabomination_client`, which holds exactly **200** more `#[test]`s on a box that can build Bevy — the same tree reads 20,0xx ↔ 20,3xx with the box, not with the tree); clippy **0**; `--bench` **195,806 / 27.49 / 611.9 / 0 stalls — byte-identical to the committed invariant**, `determinism ok`, `thread_determinism ok (3 vs 1)`, `peak_rss_mib` 28.7. **Pod: 2 fresh seeds × 2/4/6/8/10 seats × 1,000 = 10,000 games, all ten blocks 1,000/1,000 decided, every `undecided_by` column zero, zero panics.** Every audit gate clean, the three new ones included. **FRONTIER 9302 / 775.**
3. ⚠⚠ **AND `--bench` WAS NOT THE GUARDRAIL THAT MATTERED — IT PLAYS `--decks fixed`, WHICH HELD NONE OF THE 39 CARDS THIS RUN CHANGED.** The pools that do: 4 fresh seeds × cube/sos/sealed × 300 games an archetype = **30,000 two-player games, 0 panics, 0 stalls, 0 caps**; the 4 undecided (0.013 %, sealed) are all `draw`, and CR 104.4 is an outcome. **Pick the guardrail by what the diff touched, not by what is cheapest to run.**
4. 🔎🔎 **THE BIGGEST FIND IS IN THE TOOLING AND IT INVALIDATED FOUR "0" COLUMNS.** `audit_dropped_may.defs_in` — the body reader six audits import — keyed every card on a `name: "…"` literal and skipped the card when there was none. **A third of this catalog names itself positionally** (`..creature("Sidar Jabari", …)`, `fated("Fated Conflagration", …)`): **7,030 of 22,134 factories were never walked.** Fixed; three ratchets moved off zero — `another_target` 0/0 → **3 + 21**, `dropped_may` 178 → **242**, `target_zone` 455 → 641 clauses. The three invented-* columns stayed at 0, so those really are closed. **A ratchet's denominator is a number to check, not to trust.**
5. 📐📐 **AND BOTH READER BUGS WERE INVISIBLE IN THE TOTALS — VALIDATE THE ATTRIBUTION.** ① An unanchored "first call with a leading string" match takes the **token** a card mints: Conqueror's Pledge read as "Kor Soldier", Urza's Saga as "Construct". ② `slug` turned `'` into `_`, so "Acolyte's Reward" never matched `acolytes_reward` and the slug preference was failing on **all 1,162 possessive names**. Found by asking how many yielded names slug-match their factory (21,667 / 21,966 now) — a question the row counts cannot answer.
6. 🔎 **FIND: "any player may X. If a player does, Y" CLOSED ON THE FIRST ACCEPTANCE.** Two rulings say otherwise — Argothian Wurm 2007-02-01 ("all remaining players **still** get the option") and Desecration Demon 2024-11-08 ("each opponent … **even if an opponent already chose** … a **maximum of one** +1/+1 counter"). `PlayersMayAccept` now splits `on_accept` (per accepter) from the new `if_any` (once); 7 cards. ENGINE_BACKLOG's **fifty-fourth**.
7. 📐 **THE ASK LOOP HAD TO MOVE BEFORE THE APPLY LOOP AND THAT IS A RULE, NOT A STYLE.** CR 101.4a — all choices before any actions. It is also the only replay-safe order: a `wants_ui` seat later in the order suspends, and the re-run would have re-dealt damage to an accepter already paid. **The CR-correct order and the replay-safe order were the same order.**
8. 🔎 **FIND: "DEFENDING PLAYER" IS ONE SEAT AND 32 CARDS HAD IT AS "ANY OPPONENT".** CR 506.2; exact in a duel, one seat of several in a pod. `ControlledByDefendingPlayer` + `scripts/audit_defending_player.py`, **0/32**. Three were worse than imprecise: **Blaze of Glory carried no controller filter at all**, Kukemssa Pirates read *owner* for *controls*, and **Purifying Dragon declared its target slot in neither arm** of its `If`. ENGINE_BACKLOG's **fifty-sixth**.
9. 📐📐 **THE CLAUSE IS OFTEN NOT ON THE ATTACKER, AND THAT IS WHERE THE WORK WAS.** Four shapes: an **instant** cast in combat (Yare), an **Aura/Equipment** whose host attacks (Pretender's Claim), a trigger firing off *another* creature's attack (Nazahn), and a **post-teardown** trigger whose attack record is gone (Kusari-Gama). The first three go through `defending_player_in_combat` (host, then the combat's **single** defender; two is ambiguous and takes nothing). The fourth needs a different reference entirely — the damaged **blocker's controller**, which *is* that seat. `PlayerRef::DefendingPlayer` ends in the same call now so the two can never disagree.
10. ✅ **The tenth target deck is Adriana, Captain of the Guard (RW)**, and it exists because of a census: **not one of the nine decks played a single multiplayer-native mechanic.** Monarch (CR 725), goad (701.15), melee (702.121), voting (701.38), tempting offer + join forces (207.2c), the initiative (726) and myriad (702.116) were all implemented, all tested, and **none had ever resolved in bot self-play**. 27 such cards; `--seats 10` reaches it. **A mechanic the field never plays is a mechanic self-play never crashes on.**
11. ⚠⚠ **THE TENTH SEAT BENDS THE TURN CURVE AND THE FIRST HYPOTHESIS WAS WRONG.** Linear at ~12.9 turns a seat; the tenth adds **+24.4**, reproducibly at two seeds. Grand Melee was the suspect (its "all creatures block each combat if able" cancels exactly the attacks goad forces) — cut for Impact Tremors, and the A/B moved turns **124.15 → 123.07**. Not the cause. It did move the deck, 4.6 → **7.2 %**. 💡 Remaining suspects: **Protector of the Crown** (redirects *all* damage dealt to its controller) and the monarch's extra card a turn. One A/B each, and DECK_FEATURES carries the open lead rather than a guess.
12. 💡💡 **THE METHOD THAT PAID TWICE: ASK A RULING, NOT MEMORY — AND ASK IT OF A DIFFERENT CARD.** `fetch_oracle.py --rulings` is new (the offline cache carries oracle text only, which is why the Longhorn Firebeast row sat allowlisted as "waiting on a ruling" for a whole session). Longhorn Firebeast has **no rulings at all**; **Vexing Devil, same template, answered it in one call.** `audit_singular_fanout` is 19 → 2 → **1**.
13. 💡 **THE OPEN QUEUE, IN THE ORDER IT LOOKS WORTH TAKING.** ① The 21 slot-relative + 3 source-relative `another_target` rows and the 64 new `dropped_may` rows that item 4 uncovered — a known class with a known fix, just re-opened. ② `target_walkers`'s invariant is **one-directional**: it proves every `TargetFiltered { slot }` is answerable, never that every `Target(n)` has a `TargetFiltered` somewhere — Purifying Dragon is what that hole looks like, and the reverse walk is unwritten. ③ The 23 cube-pool `dropped_may` rows (Restoration Angel, Liliana's −2, Grapple with the Past, Grisly Salvage, Beastmaster Ascension, Jace TMS; Deep-Cavern Bat and Elite Spellbinder may be `RESOLVER_OPTIONAL` instead) — the 2-player trained path, so a dropped choice is live there. ④ Exert / **CR 508.1g** (the comments cite 702.137/702.83 and both are wrong; the shape is a new `GameAction` beside `DeclareAttackersBanded`, because `Attack` is `Copy` and built at ~1,400 sites). ⑤ COMMANDER_BACKLOG's remaining ~46 top-1000 rows. ⑥ ⚠ **THE GAP THAT HAS NOT MOVED ALL RUN IS SECTION 1, NOT THE STAPLES**: top-250 is complete and top-1000 is 954/1000, while **173 of the most-built COMMANDERS are absent and only 47 of the top 100 by decks built are in**. `sets::decks::recent331` opens it with the slice worth having before a deck exists — commanders whose text is about the WHOLE TABLE (Nekusar #18, Kenrith #20, Zedruu, Phelddagrif), all four needing no new primitive and all tested at four seats because two seats half-answer them. Miirym #28 landed too (its two riders are the card: **nontoken** stops the copy copying itself forever, **isn't legendary** stops CR 704.5j eating the pair). ⚠ **Kaalia #8 is the next rung and its blocker is named**: `PutFromHandOntoBattlefield` has `tapped`, `haste`, `sacrifice_eot` and `return_eot` but no **attacking** — CR 508.4 puts a creature onto the battlefield attacking a chosen defender, and only `CreateTokenAttacking` can do that today, for tokens. Isshin's trigger-doubling and Jodah's cascade are not free either. ⚠⚠ **`grep -rn "pub fn <slug>" crabomination_catalog/src` before writing one** — Ayara was drafted and dropped because the other session had shipped it mid-file, and the generated backlog a batch is sized from is a snapshot, not a lock.
14. ⚠ **Gotchas that each cost a round this time.** ① A **"may"** needs `always_yes`/a scripted `Bool(true)` or the AutoDecider declines and the test sees nothing happen — a silent pass-looking failure. ⚠⚠ **And the same fact hid a defect for a whole branch: `Effect::MayDo` asks `ctx.controller`, so a printed "TARGET OPPONENT may" / "that player may" asks the wrong seat, and because the AutoDecider declines for everyone the two seats "agree" and no aggregate moves.** Divine Gambit shipped with a comment arguing exactly that ("the auto outcomes are equivalent"); its body already named the right player in both halves and only the ASK was misrouted. `scripts/audit_may_seat.py` is the third column of the may family (0/115), and the instrument is a `wants_ui` seat on the opponent ALONE — the engine suspends only for a UI seat, so `pending_decision.is_some()` iff the question reached them. An outcome-only test passes either way. ENGINE_BACKLOG's fifty-eighth. ② `Selector::EachPermanent(filter)` does **not** thread the ability's source, so a source-relative atom inside one is always false; the mass-selection form is `Selector::ControlledBy { who, filter }`. ③ A new `SelectionRequirement` needs an arm in `evaluate_requirement_static_hinted` **and** `evaluate_requirement_on_card_inner`; three of the four walkers end in a permissive `_ =>` and it compiles either way. ④ `R` is not aliased in every catalog file — `decks/innistrad.rs` wants `SelectionRequirement::`.
15. ⚠ **The rules table is still DONE and that is still the main claim**: FEATURE_ROADMAP's Commander and 800-series sections are all ✅ bar CR 800.4i (unreachable) and an N-seat encoder (⛔ by design). **What is left is cards, and the tooling that finds what is wrong with them** — which item 4 just showed was measuring two thirds of them. `CR_COVERAGE.md` is generated: take either side of a conflict and re-run `scripts/cr_coverage.py`.
