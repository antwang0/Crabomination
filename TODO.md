...
5. 💡💡 **THE TOP LEAD IS NOW A BOT-STRENGTH ONE AND IT IS SIZED: `CRAB_PAY_FAILS` says 28.5 % of `cube`'s payments FAIL** (2,486 / 8,712 — generic 974, **coloured 1,428**, hybrid 84; `sealed` 13.0 %, `fixed` **0**), and 81 % of them had already built a mana source table. ⚠ `pay_census`'s own doc says the coloured class is where a **correctness** bug lives ("auto-tap stranded a colour it could have covered … a payable line becomes invisible"), and **nobody has separated that from genuinely unpayable**. `CRAB_PAY_FAILS=2` names each failure; a bipartite match over the source table decides it. **A refused payable cast is a lost game, not a slow one.**
