...
10. **Perf queue: ✅ (O)'s `TokenDefinition` row TAKEN as `(-364)` (one level up), `GameState::clone`'s two lines split ✅ `(-362)` / ❌ `(-363)`.** ❌ **The auto-tap `events` buffer is DECLINED, censused by consumer, no build spent**: 15 of its 21 consumers *move* the buffer into their own accumulator, so an out-param buys at most ~29 % of the row for a five-site signature change. Open on the shortlist: `combat.rs:3924 lethals` (6,274 / 5,342), `affected_from_requirement` `types` (3,684), `active_team_members` (2,642, ONE caller), `computed_permanent_hinted` (5,852 sealed). Also open: **(H)** (price the clone side first), **(N)**, **(P)**.
