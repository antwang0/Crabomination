...
3. **Perf — FOUR ROWS TAKEN, cumulative fixed -0.879 / cube -1.725 / sealed -0.908 % against the `(-335)` tip.** `(-336)` the block planner's spare-capacity pass behind a board gate (**confirmed**, the pending A/B this session opened by finishing — it reproduces to three decimals); `(-337)` `declare_blockers`' six per-attacker requirement asks fold to one `match` pass (self -13/-18/-15 %); `(-338)` the same on `declare_attackers_banded` (self -8/-18/-10 %), plus the must-attack loop's guard reordered so the controller test runs before three subset finds; `(-339)` the 53 scans. Per-row numbers in PERF's Log and the Baseline's tenth-box block.
...
12. **Best next moves, in order.** (a) ✅ `(-339)`'s grep on the OTHER zones is **already censused — see PERF candidate (H), and read it before writing any code**: 138 production sites (`hand` 68 / `exile` 29 / `graveyard` 29 / `library` 12), of which **73 are in functions that never execute in a six-game run**, and the obvious fix is the wrong one — `FIND_HINTS` is 64 bytes a pile and `Clone` copies it, so an eight-pile table adds ~512 bytes to every `GameState` clone, which is already 1.05 % of `cube` on the probe path. Price the clone side first or find a shape that does not widen the cloned struct. (b) Queue item (E), `pick_blocks_inner`'s body, which is the one hot function `(-319)`'s "do not spend another `profiling-lines` build" verdict does not cover. (c) A fresh-seed block from 1382 as the FIRST gate after any lane/memo change. (d) leftover context: `audit_dropped_may` 199; `audit_keyword_drift`'s last 4 each need a PRIMITIVE; card skips with a card behind them: `nocolors` 950, `noback` 60, `nosubtypes` 43, `nokeywords` 29.
