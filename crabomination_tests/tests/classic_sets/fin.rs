...
fn malboro_bad_breath() {
    ...
    for _ in 0..5 {
        let id = g.next_id();
        g.players[1].library.push(crabomination::card::CardInstance::new(id, catalog::grizzly_bears(), 1));
    }
...
fn sephiroth_planets_heir_etb_and_death_growth() {
    ...
    assert_eq!(
        g.battlefield_find(seph).unwrap().counter_count(crabomination::card::CounterType::PlusOnePlusOne),
        1,
...
fn vanille_mills_and_returns_permanent() {
    ...
    for _ in 0..3 {
        let id = g.next_id();
        g.players[0].library.push(crabomination::card::CardInstance::new(id, catalog::forest(), 0));
    }
...
fn rydia_landfall_loots() {
    ...
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        ...
        crabomination::decision::DecisionAnswer::Target(Target::Permanent(discardable)),
    ]));
    g.perform_action(GameAction::PlayLand(land)).expect("play land");
    drain_stack(&mut g);
    assert!(
...
fn edgar_draws_per_artifact() {
    ...
    for _ in 0..4 {
        let id = g.next_id();
        g.players[0].library.push(crabomination::card::CardInstance::new(id, catalog::island(), 0));
    }
    let hand0 = g.players[0].hand.len();
...
fn warrior_of_light_impulse_on_legendary_cast() {
    ...
    g.players[0]
        .library
        .insert(0, crabomination::card::CardInstance::new(tifa, catalog::tifa_lockhart(), 0));
...
fn coliseum_behemoth_modal_draw() {
    ...
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new(vec![
        crabomination::decision::DecisionAnswer::Mode(1), // draw a card
    ]));
    let hand0 = g.players[0].hand.len();
...
fn goobbue_gardener_taps_for_green() {
    ...
    assert_eq!(g.players[0].mana_pool.amount(crabomination::mana::Color::Green), 1, "added one green");
}
...
fn al_bhed_salvagers_drains_on_death() {
    ...
    let bolt = g.add_card_to_hand(0, catalog::lightning_bolt());
    g.players[0].mana_pool.add(crabomination::mana::Color::Red, 1);
    crabomination::game::cast_at(&mut g, bolt, Target::Permanent(fodder));
    drain_stack(&mut g);
...
fn ashe_digs_for_artifact() {
    ...
    let art = g.next_id();
    g.players[0].library.insert(0, crabomination::card::CardInstance::new(art, catalog::bonesplitter(), 0));
    for _ in 0..4 {
        let id = g.next_id();
        g.players[0].library.insert(1, crabomination::card::CardInstance::new(id, catalog::forest(), 0));
    }
...
fn deadly_embrace_draws_per_death() {
    ...
    let gid = g.next_id();
    g.players[0].graveyard.push(crabomination::card::CardInstance::new(gid, catalog::grizzly_bears(), 0));
...
fn gysahl_greens_makes_bird() {
    ...
    drain_stack(&mut g);
    let birds = g.battlefield.iter()
        .filter(|c| c.is_token && c.definition.subtypes.creature_types.contains(&crabomination::card::CreatureType::Bird))
        .count();
...
fn matoya_draws_on_scry() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_ability(matoya, 0, None);
    let events = g
        .resolve_effect(
            &crabomination::effect::Effect::Scry { who: crabomination::effect::PlayerRef::You, amount: crabomination::effect::Value::ONE },
            &ctx,
        )
        .unwrap();
...
fn matoya_draws_on_surveil() {
    ...
    let events = g
        .resolve_effect(
            &crabomination::effect::Effect::Surveil { who: crabomination::effect::PlayerRef::You, amount: crabomination::effect::Value::ONE },
            &ctx,
        )
        .unwrap();
    g.dispatch_triggers_for_events(&events);
...
fn commune_with_beavers_digs_for_a_creature() {
    ...
    g.players[0].mana_pool.add(crabomination::mana::Color::Green, 1);
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Search(Some(want)),
    ]));
    g.perform_action(GameAction::CastSpell {
        card_id: spell,
        target: None,
...
fn crossroads_village_taps_for_chosen_color() {
    use crabomination::decision::{DecisionAnswer, ScriptedDecider};
    let mut g = two_player_game();
    g.decider = Box::new(ScriptedDecider::new([DecisionAnswer::Color(crabomination::mana::Color::Blue)]));
    ...
    assert_eq!(g.players[0].mana_pool.amount(crabomination::mana::Color::Blue), 1, "added blue (chosen)");
}
...
fn sorceresss_schemes_returns_instant() {
    ...
    assert_eq!(g.players[0].mana_pool.amount(crabomination::mana::Color::Red), 1, "added {{R}}");
}
...
fn from_father_to_son_fetches_vehicle_to_hand() {
    ...
    g.players[0].mana_pool.add_colorless(1);
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Search(Some(veh)),
    ]));
    g.perform_action(GameAction::CastSpell {
        card_id: spell, target: None, additional_targets: vec![], mode: None, x_value: None,
...
fn call_the_mountain_chocobo_fetches_and_makes_bird() {
    ...
    g.players[0].mana_pool.add_colorless(3);
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Search(Some(mtn)),
    ]));
    g.perform_action(GameAction::CastSpell {
        card_id: spell, target: None, additional_targets: vec![], mode: None, x_value: None,
...
fn blitzball_mana_and_conditional_draw() {
    ...
    // Mana ability works.
    g.perform_action(GameAction::ActivateAbility {
        card_id: ball, ability_index: 0, target: None, additional_targets: vec![], x_value: None, mode: None,
    }).expect("tap for mana");
    ...
    assert!(g.perform_action(GameAction::ActivateAbility {
        card_id: ball, ability_index: 1, target: None, additional_targets: vec![], x_value: None, mode: None,
...
fn the_gold_saucer_coinflip_treasure_and_sac_draw() {
    ...
    g.players[0].mana_pool.add_colorless(2);
    g.perform_action(GameAction::ActivateAbility {
        card_id: saucer, ability_index: 1, target: None, additional_targets: vec![], x_value: None, mode: None,
    ...
    let hand0 = g.players[0].hand.len();
    g.perform_action(GameAction::ActivateAbility {
        card_id: saucer, ability_index: 2, target: None, additional_targets: vec![], x_value: None, mode: None,
...
fn ether_adds_mana_and_copies_next_spell() {
    ...
    assert_eq!(g.players[0].mana_pool.amount(crabomination::mana::Color::Blue), 1, "added {{U}}");
    let life1 = g.players[1].life;
    let bolt = g.add_card_to_hand(0, catalog::lightning_bolt());
    g.players[0].mana_pool.add(crabomination::mana::Color::Red, 1);
    g.perform_action(GameAction::CastSpell {
...
fn summon_gf_ifrit_chapter_three_adds_red() {
    ...
    drain_stack(&mut g);
    assert_eq!(g.players[0].mana_pool.amount(crabomination::mana::Color::Red), 1, "chapter III added {{R}}");
}
...
fn cr_707_2_token_copy_enters_tapped() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_ability(
        crabomination::card::CardId(0), 0, Some(Target::Permanent(src)),
    );
    g.resolve_effect(&crabomination::effect::Effect::CreateTokenCopyOf {
        extra_keywords: vec![],
...
fn qutrub_forayer_destroys_damaged_creature() {
    ...
    g.resolve_effect(&crabomination::effect::Effect::DealDamage {
        ...
        crabomination::card::CardId(0), 0, Some(Target::Permanent(foe)),
    )).unwrap();
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new(vec![
        crabomination::decision::DecisionAnswer::Mode(0), // destroy the damaged creature
    ]));
...
fn thunder_magic_thundara_tier() {
    ...
    g.players[0].mana_pool.add(crabomination::mana::Color::Red, 1); // base {R}
...
fn restoration_magic_curaga_shields_and_gains() {
    ...
    g.players[0].mana_pool.add(crabomination::mana::Color::White, 2); // base {W} + Curaga {W}
    g.players[0].mana_pool.add_colorless(3);
    let life0 = g.players[0].life;
    g.perform_action(GameAction::CastSpellSpree {
        card_id: id, spree_modes: vec![2], target: None,
...
fn summoners_grimoire_cheats_creature_on_attack() {
    ...
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Cards(vec![big]),
    ]));
    advance_to(&mut g, TurnStep::DeclareAttackers);
    g.perform_action(GameAction::DeclareAttackers(vec![Attack {
        attacker: hero, target: AttackTarget::Player(1),
...
fn the_wandering_minstrel_town_scaling() {
    ...
    g.players[0].mana_pool.add_colorless(3);
    g.perform_action(GameAction::ActivateAbility {
        card_id: minstrel, ability_index: 0, target: None, additional_targets: vec![], x_value: None, mode: None,
...
fn the_lunar_whale_unlocks_top_of_library_on_attack() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(whale, 0, None, 0);
    g.resolve_effect(&eff, &ctx).unwrap();
...
fn tellah_scales_with_mana_spent() {
    ...
    let hand0 = g.players[0].hand.len();
    let life1 = g.players[1].life;
    let mut ctx = crabomination::game::effects::EffectContext::for_trigger(tellah, 0, None, 0);
    ctx.mana_spent = 8;
    g.resolve_effect(&eff, &ctx).unwrap();
    g.check_state_based_actions();
...
fn ragnarok_death_destroys_and_reanimates() {
    ...
    let mut ctx = crabomination::game::effects::EffectContext::for_trigger(ragnarok, 0, None, 0);
...
fn omega_stuns_and_gains_by_nonbasic_lands() {
    ...
    let mut ctx = crabomination::game::effects::EffectContext::for_trigger(omega, 0, None, 0);
    ctx.targets = vec![Target::Permanent(foe)];
    g.resolve_effect(&eff, &ctx).unwrap();
    let f = g.battlefield_find(foe).unwrap();
    assert!(f.tapped, "target tapped");
    assert_eq!(f.counter_count(crabomination::card::CounterType::Stun), 2, "two stun counters");
...
fn yshtola_rhul_blinks_and_extends_first_end_step() {
    ...
    let mut ctx = crabomination::game::effects::EffectContext::for_trigger(ysh, 0, None, 0);
...
fn kain_jump_and_traitor_trigger() {
    ...
    let mut ctx = crabomination::game::effects::EffectContext::for_trigger(kain, 0, Some(Target::Player(1)), 0);
...
fn cecil_flips_at_half_starting_life() {
    ...
    let mut ctx = crabomination::game::effects::EffectContext::for_trigger(cecil, 0, None, 0);
    ctx.event_amount = 2;
    g.resolve_effect(&eff, &ctx).unwrap();
...
fn louisoixs_sacrifice_counters_noncreature() {
    ...
    match c.effect {
        crabomination::card::Effect::CounterSpell { what: crabomination::card::Selector::TargetFiltered { slot: 0, ref filter } } => {
...
fn kefka_dfc_stats_and_back_draw() {
    ...
    let mut ctx = crabomination::game::effects::EffectContext::for_trigger(kefka, 0, None, 0);
    ctx.event_amount = 3;
    g.resolve_effect(&eff, &ctx).unwrap();
...
fn kefka_eight_mana_edicts_and_transforms() {
    ...
    let victim = g.add_card_to_battlefield(1, catalog::grizzly_bears());
    g.players[0].mana_pool.add(crabomination::mana::Color::Blue, 8);
    g.perform_action(GameAction::ActivateAbility {
        card_id: kefka, ability_index: 0, target: None, additional_targets: Vec::new(), x_value: None, mode: None,
...
fn kefka_back_trigger_gated_to_your_turn() {
    ...
    for _ in 0..2 { g.add_card_to_library(0, catalog::island()); }
    let hand0 = g.players[0].hand.len();
    g.dispatch_triggers_for_events(&[crabomination::game::GameEvent::LifeLost { player: 1, amount: 2 }]);
    drain_stack(&mut g);
    ...
    g2.dispatch_triggers_for_events(&[crabomination::game::GameEvent::LifeLost { player: 1, amount: 2 }]);
    drain_stack(&mut g2);
...
fn clives_hideaway_is_hideaway_town() {
    ...
    assert!(
        c.triggered_abilities.iter().any(|t| matches!(t.effect, crabomination::card::Effect::Hideaway { .. })),
        "has Hideaway on ETB"
    );
...
fn elixir_reshuffles_nonlands_and_gains_life() {
    ...
    g.players[0].mana_pool.add_colorless(5);
    g.perform_action(GameAction::ActivateAbility {
        card_id: elixir, ability_index: 0, target: None, additional_targets: Vec::new(), x_value: None, mode: None,
...
fn yuna_end_step_reanimates_with_finality() {
    ...
    let mut ctx = crabomination::game::effects::EffectContext::for_trigger(_yuna, 0, Some(Target::Permanent(ench)), 0);
...
fn vincents_limit_break_death_gigas_returns_on_death() {
    ...
    g.resolve_effect(&crabomination::effect::Effect::DealDamage {
        ...
        crabomination::card::CardId(0), 0, Some(Target::Permanent(cat)),
    )).unwrap();
    g.check_state_based_actions();
    drain_stack(&mut g);
...
fn garnet_attack_drains_sagas_and_grows() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(garnet, 0, None, 0);
    g.resolve_effect(&eff, &ctx).unwrap();
    assert_eq!(
...
fn reno_and_rude_impulses_from_victim_library() {
    ...
    let mut ctx = crabomination::game::effects::EffectContext::for_trigger(reno, 0, Some(Target::Player(1)), 0);
    ctx.targets = vec![Target::Player(1)];
    g.resolve_effect(&eff, &ctx).unwrap();
...
fn eden_mills_then_sacrifices_to_return_permanent() {
    ...
    g.decider = Box::new(ScriptedDecider::new([DecisionAnswer::Bool(true)])); // yes, sacrifice
    g.perform_action(GameAction::ActivateAbility {
        card_id: eden, ability_index: 1, target: None, additional_targets: vec![], x_value: None, mode: None,
...
fn sin_exiles_and_copies_tapped() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(sin, 0, None, 0);
    g.resolve_effect(&eff, &ctx).unwrap();
...
fn quina_adds_a_frog_to_token_mints() {
    ...
    g.players[0].mana_pool.add_colorless(2);
    g.perform_action(GameAction::ActivateAbility {
        card_id: quina, ability_index: 0, target: None, additional_targets: vec![], x_value: None, mode: None,
    }).expect("eat the Frog");
    drain_stack(&mut g);
...
fn triple_triad_grants_lesser_mv_plays() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(tt, 0, None, 0);
    g.resolve_effect(&eff, &ctx).unwrap();
...
fn vanille_and_fang_meld_into_ragnarok() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_trigger(vanille, 0, None, 0);
    g.resolve_effect(&eff, &ctx).unwrap();
