...
3. **Gates at the closing tip `7f9520ed`:** suite **19,768 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); clippy **0** over the workspace; `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean; `--bench` (release) **195,806 / 27.49 / 611.9 / 0 stalls** — **byte-identical to the committed invariant**, `determinism ok`, `thread_determinism ok (3 vs 1)`, `peak_rss_mib` 25.9. Pod smoke: **12,000 games over 2/3/4/5 seats at fresh seeds 3201-3203, 100 % decided, 0 stalls, every `undecided_by` column zero**, no panic or error line. **FRONTIER 3204.**
