...
5. 💡💡 **THE TOP LEAD IS PERF'S NEW CANDIDATE (S), CENSUSED AND PRICED, NO BUILD SPENT: `CowBox<Vec<T>>::push`'s UNSHARE PATH COSTS TWO ALLOCATIONS — 36,724 of `cube`'s 664,995 (5.5 %), ~10.2 M Ir, 0.69 %.** `cow.rs:107` is the buffer, `cow.rs:110` is the `Arc` box, 18,362 shared-path pushes apiece; top callers `send_to_graveyard` 12,888 / `finalize_cast` 5,358 / `resolve_top_of_stack_inner` 4,068. **The device already exists in the tree** — `fx_pool::alloc_with` (`mod.rs:1029`) parks a replaced `Arc` and takes one back once its refcount has dropped, `(-166)`'s rule, 68-82 % hit rate one level out. ⚠ Two traps are written down in (S): a `thread_local!` in a generic body is ONE static for every instantiation (use a `CowPool` trait with a concrete static per impl), and the park point must **not** be `impl Drop for CowBox` because `into_inner(self)` moves out of `self`.
...
10. **Perf queue: ✅ (O)'s `TokenDefinition` row TAKEN as `(-364)` (one level up), its `lethals` row as `(-365)`, `GameState::clone`'s two lines split ✅ `(-362)` / ❌ `(-363)`.** ❌ **The auto-tap `events` buffer is DECLINED, censused by consumer, no build spent**: 15 of its 21 consumers *move* the buffer into their own accumulator, so an out-param buys at most ~29 % of the row for a five-site signature change. Open on the shortlist: `affected_from_requirement` `types` (3,684), `active_team_members` (2,642, ONE caller), `computed_permanent_hinted` (5,852 sealed). Also open: **(H)** (price the clone side first), **(N)**, **(P)**.
