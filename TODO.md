...
13. 💡 **THE OPEN QUEUE.** ① `audit_dropped_may` **208** — the self-cost subset is mined out (13 rows, 1 real), so the next cut is a different question, not more of the same. ② The **council ballot's bot policy**: a `Decision::ChooseTarget` over candidate permanents, where the bot's hostile ranking can make an opponent vote for its **own** best permanent, because the list is defined from the controller's perspective. Legal and terminating, which is why 18,000 pod games never saw it — **a crash-freedom smoke cannot find a bad decision, only an impossible one.** ③ The 69-row non-Aura "invented target slot" census in INCOMPLETE_CARDS (the karoo shape), a divergence only under shroud or a response. ④ COMMANDER_BACKLOG's remaining top-1000 rows. ⑤ `target_walkers`'s invariant is **one-directional** — it proves every `TargetFiltered` is answerable, never that every `Target(n)` has one.
