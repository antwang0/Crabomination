...
- ✅ **CR 702.85 — Heroic** — `shortcut::heroic(body)` +
  `Predicate::CastSpellTargetsSource` (a `SpellCast`/`YourControl` trigger that
  reads the just-cast spell's targets off the stack). Hero of the Pride
  (team +1/+0), Phalanx Leader (+1/+1 counter on each of your creatures).
  Tests `cr_702_85_*`.
- ✅ **CR 702.105 — Dethrone (carded)** — `shortcut::dethrone()` now ships on
  real cards (Enraged Revolutionary 2/1, Grenzo's Cutthroat 1/1 first strike),
  not just a test-granted trigger. Test `enraged_revolutionary_dethrone_on_card`.
- ✅ **CR 700.5 — devotion cost reduction** — `StaticEffect::
  SelfCostReducedByDevotion { colors }` (card-intrinsic, generic-only, read by
  `cost_reduction_for_spell`). Daybreak Chimera ({X} less = devotion to white).
  Test `daybreak_chimera_devotion_reduces_generic`.
