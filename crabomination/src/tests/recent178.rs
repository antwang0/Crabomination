...
fn fiendish_panda_counter_and_reanimate() {
    ...
    drain_stack(&mut g);
    let counters = *g.battlefield_find(panda).unwrap().counters.get(&crate::card::CounterType::PlusOnePlusOne).unwrap_or(&0);
    ...
    target.subtypes.creature_types = vec![crate::card::CreatureType::Elf];
...
fn salvation_swan_blinks_nonflyer() {
    ...
    bird.name = "Test Bird";
    bird.subtypes.creature_types = vec![crate::card::CreatureType::Bird];
