...
* `Effect::AddRadCounters` — **CLOSED 2026-09-13**: it was genuinely waiting,
  ...
  The count reads **1 dead primitive**, not 3 or 2. (`sets::pip` adds three
  more the same day — Contaminated Drink, Glowing One, Feral Ghoul — landed
...
* `Effect::GrantCastBackFromGraveyard` — waiting for a card that **does not
  ...
  fabricated Pestilent Cauldron claim was still sitting in **seven** doc
  comments in the code — `Effect::GrantCastBackFromGraveyard`,
  `CardInstance::may_cast_back_from_graveyard`, the bot test that sets the flag
  by hand, and, on the graveyard half of the lane, `net.rs`'s `back_castable`,
  `ActionAffordances::back_castable`, the `affordances` sweep and the
  `actions` hop-to-hand comment — plus a ✅ row in `INCOMPLETE_CARDS` claiming
  the rider "now works" directly above the 🟡 row saying it does not exist.
  A reader who met any of them would have gone looking for an ability that
  does not exist. All eight carry the correction now, plus the half this entry
  did not name: **the printed shape is Disturb** (CR 702.145), a KEYWORD that
  reaches `CastSpellBack` through `Keyword::Disturb(_)` on nine catalog cards
  — so the variant is neither a card in waiting nor a duplicate of a live one.
  **A triage that does not reach the code it is about is half a triage**, and
  the grep that proves it reached is the card NAME, not the variant name.* `Effect::ExileTopAndMayCastUpToMv` — **DELETED**, because it was not
