...
2. **Gates at the `b4ebba46` tip, all re-run:** `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant, `determinism ok`, `thread_determinism ok (3 vs 1)`; suite **19,598 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); clippy **0**; `cargo check --profile release-fast` clean. Sweep: **6 cells / 29,600 games / 0 failures** at seeds 1412..1413, `cap 0 / board 0 / stuck 0 / draw 2`, **FRONTIER 1414**. ⚠ `peak_rss_mib` reads **25.2 / 27.2 / 25.2 across three runs of one binary** — arena variance, not a reading.
...
11. **Perf queue: (J) and (K) CLOSED as diffuse, (L) taken as `(-344)`, (M) taken as `(-345)` + `(-346)`, its owed half refuted as `(-347)`.** Open: (A) `ManaCost::cmc`, (B) the `CardCold` `Arc::ptr_eq` gate, (C) `blocker_pair_block`'s pair count, (H) the other zones' by-id scans. ⚠ **Both (J)/(K) reads cost NOTHING:** `cg_lines.py` needs no `profiling-lines` build — one 16-second `--dump-instr=yes` callgrind line-profiles every body at once. **Line-profile a candidate before building for it.**
...
13. **Tip absolutes at `b4ebba46` (`profiling-fast --no-default-features`, system allocator, FOURTEENTH box @ 2.80 GHz): fixed 578,092,679 / cube 1,509,781,250 / sealed 1,630,933,303.** ⚠ **Build times are CLOSED as a lever** — PERF's own measurement kills the file-split idea (a 138x file-size difference buys nothing, and a split of a hot body without `#[inline]` across the new seam COSTS ~0.1 %).
