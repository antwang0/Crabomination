...
fn touch_of_the_void_deals_three() {
    ...
    assert!(catalog::touch_of_the_void().keywords.contains(&crabomination::card::Keyword::Devoid));
}
...
fn complete_disregard_cannot_hit_big_creatures() {
    ...
    g.players[0].mana_pool.add(Color::Black, 1);
    g.players[0].mana_pool.add_colorless(2);
    crabomination::game::cast_at(&mut g, id, Target::Permanent(hill));
...
fn murderous_compulsion_only_hits_tapped() {
    ...
    assert!(matches!(
        ...
        crabomination::card::Keyword::Madness(_)
    ));
}
...
fn scion_summoner_and_brood_monitor_make_scions() {
    ...
    g.players[0].mana_pool.add(Color::Green, 1);
    g.players[0].mana_pool.add_colorless(2);
    crabomination::game::cast(&mut g, ss);
    assert_eq!(scion_count(&g), 1);
    let bm = g.add_card_to_hand(0, catalog::brood_monitor());
    g.players[0].mana_pool.add(Color::Green, 2);
    g.players[0].mana_pool.add_colorless(4);
    crabomination::game::cast(&mut g, bm);
...
fn breaker_of_armies_has_all_must_block() {
    assert!(catalog::breaker_of_armies().keywords.contains(&crabomination::card::Keyword::AllMustBlock));
}
...
fn ulamogs_crusher_annihilator_sacrifices_two() {
    ...
    assert!(catalog::ulamogs_crusher().keywords.contains(&crabomination::card::Keyword::MustAttack));
}
...
fn drowner_of_hope_sacs_scion_to_tap() {
    ...
    crabomination::game::cast(&mut g, id); // ETB mints two Scions
    assert_eq!(scion_count(&g), 2);
...
fn sifter_of_skulls_mints_on_nontoken_death() {
    ...
    g.players[0].mana_pool.add(Color::Red, 1);
    crabomination::game::cast_at(&mut g, bolt, Target::Permanent(bear));
...
fn scour_from_existence_exiles_permanent() {
    ...
    crabomination::game::cast_at(&mut g, id, Target::Permanent(land));
...
fn dominator_drone_drains_with_another_colorless() {
    ...
    g.players[0].mana_pool.add_colorless(2);
    let before = g.players[1].life;
    crabomination::game::cast(&mut g, d1);
    ...
    g.players[0].mana_pool.add_colorless(2);
    let before = g.players[1].life;
    crabomination::game::cast(&mut g, d2);
...
fn flayer_drone_drains_on_devoid_creature_enter() {
    ...
    g.players[0].mana_pool.add(Color::Blue, 1);
    g.players[0].mana_pool.add_colorless(1);
    crabomination::game::cast(&mut g, mist);
...
fn spawnsire_makes_two_spawn() {
    ...
    let spawn = g.battlefield.iter().filter(|c| c.definition.name == "Eldrazi Spawn").count();
    assert_eq!(spawn, 2);
    assert!(catalog::spawnsire_of_ulamog().keywords.contains(&crabomination::card::Keyword::Annihilator(1)));
}
...
fn blight_herder_processes_two_then_makes_three_scions() {
    ...
    crabomination::game::cast(&mut g, bh); // cast trigger processes on the stack
...
fn natural_state_destroys_cheap_artifact() {
    ...
    g.players[0].mana_pool.add(Color::Green, 1);
    crabomination::game::cast_at(&mut g, id, Target::Permanent(art));
...
fn akoum_stonewaker_landfall_makes_elemental() {
    ...
    assert!(e.definition.keywords.contains(&crabomination::card::Keyword::Haste));
}
...
fn comparative_analysis_draws_two() {
    ...
    g.players[0].mana_pool.add(Color::Blue, 1);
    g.players[0].mana_pool.add_colorless(3);
    crabomination::game::cast_at(&mut g, id, Target::Player(0));
    drain_stack(&mut g);
...
fn inverter_of_truth_swaps_library_and_graveyard() {
    ...
    for _ in 0..2 {
        let cid = g.next_id();
        g.players[0].graveyard.push(crabomination::card::CardInstance::new(cid, catalog::mountain(), 0));
    }
    let lib_before = g.players[0].library.len();
    let gy_before = g.players[0].graveyard.len();
...
fn devour_in_flames_returns_land_and_deals_five() {
    ...
    g.players[0].mana_pool.add(Color::Red, 1);
    g.players[0].mana_pool.add_colorless(2);
    crabomination::game::cast_at(&mut g, id, Target::Permanent(target));
    drain_stack(&mut g);
