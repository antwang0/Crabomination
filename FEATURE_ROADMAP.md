...
| A spell-copy chain can't grow the stack without bound | ✅ *since 2026-09-26* | `recommend::MAX_STACK` (512): `copy_stack_spell_controlled` copies nothing past it and `stop_reason` ends the game as a `BoardCap` at it. Venser, Fervent Forger copying a Replication Technique that copies Venser reached 1,692 stack items and one action never returned (`a_spell_copy_chain_stops_at_the_stack_bound`) |
