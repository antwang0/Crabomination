...
1. **FIRST:** `git fetch origin claude/modern_decks && git checkout -B claude/modern_decks
   ...
   tracker prose; ⚠ claim a candidate number at PUSH time — `(-229)` is the last claimed,
   `(-230)` is next. Container gotchas in **CLAUDE.md**; measurement in **PERF's "Standing
...
2. **Gates at the `(-229)` tip:** PERF Baseline — suite 19,255 / 0 / 5, workspace clippy,
...
3. **Two sessions ran concurrently (Log `(-216)`..`(-229)`; `(-221)`, `(-227)` refuted): session
   A `cube` -2.61 % / `fixed` -2.43 % / `sealed` -1.67 %, session B on top of it `fixed` -1.55 %
   / `cube` -1.25 % / `sealed` -0.81 %.** The rules each leg added are in PERF's newest Baseline
   closing state; the reusable devices are `Battlefield::for_each_triggerer` (a printed-trigger
   walk over the member list — four walks taken, `(-222)`/`(-224)`/`(-228)`), "diff the gates
   of a mechanic's two sides" (`(-223)`), and **when a total contradicts the device's own rows,
   diff the two self tables** (`(-229)`: a `SmallVec::extend` the inliner flipped per build,
   ~2.3 % of `cube` between the two outcomes — now a push loop).
4. **Perf leads:** thin. `block_tax_for`'s per-blocker static walk (8,018 x 437 Ir, `cube`;
   ...
   static walk (~230 Ir, no lane); the probe clones' fresh event buffer (a thread-local pool,
   `(-166)`'s device — priced in `(-227)`); `computed_permanent_hinted`'s hit-path scan (~10 M).
   `PERF.md` ~30.5 k lines (Baseline states older than the ninetieth pass are in
