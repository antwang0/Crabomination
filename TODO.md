...
1. ⚠⚠ **TWO SESSIONS SHARE THIS BRANCH. FETCH BEFORE YOU START AND BEFORE EVERY PUSH.** Rebase, never force; code before tracker prose. ⚠ `pkill -f <pattern>` and `pgrep -f <pattern>` **MATCH THE SHELL RUNNING THEM** when the pattern is in their own command line — an `until ! pgrep -f "cargo build …"` waiter never exits, and it cost two rounds this run. Wait on a **pid** (`while kill -0 $pid`). ⚠ **DO NOT EDIT THE TREE WHILE AN OPTIMIZED BUILD RUNS.** ⚠ **DISK: `rm -rf target/debug/incremental` before any optimized build.** Box: 2.10 GHz Xeon, 4 cores; release-fast engine **7m54s** measured this run, `release` does not finish in 25. Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure"**.
2. **Gates at the closing tip:** suite **20,229 / 0 / 7** (`CRAB_ANSWER_LOG=strict`); clippy **0** over the workspace; **`cargo check --profile release-fast -p crabomination --bin bot_ladder` clean — the ONLY gate in the loop that sees `debug-assertions = false`, and CLAUDE.md says run it before every push**; `--bench` **195,806 / 27.49 / 611.9 / 0 stalls — byte-identical**, `determinism ok`, `thread_determinism ok (3 vs 1)`. **Pod, `release-fast`, seed 9902, 10 seats x 2,000: all decided, every `undecided_by` zero, zero panics. Two-player, the pools `--bench` cannot reach: 10,000 games over cube/sos/sealed, 0 undecided, 0 panics.**
3. 🔎🔎 **THE FIND, AND IT IS THE SESSION'S OWN QUESTION AT A FOURTH SITE: the pod worker was the ONE game worker still on the 2 MiB `scope.spawn` default**, so at every seat count from four up the **first** game aborted the process ("has overflowed its stack"). Not a runaway — `RUST_MIN_STACK=4194304` on the unchanged binary finishes the same games. Three siblings already take an explicit **32 MiB** with a comment each saying why (`recommend.rs`'s search workers, the two-player worker in the *same file*, `deck_gauntlet`); the pod worker was added after all three. ⚠ **So the Commander smoke could only ever be run on an OPTIMIZED build, and every committed pod figure before `88f07f22` is a statement about `release-fast` and only that.** A guardrail run only one way cannot report what it does the other.
4. 🔎 **THE INSTRUMENT IS NEW AND IT PAID ON ITS FIRST RUN: `bot_ladder --commander --card-census`** totals the action census by card and names, per seated deck, what a run never cast, played or activated. The smoke test answers "did the format survive"; this answers "did the **deck**". **10 seats, 2,000 games, seed 9902, 434 distinct cards: NINE of the ten target decks played EVERY card in the list.**
5. ✅ **FIXED, AND THE CENSUS IS WHAT FOUND IT AND WHAT PROVED IT: Yuriko's Agony Warp was castable by NO PATH** — 0 casts in 2,000 games, the only card of the ten decks' 1,000 a run never played. Two halves, both in `bot.rs`: the hand sweep drops `is_combat_trick` **unconditionally, with no step check**, so `pick_combat_trick` is such a card's only route onto the stack; and that function's `pump_amounts` **sums a `Seq` without reading which slot each clause names**, folding `-3/-0` + `-0/-3` into one `(-3,-3)` aimed at our own creature. A multi-slot pump is no longer a trick. Same seed before/after: **434 distinct cards with Agony Warp unplayed → 435, all ten decks complete.**
6. ⚠⚠ **THE BLAST RADIUS WAS SEVEN AND REASONING SAID ONE.** A source census keyed on the literal `slot:` spelling found two candidates; the catalog-wide test written to *measure* the same claim returned **seven** (the "another target creature" cycle). **Measure a blast radius over the catalog, never derive it from a grep.** 🔎 And the seventh is why this was a **wrong action**, not only a missed one: **Seeds of Strength** is three separate `+1/+1` pumps on three creatures, which the fold turned into **`+3/+3` on one** — a 3x over-valuation the picker acted on, where the mixed-sign cycle merely folded to `(0,0)`. ✅ None of the seven is in `cube`/`sos`/`sealed`/`fixed` (six appear nowhere in `crabomination/src`), which is *why* the traces held; a test pins the names so an eighth arrives with its own bench reading.
7. ✅ **The third side of the target-slot invariant is PINNED and it came back 0.** `target_walkers` proves every declared slot is *answerable*; `no_body_declares_a_slot_it_never_binds` proves the `requires_target()==false` case; `no_body_reads_a_slot_the_fill_loop_never_reaches` now proves every slot a body **reads** is one the cast-time fill loop reaches. The gap it closes is silent, not loud — both read sites are `ctx.targets.get(n)`, so an undeclared slot is `None` and half a card does nothing with no panic and nothing in a log.
8. ⚠⚠ **AND THE TWO FALSE POSITIVES IT COST, BOTH THE SAME MISTAKE: DO NOT RE-DERIVE A WALKER, CALL IT.** The first draft read "declared" as a literal `TargetFiltered` and called **five correct cards** broken — a slot is also declared by `ControlledBy { who: Target(n) }`, by `Value::CreatureCountControlledBy(Target(n))` and by every other player-reading selector, via `implicit_player_for_ref_slot`. The second asked only the **unkicked** walk and read Rushing River as broken, whose second target is kicked-only (CR 702.33). ⚠ A third: the walk must mirror the engine's **16-slot cap**, not just its break condition — a few arms answer the same filter for every index and an uncapped walk up **hangs**.
9. ⚠⚠ **A TRACKER THAT MIXES HISTORY WITH OPEN WORK COSTS A SEARCH, AND INCOMPLETE_CARDS DID.** Cross-referencing the ten decks' 410 card names against it returned 48 mentions and **every substantive one was a row in a "N real, all fixed" table** — the "Was" column is the *old* code. Eight were checked against Scryfall and the catalog body before the shape became obvious. Nothing was stale; the reader was. A **reading key** now names the three shapes that are open work (a 🟡 row, a "Residue" list, a numbered "No … primitive" section). **Read a table's heading before reading its rows as a worklist.**
10. 💡 **THE OPEN QUEUE.** ① **Per-slot trick evaluation** — the shape left behind: the HOLD predicate (`contains_temp_stat_leaf`) is `any` and the PICK predicate (`all_temp_pumps`) is `all`, so an instant between them is held out of the main phase and refused by the picker. It would make these cards *tricks* rather than merely castable, and it touches an adopted A/B-measured weight (+3.6 at round 66), so it needs its own A/B. ② `audit_dropped_may` **208** — the self-cost subset is mined out; the next cut is a different question. ③ The 69-row non-Aura "invented target slot" census (karoo shape). ④ COMMANDER_BACKLOG's remaining top-1000 rows. ⑤ **Extend `--card-census` to the two-player ladder** — cube/sos/sealed have no per-card coverage answer at all, and this run showed what that answer is worth.
11. ✅ **Checked this run and NOT gaps, so nobody re-checks them:** "the player to your left / right" appears on exactly **two** shipped cards and both are Conspiracy **draft** abilities, not gameplay — the engine's turn-order primitive (`seats_in_turn_order_from`) covers the rest. `audit_target_fields` is **0 aimed**. The three `HashMap`-walk-order sites in this file's determinism table are free choices under the rules and were **deliberately** recorded rather than fixed; do not re-litigate them.
12. 📐📐 **RESTORING A PRINTED "may" IS TWO EDITS AND THE SECOND IS THE POLICY.** `AutoDecider` declines *every* `OptionalTrigger`, so a `MayDo` wrapper without an answer is a card turned **off**. Mask of Memory proved it: `effect_imposes_self_cost` folds a `Seq` with `any` and a self-discard is on the list, so the wrapper alone would have left the Equipment a vanilla +0/+0. **A loot is not a cost** (`is_net_positive_loot`). Assert new rows through `optional_trigger_beneficial`, never through the wrapper's existence.
13. ⚠⚠ **A PROBE THAT SHOWS NOTHING HAPPENING IS A CLAIM ABOUT THE FIXTURE UNTIL IT IS A CLAIM ABOUT THE CARD.** Three lied in one sitting: `destroy_permanent` into a discarded `Vec` throws the death events away; `move_card_to_battlefield_for_test` does not reach a constellation watcher (it needs a **cast**); an existing test that scripts `DecisionAnswer::Target` proves the scripted path and says nothing about the headless one. **Measure the old and new shapes together before calling anything a bug** — and prove a new catalog-wide invariant non-vacuous with a **negative control** (this run: moving Harmless Offering's `to: Target(1)` to `Target(3)` must fail it) plus a floor on how many bodies it reached.
14. ⚠ **Gotchas that each cost a round.** ① A **"may"** needs `always_yes`/a scripted `Bool(true)` or the AutoDecider declines and the test sees nothing. ② **Script exactly as many bools as the card asks**: `ScriptedDecider` falls back to `AutoDecider` when exhausted. ③ `Selector::EachPermanent(filter)` does **not** thread the ability's source; the mass form is `Selector::ControlledBy { who, filter }`. ④ A new `SelectionRequirement` needs an arm in `evaluate_requirement_static_hinted` **and** `evaluate_requirement_on_card_inner`. ⑤ `R` is not aliased in every catalog file. ⑥ `cargo test` runs a binary's tests in ONE process and `land_tap_fast_path`'s seven share global atomics — they fail there and pass under **nextest**, which is the gate. Do not "fix" them.
15. ⚠ **The rules table is still DONE and that is still the main claim**: FEATURE_ROADMAP's Commander and 800-series sections are all ✅ bar CR 800.4i (unreachable) and an N-seat encoder (⛔ by design). What is left is **cards, and the tooling that finds what is wrong with them** — and `--card-census` is now the tool that says which cards a deck never reaches. `CR_COVERAGE.md` is generated: take either side of a conflict and re-run `scripts/cr_coverage.py`.
