...
| Omen (CR 702.183) | ✅ | `CardDefinition.omen` (reuses the `Adventure` shape) + `GameAction::CastOmen` + `CardInstance.omen_casting`; the creature card is cast as its instant/sorcery Omen half and shuffles into its owner's library on resolution *or* counter (handled at the `route_to_graveyard` funnel). Client right-click "Cast the Omen"; `KnownCard.{has_omen,omen_label,omen_needs_target}`. Tarkir Dragon cycle in `decks::omen`: Marang River Regent, Dirgur Island Dragon, Bloomvine Regent, Scavenger Regent, Twinmaw Stormbrood, Disruptive Stormbrood. |
| Tarkir: Dragonstorm (non-Omen) | ✅ | `decks::tarkir` — Sarkhan's Resolve (ChooseMode pump/destroy-flyer), Dragonback Lancer (Mobilize 1), Sibsig Appraiser (look-two pick/bin), Defibrillating Current (mono-hybrid burn+lifegain), Mardu Devotee (ETB scry + once-per-turn 3-color mana), Sibsig Host (each-player mill 3), Stormscale Scion (Dragon anthem + Storm), Roilmage's Trick (Converge -X/-0 + draw), Kishla Skimmer (leaves-graveyard draw), Inevitable Defeat (CantBeCountered + exile + drain), Magmatic Hellkite (destroy nonbasic + opponent ramps a stunned basic via `ControllerOf(target)`), Hardened Tactician (`{1}`, sac a token: draw). |
