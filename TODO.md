...
5. ✅✅ **THE MIRROR METHOD IS TAKEN AND IT IS THREE COLUMNS AND 21 CARDS.** ENGINE_BACKLOG's own "CLOSED WITH A REASON 2026-09-12" said an invented-ability column needs a `have` set from the card's **OWN literal**, no helper expansion, no global table — that is exactly what these are, which is why they read 0 where the `audit_oracle_verbs` inversion read 14,350. `audit_invented_may` (a choice the print does not offer) **3→0**; `audit_invented_trigger` (an event the print does not name) **45→8→0**; `audit_invented_rider` (a clause bolted onto a verb the card gets right — a **table**, one line per rider) **14→0**. All three at 0 at the tip; keep them there. 📐 **The rider column is the one to extend**: its first row was written for one card and named eight (Mortify shipped `DestroyNoRegen` with a *test asserting the wrong rule*). Rows so far: `DestroyNoRegen`, `CantBeRegeneratedThisTurn`, `once_per_turn`, `sorcery_speed`. ⚠ **Three reader bugs, all general to any name-keyed audit**: `name:` is NOT where this catalog keeps the card's name (most creatures spread `..creature("Name", ..)`, so the only `name:` is a token's); the slug must fold accents and drop apostrophes; and a phrase set that is too tight manufactures findings — 38 of the first 45. **A factory also holds its TOKENS' rules text and the card's oracle does not** (Roadkill Rodney) — bucket those, do not drop them.
