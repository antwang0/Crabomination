...
7. **Bugs — a printed "you may draw a card" is a CHOICE, 26 cards fixed and the class gated.** Eighteen came off `audit_dropped_may` (a mandatory draw can deck its controller, CR 104.3c, and the bot loses a decision); **the new gate found eight more the script could not reach** — `core_rules::structural_audit::every_printed_you_may_draw_is_a_choice` joins the committed Scryfall cache against each definition's `{:?}` rendering (`wants_converge`'s single-oracle device), vacuity guard at 30 of 50. **The device generalises: any printed phrase the oracle states and the `{:?}` can test is one more gate**, and the script's source-text scan is strictly weaker.
8. **Seeds: frontier 1304 — THIS RUN TOOK 1300..1303: 12 cells / 59,200 games / 0 failures**, `cap 0 / board 0 / stuck 0 / draw 2`, clean, no 50,000-action re-run needed. `target-audit/overflow` with `-C debug-assertions=yes` and `CRAB_ANSWER_LOG=strict`. Say which block you are taking in this line *before* you take it. All standing audits 0: `audit_panics` 0 bare, `audit_stash_in_loop` 0 unexplained, `audit_seat_from_selector` 0 open, `audit_target_walkers --check` 0, `audit_doc_drift` 0 body-wrong / 0 doc rot / 0 stale notes, `audit_variant_coverage` 0 dead capability.
