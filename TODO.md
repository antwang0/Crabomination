...
2. **Gates at `7c4e94d6` (before the residual session's `e12ccee0` restricted-mana payment, which may move pod readings):** suite **20,274 / 0 / 7**; clippy **0**; `release-fast` check clean; `--bench` **27.49 / 611.9 / 0 undecided — unchanged**, `determinism ok`. **Pod, seed 9941, 1,000 games at each of 2..15 seats: 13,994 / 14,000 decided, zero panics**; turns/game 19.82 / 32.49 / 44.95 / 57.16 / 64.43 / 78.40 / 91.97 / 101.62 / 124.14 / 136.10 / 147.54 / 159.35 / 170.78 / 181.85 (2..9 byte-equal to the last run; 10+ moved with the concurrent session's Mind Flayarrrs cards). **`--card-census` at 15 seats: 731 distinct, none unplayed.** ⚠⚠ **The pod action budget is now PER SEAT past ten seats** (`bot_ladder`: 50,000, or 5,000 a seat above ten — 2..10 unchanged): the flat cap's rate went 0.4 / 2.4 / 6.8 % at 15 / 16 / 17 and every capped game was an ordinary one. Now **1,000 / 1,000 decided at 11, 14, 15, 16, 17 seats (seed 9941)**; 17 seats at seed 9980 likewise, `--card-census` 841 distinct, every list fully played. Suite **20,302 / 0 / 7** at the push. ⚠ **The action cap: 0 at 2..14 seats, 0.6 % at 15 (6 games, 257-322 turns)** — all ATTRITION, not loops, as every capped game read so far (`CRAB_CAP_DIAG=1 --first I --games 1` dumps the board). The longest game runs 1.7-2.8x the mean; the 15-seat mean is 27,490 of 50,000. Cap left alone on purpose.
...
7. 💡 **THE OPEN QUEUE.** ① **Wretched Ranks, Tramplesaurus Rex and Keen Engineering landed as seats 16-18** (Keen's residual: Steel Hellkite counts noncombat damage). **Reap the Tides landed as seat 19** (residual: Stumpsquall Hydra never offers an opponent's commander). **Corrupting Influence (ONC) LANDED as seat 20** (session `01StaUuo`, 2026-09-23 — all 13 cards; it was already built when the claim above it landed, sorry). **Next lists: Sliver Swarm (CMM, Sliver Gravemother) LANDED as seat 23** (residual: Descendants' Fury's "one of them" is any attacker that damaged a player this turn); **⚠ Vampiric Bloodlust (C17, Edgar Markov, 16 missing) is CLAIMED by the `modern_decks` Commander routine (2026-09-23 22:00) — don't duplicate; Sneak Attack (ZNC, Anowon) LANDED as seat 21** (residual: Whispersteel Dagger opens the whole graveyard; Master Thief's closed by `GainControlWhileYouControlSource`); **Jeskai Striker (TDC, Shiko and Narset) LANDED as seat 22** (residuals: Shiny Impetus re-goads each combat, Tempest Technique's storm copies keep the target); **Quick Draw (OTC, Stella Lee) LANDED as seat 24** (residuals: Crackling Spellslinger's storm count is read at trigger resolution, Forger's Foundry's "may" has no prompt); **⚠ Calling All Angels (FDC, 17) is CLAIMED by session `01StaUuo` (2026-09-23 22:40) — don't duplicate** — `precon_scan.py`'s factory regex now also matches `-> crate::card::CardDefinition`, re-run it before picking. The 18-seat board cap was a bot bug (overkill onto one seat) — fixed by `server/pod_attack.rs`. ② **Residuals left** (DECK_FEATURES; the rest were closed 2026-09-23 by the residual session — Myriad Landscape, Herald's Horn, Overcharged Amalgam, Psionic Ritual's tap-replicate, Grell's Horror grant, River Kelpie): Colossal Grave-Reaver picks the batch's first creature (needs the milled BATCH recorded, not `milled_ids_this_turn`), Lethal Scheme's connive (needs convokers on the resolving spell — it is in no zone), Grell's blue-as-any rider, Shigeki, Cephalid Coliseum, Steward/Loam pick at resolution, Master of Dark Rites / New Blood, Sewer Nemesis always an opponent. ③ **Bots never emit 14 cast variants** (ENGINE_BACKLOG "cast variants NO bot ever emits"; escape and replicate are done) — take one when a pod list needs it. ④ `AdditionalCastCost::OneOf` picks for a human too — a cast-time prompt is the follow-up. ⑤ The older queue: per-slot trick evaluation (needs an A/B), `audit_dropped_may`, the 69-row invented-target census, `--card-census` for the two-player ladder.
8. ✅ **The residual session (2026-09-23), gates at `d4f6a334`:** suite **20,287 / 0 / 7**; `--bench` **195,806 decisions / 27.49 / 0 stalls — byte-identical**, `determinism ok`, `thread_determinism ok`; fixed-pool Ir **+0.025 %** vs `13637a7f` (cube +0.26 %, bots take new lines there); pod seed 9991 × 300 at 2..16 seats: every block decided through 14 seats, 16 seats 288/300 (12 action caps, all 247-294-turn games — the flat-cap finding above, not a loop); a debug-build pod (debug_asserts on, `CRAB_ANSWER_LOG=strict`) at 4/6/16 seats: clean. **Three bug CLASSES closed** (ENGINE_BACKLOG "FIXED 2026-09-23 (the residual session)"): ① cast origin read as "not from hand" — every **command-zone cast** counted as a graveyard cast (River Kelpie, Ash Zealot, `CastFromGraveyard`); ② modal triggers targeted/re-checked mode 0 and seven push sites never picked a mode (Shambling Ghast, Hullbreaker); ③ spend-restricted mana never auto-paid (`pay_with_restricted_sources`, CR 106.6 — Secluded Courtyard, Unclaimed Territory, Ancient Ziggurat). Also bot escape + replicate, Psionic Ritual's tap-replicate (`Keyword::ReplicateTap`), Myriad Landscape, Herald's Horn, Overcharged Amalgam, Grell, Radiant Destiny, Howlsquad Heavy, Archangel of Tithes, Silver-Fur Master (`NinjutsuCostReduction`). A mana-costed restricted source (Castle Garenbrig) is tried last.
