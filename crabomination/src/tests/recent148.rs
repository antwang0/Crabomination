...
fn popular_egotist_sacrifice_drains() {
    ...
    assert!(
        g.computed_permanent(egotist).unwrap().keywords.contains(&crate::card::Keyword::Indestructible),
...
fn cursed_windbreaker_manifests_and_equips() {
    ...
    assert!(
        g.computed_permanent(host).unwrap().keywords.contains(&crate::card::Keyword::Flying),
