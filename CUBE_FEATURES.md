...
|---|---|---|
| Equipment + equip-cost ability | 🟡 | `GameAction::Equip` + `equipped_bonus` (Shuko, Lavaspur Boots). Board-scaled bonus (`EquipBonus.scale` — Nettlecyst), equipment-granted triggers (`EquipBonus.triggered_abilities` — Sword of Body and Mind), Reconfigure (`Keyword::Reconfigure`, attach/unattach — Lion Sash), living weapon (Batterskull, Flayer Husk), and equip-triggers that charge the *Equipment* (`triggers_on_equipment` — Umezawa's Jitte) all ship. Equip at instant speed (`StaticEffect::ControllerEquipAtInstantSpeed` — Leonin Shikari), equip-cost reduction (`EquipCostReduction` — Auriok Steelshaper), per-Equipment CDA (`DynamicPt::BasePlusPerAttachedEquipment` — Goblin Gaveleer), "while equipped, has [kw]" (`SelfHasKeywordWhile` — Kor Duelist), two-target Attach via spell/ETB (Magnetic Theft, Maul, Embercleave) ship. Remaining 🟡: while-equipped *anthems* (Auriok's Soldier/Knight buff), conditional double strike by attached count (Balan). |
