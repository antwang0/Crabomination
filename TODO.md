...
1. ⚠ **Several sessions share this branch: fetch before you start and before every push; rebase, never force.** A new pod seat goes at the END of `target_decks()`. Disk fills silently: `rm -rf target/debug/incremental` + `find target/debug/deps -maxdepth 1 -type f -size +50M -mmin +20 -delete` before an optimized build (release-fast `bot_ladder` ~13 min cold, ~4 min engine-only; suite ~9 min). The previous NEXT, verbatim, is ENGINE_BACKLOG's last section.
2. **State (2026-09-26, session `01LEHArn`):** every Commander precon MTGJSON lists (181) is seated card for card — seats 1-183, `precon_scan.py` finds nothing missing, and a fresh `DeckList.json` has no new list. Rules table done bar CR 800.4i (unreachable) and the N-seat encoder (by design). **Work left is bugs the pods find and the INCOMPLETE_CARDS Commander residual rows.**
3. **Gates (session `01XSSbzm`, tip after the auto-tap re-bless):** suite **23,024 / 0 / 7**; workspace clippy **0** (client excluded); `--bench` **197,136 byte-identical** at every build this run; cube / sos / sealed 300 games each decided (seed 99101).
4. **Pod smoke this run, every deck, fresh seeds, 300 games a group:** round 1 (seeds 104001-109023, 3-8 seats, 67,500 games) and round 2 at the rebased tip (125000-129022, 4-8 seats, 49,200) — **zero panics**; every draw read by `cap_diagnosis` is a CR 104.4a all-seats-dead sweep; the caps left are genuine engines (Scute Swarm past six lands, Storm Herd at 473 life, a Rootha copy chain). Debug strict (`CRAB_ANSWER_LOG=strict`) 3/4/6/8 seats, 24 games a group: clean. `scratchpad/census.sh`-style loop over `--pod-decks` groups, `CRAB_CAP_DIAG=1` on a capped `--first i --games 1`.
5. **This run's finds (all fixed, tests cite the rule):** the **"choose an opponent" class** — Gift gave EVERY opponent the gift (21 cards, CR 702.174a), the Siege protector / Tribute / Demonstrate took the lowest seat (`choose_player.rs::choose_opponent_at_once`, one non-suspending ballot); CR 704.5w-x protector re-seat; encore tokens each attack their own opponent (CR 702.141a); ⚠ **auto-tap orphaned a prompting seat's any-color color prompt** mid-payment and the answer later minted mana off an untapped Command Tower (a 400-cast ward loop, CR 104.4b draw; re-blessed the pod table); ⚠ **Watchful Radstag** copied itself before its evolve counter landed (5 board caps; `shortcut::evolve_then`, CR 702.100b); Battle for Bretagard copied duplicate names (board cap; `Selector::OnePerDistinctName`); CR 726.4 heir ventures; Humble Defector targets; Stormsculptor hits battles (`R::ProtectedByOpponent`); Mirkwood Trapper's second ability (`EventScope::AnyPlayerAttacks`); the bot aims a *granted* MustAttackChosenPlayer.
6. **Method that paid:** debug-build strict pods find what release pods wrap silently (the overflow); an 8-seat census finds board and loop pathologies; a draw is the no-progress watch catching a loop — replay with `CRAB_CAP_DIAG=<n>`, and a 4-line per-action `eprintln!` in `play_one_pod_game_censused` (not committed) names the loop in one build.
7. **Leads:** Y'shtola (163) won 56 % of six-seat pods (seed 17302) — what her drain does that the attack decks don't; constant shrinks (Disfigure) are still combat tricks (moves 2-player traces); INCOMPLETE_CARDS' Commander rows are mostly "the engine picks" choices. **Added by `01XSSbzm`:** a pod bot spends hundreds of energy one {E} at a time on Aetherwind Basker pumps (seed 125034 4-seat game 153, the one action cap); six "an opponent chooses" sites still take the lowest live seat instead of asking the controller (`effects/mod.rs` Development / the two `chooser` arms / Cuombajj, `actions.rs:11048`, `mod.rs:5909`); `audit_target_opponent` / `audit_that_player` leave 11 non-pod cards plus Ral Zarek (SOS pool — moves 2-player play).
...
9. **Session `015BCEt5` (2026-09-26):** the player-choice batch is done — seat ballots for "choose a player / an opponent" (`effects/choose_player.rs`, 12 as-enters cards + the Offerings, Plargg, Sower/Bitter Feud pairs), protection from a player (Guardian Archon, Eon Frolicker, Curses shed), per-attacked-player trigger targets (Karazikar, Firkraag), Myth Unbound per partner. Left in it (entangled with kicker/modal/divided slots): Domineering Will, Maestros Confluence, Waste Management, Fiery Justice, Abstract Performance, Bladegriff Prototype. ⚠ **CR 109.2**: a board slot reached graveyard cards (bench moved, line 10). 💡 **`--card-census` now lists never-activated abilities** (~1,500 across all seats, seed 53000); `server/generic_sink.rs` activates the uncovered ones in pods when they beat passing (29 → 17 in one pod; pod throughput -9 %, PERF candidates); still never used: X abilities (Geth), discard/graveyard costs, untap-a-land ramp (Arbor Elf). **Later the same session:** 29 COMMANDER_BACKLOG cards (`decks::cmdr_legends`, `decks::cmdr_top1000` — 17 commanders incl. Jodah, Sauron, Tergrid, Najeela, Tiamat; Ojer Taq with a creature-token tripler), the identity-gap ratchet down to 5 (the adventure halves, Mythos of Illuna, Augusta no longer a standalone factory). ⚠ **CR 712.2**: a transforming DFC's land back face (Search for Azcanta, Dowsing Dagger, Ojer Taq) was a land drop from hand — `is_transforming_dfc` + a catalog ratchet. ⚠ The pod golden table moved under another session's any-color auto-tap fix (34774e835, bisected, re-blessed). Pod sweeps 4/6/8 seats (seeds 64000/65000/66000) all decided; cube seed 26601 now 2 legal draws.
10. ⚠ **`--bench` invariant is now 197,136** (200,190 → 200,398 by `01VVD5mW`'s CR 603.10a fix, creatures dying in one batch seeing each other die; → 197,136 by `015BCEt5`'s CR 109.2 fix, the bot no longer aims removal at creature cards in a graveyard). Bisects and pools in PERF. The same session closed ~45 Commander residual rows (ENGINE_BACKLOG `01VVD5mW`); open there: Tamiyo, Field Researcher's granter, Knight of New Alara's layer-5 read, Beckett Brass's combat-only tally.
11. **Session `012put2X` (2026-09-26): ⚠ a pod can HANG, not only cap — the action cap counts actions, and one action that never returns is invisible to it.** A census block sitting at 99 % CPU for 15+ minutes is one: `gdb -p <pid> -batch -ex "thread apply all bt"`, then bisect `--first i --games 1` under `timeout 20` (wrap each census group in `timeout`). Two found and closed: token doublers with no cap (`GameState::doubled_token_count` sizes every mint loop) and a Venser/Replication Technique copy chain (`recommend::MAX_STACK` = 512 *spells*; counting every stack item cut three legit 700-trigger games). Also Rot Hulk targets on entry (it returned itself: a 3,730-activation loop) and ~15 residual rows (ENGINE_BACKLOG `012put2X`). Gates: suite 22,989 / 0 / 7 strict, clippy 0, `--bench` 197,136 byte-identical, cube/sos/sealed 300 each decided; release censuses ~400k pod games; at the fixed tip, every deck at 2-8 seats (seeds 81001+ and 103001+, 158,500 games) had only CR 104.4a simultaneous-loss draws and >1,024-permanent board caps, zero hangs or panics; strict debug censuses at 4/6/8 seats (1,380 / 620 / 276 games) found one bot-gate `debug_assert!` (fixed: restricted floating mana) and nothing else.
