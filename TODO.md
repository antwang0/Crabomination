...
4a. 🔎 **A SECOND DEVICE, AND IT FOUND `(-342)`: GROUP THE LINE PROFILE BY SOURCE LINE ACROSS THE WHOLE PROGRAM, not by function.** `cg_lines.py <instr dump> <binary> --rows 0`, then sum the `file:line` column. `card.rs:13-14` — `CardId`'s derived `PartialEq` — is **22,214,350 Ir = 1.47 % of `cube`** over ~90 functions, i.e. `(-339)`'s whole class stated as ONE NUMBER instead of grepped for; `restore_payment_state` reading **590 id compares a call** is an O(n x m) shape written down in one line, in a function no self table ranks. Open leads from the same grouping: **`core/src/sync/atomic.rs` is 41,046,116 = 2.71 %**, 7.40 M of it inside `compute_permanents::{{closure}}` alone and UNREAD — that is 32.7 Ir a computed id for a relaxed load plus a relaxed store, so **ask whether the attribution survives a disassembly before trying to make atomics cheaper**. ❌ And `debug_flags`' `{:?}` scan (`is_contained_in` + `SmallVec` Debug + `String::write_str`) is ~12.6 M / **0.83 %** and is ONCE PER DISTINCT CARD NAME PER PROCESS — it saturates, no 10-30 k-game run pays it, **do not rank it**. ⚠⚠ **AND THE TRAP IN THAT DEVICE COST A BUILD: a line attributed to `card.rs:13` INSIDE a function does not tell you WHICH scan it is.** `find_by_id` is `#[inline]`, so its own compares land on the caller's row under the caller's source line. `computed_permanent_hinted`'s 2.15 M read as four compares a call against the freeze scope's `perms` list; a `[u8; 16]` slot table on `LayerFreezeState` (the `FIND_HINTS` contract, validated against the entry's id so needing no invalidation) reads **cube +0.151 / fixed +0.230 / sealed +0.242 %** and leaves `computed_permanent_hinted` self **unchanged to 0.05 %** — the 2.15 M was the inlined battlefield scan, and the 16 bytes went into a struct `GameState::clone` copies 22,034 times. **Confirm the scan is the caller's own before front-ending it.** 🔎 Also measured: **`profiling-lines` is codegen-identical to `profiling-fast`** (1,514,412,257 vs 1,514,422,113 on `cube`, **0.0007 % apart**) and costs **8m19s cold**, not the ~28 min the candidates entry assumed.
