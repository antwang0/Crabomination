...
9. 💡 **AND THE CARD LEAD IS NOW UNBLOCKED, WHICH IS ITSELF THE FIND.** The target-deck defect list was filed as "eight cards, all in the `cube` pool, take them as one batch with one bench reading" — **two were fixed and neither moved `--bench` or a golden trace.** Pool *membership* is not pool *reach*: the bench plays `--decks fixed` and the traces are fixed-seed games, and a card has to be drawn and played in one to move a number. Five are left (Ghost Vacuum, Obstinate Baloth, Spark Double, Wall of Roots, Delver of Secrets) and they can go one or two at a time. ⚠ **Two of the eight rows were stale** (Simian Spirit Guide and Arcane Signet both already shipped) — read the definition before believing the row.
10. **Cards and decks, what is left:** ⏳ 32 of 33 "can be your commander" non-creatures are unimplemented — *cards are chosen to complete decks*, so they wait on a sixth target deck wanting one. ⏳ `audit_each_opponent`'s residuals are down to **six** (Malcolm closed: the subject-keyed batch makes "a Treasure for each opponent dealt damage" exact); none of the six is in a target deck. ⏳ **CARD_BACKLOG's "Target-deck card defects" table is the best card work left** — eight cards, Ghost Vacuum first (three decks, and `ExileLink` already exists). ⚠ It used to say the eight were blocked on pool membership; **that was wrong on the facts** and the table now says so.
