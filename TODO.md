...
11. **Perf queue: (J)/(K) CLOSED as diffuse, (L) = `(-344)`, (M) = `(-345)` + `(-346)` with its owed half refuted as `(-347)`, (A) = `(-348)`.** Open: 💡**(N) NEW AND THE BEST OF THEM — memoize `death_sweep_scope` on `Battlefield::writes` + `ContinuousEffects::writes`.** `check_state_based_actions_into` is 2.85 % of `cube` and diffuse, but its two largest named lines are `card_death_possible`'s five-field early-out run per permanent per priority pass, and its own doc has **three refutations of the fusion device** — the body is not the lever, the repeated ANSWER is. Key is ~6 Ir against a ~300 Ir walk, so break-even is a 2 % hit rate; ⚠ **the width is the risk — put the member BITMASK on `Battlefield` (the `attach_fold` / `trig_members` precedents), not a `SmallVec` on `GameState`, where `(-344)`'s 8 bytes cost 529 k Ir.** Census both numbers first. Then **(B)** the `CardCold` `Arc::ptr_eq` gate — ⚠⚠ **and it now needs `bench_ab.py` WALL CLOCK, not an Ir row**: one process-wide `Arc` puts every card's refcount on one cache line and `CowBox<CardCold>` is cloned 68,610 times a `cube` run, which callgrind is blind to; **(H)** the other zones' by-id scans. ❌ **(C) is CLOSED — its numbers were stale**: `(-333)` already took three quarters of `blocker_pair_block` (17,528 calls / 0.16 % now, against the 72,918 / 0.58 % the entry reasons from). ❌ `call_mut` at 1.23 % of `cube` is **not** a lead either: 98.6 % is std's own `Filter` fold, a no-LTO artifact. 📐 **ONE 16-SECOND `--dump-instr=yes` DUMP CLOSED FOUR MORE BODIES THIS RUN** (`blocker_self_block`, `simulate_attack_leaf`, `quality_band_assigner`, `dispatch_triggers_for_events_slow` — the program's largest self row at 5.01 %, diffuse over 258 lines). **Line-profile and re-size a candidate before building for it; both cost nothing.** ⚠ **Both (J)/(K) reads cost NOTHING:** `cg_lines.py` needs no `profiling-lines` build — one 16-second `--dump-instr=yes` callgrind line-profiles every body at once. **Line-profile a candidate before building for it.**
