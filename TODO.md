...
2. **Gates at this run's own-tree tip `c44cbbd7`:** suite **19,600 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); clippy **0**; `cargo check --profile release-fast` clean; sweep **6 cells / 29,600 games / 0 failures** at seeds 1416..1417 (`cap 0 / board 0 / stuck 0 / draw 4`), plus a second block at 1418..1419 on the rebased tip — **FRONTIER 1420**. `--bench` and its invariant: see PERF's Baseline, and read the `143980a7` warning above before comparing to it.
