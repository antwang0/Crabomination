...
2. **Gates at the tip, all re-run this run:** suite **19,549 / 0 / 5** (`CRAB_ANSWER_LOG=strict`), clippy **0** (`--workspace --all-targets --exclude crabomination_client`), golden traces unmoved, `cargo check --profile release-fast` clean, `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant with `determinism ok` and `thread_determinism ok (3 vs 1)`. ⚠⚠ **FOURTH BOX AGAIN, 4 cores, rustc 1.95.0, 444 games/s — no absolute in PERF is comparable across boxes; the counters are.** **The A/B base was RE-TAKEN here**: 624,133,866 / 1,657,007,991 / 1,732,783,859 (0.09-0.19 % under the last session's closing numbers — toolchain, not a change). A `profiling-fast` engine rebuild is **2m34s**, a three-pool callgrind ~6 min, a cold `release-fast` catalog **7 min**, `--release` (LTO) **22 min**.
3. **Perf — `(-311)` TOOK THE HEAD'S #1, the state-level gather memo: -0.911 / -1.490 / -0.517 % (fixed / cube / sealed), 42-46 % of every gather off the program.** `(-303)` had priced it at ~1.8-2.5 % and stopped on its key. ⚠⚠ **The blocker it stopped on did not exist**: `Player` is already a CoW handle, so its `DerefMut` is already the one route into a seat, and a per-seat `writes: u32` there covers the ~2,400 mutation-shaped sites `players_mut()` would have had to touch. `CardPile` and `ContinuousEffects` took the same counter beside the memo word they already clear. **A CoW handle is a write chokepoint; every group made copy-on-write for allocation reasons is a free epoch.**
4. **Perf — the rule the first cut leaves, and it is the transferable half.** The first cut read **-0.503 / -1.263 / -0.140 %** and the *whole* gap was overhead, not hit rate: (a) the evicted box was dropped rather than parked into `fx_pool` (21,488 `Arc::drop_slow`), (b) three mutex locks per hit where one does, (c) ⚠ **`Struct { field, ..Default::default() }` over a type with large inline storage is a COPY** — two extra `memcpy` calls per `GameState::clone`, 130,496 -> 191,264 on `sealed`. **A memo is priced by its entry point as much as by what it skips**: this one is ~150 Ir against a ~1,000 Ir gather and each of the three cost more than that.
5. ⚠⚠ **Bug — THE SWEEP CAUGHT A KEY COLLISION THE SUITE COULD NOT, IN SEVENTEEN SECONDS.** `(-311)`'s first key folded the counters into one `u64` opening `h = battlefield.writes(); h ^= continuous_effects.writes()` — **a raw XOR of two small counters is not injective in the pair** ((1,0) and (2,3) fold alike), and `cube` seed 1309 served a crewed Vehicle's `AddCardType(Creature)` off a memo stamped before the crew. The five scalars are stored and **compared** now (`GatherKey`); only the variable-length halves fold, each value spread across 64 bits first. ⚠ **The collision never moved the hit rate** (gather counts identical to four digits) — only the recompute-and-compare audit saw it. **A 64-bit fold is a probabilistic witness; anything gating game state wants a compared one.**
6. ❌ **Perf — `(-312)` BUILT the follow-on this line used to propose and it is +1.112 / +1.183 / +1.068 %. The `ComputedPermanent` memo lane is CLOSED; do not open a fourth.** The key was sound (`apply_layers_one_gated` is a free function over `&CardInstance` and `&[ContinuousEffect]`, so a `GatherKey` witnesses a computed permanent too). **The HIT is what fails**: `compute_permanent_pass`, 61.7 M / 3.78 % of `cube` over 258,722 passes, moved **-1.72 M, 2.8 %** — the key moves on every board write and the bot writes between every pair of asks, so `computed_permanent_hinted`'s 42 % in-scope rate is most of what there is. ⚠⚠ **And `end_of_scope`'s drain WAS `cp_pool`'s feed** (`(-163)`'s largest allocation site), which a *carried* entry cannot replace — it is shared with its probe and fails the pool's uniqueness check: `Arc::drop_slow` +7.91 M, allocator +10.7 M. **A memo whose entries a pool recycles is priced by the pool it starves.** `(-304)`, `(-309)`, `(-312)` all refuted in front of that row.
7. **Perf — the rest of the queue is unchanged and PERF's head carries it.** `(-309)` (id-compare family) and `(-307)` (combat dispatch) are CLOSED, do not re-open. The two sized-but-untaken rows are **`can_afford_in_state_with`** (3.49 % of `sealed`, **2.86 full cost re-derivations a call** across five callees at exactly 49,140 calls each — a per-decision memo, but price the key against `(-88)` first) and **`dispatch_triggers_for_events_slow`** (6.09 % of `sealed` self, line profile FLAT at 267 lines under 0.37 % — it needs the structural half, a CR 603.3b ordering question before a perf one).
8. **Seeds: frontier 1328 — THIS RUN TOOK 1308..1327 IN THREE BLOCKS: 60 cells / 296,000 games / 0 failures**, the largest block the record carries and deliberately so (`(-311)` is a memo over game state). 1308..1311 `cap 0 / board 0 / stuck 0 / draw 0`; 1312..1319 `cap 6 / draw 12`, **all six cleared at `CRAB_MAX_ACTIONS=50000`** (slow, not stuck — `cube`/`all` 1316, saturated-life); 1320..1327 `cap 0 / draw 2`. ⚠ **The block ran at the final tip on purpose and it is the reason this run shipped something sound**: the memo's audit only fires on a hit, and a hit needs a board the suite does not build. Say which block you are taking in this line *before* you take it. All standing audits 0: `audit_panics` 0 bare (70 sites, 59 guarded, 11 lock-poison), `audit_stash_in_loop` 0 unexplained, `audit_seat_from_selector` 0 open, `audit_target_walkers --check` 0, `audit_doc_drift` 0 body-wrong / 0 doc rot / 0 stale notes, `audit_variant_coverage` 0 dead capability (1 dead primitive, `GrantCastBackFromGraveyard`, unchanged).
9. **Best next moves.** (a) ⚠ **NOT the `ComputedPermanent` memo — line 6 closed it this run.** The largest untaken rows are `dispatch_triggers_for_events_slow` (4.64 % of `cube` / 6.09 % of `sealed` self, line profile FLAT over 267 lines, so it needs the structural half — merging `perform_action_inner`'s batches, a CR 603.3b ORDERING question before a perf one) and `compute_permanent_pass` itself (238 Ir of self per permanent, and `(-308)` has had one line out of it — the next one wants a `profiling-lines` dump, cold, and ⚠ `(-310)`'s rule about inlined frames). (b) `can_afford_in_state_with`'s 2.86 re-derivations (line 7). (c) ⚠ **Re-run the fresh-seed sweep as the FIRST gate after any memo or epoch change, not the last** — it is the only gate that reached `(-311)`'s collision, it costs 12 cells / ~9 minutes on a warm `target-audit/overflow` build, and the suite is not a substitute. (d) **`audit_dropped_may` reads 199, down from 223, and the 24 that went were one FALSE-POSITIVE CLASS, not 24 fixes**: `Effect::PutFromHandOntoBattlefield` carries its "may" in the *resolver* (`choose_up_to_cards`, min 0) and not in the definition, so the shape-reading script could not see it. The exemption is **conditioned on the engine and re-checked every run** — flip the helper name and all 24 come back. ⚠ **`LookPickToHand` is the next one and it is NOT safe at this granularity, measured**: its `take == 1` path rides `Decision::SearchLibrary` (declining available whatever `optional` says, 11 findings) but "you may put" on those same cards also matches the *rest* disposition (Bucolic Ranch, Break Out), which is a real choice — it needs the span matched to the pick, not to the variant. The remaining buckets: put 73, exile 20, return 16, collect 12, begin 12 (companion/Leyline, genuinely unmodelled). (e) `audit_keyword_drift`'s last 4 each need a PRIMITIVE (bestow / escalate / flashback / kicker, one card each). (f) The skips with a card behind them: `nocolors` **950**, `nosubtypes` 43, `noback` 60, `nokeywords` 29.
