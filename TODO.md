...
8. ❌❌ **Perf — the `release-fast` TRAP IS WIDER THAN THE FILE SAID, and `cg_calls.py`'s own docstring was WRONG. Refuted with NO BUILD SPENT; the script is corrected.** The trap was filed for "a non-generic `crabomination_base` leaf". It also covers **std generics whose body is a closure**: `LocalKey<T>::with` reads **148,514 calls / 0.67 % of `cube`** with `FnOnce::call_once` under it — two non-inlined calls per `cp_pool` access, the textbook restructure-the-call-site shape — and in `target/release/bot_ladder` the whole TLS access is inlined into `cp_pool::alloc` (115 instructions, **zero** `LocalKey`/`call_once` calls, 12 direct `%fs:` refs). ⚠⚠ **THE INSTRUMENT IS FREE AND NOBODY HAD USED IT THIS WAY: the `--bench` gate already builds the `release` binary, so `nm -C` + `objdump -d` the byte range settles "artifact or real" for any row without a build.** Do it before ranking a std generic or a small leaf. ⚠ Grep with `-P '\bcall\b'` — `'\tcall'` matches nothing and reads as "no calls", which is how this nearly became a row instead of a refutation.
