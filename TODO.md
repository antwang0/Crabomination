...
6. **Best next moves.** (a) **PERF's candidates head has a sized, unstarted lead**: the other half of the `cast_candidates` tree — 2.86 full cost re-derivations per affordability check (five callees at *exactly* 49,140 calls each), and 26.6 printed-requirement evaluations per target enumeration against a `first_legal_graveyard_card`-shaped short-circuit that already exists in the same module. Read what `cast_candidates`' two callers actually need (an enumeration or a witness) before building either. (b) **The class `(-295)` opened is only half taken**: `sac_other_filter` now reaches its priced owner, but `bounce_other_filter`, `exile_other_filter` and `discard_cost` have **no priced owner at all** — `ability_sink_bits` does not set `AB_SAC` for them and `pick_sacrifice_value` does not consider them. (c) The follow-up named in code: `pick_removal_sacrifice`'s favourable-trade test reads the SOURCE's power, so a destroy-shaped sacrifice-other ability has no trade math and `pick_removal_destroy` keeps the narrow gate on purpose. (d) The skips with a card behind them: `nocolors` **950**, `nosubtypes` 43, `noback` 60, `nokeywords` 29 — say which you are taking. (e) `audit_keyword_drift`'s last 4 each need a PRIMITIVE. (f) `audit_variant_coverage` is at ONE dead primitive, closed with a reason (`GrantCastBackFromGraveyard`); do not go looking.
