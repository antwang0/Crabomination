...
fn keldon_vandals_artifact_kill() {
    ...
    let mut ctx = crabomination::game::effects::EffectContext::for_trigger(vandals, 0, Some(Target::Permanent(mox)), 0);
...
fn cr_310_7_portent_tracker_battle_defense() {
    ...
    let base = {
        ...
        c.counter_count(crabomination::card::CounterType::Defense)
    };
    g.step = crabomination::game::TurnStep::PreCombatMain;
    g.active_player_idx = 0;
    g.priority.player_with_priority = 0;
    ...
    drain_stack(&mut g);
    assert_eq!(
        g.battlefield_find(siege).unwrap().counter_count(crabomination::card::CounterType::Defense),
...
fn cr_702_29b_stolen_echo_owed_by_new_controller() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_spell(1, Some(Target::Permanent(riders)), 0, 0);
    g.resolve_effect(
