...
2. **Where it stands:** FEATURE_ROADMAP's **"Commander status"** is the map and it is current — CR 903 core, the 800-series (**now incl. 800.4f/g and 800.4a's effect-elimination half**), the whole variant list. There is **no ⚠ row left** in the 800-series table. **Read that table, don't re-audit.** DECK_FEATURES: 5/5 decks complete.
3. **The runner:** `crabomination::pod` + `bot_ladder --commander [--seats N] [--games N] [--seed N]`. `pod::tests` validates every list every run. ⚠ The Partner deck is **last** on purpose: `--seats 5` is what reaches it.
4. 🔎🔎 **THE FIND, AND IT IS A METHOD: WHEN A DUMP AND AN AUTHORITY DISAGREE, CHECK WHAT THE DUMP IS NOT PRINTING.** The forty-second find ("an activation completed with no mana anywhere") was **the estimate, not the engine**: the pool held a Path of Ancestry *rider* pip and `ManaPool::total`/`amount` — which the blame string prints — exclude the restricted bucket wholesale. Two regression tests saying "the engine rejects this in a duel" made it look state-dependent and pointed at `stack=3`; it was not. One `eprintln!` of the **whole** `ManaPool` at the payment site named it in the first run. ENGINE_BACKLOG has it as FIXED.
5. 🔎🔎 **AND THE SECOND METHOD, WHICH FOUND TWO MORE: RUN THE ASSERTIONS BUILD OVER PODS.** `RUSTFLAGS="-C debug-assertions=yes" CARGO_TARGET_DIR=target-audit cargo build --profile overflow -p crabomination --bin bot_ladder` (Cargo.toml's recipe, ~8 min) then `--commander --seats N --games 1000`. ⚠⚠ **`rc=$?` after a pipe reads the PIPE's status, not the binary's** — an abort (134) reads as 0 and a whole sweep looks clean. Redirect to a file, then grep. It turned up the layer-5 colour gate on the first 1,000 games.
6. **This run shipped:** the rider-mana estimate fix (`SpendRestriction::allows` short-circuits on `is_rider()`, `ManaPool::rider_amount`, `available_mana`); the ranked auto-targeter it unblocked (`targeting.rs` → `default_hostile_opponent`, `first_alive_opponent_of` deleted); **CR 800.4f/g** (`game/departed.rs` — `route_ask` + `seat_prompts`, which replaced 102 hand-written `wants_ui` reads); **CR 800.4a** for a seat an *effect* eliminates (`Player::left_game`); the combat-damage trigger's subject binding (Phage and Entropic Specter had dead abilities); **CR 613.5**'s colour gate on the printed fast path.
7. ⚠⚠ **`opponents_of` DOES NOT FILTER THE ELIMINATED AND `Value::OpponentCount` READS IT — THIS IS THE BEST OPEN LEAD.** `mod.rs:5463`, 46 call sites. "Draw a card for each opponent" in a four-seat pod where two are dead draws **three**, not one. Two shapes among the callers: iteration/counting (clearly wrong today) and `opponents_of(x).contains(&y)` membership (a departed y controls nothing, so filtering is safe there too). Duel-safe by the usual argument. Do it in place, one answer, not an `opponents_alive_of` beside it.
8. **Deck balance is still the loudest number** and it did not move this run: seed 43, 4,000 four-seat games reads Sigarda 42.7 % / Tatyova 26.5 % / Hanna 23.1 % / **Judith 7.8 %**. The aristocrats list does not assemble; nobody has looked at *why* yet. `CRAB_CAP_DIAG=1` names an undecided pod game's board, but there are none — this is a losing deck, not a stalling one.
9. **What CR 800.4 still approximates, on purpose:** an ask **already materialized** when its seat leaves is dropped, not re-seated — the legal set would have to be re-derived against the post-departure board, which is a different capability. Rare now that `route_ask` stops one being posed to a departed seat in the first place. 800.4i's last-known-information clause is unmodelled.
10. **Cards and decks, what is left:** ⏳ 32 of 33 "can be your commander" non-creatures are unimplemented; the flag and validator are done, so each is a plain card. ⏳ `scripts/audit_each_opponent.py`'s seven residuals are in INCOMPLETE_CARDS with what each needs. ⏳ **Obscuring Haze** is the free-spell cycle's missing member and CARD_BACKLOG names its blocker: an *all-damage* fog filtered by **source**, where the engine has only a combat-only one and a single-target one.
11. ⚠ **The JoinForces/TemptingOffer latent bug** is a **loop** arm rebuilding no continuation: a suspend in iteration *k* silently drops *k+1..n*. Forbidden Ritual's arm (`effects/mod.rs` ~3965) and `SearchUpToN` show the splice; what blocks copying it is that each remaining iteration needs a *different* bound player and no `Effect` pins one. `ForEachOpponent` has **one** catalog card; price it before building it.
12. ⚠⚠ **Golden traces are the Commander guardrail too, and the test is POOL MEMBERSHIP.** With no commander seated every Commander change must be byte-identical in a duel. Every engine change this run is gated on "somebody has left the game" (which one elimination ends in a duel) or short-circuits to the single opponent — that is *why* the traces held, and the next one needs the same argument made out loud.
...
15. ⚠ **Four gotchas that each cost a debugging round.** ① A `ScriptedDecider` answers in ASK ORDER — to answer one question and leave the rest, write a four-line `Decider` matching that `Decision` variant and deferring to `AutoDecider`, and note `AutoDecider` **declines every `OptionalTrigger`**. ② A "target player" slot **auto-picks an OPPONENT** (`prefers_friendly_target` declares the shape). ③ `Selector::TriggerSource` binds the **damaged** player at resolution while a trigger's `filter` is evaluated against the **dealer** at gather time; twenty catalog cards rely on it and it is **not** a bug class. ④ New Commander cards live in `catalog/src/sets/cmdr.rs`, registered in `all_factories.rs` — free of the net vocab, but **`cargo check --workspace --all-targets` is the gate that sees the bins**.
