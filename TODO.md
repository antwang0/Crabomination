...
12. ⚠ **A name-keyed script that files a site by "the first string literal in the body" mis-files any factory that ends in `..creature("Name", …)`** — it picks up the *previous* factory's name (Longhorn Firebeast came back as "Gurzigost"). Key on the `fn` ident. Worth sweeping the other `scripts/audit_*.py` for the same shape.
