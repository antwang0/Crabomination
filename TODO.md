...
1. ⚠⚠ **TWO SESSIONS SHARE THIS BRANCH. FETCH BEFORE YOU START AND BEFORE EVERY PUSH, AND RE-RUN THE SUITE AFTER EVERY REBASE.** `cargo nextest` fail-fast hides the rest — use `--no-fail-fast`. A concurrent session landed the pile-pair fix *while this one was writing the same fix*; the collision cost a rebase and a reverted variant, so **check `git log origin/<branch>` for your item before you build it**. Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure"**.
...
3. **Gates at the closing tip:** suite **20,018 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); clippy **0** over the workspace (five warnings in the new `recent_b` batch tests fixed on the way past); `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean; `--bench` **195,806 / 27.49 / 611.9 / 0 stalls**, `determinism ok` — **byte-identical to the committed invariant at seven separate tips this run**, including the two that changed six `cube`/SOS pool cards and the three that changed only `pod/decks.rs`. Pod smoke at the tip: **3,000 games each at four and five seats (seed 43), 100 % decided, 0 stalls**; at **six** seats 2,000 at seed 43 and 3,000 at seed 99, **1 action cap each** — see item 9. Earlier in the run: the assertions sweep over **8,000 pod games at 2/3/4/5 seats, fresh seeds 2005-2006, 0 assertions**, plus the two-player grid on fixed/cube/sos/all at seed 2005 (4,080 games, clean). Ir guardrail, the clean single-commit A/B (`3964e1c5` → `a09f4704`): **fixed +0.025 / cube +0.030 / sealed +0.025 %** — all three together, a codegen row. **FRONTIER 3204.**
...
5. 📐📐 **THE METHOD GENERALISES AND IT IS THE BEST NEXT MOVE: ASK EVERY PRINTED-CLAUSE RATCHET THE OTHER WAY.** The family already has `audit_each_opponent`, `audit_dropped_may`, `audit_keyword_drift`, `audit_bottom_random`, `audit_target_walkers`. Each reads *print → body* in one direction; each has a mirror nobody has run. **A mirror needs three gates to stay precise**: the printed clause, the *absence* of the clause that would make the body right, and a needle in a **recipient** position (not a `Predicate::`, not `PlayersMayAccept`, not a `Gift` promisee).
...
11. **Cards and decks: 6/6 target decks complete**, and the two standing deck items are closed. ✅ **Board wipes**: Wrath of God + Fumigate (Sigarda), River's Rebuke (Hanna), Evacuation + Bane of Progress (Tatyova) — every deck can now break a stalled board, and the four-seat spread **tightens 25.1 → 23.6 points** (39.8/14.7/20.0/25.5 → **40.9/17.1/17.3/24.7**, seed 43, 3,000 games) at ~9 % longer games. ⚠ **The shape of the wipe is the finding**: Hanna took *Supreme Verdict* first and dropped 20.0 → 15.3 %, because a wide artifact board cannot afford a symmetric wrath — the same error the aristocrats experiment made in a different costume. ✅ **A sixth deck**: **Edgar Markov BRW**, the pod's first three-colour identity and first **Eminence** commander, appended after `pod_field(5)` so nothing existing is re-blessed; `--seats 6` reaches it. ⏳ 32 of 33 "can be your commander" non-creatures now wait on a *seventh* deck wanting one.
...
13. ✅ **THE ASSERTIONS BUILD IS THE CHEAP GATE, NOT THE EXPENSIVE ONE — 7m45s here, a third of a `release`.** `RUSTFLAGS="-C debug-assertions=yes" CARGO_TARGET_DIR=target-audit cargo build --profile overflow -p crabomination --bin bot_ladder`, then `RUST_MIN_STACK=33554432 …`. Taken at the closing tip: **4,000 pod games** (2/3/4/5 seats, seeds 4101-4102) and **4,080 two-player games** (`fixed`/`cube`/`sos`/`all`, seed 4101) — no panic, no assertion, no overflow, 100 % decided. ⚠⚠ `rc=$?` **AFTER A PIPE READS THE PIPE'S STATUS** — an abort (134) reads as 0. Redirect to a file, then grep.
