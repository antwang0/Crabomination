...
fn echoing_equation_copies_until_end_of_turn() {
    ...
    g.step = TurnStep::PreCombatMain;
    g.priority.player_with_priority = 0;
    crabomination::game::cast_at(&mut g, id, Target::Permanent(big));
...
fn torrent_sculptor_etb_exiles_is_and_grows() {
    ...
    assert!(s.has_keyword(&crabomination::card::Keyword::Ward(crabomination::card::WardCost::generic(2))), "has Ward 2");
}
...
fn blex_anthems_kin_and_gains_life_on_death() {
    ...
    crabomination::game::cast_at(&mut g, bolt, Target::Permanent(blex));
...
fn lisette_pumps_team_on_lifegain_when_paid() {
    ...
    g.step = TurnStep::PreCombatMain;
    g.priority.player_with_priority = 0;
    crabomination::game::cast_at(&mut g, vc, Target::Permanent(opp));
...
fn kianne_studies_top_card_land_to_hand_else_study_counter() {
    ...
    g.perform_action(GameAction::ActivateAbility {
        card_id: kianne, ability_index: 0, target: None, additional_targets: Vec::new(), x_value: None, mode: None,
    }).expect("Kianne study");
    drain_stack(&mut g);
...
fn kianne_fractal_counts_distinct_study_mana_values() {
    ...
    for c in [catalog::lightning_bolt(), catalog::grizzly_bears(), catalog::lightning_bolt()] {
        let id = g.next_id();
        let mut card = crabomination::card::CardInstance::new(id, c, 0);
        card.add_counters(CounterType::Study, 1);
        g.exile.push(card);
    }
    g.perform_action(GameAction::ActivateAbility {
        card_id: kianne, ability_index: 1, target: None, additional_targets: Vec::new(), x_value: None, mode: None,
...
fn imbraham_exiles_top_x_with_study_counters() {
    ...
    g.perform_action(GameAction::ActivateAbility {
        card_id: imb, ability_index: 0, target: None, additional_targets: Vec::new(), x_value: Some(2), mode: None,
    }).expect("Imbraham X=2");
    drain_stack(&mut g);
    for id in ids {
...
fn uvilda_hones_instant_then_makes_it_castable_for_four_less() {
    ...
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Discard(vec![behold]),
    ]));
    g.perform_action(GameAction::ActivateAbility {
        card_id: uvilda, ability_index: 0, target: None, additional_targets: Vec::new(), x_value: None, mode: None,
    }).expect("Uvilda hone");
    drain_stack(&mut g);
