...
7. **Perf — the sized queue is in PERF's "Perf candidates", top first:** (A) `ManaCost::cmc` — the filed "needs a sixth `CardMemo` word" blocker is WRONG, word 1 has bits 55-62 free, ~90 k of the 148 k calls want the *printed* cost; (B) `clear_end_of_turn_effects` 0.48/0.28/0.27 %, the 26-field guard IS the row, only a dirty bit is left and that is a `CardData` write to price first; (E) **`pick_blocks_inner` itself** — now 0.74 % self on `cube` after `(-336)` took 23 % off it, 2,582 Ir a call, still never line-profiled; ✅ (C)(F)(G) all taken. Still unread with `(-329)`'s checklist: `resolve_top_of_stack_inner` 0.81 %, `advance_step` 0.67 %, `cast_candidates` 1.06 %.
