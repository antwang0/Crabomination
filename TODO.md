...
8. ❌ **Perf — refuted this run with NO BUILD SPENT, and the method is the point: a battlefield-keyed lane in front of the continuous-effects gather.** `gather_continuous_effects_inner` is **2.31 / 1.93 / 2.06 % self**, the largest row in the program never to appear in a candidates list, and on `fixed` **88 % of it is its own O(board) opening walk** on decks that contribute no statics. But `LayerFreezeState::cross`'s own doc already filed the census: 48.0/49.2/48.9 % of gathers found the *board-and-seats* half of the key unchanged, and the shipped whole-key memo already takes 45.7/44.5/42.1 % off the program — so only ~6 % of the gathers that still reach the walk have an unchanged board. **A gather that misses the cross memo misses because the BOARD moved.** Worth ~0.08 %. ⚠ **Before building a memo in front of row X, look for an existing census of a memo keyed on a SUPERSET of X's inputs — its hit rate is an upper bound on yours.**
