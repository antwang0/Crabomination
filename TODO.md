...
1. **FIRST:** `git fetch origin claude/modern_decks && git checkout -B claude/modern_decks origin/claude/modern_decks`. Rebase, never force; code before tracker prose; **fetch before every push AND before you start**. ⚠⚠ **THREE SESSIONS RAN ON THIS BRANCH TODAY AND THE COLLISION COST NOTHING THIS TIME** — each fetched before pushing and rebased one commit. ⚠ But an ML session landed **`143980a7` (Round 76b, 731 lines of `server/bot.rs`)** after this run's last measurement, so **THE PILOT PLAYS DIFFERENT GAMES AND EVERY ABSOLUTE IN THIS FILE AND IN PERF'S BASELINE IS STALE: RE-TAKE THE BASE, DO NOT PREDICT IT.** A/B rows are unaffected (one tree per row). Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure" + "Standing rules"**, the 2026-09-12/13 detail is in **ENGINE_BACKLOG's "moved verbatim"** block. NEXT is 15 lines and stays 15.
2. **Gates at this run's own-tree tip `c44cbbd7`:** suite **19,600 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); clippy **0**; `cargo check --profile release-fast` clean; sweep **12 cells / 59,200 games / 0 failures** at seeds 1416..1419, two blocks (`cap 0 / board 0 / stuck 0 / draw 8`) — **FRONTIER 1420**; `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant at both tips, `determinism ok`, `thread_determinism ok (3 vs 1)`. 📐 **The invariant survived the ML session's pilot change because `--bench` is a `gang` mirror and Round 76b moved the SEARCH pilot — the bench does not cover `dflt`, and an ML change to it reads as "no change" here.**
3. 🔎🔎 **THE FIND: A "HAS THIS EVER BEEN WRITTEN" BIT BELONGS AT THE CoW GROUP'S ONE `&mut` ROUTE, NOT AT ITS FIELDS' WRITE SITES.** Candidate (B) sat open for two passes priced as a dirty bit over `CardData`'s 26 cleanup fields = **114 write sites**, a refactor with a rules-bug risk. Eleven of the 26 live in `CardCold`, which is *one* `CowBox` with *one* `DerefMut` — so `CardData::cold_written` is three lines, needs no write site touched, and `cold_pristine()` is authoritative for **all thirty-five** of that group's fields. **When a bit looks like N write sites, ask whether the fields share a CoW group.**
4. 📐 **ONE BYTE, THREE ROWS, AND THE THIRD WAS THE BIGGEST: `(-349)` -0.149/-0.098/-0.090, `(-350)` -0.064/-0.042/-0.034, `(-351)` -0.121/-0.137/-0.096.** The entry priced (B) at ~0.12 % of `cube` for one body; the bit was worth **0.28 %** across the cleanup guards, the keyword family and the layer pass, because what was built is *a fact about a card*, not a memo for a row. **Ask what else a bit answers before spending it on the row that suggested it.**
5. 💡 **THE OPEN HALF, AND IT IS THE BEST-PRICED LEAD ON THE QUEUE: only three families have taken `cold_pristine()`.** Unsurveyed hot readers of `CardCold`, by site count — `may_play_until` 49, `named_card` 39, `exiled_by` 26, `goaded_by` 22, `front_face`/`face_up_def` 37, `granted_activated_*` 34. **Census them the way (H) was censused** (grep the field, take the enclosing `fn`, join against a `cube` dump — `scripts/` has no script for it, it is ten lines of python and it found `compute_permanent_pass` in one run); 140 fns read a cold field and only 54 have a self row.
6. 📐 **A FLAT PROFILE MEANS RANK SHAPES, NOT BODIES.** Eleven bodies have been line-profiled and closed as diffuse and the largest single source line is 0.97 % — none of which stopped a device worth 0.30 % that touches five of them. **Ask what every body does that could be done once**, not which body is hot.
7. ⚠⚠ **THE BOX IS NOT THE CPU.** Same Xeon @ 2.80 GHz string, same core count, same tree — and the base read **+0.169 / +0.086 / +0.089 %** above the filed tip, which is *this file's own recorded twelfth-vs-eleventh step*. So: **a base that misses its prediction by an amount PERF has seen before is a box; a novel miss is a tree.** Never quote a row against a tip measured elsewhere.
8. 📐 **RUN-TO-RUN JITTER IS ±330 Ir ON AN IDENTICAL BINARY (±0.00002 %)** — two runs of one file read -232 / -330 / +161. The ±0.001 % floor this branch quotes is the cost of a **rebuild** of value-identical source, not of a re-run: **a suspicious row can be re-measured for free.**
9. 🔎 **THE THIRTY-SIXTH FIND IS CLOSED AND ITS SHAPE GENERALISES: a state-free walker that needs one board fact takes it as an ARGUMENT from the caller that already holds `&GameState`.** Temple Thief's `IsEnchanted` half was dead; `audit_block_restriction_filters_*`'s `KNOWN_DEAD` list is now **empty** (0 of 30 filters). ⚠ **And the follow-up is the rule worth more than the card: `(-352)` — when the ask rate is near zero and the answer is not free, pass a THUNK, not a value.** Answered eagerly the same fact cost +0.12 % of `cube` at a per-pair call site.
10. ⚠ **EVERY GATE THIS BRANCH LANDS CARRIES A RECOMPUTE-AND-COMPARE `debug_assert!` ON ITS CHEAP PATH** — and `(-349)` adds a second kind the compiler owns: `cold_membership_check` names `eot_wear_off_cold!`'s fields on a `&CardCold`, so mis-filing one is a **compile** error, not a runtime one. **Prefer the ratchet the compiler can hold.** The sweep is where the runtime ones fire, not the suite.
11. **Perf queue: ✅ (B) TAKEN as `(-349)`/`(-350)`/`(-351)`; (L) = `(-344)`, (M) = `(-345)`+`(-346)` (owed half refuted as `(-347)`), (A) = `(-348)`; (J)/(K)/(C)/(N)/(D)/(E)/(I) CLOSED.** Open: **(H)** the other zones' by-id scans — ⚠ its filed shape is refuted (a `FIND_HINTS` clone per pile is 512 bytes on `GameState`); **price the clone side first**. ❌ `call_mut` at 1.23 % of `cube` is not a lead (98.6 % is std's `Filter` fold, a no-LTO artifact). **UNREAD leads, both sized:** the allocation table — **755,903 allocations / ~149 M Ir = 9.9 % of `cube`** across `malloc`/`free`/`_int_*`, the biggest single thing left and known-diffuse since `(-90)`; and `Player::deref_mut` under `declare_attackers_banded`, 27,312 calls / 2.69 M inclusive / 98.6 Ir a call, the `(-280)` CoW family on the *seat*.
12. **THE GLOBAL LINE TABLE SAYS THE PROFILE IS FLAT — do not re-derive it.** Largest single source line **0.97 %** (`Arc::clone_from_ref_in`'s memcpy), then 0.80 / 0.56 / 0.41 %, and **12,537 lines hold 86.9 %**. Rank by **self per call**. ⚠ **`Battlefield`'s lane word has exactly ONE two-bit field left** (shift 62), with a `const` assertion in `zone.rs`; a thirty-third presence question needs a second word or a shared lane.
13. **This run's own-tree absolutes (`profiling-fast --no-default-features`, system allocator, FOURTEENTH box @ 2.80 GHz): base `a1ff92f3` 578,482,885 / 1,509,987,882 / 1,630,351,663 -> `c44cbbd7` 576,435,672 / 1,505,398,754 / 1,626,393,502, run total -0.354 / -0.304 / -0.243 %** with a rules fix among them that ended up paying for itself. ⚠ **Build times are CLOSED as a lever** — PERF's own measurement kills the file-split idea.
14. ⚠⚠ **FOUR TRAPS, ALL LIVE.** `cargo build --profile profiling-fast -p crabomination --bin bot_ladder --no-default-features` — **both flags load-bearing**; `nm <bin> | grep -cE " (T|t) (_)?mi_"` = 0 before filing a number. ⚠ **DO NOT EDIT THE TREE WHILE A BASE BINARY IS BUILDING.** ⚠ **Never overlap two sweep blocks on four cores.** ⚠⚠ **`cargo build --release` is NOT the ~10 min this file used to record — cold and contended it took over 35 minutes** (cgu 1 + thin LTO on the engine); start it before you need it. ⚠ **DISK reached 96 % mid-run with 1.9 GB left**: `target/debug/incremental` was 17 GB and `rm -rf` returned all of it; `find target/debug/deps -maxdepth 1 -type f -size +50M -mmin +60 -delete` returned 4 GB more and rebuilt nothing.
15. **Cards & rules, leftover only:** `audit_dropped_may` 199; `audit_keyword_drift`'s last 4 (bestow / escalate / flashback / kicker) each need a PRIMITIVE; card skips `nocolors` 950 / `noback` 60 / `nosubtypes` 43 / `nokeywords` 29; `audit_incomplete` 858 documented approximations. ✅ Temple Thief's `IsEnchanted` is DONE (see 9). ⚠ 605 `definition.is_*()` reads remain and **that is NOT a grep to run to the end** — most are on cards in zones where the printed answer is the only one there is.
