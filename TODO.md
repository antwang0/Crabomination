...
2. **Gates at the closing tip:** suite **20,260 / 0 / 7** (`CRAB_ANSWER_LOG=strict`); clippy **0**; `release-fast` check clean; `--bench` **195,806 / 27.49 / 611.9 / 0 — byte-identical** at the merged tip, `determinism ok`. **Pod, seed 9941, 1,000 games at each of 2..14 seats: 12,999 / 13,000 decided, zero panics**; turns/game 19.82 / 32.49 / 44.95 / 57.16 / 64.43 / 78.40 / 91.97 / 101.62 / 124.08 / 135.81 / 148.11 / 159.34 / 170.61. **`--card-census` at 14 seats: every card of all fourteen lists played, 672 distinct.** ⚠ **The 14-seat action cap: 0.1-0.3 % (seeds 9940/9941/9950) — every capped game read so far is a 292-314-turn ATTRITION game** (9-10 of 14 alive, lands-only boards, evenly spread priority passes), not a loop: `bot_ladder` now prints each undecided game's index and the longest game, and `CRAB_CAP_DIAG=1 --first I --games 1` replays one with a board dump. The longest game runs 1.7-2.8x the mean at every seat count; the 14-seat mean is 24,196 of 50,000. Cap left alone on purpose.
3. 🔎🔎 **THE FIND: OFFICIAL PRECONS ARE REACHABLE, AND THE FIELD NOW HAS TWO.** MTGJSON's deck files say how a precon splits (the Scryfall cache can't), and `scripts/precon_scan.py` ranks all ~200 Commander precons by cards the catalog lacks. **Sultai Arisen (TDC, Teval) was 99/100 already, Mind Flayarrrs (CLB, N'ghathrod) 93/100, Blood Rites (LCC, Clavileño) 91/100, Heads I Win (SLD, Zndrsplt/Okaun) 91/100.** All four are pod seats 11-14, card for card, appended so every committed reading below 11 seats is unchanged. Clavileño wins 27 % of 13-seat pods — Edgar's table-wide Vampire drain again, not a deck bug. ⚠ The scan's first draft undercounted by matching only `name:` literals; it now also matches factory slugs — **a card built by a helper (`land("…")`) has no `name:` field**.
