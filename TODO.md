...
1. ⚠⚠ **TWO SESSIONS SHARE THIS BRANCH. FETCH BEFORE YOU START AND BEFORE EVERY PUSH, AND RE-RUN THE SUITE AFTER EVERY REBASE.** `cargo nextest` fail-fast hides the rest — use `--no-fail-fast`. A concurrent session landed the pile-pair fix *while this one was writing the same fix*; the collision cost a rebase and a reverted variant, so **check `git log origin/<branch>` for your item before you build it**. ⚠⚠ **It happened again and it cost more: two sessions built the board wipes and two built a sixth deck, on the same afternoon, from the same DECK_FEATURES bullet.** The wipe work was *dropped* (the other side had measured a Supreme Verdict variant this one had declined to run, and its field was tighter); the deck work survived as a **seventh**, because appending never re-blesses. **Rule: a tracker bullet is a claim ticket — fetch and grep `git log origin/<branch> --oneline -20` for its words BEFORE the first edit, and prefer a shape that APPENDS to one that rewrites.** Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure"**.
2. ⚠⚠ **DO NOT EDIT THE TREE WHILE AN OPTIMIZED BUILD IS RUNNING — it cost two whole builds this session.** rustc reads a crate's sources at the start, so the artifact is from the pre-edit tree *and* cargo re-runs the chain. On this box (4 cores, 15 GB): `release` bot_ladder **23m24s** after a catalog change, `release-fast` ~13 min, the debug suite 95-102 s. **Batch every catalog edit, then build once.**
3. **Gates at the closing tip `85e702be`:** suite **20,032 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); clippy **0** over the workspace; `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean; `--bench` **195,806 / 27.49 / 611.9 / 0 stalls**, `determinism ok`, `thread_determinism ok (3 vs 1)` — **byte-identical to the committed invariant at every tip this run**, including the one that changed ten card definitions. Pod smoke: **21,000 games** on `release-fast` over 2..7 seats (seeds 5101 / 6201 / 7301, 3,000 apiece), **100 % decided, every `undecided_by` column zero**. Assertions sweep (`overflow` + `-C debug-assertions=yes`, 8m45s): **6,000 pod games** over 2/3/4/5/6/7 seats at seeds 8101-8102 plus the two-player grid on fixed/cube/sos/all — no panic, no assertion, no overflow, 12/12 blocks 100 % decided. ⚠ Item 9's six-seat action cap did **not** reproduce in 7,000 six-seat games here; it is ~1 in 2,000, not gone. **FRONTIER 8103.**
4. 🔎🔎 **THE FIND, AND IT IS A CLASS OF 126: A PRINTED "*TARGET* OPPONENT" MODELLED AS A FAN-OUT IS INVISIBLE IN A DUEL AND HITS THE WHOLE TABLE IN A POD.** `audit_each_opponent` only ever asked the other direction. `scripts/audit_target_opponent.py` reads **126 → 1** (Consumed by Greed, whose gift branch owns slot 0). Blood Artist drained 3 a death; Thoughtseize stripped every hand; Bojuka Bog exiled every graveyard. CARD_BACKLOG's **"TARGET-clause class"** and ENGINE_BACKLOG's **forty-fourth find** have the work and the traps.
5. ✅✅ **THE MIRROR METHOD IS TAKEN AND THE RIDER TABLE IS WHERE IT KEEPS PAYING — FOUR COLUMNS, 14 ROWS, 25 CARDS AND AN ENGINE BUG.** ENGINE_BACKLOG's own "CLOSED WITH A REASON 2026-09-12" said an invented-ability column needs a `have` set from the card's **OWN literal**, no helper expansion, no global table — that is why these read 0 where the `audit_oracle_verbs` inversion read 14,350. `audit_invented_may` **3→0**; `audit_invented_trigger` **45→8→0**; `audit_invented_rider` (a **table**, one line per rider) **14→0**, then 0 again at 14 rows over **1,248** bodies; `audit_synthesised_name` **27→0**. 📐📐 **THE RECIPE IS MECHANICAL: every `clause_ratchet` row in `core_rules/catalog_registration.rs` is one rider row, asked the other way.** Ten rows landed that way this run and named four cards — Cephalid Coliseum (three deviations under a doc calling one of them deliberate), Glimmerpost, Deep-Sea Kraken (uncounterable instead of unblockable, wrong suspend cost), Gleaming Overseer. **The ratchets that still have no mirror are the next ten lines of that table.** ⚠⚠ **AND THE OVERSEER'S CARD FIX DID NOT WORK, WHICH IS THE FORTY-SIXTH FIND**: `affected_from_requirement` returns `AllWithCreatureType` as soon as a creature type is named, and that variant has no `token` field — so `IsToken` was written, dropped, and the static stayed wide. Same for `color` / `colorless` / `owned_by_controller`, and for `AllOpponents` / `AllWithCounter`. It repaired **Eternal Skylord** with no card edit. **A dropped leaf widens; grep the catalog for the shape before calling it one card.** ⚠ **Reader traps, all general to a name-keyed audit**: `name:` is NOT where the catalog keeps the name (`..creature("Name", ..)`); the slug must fold accents and drop apostrophes; a tight phrase set manufactures findings (38 of the first 45) and **a KEYWORD is printed wording** (modular, prepared, "hexproof from" vs "can't be the target of"); a needle inside `HasKeyword(…)` is a **read, not a rider**; and a factory holds its TOKENS' text where the oracle does not (Roadkill Rodney) — bucket those.
6. ⚠⚠ **A "TARGET PLAYER" SLOT AIMS AT THE CASTER unless the seat's `hostile_player_targets` flag is on.** It is on in `EvalWeights::default()` (round 67) and **off in a bare `two_player_game()`** — 24 tests needed the flag, not a target. `player_slot_is_hostile` is the list of shapes whose polarity is not in doubt; a variant missing from it aims at the caster in bot play too.
7. ⚠⚠ **AND THE POLARITY CLASSIFIER READ `any` OVER A `Seq`.** Oildeep Gearhulk is `Seq[DiscardChosen(target player), Draw(that player)]`: the rider aimed at the *same* seat read as a gift, so the trigger aimed at its own controller and the Gearhulk made its **caster** discard. `friendliness_of_targeting_children` reads the **first** targeting child now. ⚠ The obvious alternative — a slot-aware picker — was written and **reverted**: on a modal body with `mode: None`, `slot_owner` resolves to the first *mode*, not the chosen one.
8. 📐 **THE TWO CATALOG-WIDE TARGET RATCHETS EARNED THEIR KEEP IN ONE RUN.** `every_declared_target_slot_is_answerable` and `cr_601_2c_every_catalog_target_filter_is_surfaced` named **six** variants with no walker arm for the player slot the new cards declare. A card that declares a slot no walker answers resolves against an **empty target list** — a silent, total fizzle with nothing logged.
9. 💡💡 **TWO NEW LEADS, BOTH FROM THE SIXTH DECK, AND THE FIRST IS THE BIGGER ONE.** ① **Eminence is worth roughly three seats' share to this bot**: Edgar Markov wins **52.1 %** of six-seat pods, and two rounds of nerfing proved it is the *commander*, not the list — cutting the seven best tribal payoffs took it **up** (61.6 → 64.0 %) and only cutting Vampire density moved it (64.0 → 52.1 %). A free 1/1 per Vampire spell, from the command zone, from turn one, no card spent and no tax paid; none of the other five commanders does anything from the command zone. Either the other five get command-zone value or six seats stays lopsided — **do not tune Edgar's 99 again, it has been tried twice**. ② **Six seats is the pod's only configuration with a stall**: ~0.03 % action-capped (1/2,000 at seed 43, 1/3,000 at seed 99) against **0/3,000 at four and five seats both**, and `CRAB_CAP_DIAG=1` names it — pod seed `11643393128411363082`, turn 29, 50,002 actions, the **Krark seat holding 3,990 floating mana** with three untapped permanents. A mana source the bot activates and cannot spend; start from that seed.
10. ✅ **`Effect::BindScratch` closed the last two loop-splice rows that wanted a primitive** (`Vote`'s `PerVote`, `RollDie`). `scripts/audit_loop_splice.py` reads **13/13/0/0** and grew a **staleness** half (an `ALLOW` entry whose site is gone now fails — it found three dead ones immediately) and a `scope_at` that reads a multi-line arm pattern. Four rows stay open and **two of them are catalog judgements, not code** (`SacrificeAnyNumber`, `MayPayRepeatedly`, `FlipCoinsChooseCount` — every body is choiceless today).
11. **Cards and decks: 7/7 target decks complete.** ✅ **A seventh deck: Freyalise, Llanowar's Fury** — mono-green, and the pod's only **non-creature** commander, so CR 903.3a's `can_be_commander` is piloted rather than only validated. Appended after `pod_field(6)`, nothing re-blessed, `--seats 7` reaches it. Seed 7301, 3,000 games at seven seats: 16.8 / 5.9 / 6.1 / 10.1 / 2.1 / **51.4** (Edgar) / **7.7** (Freyalise), 100 % decided. ⚠ **Read 7.7 % against item 9's Eminence finding, not against 1/7**: Edgar takes half the table, so that is 15.8 % of the non-Edgar remainder against a 16.7 % even share — mid-field. `scripts/pod_deck_candidates.py` is how a list is picked now (implemented factories ∩ identity subset ∩ Commander-legal; mono-green had 4,323). ⏳ 32 of 33 "can be your commander" non-creatures still unimplemented — an **eighth** deck no longer needs one, so they are ordinary card work. ⚠ Two pod tests found their deck with `last()`; appending broke one. Both search by **shape** now.
12. ⚠ **POOL MEMBERSHIP IS A MEASUREMENT, NOT A VETO — and the cheap pre-check is two deck lists.** `--bench` plays `--decks fixed` = `bot_ladder::archetypes()`, 38 vanilla/burn/one-clause cards, and `golden_trace.rs` has two more. **Not one of the 126 changed cards is in either**, which is why this run could predict the bench before it built it. ⚠⚠ **AND WHEN THE COMMITTED POD TABLE AND THE AGGREGATE DISAGREE, THE AGGREGATE WINS.** `cr_903_seeded_pod_outcomes_match_the_committed_table` is a re-bless gate over **three** games; read turns/game over thousands, and read the two-seat row as the control.
13. ✅ **THE ASSERTIONS BUILD IS THE CHEAP GATE, NOT THE EXPENSIVE ONE — 8m45s here, a third of a `release`.** `RUSTFLAGS="-C debug-assertions=yes" CARGO_TARGET_DIR=target-audit cargo build --profile overflow -p crabomination --bin bot_ladder`, then `RUST_MIN_STACK=33554432 …`. Taken at the closing tip: **6,000 pod games** over 2/3/4/5/6/**7** seats (seeds 8101-8102, 500 a block) and the two-player grid on `fixed`/`cube`/`sos`/`all` — no panic, no assertion, no overflow, every block 100 % decided. ⚠⚠ `rc=$?` **AFTER A PIPE READS THE PIPE'S STATUS** — an abort (134) reads as 0. Redirect to a file, then grep.
14. 📐 **VERIFY EVERY CR NUMBER AGAINST `MagicCompRules_20260417.txt` BEFORE COMMITTING IT.** ⚠ Two corrected this run: "whenever a player rolls one or more dice" is **not** CR 706.6 (that is *ignoring* a roll) — the ordering rule is CR 706.2 / 706.3a, the roll is over before the results table is consulted. FEATURE_ROADMAP's **"Commander status"** is the map and it is current; read it, don't re-audit.
15. ⚠ **Five gotchas that each cost a debugging round.** ① A `ScriptedDecider` answers in ASK ORDER — write a four-line `Decider` matching one `Decision` variant and deferring to `AutoDecider` instead, and note `AutoDecider` **declines every `OptionalTrigger`**. ② `Selector::TriggerSource` binds the **damaged** player at resolution while a trigger's `filter` reads the **dealer** at gather time. ③ New Commander cards go in `catalog/src/sets/cmdr.rs`; pool membership is an **explicit curated list** in `cube.rs`. ④ `add_card_to_battlefield` does **not** run `as_enters_effect`; cast it. ⑤ `resolve_answering` in `core_rules::multiplayer` is the harness for any suspend/resume assertion — `drain_stack` passes priority, which a pending decision blocks.
