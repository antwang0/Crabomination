...
token; Elemental Mascot's Opus branch exiles-top + grants may-play;
Tragedy Feaster's Ward—Discard and Prismari, the Inspiration's Ward—Pay
5 life are now enforced (CR 702.21). **Echocasting Symposium &
Improvisation Capstone promoted ⏳→✅** (both were already wired — stale
false-negatives in the audit; Improvisation Capstone's cost corrected to
{5}{R}{R}). See TODO.md for the full session breakdown (cards, CR
sections, engine/server/UI improvements).
...
| Echocasting Symposium | {4}{U}{U} | Sorcery — Lesson |  | Target player creates a token that's a copy of target creature you control. / Paradigm (Then exile this spell. After you first resolve a spell with this name, you may cast a copy of it from exile without paying its mana cost at the beginning of each of your first main phases.) | ✅ | Push (claude/modern_decks): fully wired. Echocasting Symposium uses `Effect::CreateTokenCopyOf` (copy target creature) + `RegisterParadigm`; Improvisation Capstone uses `TopOfLibraryUntilMvAtLeast` exile + `CastWithoutPayingImmediate` over `LastMoved` + `RegisterParadigm` (cost corrected to {5}{R}{R}). |
