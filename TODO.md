...
2. **Gates at the `d214cdae` tip, all re-run:** `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant, `determinism ok`, `thread_determinism ok (3 vs 1)`, `peak_rss 25.1`; suite **19,583 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, 12 golden traces unmoved); clippy **0** both feature sets; `cargo check --profile release-fast` clean. Sweep: **24 cells / 118,400 games / 0 failures** over seeds 1386..1393, **FRONTIER 1394**. `audit_stubs` 0, `audit_panics` 0 bare.
3. ⚠⚠ **`--bench` DID NOT MOVE ACROSS SIX BEHAVIOUR-TOUCHING RULES FIXES, AND THAT IS ABOUT THE POOL, NOT THE FIXES.** `--decks fixed` is mono-coloured with plain pips, no keyword grants and no keyword counters — it exercises none of what this run changed. **A rules fix that leaves `--bench` byte-identical has not been tested by `--bench`.** The suite and the fresh-seed sweep are the gates that reach them; run a sweep block after any rules change.
4. 🔎🔎 **THE METHOD THAT PAID THIS RUN, AND IT IS NOT EXHAUSTED: DIFF THE REQUIREMENT WALKERS AGAINST EACH OTHER.** There are **six** places a `SelectionRequirement` leaf gets answered — `eval.rs`'s `evaluate_requirement_static_hinted` (battlefield) and `evaluate_requirement_on_card` (off-battlefield), `eval.rs`'s `printed_requirement_impl`, `layers.rs`'s `requirement_matches_card`, `mod.rs`'s `blocker_matches_block_filter`, and `layers.rs`'s `AffectedPermanents::{All, AllOpponents, AllWithCreatureType, AllWithCounter}` field gates (which is where `affected_from_requirement` sends a *simple* filter, so a fix to the walker alone does not fix the anthem — the colour test caught exactly that). 116 arms are shared between the first two and **50 differ textually; ~45 are still unread.** The recipe: a 20-line `re` script that extracts each `R::` arm's body per line range, strips comments and whitespace, and prints the arms that disagree. **Six finds in one run, with every `audit_*` script clean.** ENGINE_BACKLOG's twenty-seventh through thirty-second finds are the write-ups.
5. **Bugs fixed this run, all one class:** (a) `has_toxic`/`has_modular` read printed + `granted_keywords_eot` only → `CardInstance::has_keyword_tag`. (b) `layers::requirement_matches_card`'s `HasKeyword`/`HasToxic`/`HasModular` read `definition.keywords` alone, so a keyword-filtered static missed every granted keyword. (c) six copies of the morph keyword set unified. (d) `WithAnyCounter` and `HasNoCounters` were **not complements** ten arms apart in one walker — a keyword counter (CR 122.1b) answered false to both. (e) `blocker_matches_block_filter`'s five *type* leaves read the print while its own doc said "computed" — a retyped creature could not block "except by Walls". (f) **seven** hand-written answers to "is this object green", the layer pass's walking `ManaSymbol::Colored(_)` alone, so a `{G/W}` pip and a token's colour indicator both read as colourless. (g) a two-value static filter ("Elf Warriors you control") collapsed to its last leaf. All seven have a regression test verified to FAIL on the pre-fix line.
6. ⚠⚠ **THE MIMALLOC TRAP COST THIS RUN A 7m29s BUILD AND IT LOOKS LIKE A WIN.** `cargo build --profile profiling-fast --bin bot_ladder` — **no `-p`** — keeps mimalloc and reads **-5.9 / -6.7 / -6.9 %**. Both `-p crabomination` AND `--no-default-features` are load-bearing. Check `nm <bin> | grep -cE " (T|t) (_)?mi_"` = 0 **before** you file a number.
7. **Tip absolutes at `d214cdae`, twelfth box (`profiling-fast`, system allocator): fixed 578,978,442 / cube 1,510,152,125 / sealed 1,632,364,300.** The run's whole movement is -0.035 / +0.019 / +0.026 % and is accounted for in PERF. The `012e0e21` opening set reproduced the eleventh box's to +0.09..0.20 % on a different CPU. 🔎 `cg_lines.py` needs **no** `profiling-lines` build: `.debug_line` survives unpacked split debuginfo, so the `(-342)` source-line grouping device is one `--dump-instr=yes` callgrind (**15 s**) plus 7 s of script.
8. **Perf — TWO CANDIDATES BUILT, MEASURED AND REVERTED. Read both refutations in PERF's candidates head before planning perf work; each closes a family.** (a) `(-303)`'s cross-scope key applied to `LayerFreezeState::perms`: sound, **+0.70 % uncapped on `cube`**. Carried hit rate **3.1 %** — a freeze scope opens on an unmoved board once in thirty-two — against the cross *gather* memo's 23.7 %, because a gather is asked once a scope and a pass once a permanent. And **`cp_pool` is worth ~40,000 allocations a six-game `cube` run** (5.3 % of 757,012); its feed IS the scope-end release, so the two devices are mutually exclusive. (b) `(-129)`'s outlining on `affected_includes_gated`: **+0.026 %**, prologue unchanged. **`out/call ≈ 0` is necessary but not sufficient** — the surviving arms wanted the registers themselves. Read the hot arms' disassembly before spending a build.
9. **The profile is FLAT and the candidates list is thin — say so rather than re-deriving it.** Top single line **0.97 %**; the `(-342)` grouping is all slice iteration and pointer machinery. `card.rs:13` (`CardId::eq`) is down to 17.18 M / 1.14 %. Allocation is 757,012 a `cube` run and **diffuse** (largest single `finish_grow` site is 7 % of regrowths). Open queue in PERF: (A) `ManaCost::cmc`, (B) the `CardCold` `Arc::ptr_eq` gate (~0.12 %, a base change at ~7 min a build), (C) `blocker_pair_block`'s pair count, (H) the other zones' by-id scans (censused; the obvious fix is wrong).
10. ⚠ **THE CONTAINER IS SUSPENDED BETWEEN TURNS — background waits DO NOT PROGRESS.** Run builds and A/Bs in the FOREGROUND with the 600 s tool timeout; when one overruns and is backgrounded, **re-issue the same `cargo build` in the foreground** — it blocks on the artifact lock and that block *is* the wait. ⚠⚠ **DISK hit 20 KB free twice and a `cargo check` died with `No space left on device` mid-query-cache.** `rm -rf target/debug/incremental target-audit && find target/debug/deps -maxdepth 1 -type f -size +50M -delete` returns 15-23 GB and rebuilds nothing. Do it **before** each optimized build.
11. **Box timings (twelfth box, 4 cores, 15 GB — roughly 2.5x the eleventh):** cold `profiling-fast` 7m29s, **engine-only 2m08s**, a `crabomination_base` change 7m20s, cold `release` 7m18s, `overflow` 6m58s, full suite 1m55s warm / 4m25s cold, workspace clippy 3m44s, `cargo check --profile release-fast` 1m01s, three-pool callgrind ~35 s in parallel, one `cube` callgrind 15 s, a 6-cell sweep block 4m10s.
12. **Cards & rules, leftover only:** `audit_dropped_may` 199; `audit_keyword_drift`'s last 4 (bestow / escalate / flashback / kicker) each need a PRIMITIVE; card skips `nocolors` 950 / `noback` 60 / `nosubtypes` 43 / `nokeywords` 29; `audit_incomplete` 858 documented approximations.
