...
fn doorkeeper_thrull_suppresses_artifact_etb() {
    ...
    let etb_fired = crate::game::actions::etb_trigger_multiplier(&g, 0, Some(sal));
...
fn sanctuary_wall_taps_and_stuns() {
    ...
    assert!(g.battlefield_find(enemy).unwrap().counter_count(crate::card::CounterType::Stun) > 0);
    assert!(g.battlefield_find(wall).unwrap().counter_count(crate::card::CounterType::Stun) > 0);
}
