...
1. **FIRST:** `git fetch origin claude/modern_decks && git checkout -B claude/modern_decks origin/claude/modern_decks`. Rebase, never force; code before tracker prose; **fetch before every push AND before you start**. Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure" + "Standing rules"**, ⚠ the 2026-09-12/13 detail (printed-body skip taxonomy, the `EventKind` fan-out census, the modal census, the collision log, the seed discipline) is in **ENGINE_BACKLOG's "The 2026-09-12/13 handoff detail, moved verbatim"** — read it before taking a skip kind or a seed. NEXT is 15 lines and stays 15.
2. **Gates — ✅ RE-RUN AT THE COMBINED `(-326)` TIP (both sessions' work together, which no single session had done): suite 19,549 / 0 / 5 (`CRAB_ANSWER_LOG=strict`), clippy 0 (`--workspace --all-targets --exclude crabomination_client`), golden traces unmoved, `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean.** ⚠ **`(-327)` landed AFTER that run and is covered only by its own commit's gates** — re-run the combined gate at the current tip before trusting it. ⚠ **TWO SESSIONS RAN ON THIS BRANCH TODAY AND ON DIFFERENT BOXES** — `(-320)`/`(-321)` the sixth box, `(-322)`..`(-326)` another; PERF's Baseline carries the box warning and every row on both sides is a same-box A/B, so **DELTAS carry and ABSOLUTES do not**. The per-tip gates each session ran: suite **19,549 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, nextest), clippy **0** (`--workspace --all-targets --exclude crabomination_client`, and again `--features trig-census`), golden traces unmoved, `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean, `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant. ⚠⚠ **SIXTH BOX, 4 cores, rustc 1.95.0 — no absolute in PERF is comparable across boxes; the counters and the Ir are.** The A/B base was re-taken here and reproduces the `(-319)` record to six sig figs (609,526,234 / 1,608,297,043 / 1,696,358,940 vs filed 609,523,889 / 1,608,296,018 / 1,696,358,147). Timings this box: cold catalog+engine `profiling-fast` **~8 min** (deps cached), warm engine-only **~2m30s**, `cargo check --profile release-fast` ~1m20s, a three-pool callgrind ~1 min, the debug suite **81 s to run**.
3. **Perf — ONE BIG ROW TAKEN, `(-320)`: `bot::legal_blockers` reads the printed creature gate before the layer view — fixed -2.338 / cube -1.446 / sealed -1.231 %, the row itself -68.7 / -55.5 / -55.0 %.** It asked `computed_permanent` about every permanent the seat controls; CR 509.1a's creature check reads the *computed* type line, but with no type change in scope that IS the printed line, so a printed non-creature never builds its view. Gated on `card_type_change_in_scope()` (the slot `death_sweep_scope`/`compute_battlefield_creatures` use), `debug_assert!` audits it. `compute_permanent_pass` calls fell 88,322/258,722/227,102 -> 67,412/226,544/191,598. Found the `(-315)`/`(-316)` way — the CALLEE table of a bot-spine row (`pick_blocks_inner` -> `legal_blockers` -> `computed_permanent_hinted`, 29.1 M cube).
4. **Perf — `(-321)`, the second row: the CR 509.1b tapped-block walk goes behind `PresenceGate::TappedBlock` — fixed -0.009 / cube -0.042 / sealed -0.021 %, `blocker_self_block` self -3.3 / -8.1 / -5.4 %.** ⚠⚠ **It has ZERO callgrind edges** — `tapped_creatures_can_block` inlines wholly into `blocker_self_block` — so no self / callee / call-count table could ever name it. It was found by *reading the source* of the function `(-320)` had just put in view. **That is the state most of the 285 `battlefield.iter().any(` sites are in: the profile says which FUNCTION to open, the walks inside have to be read, not ranked.** Two transferable rules together: (i) a whole-board walk reading the COMPUTED line only needs the PRINTED one when nothing can change it (`(-320)`, ask `card_type_change_in_scope()` once for the board); (ii) a seat-scoped walk's seat-INDEPENDENT superset is the memoizable half, and `false` there is authoritative for every seat (`(-321)`) — and the unscoped caller pays nothing, because the superset is the same iteration minus the seat compare.
5. ❌ **Perf — the `(-320)` gate on `permanent_value_with` is BUILT AND REFUTED (fixed +0.009 / cube -0.092 / sealed +0.144 %, flat/mixed, reverted).** The `supertype_change_in_scope()` primitive it needed is sound and was verified (two gather emitters: `AllNonlandPermanentsAreLegendary` + `ring_temptations>=1`), but it does not pay: in `eval_material` almost every valued non-land permanent IS a creature, so the artifacts/enchantments the gate skips are few, and the presence-slot reads + split call boundary cost what the skipped views save. `(-320)` won because `legal_blockers` walks the whole seat's board (mostly non-creatures); this walks a set already mostly creatures. **A printed-shape gate is priced by how much of its walk is the printed-negative case.** Do not rebuild without a different eval structure. See candidates.
6. 🆕 **Perf — A CONCURRENT SESSION TOOK `(-322)`..`(-327)` OFF THE SAME DEVICE AND THE BRANCH TIP IS `(-327)`: cumulative fixed -0.927 / cube -1.309 / sealed -0.851 % on its box.** `(-322)` the block pair gate (one walk per side, not nine; `blocker_pair_block` self -44 %), `(-323)` the block-tax note (`attack_block_keyword_tax` calls 65,610 -> 39,068 on cube), `(-324)` the combat-participation cap behind the board lane (row -87 %, ⚠ **lane shift 60 is spent, 62 is the LAST one**), `(-325)` the two zone walkers scanning push-only zones back to front, `(-326)` one shared lane walk for the two grant-source callers, `(-327)` the Soulbond pairing walk behind the gate-keyword lane (row -74/-82/-74 %). ⚠⚠ **`(-326)` AND `(-327)` BOTH LANDED WITH NO PERF RECORD** — their Log entries and Baseline rows are transcribed here from `d75575c6`/`cd25d07f`'s commit messages, marked as transcribed, not re-measured; **if that session files them itself, reconcile rather than leaving both.** **Combined with `(-320)`/`(-321)`, the two sessions took ~2.5-2.8 % off every pool today.**
7. **Perf — the three big self rows are STRUCTURAL/FLAT (line-profile verdict from the `(-319)` run):** `dispatch_triggers_for_events_slow` 4.65 % cube self (needs the CR 603.3b batch-merge, an ordering question first), `compute_permanent_pass` 3.84 % (238 Ir/permanent), `check_state_based_actions_into` 3.02 %. `available_mana` CLOSED (line 8 of the prior NEXT). The allocation census is flat (759,943 `__rust_alloc`, top grower 7 % of growths, no `with_capacity` moves a pool).
8. ❌ **Perf — sized and DECLINED, so nobody spends a build:** `main_phase_action_with`'s `has_magecraft`/`has_opus` (0.02-0.05 % the pair, and the mana word has no free bits). `ComputedPermanent` memo lane CLOSED (`(-312)`), as are `(-309)`, `(-307)`, the target enumerator, and `available_mana` (`e8308d53`/`(-317)`). Do not re-open without a device none of the three prior ones was.
9. **Seeds: TWO blocks this run, 24 cells / 118,400 games / 0 failures, `cap 0 / board 0 / stuck 0 / draw 0` on both. FRONTIER 1354.** ⚠ **The prior NEXT said "frontier 1342" and that was STALE — PERF's Baseline is the authority and it said 1350.** 1342..1345 was therefore a RE-TAKE (last swept at the `(-318)` tip); it is still the block that audits `(-320)`'s new `debug_assert!` on dflt-pilot boards the suite never builds, and 1350..1353 is the frontier block. Both on `target-audit/overflow` with `-C debug-assertions=yes` and `CRAB_ANSWER_LOG=strict`. **Read the frontier out of PERF's Baseline, not out of this line.** Standing audits re-run at the start of the run and all 0 (`audit_panics` 0 bare / 70 sites, `audit_engine_stack` 0, `audit_stash_in_loop` 0 unexplained, `audit_seat_from_selector` 0 open, `audit_target_walkers --check` 0, `audit_doc_drift` 0/0/0).
10. **Bugs/robustness: nothing open.** Structural audits (`audit_incomplete` 1 finding = Elite Interceptor, triaged not-a-gap; `audit_stubs` 0), determinism, panic census all clean. TODO's robustness section has no open entries.
11. **Best next moves, in order.** (a) ⚠ **The combat/block functions this line used to point at are NOW TAKEN — `blocker_pair_block`, `attack_block_keyword_tax` and `combat_participation_cap` are `(-322)`/`(-323)`/`(-324)`. Do not re-open them.** The device still works and is the branch's best one; apply it to a function the two sessions have NOT opened, and **price the population first** (line 5: a printed-shape gate is worth only its printed-negative fraction). ⚠ **Only lane shift 62 is left**, so the next lane is the last one — spend it on a walk sized first, not the next one found. (b) The fresh-seed sweep as the FIRST gate after any epoch/memo change — frontier 1354, **read it from PERF's Baseline, not from this line** (the previous NEXT's frontier was stale by 8). (c) The three structural rows (line 7) are a count/structure question, not a line. (d) leftover context: `audit_dropped_may` 199 (`LookPickToHand` not safe at this granularity — put 73, exile 20, return 16, collect 12, begin 12); `audit_keyword_drift`'s last 4 each need a PRIMITIVE; card skips with a card behind them: `nocolors` 950, `noback` 60, `nosubtypes` 43, `nokeywords` 29.
