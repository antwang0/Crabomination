...
5. **This run shipped, in order:** CR 601.2f alt-cost casting from the command zone (`cast_spell_alternative_from` over `AltCastZone`, tax included, bot offers both casts); CR 113.6b triggers (`EventSpec.zone`) and statics (`CardDefinition.statics_in_command_zone`) that function from the command zone; CR 702.49d commander ninjutsu plus CR 602.2b's auto-tap and the bot's first-ever `GameAction::Ninjutsu` candidate; CR 207.2c join forces (`Effect::JoinForces` over `ask_seat_amount`); eight cards in `sets/cmdr.rs` (Edgar Markov, Oloro, Arahbo, Yuriko, The Ur-Dragon, Minds Aglow, Collective Voyage, Mana-Charged Dragon).
...
11. 📐 **A "target player" slot auto-picks an OPPONENT by default**, so a targeted gift (the Partner-with fetch) silently went to the wrong seat and looked like "nothing happened". `prefers_friendly_target` is where the shape is declared. **A test that asserts only "something moved" cannot see this** — assert the seat.
...
14. ⚠ **`Selector::TriggerSource` binds the damaged PLAYER at resolution on a dealer-side damage kind**, but a trigger's own `filter` is evaluated against the dealer at gather time, so both `with_filter(EntityMatches{TriggerSource, …})` and `dealt_by(…)` gate correctly on it. Twenty catalog cards use the first; it is not a bug class. Verified by flipping Yuriko's filter to a type she doesn't have and watching the trigger stop firing.
