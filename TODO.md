...
1. ⚠⚠ **TWO SESSIONS SHARE THIS BRANCH. FETCH BEFORE YOU START AND BEFORE EVERY PUSH.** Rebase, never force. ⚠ **DISK: this box hit 96 % mid-run** — `rm -rf target/debug/incremental` and `find target/debug/deps -maxdepth 1 -type f -size +50M -mmin +20 -delete` before any optimized build (26 GB came back). Box: 2.10 GHz Xeon, 4 cores; `release-fast` `bot_ladder` **7m35s**, full suite **119-128 s**, workspace clippy ~2.5 min. Gotchas in **CLAUDE.md**; wait on a **pid**, never `pgrep -f`.
2. **Gates at `b38f1065` (the Commander routine, 2026-09-23):** suite **20,429 / 0 / 7**; clippy **0**; `release-fast` builds; `--bench` **195,806 / 27.49 / 611.9 / 0 stalls — byte-identical**, `determinism ok`, also after the CR 511.3 combat fix; 2-player cube/sos/sealed seed 10061 × 300: 7,499 / 7,500 decided (one rules draw). **Pod seed 10062 × 300 at 2/4/6/8/12/16 seats: all decided, zero panics**; 25 seats (seed 10051): 289 / 300 decided, 10 action caps, 1 board cap — **not a loop**: Gravemother's encore at 24 opponents minted 21 Brood Slivers, 813 tokens in one combat (DECK_FEATURES). ⚠ Older readings in item 8 predate seats 22-25.
3. 🔎🔎 **THE FIND: OFFICIAL PRECONS ARE REACHABLE, AND THE FIELD NOW HAS FIVE.** MTGJSON's deck files say how a precon splits (the Scryfall cache can't), and `scripts/precon_scan.py` ranks all ~200 Commander precons by cards the catalog lacks. **Sultai Arisen (TDC, Teval) was 99/100 already, Mind Flayarrrs (CLB, N'ghathrod) 93/100, Blood Rites (LCC, Clavileño) 91/100, Heads I Win (SLD, Zndrsplt/Okaun) 91/100, Goblin Storm (SLD, Zada) 90/100.** All five are pod seats 11-15, card for card, appended so every committed reading below 11 seats is unchanged. ⚠⚠ **Goblin Storm's first 15-seat run found CR 508.4 broken engine-wide**: every put-onto-the-battlefield-attacking site emitted `AttackerDeclared`, so General Kreat's attack trigger fed on its own tokens (922 Goblins by turn 57; 8 board caps in 1,000) — `game/enter_attacking.rs` now funnels all six. **A board cap is a loop until proven otherwise; an action cap has so far always been attrition.** Clavileño wins 27 % of 13-seat pods — Edgar's table-wide Vampire drain again, not a deck bug. ⚠ The scan's first draft undercounted by matching only `name:` literals; it now also matches factory slugs — **a card built by a helper (`land("…")`) has no `name:` field**.
4. ✅ **Seven primitives, each built for the class, not the card:** `SpendRestriction::SpellFromGraveyard` + `SpellKind::from_graveyard` on every graveyard-cast path (CR 106.6/601.2a — retrace, escape and aftermath used to pay with an EMPTY kind, so no restricted mana could fund them at all); `ControlledHaveAbilitiesOfExiledWithSource` (Steward); `GraveyardCardsHaveEscapeMatching.once_per_turn` (Kotis); `game::could_produce` (CR 106.7 — Fellwar/Orchard read only basic TYPES and made {C} from nothing); `Effect::ExileIfLeavesBattlefield` (CR 614 — fixed Geth, Whip of Erebos, Gruesome Encore, Greenwidow too); `AdditionalCastCost::OneOf` (CR 601.2b); `TriggerZone::WhileSuspended` (CR 702.62b); `Player::milled_ids_this_turn` (+ `PutIntoGraveyardFromLibraryThisTurn`, fixes the pod commander N'ghathrod, The Weaver King, Welcome the Dead); `Effect::ExileLinkedTo` (Timothar's Bat); the emblem matcher now delegates to `event_kind_matches` (it kept its own ten-kind whitelist, so Daretti's emblem was dead).
5. 🔎 **Three latent engine bugs the precon cards exposed:** ⚠⚠ **persist and undying read only PRINTED keywords** (three sites), so every grant — Undying Evil, Mikaeus, Haunted One, Dusk Legion Sergeant — showed the keyword and did nothing; Undying Evil's test asserted only that the keyword appeared. `SharesCreatureTypeWithSource` read only a *departed* source's LKI, so it was false for every living source; `DynamicPt::ChosenPlayerGraveyardMatching` never scaled toughness (a 0/0 Sewer Nemesis died on entry). **A new card is the cheapest probe of an old primitive** — both were found by cards that "only needed wiring".
6. ⚠ **`GameState` is size-capped (`cow::tests::game_state_stays_small`, 1,680 bytes) and a lone `Option<CardId>` field broke it.** Folding it into the existing library-top marker as `casting_hop: Option<(CardId, HopFrom)>` cost zero bytes: the tag sits in the `Option`'s padding. Look for a slot to share before adding one.
7. 💡 **THE OPEN QUEUE.** ① **Seats 16-25 are official precons**; this run landed **Sliver Swarm (CMM, seat 23)**, **Vampiric Bloodlust (C17, seat 25 — a second Edgar)** **Guided by Nature (C14, seat 27)** and **Grave Danger (SCD, seat 29)**; the other session landed Jeskai Striker (22), Quick Draw (24) and **Calling All Angels (FDC, seat 26 — Giada, the first mono-white seat, no residuals)**. Residuals in DECK_FEATURES: Descendants' Fury (any attacker that hit a player this turn), Bloodlord (bloodthirst checked at the cast trigger), Mathas (grant lasts while Mathas stays), Kheru (exiles face up), plus the older list (Colossal Grave-Reaver, Lethal Scheme, Grell, Steward/Loam, Sewer Nemesis, Whispersteel Dagger, Shiny Impetus, Tempest Technique). **Guided by Nature (C14) LANDED as seat 27.** **Grave Danger (SCD, Gisa and Geralf) LANDED as seat 29.** **Swell the Host (C15, Ezuri) LANDED as seat 32.** **Angels: They're Just Like Us (SLD, Gisela — the first melded commander) LANDED as seat 33.** **Built From Scratch (C14, Daretti) LANDED as seat 34.** **Vampiric Bloodline (VOC, Strefan) LANDED as seat 36.** **Plunder the Graves (C15, Meren of Clan Nel Toth) LANDED as seat 37** (no residuals: Wretched Confluence chooses its modes at cast). **The pod budget counts plays, not priority passes** (`PLAYS_PER_SEAT = 1,000`, `PASSES_PER_PLAY_BACKSTOP = 200`): passes grow with seats² and were ~90 % of actions, so wide pods capped on passing alone. **Animated Army (BLC, Bello — the first Gruul seat) LANDED as seat 28** (residuals: Evercoat Ursine can't play a hidden land, Grothama's fight offer is its own trigger, Tendershoot's ascend is ETB+upkeep). **Graveyard Overdrive (M3C, Disa the Restless — the first Jund seat) LANDED as seat 31** (residuals: Disa's "not from the battlefield" reads "not put there from the battlefield this turn", Find picks at resolution, Ziatora sacrifices the weakest other creature). **Sworn to Darkness (C14, Ob Nixilis of the Black Oath) LANDED as seat 35** (residuals: Infernal Offering's opponents are the engine's pick and returns take the first creature card; Profane Command's modes are resolution-time picks). It found `Drain`'s "life lost this way" class (nine cards, CR 119.3), the self-copied Destroy, emblem-granted abilities, and two no-bot-path cards (Tempt with Mayhem, Wake the Dead — both now cast). **Quantum Quandrix (C21, Adrix and Nev) LANDED as seat 39** (residuals: Esix copies the engine's pick, Primal Empathy's counter placement, Ruxa's printed "no abilities"); it found eight statics dropped by the layer router (ENGINE_BACKLOG). **Fae Dominion (WOC, Tegwyll) LANDED as seat 45** (residuals: Blightwing's face-up exile, Halo Forager's MV 0, Illusionist's Gambit's turn-long grants, Puppeteer Clique's end step). **⚠ Entropic Uprising (C16, Yidris, Maelstrom Wielder, 17) is CLAIMED by session `01StaUuo` (2026-09-24 run 5) — don't duplicate.** **⚠ Endless Punishment (DSC, Valgavoth, Harrower of Souls, 15) is CLAIMED by session `01StaUuo` (2026-09-24 run 5) — don't duplicate.** **Invent Superiority (C16, Breya — the first WUBR seat) LANDED as seat 48** (residual: Armory Automaton takes only your Equipment). **Lorehold Legacies (C21, Osgir) LANDED as seat 43** (residuals: Archaeomancer's Map's land comparison, Key to the City's "up to one", Laelia's library-exile event); it found the X-naming graveyard-exile cost bug (ENGINE_BACKLOG). **Forged in Stone (C14, Nahiri) LANDED as seat 30** (residuals: Arcane Lighthouse strips once, Benevolent Offering's opponent is picked, Nahiri takes your first Equipment). **Seize Control (C15, Mizzix) LANDED as seat 38** (residual: Mystic Confluence runs its default picks). **Rebellion Rising (ONC, Neyali) LANDED as seat 41** (residuals: Collective Effort's escalate at resolution, Goldwardens' Gambit auto-picks unattached Equipment, Neyali counts token attacks on planeswalkers). **⚠ Arm for Battle (CMR, Wyleth) was claimed twice (session `01P5KQkY` at 06:05, `01546QdT` at 06:09) and LANDED as seat 47 from `01P5KQkY` — its cards are in `decks/cmdr_wyleth.rs`; don't duplicate.** **Devour for Power (CMD, The Mimeoplasm) LANDED as seat 58** (residuals: Mimeoplasm's picks, Desecrator Hag's tie). **⚠ Open Hostility (C16, Saskia the Unyielding, 16) is CLAIMED by the `modern_decks` Commander routine (2026-09-24 run 4, session `01XnuL2a`) — don't duplicate.** **Political Puppets (CMD, Zedruu) LANDED as seat 55** (Trade Secrets, banned, swapped for Divination). **Heavenly Inferno (CMD, Kaalia) LANDED as seat 53**. **Chaos Incarnate (SCD, Kardur) LANDED as seat 49** (it found CR 603.10 broken for "whenever an attacking creature dies"). **Wade into Battle (C15, Kalemne) LANDED as seat 46** (its debug pod found the Fool's Demise re-attach loop, CR 704.5m). **Peer Through Time (C14, Teferi — the fifth planeswalker commander) LANDED as seat 42** (it found CR 601.2c broken: a fixed multi-target count wasn't a minimum — Hex took five; residuals in DECK_FEATURES). **⚠ Breed Lethality (C16, Atraxa, Praetors' Voice, 17) is CLAIMED by the `modern_decks` Commander routine (session `01546QdT`, 2026-09-24 10:40) — don't duplicate.** **Primal Genesis (C19, Ghired) LANDED as seat 56** (residuals: Cliffside Rescuer's protection is from what opponents control, Tahngarth attacks its borrower's default opponent). **Feline Ferocity (C17, Arahbo) LANDED as seat 52** (residuals: Divine Reckoning keeps the highest-MV creature, Stalking Leonin's opponent is picked openly). **⚠ Mirror Mastery (CMD, Riku of Two Reflections, 16) is CLAIMED by the `modern_decks` Commander routine (session `011A87o2`, 2026-09-24 10:00) — don't duplicate.** **⚠ Token Triumph (SCD, Emmara, Soul of the Accord, 15) is CLAIMED by the `modern_decks` Commander routine (2026-09-24 run 2, session `01P5KQkY`) — don't duplicate.** **Stalwart Unity (C16, Kynaios and Tiro) LANDED as seat 57** (four residuals in INCOMPLETE_CARDS; Collective Voyage fixed on the way — it ramped one seat). **Land's Wrath (ZNC, Obuun) LANDED as seat 54** (residuals: Scaretiller's mode, Mending's regrow pick, Trove Warden returns on any leave). **The bot now casts choose-two/choose-three spells** (`mode_combinations`), and Righteous/Wretched Confluence are cast-time modes; **a pod seat cycles** at an opponent's end step. **Eternal Might (DRC, Temmet; Hashaton in the 99) LANDED as seat 50** (residuals: the 4/4 Zombie copies add the type, the Gift and Rot Hulk auto-pick). **Arm for Battle (CMR, Wyleth) LANDED as seat 47** (CR 205.4e legendary sorceries now enforced; its two residuals — Dawn Charm's counter filter, Timely Ward's flash condition — closed after). **Counterpunch (CMD, Ghave, Guru of Spores) LANDED as seat 44** (residual: Footbottom Feast picks at resolution). **CR 601.2c: a required target nothing could fill now rejects the cast** (`game/required_target.rs`; optional slots, stack filters and post-cast mode choices stay out). ⚠ Still open: a cast with `target: None` while a legal target EXISTS is accepted and resolves as a no-op (Doom Blade) — the client/bot always aim, but the engine doesn't auto-aim at resolution. **Call the Spirits (C15, Daxos the Returned) LANDED as seat 40** (residuals: Righteous Confluence offers its non-targeting mode combinations as one choice; Sandstone Oracle's opponent is the one with the most cards in hand). **Next lists by `precon_scan.py --top 45` (the 0-missing rows crowd the default 30): Peer Through Time (17); Lorehold Legacies, Growing Threat, Enduring Enchantments, Fae Dominion, Eternal Might (18)** · Condemn (Nahiri) is unplayed at 31 and 32 seats — claim in this line before starting. ② ⚠ **CR 511.3 was broken engine-wide until this run**: combat was torn down as regular damage was dealt; now `remove_all_from_combat` runs as the end of combat step ends — re-check any card whose post-damage or end-of-combat body was written around the old timing. ③ `Effect::ExileFromHand` asks a bot seat now; a prompting seat still takes hand order (UI picker open). ④ **Bots never emit 6 cast variants** (ENGINE_BACKLOG). ⑤ `AdditionalCastCost::OneOf` picks for a human too. ⑥ The older queue: per-slot trick evaluation, `audit_dropped_may`, the invented-target census.
8. ✅ **The residual session (2026-09-23/24), gates at the tip:** suite **20,709 / 0 / 7**; clippy **0**; `--bench` **195,806 / 0 stalls — byte-identical after every change**; cube/sos/sealed **0 undecided**; each pod re-bless's 3,000-game 4-seat aggregate within noise; **24-seat `--card-census`: every card of all 24 lists played**. **Debug pods (`CRAB_ANSWER_LOG=strict`) found three bugs on their first seed (25 seats, 9201) — now 24/24 decided, clean.** Bug CLASSES closed (ENGINE_BACKLOG "FIXED 2026-09-23 (the residual session)"): ① **costs spelled as effects — 46 cards** paid a printed sacrifice / discard / life cost at resolution (a creatureless Fling, a handless Wild Guess, an X-above-life Toxic Deluge were castable) → `AdditionalCastCost` / `sac_other_filter` + `PayLifeX`, gated by `no_cost_sacrifice_is_spelled_as_the_first_step_of_the_effect`; ② **CR 800.4a: a departed player was a legal target**, and `MayDoBy`'s re-seated replay leaked its answer; ③ **power sums overflowed** on a saturated power (block planner) — the audit's look-back catches `.sum()` chains, planner facts clamp to `SCALE_CEILING`; ④ **an X spell's targets were picked with no X** — Finale of Promise was never cast; ⑤ earlier: cast origin, modal triggers, spend-restricted mana. Primitives: `spend_only_chosen_color` (Throne of Eldraine), `PhyrexianPipForSpells` (Defiler), `SpellKind.kindred` + `CreatureSpellOfType` + `CreatureOfTypeOrItsAbility` (Kindred mana, Secluded Courtyard), `PayLifeX`. `bot_ladder --pod-decks I,J,..` seats chosen decks. `sac_other_may_be_source` lets 38 "Sacrifice a creature:" creatures pay with themselves (Viscera Seer). Open: Grab the Prize (cost-time discard record), Devouring Greed, Whispersteel Dagger (one-of-them budget), Lethal Scheme (convokers).
9. 🔎 **Run debug pods, not just release ones: `CRAB_ANSWER_LOG=strict target/debug/bot_ladder --commander --seats 21 --games 24 --seed N`** (~6 min a seed). Release pods decided 997/1,000 at 21 seats while the debug pod aborted on its first seed — `debug_assert!` guards are where the pod bugs are. Found and fixed this way (ENGINE_BACKLOG "the Commander precon run"): ① single-player values/predicates handed `EachOpponent` read one seat — **10 cards**, now a whole-catalog test (`cr_800_4_no_single_player_value_or_predicate_reads_each_opponent`) plus `Predicate::ForAnyPlayer` for "if an opponent …"; ② **six per-player loops resumed a parked body unbound** (`ForEachOpponent`, Punisher, VillainousChoice, TemptingOffer, EachPlayerSacrificesUnlessDiscards, EachPlayerFlipsCoin) — Geth's Summons leaked its answer; ③ the bot aimed a goaded attacker at its goader, and dropped a must-attacker for a tax only ONE seat charged (`pod_attack::retarget_taxed_attacks`): 14 rejected declarations in 40 eighteen-seat games → 0; ④ `ControllerDealtCombatDamage` dropped its filter and batch slot.
10. **The rules table is still DONE** (FEATURE_ROADMAP's Commander and 800-series sections, bar CR 800.4i and the N-seat encoder). What is left is **precons and the residuals they surface** — and `precon_scan.py` + `--card-census` are the two instruments. `CR_COVERAGE.md` is generated: re-run `scripts/cr_coverage.py` (146/146).
