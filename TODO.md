...
3. **Gates at the closing tip `71bf6a71`:** suite **20,040 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved) taken one commit before the second rebase, and **re-running at the rebased tip**; `modern` + `classic_sets` **9,702 / 9,702** after the retarget commit; clippy **0** over the workspace; `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean; `audit_panics` **0 bare**; every mirror column 0 (`invented_rider` / `invented_trigger` / `invented_may`, `loop_splice` 13/13/0/0, `each_player` 1-allowlisted, `target_opponent` 1 known) and two **new** ones at 0 — `audit_seat_walks`, `audit_retarget`. CR_COVERAGE **146/146**. ⚠⚠ **`--bench` and the pod smoke were NOT re-taken at this tip** — the `release-fast` build was killed to free the cargo lock so the retarget commit could be verified and pushed, and a cold one is ~40 min on this box. The derivation instead: none of this run's changed cards is in `archetypes()` or `golden_trace.rs`'s two decks (item 12), the golden traces are green in the suite, and every engine change is either pod-only (`ActionCensus`, the eighth deck) or in an effect arm no `fixed` card reaches (`seats_in_turn_order_from`'s four). **Take both before trusting a number from them.**
