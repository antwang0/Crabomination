...
6. **Still ⏳, roughly in value order:** golden traces for a fixed-seed pod (the variant mechanics have landed, so a trace would stop churning — this is the next guardrail and the roadmap has been waiting on exactly this); more planeswalker commanders into `sets::cmdr` (the flag is done, 32 of 33 such cards unimplemented); mana provenance (item 7); Friends forever / Doctor's companion (no catalog card, so skip until one exists). ⚠ Check a mechanic has a consumer *before* building it.
