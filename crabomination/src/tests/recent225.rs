...
fn count_named(g: &crate::game::GameState, ctrl: usize, name: &str) -> usize {
...
fn long_goodbye_destroys_cheap_creature() {
    ...
    assert!(catalog::long_goodbye().keywords.contains(&crate::card::Keyword::CantBeCountered));
}
...
fn seraphic_steed_makes_angel() {
    ...
    assert!(g.computed_permanent(angel.id).unwrap().keywords.contains(&crate::card::Keyword::Flying));
}
...
fn mirage_mesa_taps_for_chosen_color() {
    ...
    g.resolve_effect(&crate::effect::Effect::ChooseColorForSelf, &EffectContext::for_ability(land, 0, None)).unwrap();
...
fn valgavoths_lair_is_hexproof() {
    ...
    assert!(g.computed_permanent(land).unwrap().keywords.contains(&crate::card::Keyword::Hexproof));
}
...
fn sandstorm_verge_prevents_block() {
    ...
    g.resolve_effect(&effect, &ctx).unwrap();
    assert!(g.computed_permanent(enemy).unwrap().keywords.contains(&crate::card::Keyword::CantBlock));
}
