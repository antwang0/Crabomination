...
1. **FIRST:** `git fetch origin claude/modern_decks && git checkout -B claude/modern_decks origin/claude/modern_decks`. Rebase, never force; code before tracker prose; **fetch before every push AND before you start**. ⚠⚠ **THREE SESSIONS RAN ON THIS BRANCH TODAY AND THE COLLISIONS COST NOTHING** — each fetched before pushing and rebased. ⚠ An ML session landed **`143980a7` / `30ed873b` (Round 76b, 731 lines of `server/bot.rs`)** mid-run, so **THE PILOT PLAYS DIFFERENT GAMES THAN THE OLDER ABSOLUTES IN PERF: RE-TAKE THE BASE, DO NOT PREDICT IT.** A/B rows are unaffected (one tree per row). Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure" + "Standing rules"**, the 2026-09-12/13 detail in **ENGINE_BACKLOG's "moved verbatim"**. NEXT is 15 lines and stays 15.
2. **Gates at the `4289a165` tip:** suite **19,603 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); clippy **0**; `cargo check --profile release-fast` clean; sweep **3 blocks / 18 cells / 88,800 games / 0 failures** over fresh seeds 1416..1421 (`cap 0 / board 0 / stuck 0 / draw 10`), **FRONTIER 1422**; `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant, `determinism ok`, `thread_determinism ok (3 vs 1)`. 📐 **The invariant survived the ML pilot change because `--bench` is a `gang` mirror and Round 76b moved the SEARCH pilot — the bench does not cover `dflt`, and an ML change to it reads as "no change" here.**
3. 🔎🔎 **FIND 1: A "HAS THIS EVER BEEN WRITTEN" BIT BELONGS AT THE CoW GROUP'S ONE `&mut` ROUTE, NOT AT ITS FIELDS' WRITE SITES.** Candidate (B) sat open two passes priced as 114 write sites over `CardData`'s 26 cleanup fields. Eleven of them live in `CardCold`, which is *one* `CowBox` with *one* `DerefMut` — so `cold_written` is three lines, touches no write site, and `cold_pristine()` is authoritative for **all thirty-five** of that group's fields. **-0.334 / -0.277 / -0.220 % over `(-349)`/`(-350)`/`(-351)`, and the THIRD row was the biggest** because what was built is a fact about a card, not a memo for a row. **Ask what else a bit answers before spending it on the row that suggested it.**
4. 🔎🔎 **FIND 2: THE HISTOGRAM BY LINE IS WHAT TURNS A TWENTY-BRANCH REFACTOR INTO A ONE-BRANCH COMMIT.** `cast_spell_with_convoke` takes the card out of hand and forty-nine checks put it back; "about twenty look hoistable" was the plan until `CRAB_CAST_CENSUS` keyed rejections by line and **three sites were the whole table**. `(-353)` hoisted one of them (-0.077 / -0.110 / -0.176 %). **Count by site before refactoring by inspection; it cost one 3-minute rebuild.**
5. 💡💡 **THE TOP LEAD IS THE PAYMENT AND IT IS SIZED — AND ITS CORRECTNESS HALF IS ALREADY REFUTED, so do not write the bipartite matcher.** `CRAB_PAY_FAILS` says **28.5 % of `cube`'s payments fail** (2,486 / 8,712; `sealed` 13.0 %, `fixed` **0**), 81 % after building a source table, at ~1,632 Ir each = **0.27 % of `cube`**. ❌ `pay_census`'s doc feared the coloured class hid "auto-tap stranded a colour … a payable line becomes invisible" — `CRAB_PAY_FAILS=names` says **all 1,428 coloured failures read `have: 0`**, no producer at all. ✅ **What is left is one question: 41 % of the failures are CREATURE spells whose colour the seat cannot make, and `can_afford_in_state_with_full` already calls `colors_afford` — so which probe path reaches the payment without that gate, or does `available_mana` over-count a colour?** An env-gated counter at its two exits answers it for one rebuild. ⚠ `bot.rs` is the ML sessions' hot ground; `git log` it first.
6. 💡 **AND THE COLD BIT'S OPEN HALF IS STILL THE BEST-PRICED PERF LEAD:** only three families have taken `cold_pristine()`. Unsurveyed hot readers of `CardCold`, by site count — `may_play_until` 49, `named_card` 39, `exiled_by` 26, `goaded_by` 22, `front_face`/`face_up_def` 37, `granted_activated_*` 34. **The census is ten lines of python** (grep the field, take the enclosing `fn`, join against `cg_calls.py`): 140 fns read one, only 54 have a self row, and it is what found `(-351)`.
7. 📐 **A FLAT PROFILE MEANS RANK SHAPES, NOT BODIES.** Eleven bodies are line-profiled and closed as diffuse and the largest single source line is 0.97 % — none of which stopped a device worth 0.28 % that touches five of them. **Ask what every body does that could be done once.**
8. ⚠⚠ **THE BOX IS NOT THE CPU.** Same Xeon @ 2.80 GHz string, same cores, same tree — and the base read **+0.169 / +0.086 / +0.089 %** above the filed tip, which is *this file's own recorded twelfth-vs-eleventh step*. **A base that misses its prediction by an amount PERF has seen before is a box; a novel miss is a tree.** 📐 And run-to-run jitter on an identical binary is **±330 Ir (±0.00002 %)**: the ±0.001 % floor is about the REBUILD, so a suspicious row re-measures for free.
9. ⚠⚠ **WHEN A CHANGE MOVES A CONTAINER'S *ORDER* RATHER THAN ITS CONTENTS, THE TRACE DUMP IS THE GATE, NOT THE SUITE.** `(-353)` stopped a take-and-put-back that moved the card to the end of the hand — invisible to 19,603 tests, visible to every bot decision that iterates the hand. `CRAB_DUMP_TRACES` on both binaries, byte-identical over 48 `cube` and 72 `sealed` pairings, is what licensed the row.
10. ⚠ **EVERY GATE THIS BRANCH LANDS CARRIES A RECOMPUTE-AND-COMPARE `debug_assert!` ON ITS CHEAP PATH** — and `(-349)` adds one the COMPILER owns: `cold_membership_check` names `eot_wear_off_cold!`'s fields on a `&CardCold`, so mis-filing one is a compile error. **Prefer the ratchet the compiler can hold.** The sweep is where the runtime ones fire, not the suite.
11. **Perf queue: ✅ (B) TAKEN as `(-349)`/`(-350)`/`(-351)`; (L)=`(-344)`, (M)=`(-345)`+`(-346)` (owed half refuted as `(-347)`), (A)=`(-348)`; (J)/(K)/(C)/(N)/(D)/(E)/(I) CLOSED.** Open: **(Q)** the cast body — first branch taken as `(-353)`, the payment half is item 5; **(O)** the allocation table, **755,903 allocations ≈ 9.9 % of `cube`**, censused by caller this run (`finish_grow` 20 %, `Arc::clone_from_ref_in` 11.5 %, `GameState::clone` 6.4 %) and by growth: **191,277 growths over 131 callers of which only ~10,300 are RE-growths**, ceiling 0.10-0.17 % over a dozen `with_capacity` edits; **(P)** the CoW unshare table, censused by instance and caller — `cast_spell_with_convoke` is 21 % of every unshare in the program; **(H)** the other zones' by-id scans (filed shape refuted — price the clone side first). ❌ `call_mut` is not a lead (98.6 % std's `Filter` fold). ❌ **"audit read-only `iter_mut` on hot paths" is CLOSED for the battlefield**: one caller in the whole dump and it writes.
12. **THE GLOBAL LINE TABLE SAYS THE PROFILE IS FLAT — do not re-derive it.** Largest single source line **0.97 %**, then 0.80 / 0.56 / 0.41 %, and **12,537 lines hold 86.9 %**. Rank by **self per call**. ⚠ **`Battlefield`'s lane word has exactly ONE two-bit field left** (shift 62) with a `const` assertion in `zone.rs`.
13. **Closing tip absolutes at `4289a165` (`profiling-fast --no-default-features`, system allocator, FOURTEENTH box @ 2.80 GHz): fixed 576,120,857 / cube 1,503,966,660 / sealed 1,623,872,595.** Six rows this run summing **-0.431 / -0.413 / -0.419 %**, one rules fix among them that ended up paying for itself. ⚠ **Build times are CLOSED as a lever.**
14. ⚠⚠ **FIVE TRAPS, ALL LIVE.** `cargo build --profile profiling-fast -p crabomination --bin bot_ladder --no-default-features` — **both flags load-bearing**; `nm <bin> | grep -cE " (T|t) (_)?mi_"` = 0 before filing a number. ⚠ **DO NOT EDIT THE TREE WHILE A BASE BINARY IS BUILDING.** ⚠ **Never overlap two sweep blocks on four cores.** ⚠⚠ **`cargo build --release` is NOT ~10 min — cold and contended it took over 35** (cgu 1 + thin LTO); start it before you need it, and note that a change to `bot_ladder.rs` alone relinks in **46 s**. ⚠ **DISK hit 96 % with 1.9 GB left, twice**: `target/debug/incremental` reaches 11-17 GB and `rm -rf` returns all of it; `find target/debug/deps -maxdepth 1 -type f -size +50M -mmin +60 -delete` returns 4 GB more and rebuilds nothing.
15. **Cards & rules, leftover only:** `audit_dropped_may` 199; `audit_keyword_drift`'s last 4 (bestow / escalate / flashback / kicker) each need a PRIMITIVE; card skips `nocolors` 950 / `noback` 60 / `nosubtypes` 43 / `nokeywords` 29; `audit_incomplete` 858 documented approximations. ✅ **Temple Thief's `IsEnchanted` is DONE and `audit_block_restriction_filters_*`'s `KNOWN_DEAD` is now EMPTY** (0 dead leaves over 30 filters); the shape that closed it — *a state-free walker that needs one board fact takes it as an argument, and as a THUNK when the ask rate is near zero* — is `(-352)`'s. ⚠ 605 `definition.is_*()` reads remain and **that is NOT a grep to run to the end**.
