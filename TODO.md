...
2. **Gates at the tip, all re-run this run:** suite **19,549 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, nextest), clippy **0** (`--workspace --all-targets --exclude crabomination_client`), golden traces unmoved, `cargo check --profile release-fast` clean, `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant on all three binaries with `determinism ok` and `thread_determinism ok (3 vs 1)`. ⚠⚠ **SIXTH BOX, 4 cores, rustc 1.95.0 — no absolute in PERF is comparable across boxes; the counters and the Ir are.** ⚠ **The A/B base WAS re-taken here and reproduces the record to six significant figures**: 616,268,574 / 1,626,125,678 / 1,713,378,250 against the filed 616,268,430 / 1,626,125,608 / 1,713,381,435. Timings on this box: cold catalog+engine `profiling-fast` **~70 min**, warm **engine-only 3m50s**, `target-audit/overflow` cold **10m53s**, `cargo check --profile release-fast` **2m06s**, a three-pool callgrind ~3 min, the debug suite cold ~25 min and **124 s to run**. ⚠⚠ **`md5sum` is NOT the check for a "byte-identical" claim** — `profiling-fast` embeds the source hash in debuginfo, so identical code gives different digests; the `--bench` counters and the Ir are the check. ⚠ **A CONCURRENT SESSION pushed three commits to this branch mid-run (`6644af9b`), so every gate was re-run after the rebase onto it**: suite **19,549 / 0 / 5**, clippy **0** both with and without `--features trig-census`, `cargo check --profile release-fast` clean. Fetch before you start, not only before you push.
3. **Perf — `(-315)` TOOK `grant_scan`'s ONE UNGATED PASS: fixed -0.188 / cube -0.113 / sealed -0.203 %, the row -36.4 / -29.5 / -47.1 %.** Three of the scan's four passes were already behind a presence gate; the CR 702.6e Equipment / CR 702.95 Soulbond pass walked the whole battlefield on **all 24,242 calls** a six-game `cube` run makes. `LANE_ATTACH_GRANT` (shift 56), definition-only, the two instance gates over-approximated. **`(-316)` took the CR 602.5g haste walk the same way** (`LANE_HASTE_STATIC`, shift 58): fixed -0.002 / cube -0.044 / sealed -0.028 %, row halved on all three, and the static it walks for is carried by **one card in the catalog**. ⚠ **The instrument that found both is the CALLEE table, not the self table** — neither walk has a symbol, both inline into their parent, so `cg_edges.py --rows N` could never show them.
4. **Perf — two rules, and the first is the counter-example to `(-314)`'s.** **(i) Size a presence gate by what the loop body does on a MISS, not by the loop.** `(-314)` gated a probe of an empty `Vec` — two instructions a permanent, 0.05 %; `(-315)` gates two `Option` discriminants out of a large `CardDefinition` plus an instance load, and is thirty times that. **(ii) About 40 % of what a lane takes off a row comes back** as the lane read at each call site plus `walk_and_store`'s one walk per invalidation (`(-315)` gave back 15-19 %, `(-316)` nearly all of it on `fixed`). A row under ~0.05 % of the pool is not worth a lane shift — and **only 60 and 62 are left**. `(-313)`'s own rule (ask which input can only move the answer ONE WAY) is in PERF.
5. ❌ **Perf — `available_mana` IS CLOSED for gating and for the member list, censused not argued** (`mana_census`, `trig-census` + `CRAB_MANA_CENSUS=1`). (a) **The walk is the majority** — self **16.57 M / 58 %** of the row on `cube`, **18.49 M / 68 %** on `sealed`, so more gating of the `(-314)` shape was capped at a third of the row; `(-315)`/`(-316)` then took most of that third. (b) **The `(-87)` member list IS THE BOARD**: 65-81 % of permanents satisfy a sound definition-only predicate, because lands are most of a board and every land is a mana source — **a member list is priced by how much it EXCLUDES**. (c) `relax_reachable` is clear on **100 %** of calls, so `(-314)` is fully effective. The `OnceCell` already amortises (48,596 `sealed` affordability checks -> 15,884 inits) and there is no within-tick duplicate.
6. 🔥 **Perf — ONE DEVICE ON THAT SELF IS NOT CLOSED, because it is neither a gate nor a member list: a per-definition budget summary on `mana_summary`'s free bits 51-57.** The loop's whole job for an **untapped, non-sick** permanent that `grants_nothing` — `is_countable_mana_ability` (fifteen field tests a basic land passes), `mana_ability_output`, `mana_amount_is_dynamic`, the `opaque_source` arm — is a pure function of the printed ability list. Pack `AM_COLORS` (51-55), `AM_COLORLESS` (56), `AM_EXACT` (57), where `AM_EXACT` = best == 1, no dynamic amount, no non-countable ability producing colour; clear means "run the loop", so it cannot be wrong in the permissive direction. The census sizes it: **4.4-6.3 permanents a call reach that loop**, at ~60-85 Ir each — **~0.2-0.35 %**, and it is the only thing left on the row's majority. Bit layout and the argument are in PERF's candidates.
7. ⚠⚠ **Line 6's prerequisite is a MECHANICAL MOVE and it must be its own commit.** `mana_summary_of` is in `game/actions.rs`; `is_countable_mana_ability`, `mana_ability_output`, `mana_amount_is_dynamic` and the three `accumulate_*_colors` are in `server/bot.rs`. **`actions.rs` has ZERO references to `crate::server::bot` and must keep zero** — move them to a new `game/mana_shape.rs` beside `effect_produced_colors` / `is_mana_ability` (a better seam than either oversized file), and **do not reimplement them**: two hand-written copies of one predicate is the bug class this branch keeps closing. ⚠ It touches `bot.rs`, which a concurrent session was also editing on 2026-09-15 — fetch first.
8. ❌ **Perf — sized and DECLINED this run, so nobody spends a build:** `main_phase_action_with`'s `has_magecraft` (bot.rs:7662) and `has_opus` (bot.rs:7706) are ungated whole-battlefield walks with definition-only predicates, but they are once *per tick* and their body is a `controller` compare plus a length load — `is_opus_trigger::branches` is **60,168 Ir on `sealed` (0.0035 %)**, 2,210 on `cube`. 0.02-0.05 % for the pair against two of the three remaining lane shifts. If ever taken, take them on **one** combined lane. The `ComputedPermanent` memo lane stays CLOSED (`(-312)`), as do `(-309)` and `(-307)`, and so does the **target enumerator** (it is an enumeration, not a witness; 58.8 % of `sealed` / 87.7 % of `cube` of its calls already ride `(-313)`'s ordering, and its only lever is a behaviour change needing a ladder run).
9. **Perf — the two structural rows are what is left above line 6.** `dispatch_triggers_for_events_slow` **4.65 % of `cube` / 6.15 % of `sealed`** self, line profile FLAT over 267 lines — it needs the structural half (merging `perform_action_inner`'s batches), which is a CR 603.3b ORDERING question before a perf one. `compute_permanent_pass` **3.80 % / 2.92 %**, 238 Ir of self per permanent — the next line out of it wants a `profiling-lines` dump, cold, and ⚠ `(-310)`'s rule about inlined frames.
10. **Seeds: frontier 1338 — THIS RUN TOOK 1334..1337, 12 cells / 59,200 games / 0 failures**, `cap 0 / board 0 / stuck 0 / draw 16` over `cube` / `all` / `sealed`, `target-audit/overflow` with `-C debug-assertions=yes` and `CRAB_ANSWER_LOG=strict` — so **both new lanes' `Battlefield::lane` audits, which recompute the handed predicate against the stored bit on EVERY read, ran on every one of those boards**, which is the coverage a new memo cannot get from the suite. No Scute Swarm cell this block; cells ran 43-94 s. All standing audits re-run and 0: `audit_panics` **0 bare** (70 sites, 59 guarded, 11 lock-poison), `audit_stash_in_loop` **0 unexplained**, `audit_seat_from_selector` **0 open**, `audit_target_walkers --check` **0**, `audit_doc_drift` **0 body-wrong / 0 doc rot / 0 stale notes**.
11. **Best next moves.** (a) **Line 7 then line 6** — the move is mechanical and the pack is specified; it is the largest non-structural thing on the page and the ONE device `available_mana`'s closure does not cover. (b) **The checklist line 3 opened and nobody has run to the end: 285 `battlefield.iter().any(` sites.** Take the CALLEE tree of anything over ~0.5 % and ask of each callee *which of its passes is gated* — that is how both of this run's were found, and neither was visible in a self table. (c) The two structural rows (line 9). (d) ⚠ **Re-run the fresh-seed sweep as the FIRST gate after any memo or epoch change, not the last** — it is the only gate that reached `(-311)`'s collision, and the suite is not a substitute. (e) **`audit_dropped_may` is 199 and `LookPickToHand` is NOT safe at this granularity, measured** — buckets: put 73, exile 20, return 16, collect 12, begin 12. (f) `audit_keyword_drift`'s last 4 each need a PRIMITIVE. (g) The skips with a card behind them: `nocolors` **950**, `nosubtypes` 43, `noback` 60, `nokeywords` 29.
