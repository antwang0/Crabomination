...
fn scurry_oak_squirrel_per_counter_batch() {
    ...
    g.decider = Box::new(ScriptedDecider::new([DecisionAnswer::Bool(true)]));
    let ctx = crabomination::game::effects::EffectContext::for_spell(0, Some(Target::Permanent(oak)), 0, 0);
    let events = g
        .resolve_effect(
...
fn flametongue_yearling_multikick() {
    ...
    assert!(matches!(
        ...
        Some((CounterType::PlusOnePlusOne, crabomination::card::Value::TimesKicked))
    ));
}
...
fn urban_daggertooth_enrage_proliferates() {
    ...
    let mut evs = Vec::new();
    g.deal_damage_to_from(crabomination::game::effects::EntityRef::Permanent(dino), 1, None, &mut evs);
