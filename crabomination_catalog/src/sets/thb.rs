        ...
        activated_abilities: vec![ActivatedAbility {
            ...
            effect: Effect::TapAndUntapLock {
                what: target_filtered(
                    SelectionRequirement::Creature.and(SelectionRequirement::PowerAtMostXFromCost),
                ),
            },
            ..Default::default()
        }],
...
pub fn shark_typhoon() -> CardDefinition {
    let mint = |scope| TriggeredAbility {
        ...
        effect: Effect::CreateToken {
            who: PlayerRef::You,
            count: Value::ONE,
            definition: std::sync::Arc::new(shark_token()),
        },
    };
    CardDefinition {
        name: "Shark Typhoon",
...
pub fn daxos_blessed_by_the_sun() -> CardDefinition {
    ...
    CardDefinition {
        ...
        card_types: vec![CardType::Enchantment, CardType::Creature],
        subtypes: Subtypes {
            creature_types: vec![CreatureType::Demigod],
            ..Default::default()
        },
        power: 2,
        dynamic_pt: Some(DynamicPt::DevotionToToughness {
            color: Color::White,
            base_p: 2,
        }),
        triggered_abilities: vec![
...
pub fn tymaret_calls_the_dead() -> CardDefinition {
    ...
    let zombies = || Value::CountMatching {
...
pub fn underworld_rage_hound() -> CardDefinition {
    CardDefinition {
        ...
        keywords: vec![
            Keyword::MustAttack,
            Keyword::Escape(cost(&[generic(3), r()]), 3),
        ],
        enters_with_counters: Some((
            CounterType::PlusOnePlusOne,
            Value::IfPred {
...
pub fn daybreak_chimera() -> CardDefinition {
    ...
    CardDefinition {
        ...
        static_abilities: vec![StaticAbility {
            ...
            effect: crate::effect::StaticEffect::SelfCostReducedByDevotion {
                colors: vec![Color::White],
            },
        }],
        ..Default::default()
    ...
    CardDefinition {
        ...
        triggered_abilities: vec![etb(Effect::Seq(vec![
            Effect::CreateToken {
                who: PlayerRef::You,
                count: Value::Const(2),
                definition: std::sync::Arc::new(human_soldier_token()),
            },
            Effect::GainLife {
                who: Selector::You,
                amount: Value::Const(2),
...
pub fn underworld_charger() -> CardDefinition {
    CardDefinition {
        ...
        keywords: vec![
            Keyword::CantBlock,
            Keyword::Escape(cost(&[generic(4), b()]), 3),
        ],
        enters_with_counters: Some((
            CounterType::PlusOnePlusOne,
            Value::IfPred {
...
pub fn threnody_singer() -> CardDefinition {
    CardDefinition {
        ...
        card_types: vec![CardType::Creature],
        subtypes: Subtypes {
            creature_types: vec![CreatureType::Siren],
            ..Default::default()
        },
        power: 1,
        toughness: 3,
        ...
        triggered_abilities: vec![etb(Effect::PumpPT {
            ...
            power: Value::Diff(
                Box::new(Value::ZERO),
                Box::new(Value::DevotionTo(vec![Color::Blue])),
            ),
            toughness: Value::ZERO,
            duration: Duration::EndOfTurn,
        })],
...
pub fn blight_breath_catoblepas() -> CardDefinition {
    let neg_devotion = Value::Diff(
        Box::new(Value::ZERO),
        Box::new(Value::DevotionTo(vec![Color::Black])),
    );
    CardDefinition {
    ...
    CardDefinition {
        ...
        effect: Effect::Seq(vec![
            ...
            Effect::If {
                cond: Predicate::EntityMatches {
                    what: Selector::Target(0),
                    filter: SelectionRequirement::IsEnchanted.or(SelectionRequirement::Enchantment),
                },
                then: Box::new(Effect::Seq(vec![
                    Effect::GrantKeyword {
                        what: Selector::Target(0),
...
pub fn renata_called_to_the_hunt() -> CardDefinition {
    ...
    CardDefinition {
        ...
        dynamic_pt: Some(DynamicPt::DevotionTo {
            color: Color::Green,
            base_t: 3,
        }),
        static_abilities: vec![StaticAbility {
                ...
                Effect::PumpPT {
                    what: target_filtered(
                        SelectionRequirement::Creature
                            .and(SelectionRequirement::ControlledByYou)
                            .and(
                                SelectionRequirement::IsEnchanted
                                    .or(SelectionRequirement::Enchantment),
                            ),
                    ),
                    power: Value::ONE,
    ...
    CardDefinition {
        ...
        triggered_abilities: vec![
            etb(Effect::CreateToken {
                who: PlayerRef::You,
                count: Value::ONE,
                definition: std::sync::Arc::new(satyr_cant_block_token()),
            }),
            heroic_team_pump(),
        ],
        ..Default::default()
...
pub fn chainweb_aracnir() -> CardDefinition {
    CardDefinition {
        ...
        keywords: vec![
            Keyword::Reach,
            Keyword::Escape(cost(&[generic(3), g(), g()]), 4),
        ],
        triggered_abilities: vec![etb(Effect::DealDamage {
            to: target_filtered(
                SelectionRequirement::Creature
...
pub fn archon_of_suns_grace() -> CardDefinition {
    ...
    CardDefinition {
        ...
        triggered_abilities: vec![constellation(Effect::CreateToken {
            who: PlayerRef::You,
            count: Value::ONE,
            definition: std::sync::Arc::new(pegasus_token()),
        })],
        ..Default::default()
    }
}
                    ...
                    condition: Predicate::SelectorCountAtLeast {
                        sel: Selector::EachPermanent(
                            SelectionRequirement::Planeswalker
                                .and(SelectionRequirement::ControlledByYou)
                                .and(SelectionRequirement::HasPlaneswalkerType(
                                    PlaneswalkerSubtype::Elspeth,
                                )),
                        ),
                        n: Value::ONE,
                    },
        ...
        saga_chapters: vec![
            ...
            (
                ...
                Effect::CreateToken {
                    who: PlayerRef::You,
                    count: Value::ONE,
                    definition: std::sync::Arc::new(gold_token()),
                },
            ),
        ],
        ..Default::default()
        ...
        saga_chapters: vec![
            (
                ...
                Effect::CreateToken {
                    who: PlayerRef::You,
                    count: Value::ONE,
                    definition: std::sync::Arc::new(kraken),
                },
            ),
            (
                2,
...
pub fn lagonna_band_storyteller() -> CardDefinition {
    CardDefinition {
        ...
        triggered_abilities: vec![etb(Effect::MayDo {
            description: "Put an enchantment from your graveyard on top, gain its mana value?"
                .into(),
            body: Box::new(Effect::Seq(vec![
                Effect::GainLife {
                    who: Selector::You,
        ...
        loyalty_abilities: vec![
            LoyaltyAbility {
                ...
                effect: Effect::CreateToken {
                    who: PlayerRef::You,
                    count: Value::ONE,
                    definition: std::sync::Arc::new(ashiok_nightmare_token()),
                },
                ..Default::default()
            },
            LoyaltyAbility {
            ...
            LoyaltyAbility {
                loyalty_cost: -7,
                effect: Effect::CastUpToNFromOpponentsExile {
                    count: Value::Const(3),
                },
                ..Default::default()
            },
...
pub fn enemy_of_enlightenment() -> CardDefinition {
    CardDefinition {
        ...
        toughness: 5,
        keywords: vec![Keyword::Flying],
        dynamic_pt: Some(DynamicPt::BaseMinusOpponentsHandTotal {
            base_p: 5,
            base_t: 5,
        }),
        triggered_abilities: vec![TriggeredAbility {
    ...
    CardDefinition {
        ...
        static_abilities: vec![StaticAbility {
            ...
            effect: StaticEffect::TaxOpponentSpellsTargeting {
                target_filter: SelectionRequirement::Creature.or(SelectionRequirement::Enchantment),
                amount: 1,
            },
        }],
        ..Default::default()
...
pub fn hateful_eidolon() -> CardDefinition {
    CardDefinition {
        ...
        triggered_abilities: vec![TriggeredAbility {
            ...
            effect: Effect::Draw {
                who: Selector::You,
                amount: Value::AurasYouControlledOnDyingSubject,
            },
        }],
        ..Default::default()
    }
    ...
    CardDefinition {
        ...
        effect: Effect::Seq(vec![
            Effect::PreventCombatDamageExceptDealtBy {
                except: SelectionRequirement::Creature
