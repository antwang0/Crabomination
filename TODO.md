...
2. **Gates at the `9f7b8c29` tip, all re-run:** `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant, `determinism ok`, `thread_determinism ok (3 vs 1)`, `peak_rss 25.2`; suite **19,586 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, 12 golden traces unmoved); clippy **0** both feature sets; `cargo check --profile release-fast` clean. Sweep: **36 cells / 177,600 games / 0 failures** over seeds 1386..1397, **FRONTIER 1398**. `audit_stubs` 0 of 21,797, `audit_panics` 0 bare.
3. ⚠⚠ **`--bench` DID NOT MOVE ACROSS NINE BEHAVIOUR-TOUCHING RULES FIXES, AND THAT IS ABOUT THE POOL, NOT THE FIXES.** `--decks fixed` is mono-coloured with plain pips, no keyword grants, no keyword counters and no anthem reading its own targets — it exercises none of what this run changed. **A rules fix that leaves `--bench` byte-identical has not been tested by `--bench`.** The suite and the fresh-seed sweep are the gates that reach them; run a sweep block after any rules change.
4. 🔎🔎 **THE METHOD THAT PAID THIS RUN — NINE FIXES, EVERY `audit_*` SCRIPT CLEAN — AND IT IS NOT EXHAUSTED: DIFF THE PLACES THAT ANSWER THE SAME QUESTION.** Six walkers answer a `SelectionRequirement` leaf (`eval.rs`'s `evaluate_requirement_static_hinted` / `evaluate_requirement_on_card` / `printed_requirement_impl`, `layers.rs`'s `requirement_matches_card`, `mod.rs`'s `blocker_matches_block_filter`, and `layers.rs`'s `AffectedPermanents::{All, AllOpponents, …}` field gates) — **and the value evaluator is a seventh place the same questions get answered**, which is where the biggest find of the run was. 116 arms are shared between the first two and **50 differ textually; ~40 are still unread**. Recipe: a 20-line `re` script that extracts each `R::` arm's body per line range, strips comments and whitespace, and prints the arms that disagree. ⚠ **And the routing matters as much as the arm**: `affected_from_requirement` sends a *simple* filter to `AffectedPermanents::All`, not to the walker, so fixing the walker alone did not fix the anthem. Put a control assertion in every regression test for exactly that reason — two of this run's tests passed by accident on the first cut.
5. **The nine fixes** (ENGINE_BACKLOG twenty-seventh..thirty-fourth): value-agnostic keyword accessors read 2 of 4 sources; the layer walker's `HasKeyword` read the print; six copies of the morph keyword set; `WithAnyCounter`/`HasNoCounters` were not complements ten arms apart in one walker; the block-restriction walker's five *type* leaves read the print under a doc saying "computed"; **seven** hand-written answers to "is this object green", the layer pass's walking `Colored(_)` alone so `{G/W}` and colour indicators read colourless; a two-value static filter collapsing to its last leaf; **seven P/T *comparison* arms reading the raw instance power under a comment saying the thresholds beside them read the computed view**; and **`Value::PowerOf`/`ToughnessOf` reading the raw instance power, so a 2/2 under a Glorious Anthem was flung for 2 — 227 catalog cards route through those two.** Every one has a regression test verified to FAIL on the pre-fix line.
6. ⚠ **`effective_power` IS NOT A FREE SWAP FOR `card.power()`, and the price is a GATHER, not a memo lookup.** The `Value::PowerOf` fix cost `fixed` **+0.118 %**, and the dumps say it is **84 extra `gather_continuous_effects_inner` calls** (13,170 -> 13,254): values are evaluated during *resolution*, where the state is unfrozen, so the first computed read on a state gathers and `(-303)`'s cross memo serves the repeats. Price the next such read that way. (Inside a requirement walk it is nearly free — the threshold arms already paid it.)
7. ⚠⚠ **THE MIMALLOC TRAP COST THIS RUN A 7m29s BUILD AND IT LOOKS LIKE A WIN.** `cargo build --profile profiling-fast --bin bot_ladder` — **no `-p`** — keeps mimalloc and reads **-5.9 / -6.7 / -6.9 %**. Both `-p crabomination` AND `--no-default-features` are load-bearing. Check `nm <bin> | grep -cE " (T|t) (_)?mi_"` = 0 **before** you file a number.
8. **Tip absolutes at `9f7b8c29`, twelfth box (`profiling-fast`, system allocator): fixed 579,665,125 / cube 1,510,267,940 / sealed 1,632,443,289.** The run's whole movement is +0.084 / +0.026 / +0.031 % and PERF's Baseline breaks it down row by row. The `012e0e21` opening set reproduced the eleventh box's to +0.09..0.20 % on a different CPU. 🔎 `cg_lines.py` needs **no** `profiling-lines` build: `.debug_line` survives unpacked split debuginfo, so the `(-342)` source-line grouping device is one `--dump-instr=yes` callgrind (**15 s**) plus 7 s of script.
9. **Perf — TWO CANDIDATES BUILT, MEASURED AND REVERTED. Read both refutations in PERF's candidates head; each closes a family.** (a) `(-303)`'s cross-scope key applied to `LayerFreezeState::perms`: sound, **+0.70 % uncapped on `cube`**. Carried hit rate **3.1 %** — a freeze scope opens on an unmoved board once in thirty-two — against the cross *gather* memo's 23.7 %, because a gather is asked once a scope and a pass once a permanent. And **`cp_pool` is worth ~40,000 allocations a six-game `cube` run** (5.3 % of 757,012); its feed IS the scope-end release, so the two devices are mutually exclusive. (b) `(-129)`'s outlining on `affected_includes_gated`: **+0.026 %**, prologue unchanged — **`out/call ≈ 0` is necessary but not sufficient**, the surviving arms wanted the registers themselves.
10. **The profile is FLAT and the candidates list is thin — say so rather than re-deriving it.** Top single line **0.97 %**; the `(-342)` grouping is all slice iteration and pointer machinery. `card.rs:13` (`CardId::eq`) is down to 17.18 M / 1.14 %. Allocation is 757,012 a `cube` run and **diffuse**. Open queue in PERF: (A) `ManaCost::cmc`, (B) the `CardCold` `Arc::ptr_eq` gate (~0.12 %, a base change at ~7 min a build), (C) `blocker_pair_block`'s pair count, (H) the other zones' by-id scans (censused; the obvious fix is wrong).
11. ⚠ **THE CONTAINER IS SUSPENDED BETWEEN TURNS — background waits DO NOT PROGRESS.** Run builds and A/Bs in the FOREGROUND with the 600 s tool timeout; when one overruns and is backgrounded, **re-issue the same `cargo build` in the foreground** — it blocks on the artifact lock and that block *is* the wait. ⚠⚠ **DISK hit 20 KB free and a `cargo check` died with `No space left on device` mid-query-cache.** `rm -rf target/debug/incremental target-audit && find target/debug/deps -maxdepth 1 -type f -size +50M -delete` returns 15-23 GB and rebuilds nothing. Do it **before** each optimized build.
12. **Box timings (twelfth box, 4 cores, 15 GB — roughly 2.5x the eleventh):** cold `profiling-fast` 7m29s, **engine-only 2m08s**, a `crabomination_base` change 7m20s, cold `release` 7m18s (9m11s contended), `overflow` 6m52s, full suite 1m55s warm / 4m25s cold, workspace clippy 2m10s warm / 3m50s cold, `cargo check --profile release-fast` 19 s warm, three-pool callgrind ~35 s in parallel, one `cube` callgrind 15 s, a 6-cell sweep block 4m10s.
13. **Cards & rules, leftover only:** `audit_dropped_may` 199; `audit_keyword_drift`'s last 4 (bestow / escalate / flashback / kicker) each need a PRIMITIVE; card skips `nocolors` 950 / `noback` 60 / `nosubtypes` 43 / `nokeywords` 29; `audit_incomplete` 858 documented approximations.
