...
2. **Gates at the `512aea87` tip:** suite **19,603 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); clippy **0** over the workspace; `cargo check --profile release-fast --all-targets` clean; sweep **2 blocks / 12 cells / 59,200 games / 0 failures** at fresh seeds 1422..1425, `cap 0 / stuck 0 / draw 2` (a draw is a legal rules outcome, not a stall) — **FRONTIER 1426**; `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant across all five rows, `determinism ok`, `thread_determinism ok (3 vs 1)`, `peak_rss_mib` 25.0.
