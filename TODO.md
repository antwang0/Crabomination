...
5. **This run shipped, in order:** CR 601.2f alt-cost casting from the command zone (`cast_spell_alternative_from` over `AltCastZone`, tax included, bot offers both casts); CR 113.6b triggers (`EventSpec.zone`) and statics (`CardDefinition.statics_in_command_zone`) that function from the command zone; CR 702.49d commander ninjutsu plus CR 602.2b's auto-tap and the bot's first-ever `GameAction::Ninjutsu` candidate; five cards in `sets/cmdr.rs` (Edgar Markov, Oloro, Arahbo, Yuriko, The Ur-Dragon).
6. **Still ⏳, roughly in value order:** join forces — the primitive is one `Effect` arm over `ask_seat_amount` (which already carries the APNAP cursor, the suspend and the replay log) plus `AmountKind::Mana`, and **Minds Aglow / Collective Voyage / Mana-Charged Dragon are the consumers**; more planeswalker commanders into `sets::cmdr` (32 of 33 such cards unimplemented); Friends forever / Doctor's companion (no catalog card, so skip until one exists); golden traces for a fixed-seed pod.
...
15. **Where the new cards live:** `crabomination_catalog/src/sets/cmdr.rs` — cards whose text is *about* CR 903, not about their set. Register in `all_factories.rs`.
