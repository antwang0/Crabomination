...
2. **Gates at the closing tip:** suite **19,745 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); clippy **0** over the workspace; `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean; `--bench` **195,806 / 27.49 / 611.9 / 0 stalls**, `determinism ok` — byte-identical to the committed invariant at **three** separate tips across this run's CR 603.2c work, including the one that changed six cube/SOS pool cards. Assertions sweep (`overflow` + `-C debug-assertions=yes`): **8,000 pod games at 2/3/4/5 seats, fresh seeds 2005-2006, 100 % decided, 0 undecided, 0 assertions**, plus the two-player grid on fixed/cube/sos/all at seed 2005 (4,080 games, clean) — **FRONTIER 2007**. Ir guardrail, the clean single-commit A/B (`3964e1c5` → `a09f4704`): **fixed +0.025 / cube +0.030 / sealed +0.025 %** — all three together, i.e. a codegen row, which is what a widened key and a `bool` parameter are. PERF's Baseline has the block.
