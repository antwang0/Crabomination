...
fn wartime_protestors_buffs_incoming_allies() {
    ...
    drain_stack(&mut g);
    assert_eq!(
        g.battlefield_find(ally).unwrap().counters.get(&crabomination::card::CounterType::PlusOnePlusOne)
            .copied().unwrap_or(0),
        1,
...
fn hog_monkey_grants_menace_to_counter_creature() {
    ...
    g.battlefield_find_mut(buff).unwrap().add_counters(crabomination::card::CounterType::PlusOnePlusOne, 1);
...
fn gran_gran_lesson_gated_cost_reduction() {
    ...
    let noncreature = crabomination::card::CardInstance::new(g.next_id(), catalog::boomerang_basics(), 0);
...
fn firebending_student_scales_with_power() {
    ...
    let mut g = two_player_game();
    let fs = g.add_card_to_battlefield(0, catalog::firebending_student());
    g.battlefield_find_mut(fs).unwrap().add_counters(crabomination::card::CounterType::PlusOnePlusOne, 1);
    g.clear_sickness(fs);
    attack_with(&mut g, fs);
...
fn dai_li_agents_double_earthbend() {
    ...
    let counters: i32 = [l1, l2].iter()
        .map(|&l| g.battlefield_find(l).unwrap().counter_count(crabomination::card::CounterType::PlusOnePlusOne) as i32)
        .sum();
...
fn fire_nation_warship_dies_to_clue() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_ability(ship, 0, Some(Target::Permanent(ship)));
...
fn uncle_iroh_discounts_lessons() {
    ...
    let lesson = crabomination::card::CardInstance::new(g.next_id(), catalog::firebending_lesson(), 0);
    let nonlesson = crabomination::card::CardInstance::new(g.next_id(), catalog::lightning_bolt(), 0);
...
fn beifongs_bounty_hunters_earthbends_for_power() {
    ...
    drain_stack(&mut g);
    assert_eq!(
        g.battlefield_find(land).unwrap().counter_count(crabomination::card::CounterType::PlusOnePlusOne),
        3,
...
fn sold_out_clues_on_damaged() {
    ...
    let mut events = vec![];
    g.deal_damage_to_from(crabomination::game::effects::EntityRef::Permanent(foe), 1, None, &mut events);
...
fn appa_loyal_grants_flying() {
    ...
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Modes(vec![0]),
...
fn sokka_grows_on_lesson_cast() {
    ...
    drain_stack(&mut g);
    assert_eq!(g.battlefield_find(sokka).unwrap().counter_count(crabomination::card::CounterType::PlusOnePlusOne), 1,
...
fn teo_loots_and_counters_on_nonland_discard() {
    ...
    assert_eq!(g.battlefield_find(teo).unwrap().counter_count(crabomination::card::CounterType::PlusOnePlusOne), 1,
...
fn sandbender_grows_on_sacrifice() {
    ...
    g.add_card_to_battlefield(0, catalog::grizzly_bears());
    let ctx = crabomination::game::effects::EffectContext::for_ability(sb, 0, None);
    ...
    g.dispatch_triggers_for_events(&evs);
    drain_stack(&mut g);
    assert_eq!(g.battlefield_find(sb).unwrap().counter_count(crabomination::card::CounterType::PlusOnePlusOne), 1,
...
fn sandbender_reanimates_on_death() {
    ...
    let mut g = two_player_game();
    let sb = g.add_card_to_battlefield(0, catalog::sandbender_scavengers());
    g.battlefield_find_mut(sb).unwrap().add_counters(crabomination::card::CounterType::PlusOnePlusOne, 2); // power 3
    let bears = g.add_card_to_graveyard(0, catalog::grizzly_bears()); // MV 2 ≤ 3
    g.decider = Box::new(ScriptedDecider::new([
        DecisionAnswer::Bool(true),               // yes, exile + reanimate
        DecisionAnswer::Target(crabomination::game::types::Target::Permanent(bears)),
    ]));
...
fn honest_work_shrinks_and_taps() {
    ...
    g.battlefield_find_mut(foe).unwrap().add_counters(crabomination::card::CounterType::PlusOnePlusOne, 2);
...
fn bumi_choose_up_to_lessons() {
    ...
    assert_eq!(g.battlefield_find(bumi).unwrap().counter_count(crabomination::card::CounterType::PlusOnePlusOne), 3,
...
fn bumi_no_lessons_chooses_nothing() {
    ...
    g.fire_self_etb_triggers(bumi, 0);
    drain_stack(&mut g);
    assert_eq!(g.battlefield_find(bumi).unwrap().counter_count(crabomination::card::CounterType::PlusOnePlusOne), 0,
...
fn aangs_journey_kicker_fetches_shrine() {
    ...
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Search(Some(forest)),
        crabomination::decision::DecisionAnswer::Search(Some(shrine)),
    ]));
    g.perform_action(GameAction::CastSpellKicked {
...
fn cave_of_two_lovers_tokens_and_search() {
    ...
    g.decider = Box::new(crabomination::decision::ScriptedDecider::new([
        crabomination::decision::DecisionAnswer::Search(Some(mountain)),
    ]));
...
fn zhao_taps_nonbasics_and_conquers_to_mountains() {
    use crabomination::card::{CardType, CounterType, LandType};
    let mut g = two_player_game();
