...
13. 🔎 **RUN THE ASSERTIONS BUILD OVER PODS** — `RUSTFLAGS="-C debug-assertions=yes" CARGO_TARGET_DIR=target-audit cargo build --profile overflow …` then `--commander --seats N --games 1000`. **Not taken this run** (the wall clock went to the class), so it is owed. ⚠⚠ `rc=$?` **AFTER A PIPE READS THE PIPE'S STATUS** — an abort (134) reads as 0. Redirect to a file, then grep.
