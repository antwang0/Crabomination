...
1. ⚠⚠ **TWO SESSIONS SHARE THIS BRANCH. FETCH BEFORE YOU START AND BEFORE EVERY PUSH.** Rebased **five times** mid-run this time; two conflicts, both in test files both sessions had appended to, and both resolutions were "keep both blocks" — prefer a shape that APPENDS. ⚠ One conflict was a *duplicate clippy fix*: we each invented an alias for `[(&str, fn() -> CardDefinition); N]`. **Grep the file for the house spelling before inventing one** (`type Factory` here, from `counters.rs`/`structural_audit.rs`). ⚠ **DISK: `target/debug/incremental` regrows to 15-19 GB per full suite run and a `release` build then dies as a *compile error* (`No space left on device`)** — `rm -rf` it BEFORE any optimized build, every time. Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure"**.
2. **Gates at the closing tip:** suite **20,144 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, `--exclude crabomination_client`); clippy **0** over the workspace; `cargo check --profile release-fast -p crabomination --bin bot_ladder` clean; `--bench` **195,806 / 27.49 / 611.9 / 0 stalls — byte-identical to the committed invariant**, `determinism ok (all pairs split)`, `thread_determinism ok (3 vs 1)`; `audit_panics` **0 bare**; CR_COVERAGE **146/146**; `audit_as_enters --gate` and `audit_singular_fanout --gate` both clean. **Pod smoke, fresh seed 9110: 2,000 games at each of 2..8 seats = 14,000, 100 % decided, every `undecided_by` column zero, zero panics** — turns/game 19.62 / 32.48 / 45.37 / 57.20 / 65.31 / 77.54 / 93.69. **Plus 15,000 two-player games over cube/sos/sealed at fresh seeds 6001-6002, 0 undecided.** **FRONTIER 9111 / 6003.** ⚠⚠ **QUOTE THE EXCLUSION WITH THE NUMBER.** `crabomination_client` holds exactly **200** `#[test]`s and is the only member the documented suite excludes, so a session that can build the Bevy stack reads 200 higher off the same tree. That gap read as a 200-test regression for two commits this run and cost a round trip of "corrections" in both directions before anyone compared the two *commands*. Also clean: `audit_player_static_target` (1 allowlisted) and `audit_doc_card_names` (gate 0). ⚠ This box is a **2.10 GHz** Xeon, not the SIXTEENTH box's 2.80 — wall clock here is not comparable to anything in PERF above this run's entry; the decision count is.
3. ✅✅ **THE 614.12 CLASS IS CLOSED: 89 → 7, and the seven left each need more than a field move** (a copy, a coin flip, a random pick, a reflexive attach, entering counters, and two documented approximations). ENGINE_BACKLOG's forty-eighth find carries the whole thing.
4. 📐📐 **THE WIRING WAS THE GAP, NOT THE PRIMITIVE — and that is the transferable half.** `as_enters_effect` had shipped for forty cards; what had not shipped was running it on every entry path. `game::as_enters::apply_as_enters_replacements` is the one funnel all four hops call now (cast / move / **land drop** / **token mint**). Before it the land drop applied **none** of the three appliers, so a *played* Cavern of Souls named nothing at all, and the mint applied none, which is CR 614.12's own worked example. **A class audit has to ask "what shapes does the engine already have?" as well as "what shapes do the cards ship?" — the first census priced this off the wrong denominator.**
5. ⚠⚠ **THE LAND DROP IS THE ONE ENTRY PATH THAT CANNOT REPLAY.** Every other suspending action parks *before* anything is paid and is re-run verbatim; here the land is on the battlefield and the drop is spent. `ResumeContext::LandEntry` is a **continuation** (`is_action_replay` = false): it applies the answer, resumes the parked effect, and runs `actions::finish_land_entry` — printed entering counters, self-source ETB triggers, a Saga land's lore counter — exactly once.
6. ⚠⚠ **TWO MODAL VEHICLES WERE TRIED AND BOTH WERE WRONG, AND BOTH REASONS GENERALISE.** `PlayerMayPayLifeElse` asks correctly and `AutoDecider` **declines every `OptionalTrigger`**, so every headless seat's shockland came down tapped (eleven tests red, whole-pool behaviour change). `Effect::ChooseMode` keeps `AutoDecider`'s mode 0 and the bot's outcome scoring — and it reads **`ctx.mode`, the pick made when the object went on the stack**, so inside an entry it silently takes mode 0 and never asks. **`Effect::AsEntersChooseMode` is the one that works without a stack item**; reach for it, not `ChooseMode`, in any `as_enters_effect`.
7. 📐 **A permanent that ENTERS tapped never BECOMES tapped.** `Effect::SourceEntersTapped` sets the flag instead of going through `Effect::Tap`, whose `PermanentTapped` event a "becomes tapped" watcher would see, and honours Spelunking through `lands_enter_untapped_for`, which `apply_enters_tapped_replacement` reads too.
8. 🔎🔎 **FIND TWO, AND IT IS A MULTIPLAYER CLASS: A FAN-OUT `PlayerRef` HANDED TO AN ARM THAT RESOLVES IT SINGULARLY.** `resolve_player` answers with the **first seat of the set** and drops the rest — exact in a duel, and in a pod either seat order deciding the controller's "choose an opponent" (8 cards, incl. Shinryu's "when the chosen player loses, **you win the game**") or a printed "each opponent …" reaching **one** opponent (5 cards). `PlayerRef::HostileOpponent` for the first, `Effect::EachPlayerDoes` for the second. **19 → 2**, ratcheted by `scripts/audit_singular_fanout.py`, which pairs the 102 singular-resolving arms with the catalog sites that feed one. ENGINE_BACKLOG's fifty-first find.
9. 💡 **The two allowlisted survivors are the next work there**: Longhorn Firebeast needs an ask-in-turn-order-and-**stop** arm (`AnyPlayerMayExileFromGraveyard`'s shape, not a fan-out); Unidentified Hovership needs a "the linked exile's owner" ref. 💡 And `default_hostile_opponent` is ranked for **hostile** clauses: Ingenious Mastery's Treasures and Echo Chamber's token are **gifts** and go to the biggest threat. A `default_least_hostile_opponent` sibling is the fix, and it is bot policy, not rules.
10. ⚠ **`as_enters_effect` asks a `wants_ui` seat for real only on the LAND DROP.** The cast, move and mint paths still drive the ask through `resolve_effect_driven` and the decider — standing behaviour for all forty-odd such cards, and one resume context each to close. Rules-correct either way (the choice is made, before the permanent enters); a human-seat UX gap.
11. ⚠⚠ **THE FIXTURE GOTCHA, AND IT COST THREE SEPARATE ROUNDS OF RED TESTS.** `add_card_to_battlefield` deliberately skips every entry replacement, so a test that set a chosen value with `fire_self_etb_triggers` after it sets **nothing** the moment the card's clause becomes a replacement. `GameState::add_card_to_battlefield_entering` is the fixture (sibling of `..._with_counters`), and the decider must be **scripted first** — the choice happens inside the call.
12. ⚠⚠ **`card_bodies()` IS THE SHARED BUG SURFACE OF EVERY `audit_*.py`, AND BOTH SESSIONS HIT IT INDEPENDENTLY THIS RUN — from opposite ends.** ① Filing a site by "the first string literal in the body" mis-files any factory that ends in `..creature("Name", …)`: it picks up the *previous* factory's name (Longhorn Firebeast came back as "Gurzigost"). Anchor the fallback on the `..helper("Name"` tail call, or key on the `fn` ident. ② Slicing a body "up to the next `pub fn`" puts the **next** card's doc comment inside **this** card's slice, so a doc that names a needle reads as this card using it (a Faerie with no static ability was reported for a sentence about a card two functions later) — and hides the body of any card sitting behind a private helper. `_factory_body`, brace-matched from the opening `{`, fixes both; all four fan-out audits use it now. **A row filed under the wrong card is worse than a missed row: it sends the next reader to a card that is fine.**
13. 💡 **Standing leads, best first:** ① the seven remaining 614.12 rows, by mechanic (a coin flip and a random pick would share one primitive). ② The `as_enters_effect` ask on the cast path (item 10) — three resume contexts, and it is the last human-seat gap in the class. ③ `audit_dropped_may` **198** and `audit_incomplete` are the two big unmined columns. ④ **COMMANDER_BACKLOG's top-1000 gap: 70 of 82 open** — recompute it as section 2 minus every name appearing as a string literal *anywhere* in the catalog, **not** as `name: "…"`, or a card built through a per-file helper reads as missing (it said 81-of-82 the day after five landed). Needing no primitive, by rank: Bloom Tender 257, Decanter of Endless Water 318, Lotho 409, Unnatural Growth 442, Faeburrow Elder 501, Relic of Legends 536, Displacer Kitten 550, Second Harvest 561, Champion of Lambholt 574, Saw in Half 582, Cloud Key 610, Mirror Entity 988, Teleportation Circle 992. ⑤ 🔎 **THE FOURTH FAN-OUT RATCHET AND ITS FIRST FIND — a player **static** names its seats with `PlayerStaticTarget`, which is neither a `PlayerRef` nor inside an `Effect`, so the other three audits walk past it.** Erebos, God of the Dead printed "**your opponents** can't gain life" and shipped as `Controller` — the other player — with a `description` string and a test both agreeing with the bug. ⚠ **A `description` is not a check on the model**: same hand, same moment. ⑥ 🔎 **`audit_doc_card_names.py`: an engine doc comment naming a card the catalog has never shipped.** 28 of them, harmless as design rationale — the gate is the sub-class where the doc *also* writes down an approximation for the absent card, which is a decision made before the card existed, by nobody (`DoubleDamageDealt` carried "Fiery Emancipation as ×2 stacking" for a card whose whole text is ×3). 3 → 0 this run.
14. ⚠ **Gotchas that each cost a round.** ① `Effect::ChooseMode` never asks outside a stack item (item 6). ② `PlayerRef::EachOpponent` in a `who` field an arm resolves singularly is a `debug_assert!` waiting for a pod — run `audit_singular_fanout.py` before adding one. ③ A new `Effect` variant needs an arm in **`effect/query.rs`'s exhaustive match** as well as the resolver. ④ New Commander cards go in `catalog/src/sets/cmdr.rs`; a new factory needs `python3 scripts/gen_factory_lists.py`. ⑤ `CR_COVERAGE.md` is **generated** — take either side of a conflict and re-run `scripts/cr_coverage.py`.
15. ⚠ **The rules table is DONE and that is still the handoff's main claim**: FEATURE_ROADMAP's Commander (CR 903) and multiplayer (CR 800-series) sections are all ✅ bar CR 800.4i (unreachable by construction) and an N-seat observation encoder (⛔ by design). What is left in Commander is **cards**, not rules.
