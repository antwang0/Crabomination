...
fn cosmogoyf_scales_with_exiled_cards() {
    ...
    g.exile.push(crabomination::card::CardInstance::new(
        crabomination::card::CardId(9101), catalog::grizzly_bears(), 0,
    ));
    g.exile.push(crabomination::card::CardInstance::new(
        crabomination::card::CardId(9102), catalog::lightning_bolt(), 0,
    ));
    let cp = g.computed_permanent(id).unwrap();
