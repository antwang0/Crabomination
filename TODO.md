...
11. **Cards and decks, what is left:** ⏳ 32 of 33 "can be your commander" non-creatures are unimplemented; the flag and validator are done, so each is a plain card. ⏳ `scripts/audit_each_opponent.py`'s seven residuals are in INCOMPLETE_CARDS with what each needs. ⚠ **Deck balance is the loudest number**: seed 43, 400 four-seat games reads Sigarda **45.5 %** / Tatyova 26.8 % / Hanna 22.2 % / **Judith 5.5 %** — the aristocrats list does not assemble. Board wipes are also one seat's (both Krark's).
