...
| Bot takes every decision the pod decks introduce | 🟡 | `server/bot.rs` `cast_candidates` — escape (printed and Kotis-granted) and replicate (mana / energy / tap) got candidate blocks 2026-09-23; a short payment now floats spend-restricted sources the spell may use (`pay_with_restricted_sources`, CR 106.6). Open: 14 cast variants no bot emits (ENGINE_BACKLOG, none in a pod list) |
