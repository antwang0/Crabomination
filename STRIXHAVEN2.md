...
| Colossus of the Blood Age | {4}{R}{W} | Artifact Creature — Construct | 6/6 | When this creature enters, it deals 3 damage to each opponent and you gain 3 life. / When this creature dies, discard any number of cards, then draw that many cards plus one. | 🟡 | Push XVII: death rider now wired via `Value::CardsDiscardedThisResolution` — discards your entire hand (treating "any number" as the optimal greedy answer) then draws cards-discarded+1. The "+1 floor" matches the printed "draw that many plus one" (still draws ≥ 1 from an empty hand). ETB drain wired (3-to-each-opp + gain 3). |
