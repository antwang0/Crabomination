...
3. **Perf — the run took -0.571 / -0.479 / -0.630 % (fixed / cube / sealed) in four commits, and BOTH of PERF's sized leads are now spent.** `(-298)`+`(-299)` finished the reduction walk's tail: nineteen `static_abilities` walks merged into one, then the whole tail behind `CardData::has_self_cost_reduction` (a one-bit definition memo on **word 1, bits 53-54** — word 0 is full). The row is **181.9 -> 71.0 Ir/call, -61 %**. `(-300)` took the first of `(-294)`'s two dispatch-count questions: **94,072 of 199,186 dispatches (47.2 %) are empty**, gated at the call site with `#[inline(always)]` (plain `#[inline]` leaves a 180,198-call stub and is worth 0.015-0.021 points less). ⚠ **`(-301)` is a REFUTATION to read before touching `printed_requirement_impl`**: its 40 %-self-recursion is NOT a lead — a spine loop loses +0.458 % keeping the order and **+1.410 %** not, because `.and()` nests LEFT and the innermost clause is the broad type test that short-circuits. **An order-free fold is not an order-free walk.**
4. **Perf — what is left in the #1 tree, and it is one number.** `cast_candidates`' target half is **3.61 % of sealed and 26.6 requirement evaluations per `auto_targets_for_effect_all_slots_kicked` call**; the walk is a *max* (rank by side, then power), so no witness short-circuit applies — the lead is **fewer candidates**, e.g. a printed-type pre-gate off `CardMemo`'s `type_bits` guarded by `PrintedGates::card` (unsound when a layer-4 type changer is in scope, which is the whole design question). The other open dispatch question — merging `perform_action_inner`'s batches — is now about the **105,114** that do work. ⚠ The allocator row and the `(-92)` flatness verdict are both still READ AND CLOSED; do not spend a build on either.
