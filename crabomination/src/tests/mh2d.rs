...
fn destroy(g: &mut GameState, id: crate::card::CardId) {
    let mut ctx = crate::game::effects::EffectContext::for_spell(0, None, 0, 0);
...
fn burdened_aerialist_token_sac() {
    ...
    let sac = crate::effect::Effect::Sacrifice {
    ...
    let treasure = crate::effect::shortcut::mint_treasures(1);
...
fn gilt_blade_prowler_gate() {
    ...
    g.players[0].discarded_this_turn.insert(crate::card::CardId(999_999));
...
fn junk_winder() {
    ...
    assert_eq!(catalog::junk_winder().affinity_filter,
        Some(crate::card::SelectionRequirement::IsToken));
...
fn sanctuary_raptor_token_gate() {
    ...
    let ctx = crate::game::effects::EffectContext::for_spell(0, None, 0, 0);
    let events = g
        .resolve_effect(&crate::effect::shortcut::mint_treasures(3), &ctx)
        .unwrap();
    g.dispatch_triggers_for_events(&events);
