...
fn acolyte_of_aclazotz_drains_on_sac() {
    ...
    g.perform_action(GameAction::ActivateAbility {
        card_id: acolyte, ability_index: 0, target: None, additional_targets: vec![], x_value: None, mode: None,
    }).expect("activate");
    drain_stack(&mut g);
...
fn cavern_stomper_grants_evasion() {
    ...
    g.perform_action(GameAction::ActivateAbility {
        card_id: stomper, ability_index: 0, target: None, additional_targets: vec![], x_value: None, mode: None,
    }).expect("activate");
    drain_stack(&mut g);
    assert!(
...
fn panicked_altisaur_pings_each_opponent() {
    ...
    let start = g.players[1].life;
    g.perform_action(GameAction::ActivateAbility {
        card_id: dino, ability_index: 0, target: None, additional_targets: vec![], x_value: None, mode: None,
    }).expect("activate");
    drain_stack(&mut g);
...
fn miners_guidewing_dies_triggers_explore() {
    ...
    let nid = g.next_id();
    g.players[0].library.insert(0, crabomination::card::CardInstance::new(nid, catalog::grizzly_bears(), 0));
...
fn descended_this_turn_tracks_permanent_cards() {
    ...
    let nid = g.next_id();
    g.players[0].send_to_graveyard(crabomination::card::CardInstance::new(nid, catalog::lightning_bolt(), 0));
...
fn hermitic_nautilus_pumps() {
    ...
    g.players[0].mana_pool.add_colorless(1);
    g.perform_action(GameAction::ActivateAbility {
        card_id: naut, ability_index: 0, target: None, additional_targets: vec![], x_value: None, mode: None,
    }).expect("activate");
    drain_stack(&mut g);
...
fn hidden_cave_enters_tapped_then_taps_for_color() {
    ...
    g.perform_action(GameAction::ActivateAbility {
        card_id: cave, ability_index: 0, target: None, additional_targets: Vec::new(), x_value: None, mode: None,
    }).expect("tap for blue");
...
fn volatile_fault_destroys_opponent_nonbasic_land() {
    ...
    g.players[0].mana_pool.add_colorless(1);
    g.perform_action(GameAction::ActivateAbility {
        card_id: fault, ability_index: 1, target: Some(Target::Permanent(victim)), additional_targets: Vec::new(), x_value: None, mode: None,
...
fn forgotten_monument_grants_other_caves_any_color_mana() {
    ...
    g.perform_action(GameAction::ActivateAbility {
        card_id: other, ability_index: 3, target: None, additional_targets: Vec::new(), x_value: None, mode: None,
...
fn bartolome_sacrifices_for_counter() {
    ...
    g.priority.player_with_priority = 0;
    g.perform_action(GameAction::ActivateAbility {
        card_id: barto, ability_index: 0, target: None, additional_targets: vec![], x_value: None, mode: None,
    }).expect("sac ability");
    drain_stack(&mut g);
...
fn pugnacious_hammerskull_stuns_when_alone() {
    ...
    g.perform_action(GameAction::DeclareAttackers(vec![Attack { attacker: dino, target: AttackTarget::Player(1) }]))
        .expect("attack");
    drain_stack(&mut g);
    assert_eq!(g.battlefield_find(dino).unwrap().counter_count(crabomination::card::CounterType::Stun), 1, "stunned (no other Dino)");
}
...
fn sentry_of_the_underworld_regenerates() {
    ...
    g.players[0].mana_pool.add(Color::Black, 1);
    g.perform_action(GameAction::ActivateAbility {
        card_id: sentry, ability_index: 0, target: None, additional_targets: vec![], x_value: None, mode: None,
...
fn sunshot_militia_taps_two_to_ping() {
    ...
    let start = g.players[1].life;
    g.perform_action(GameAction::ActivateAbility {
        card_id: militia, ability_index: 0, target: None, additional_targets: vec![], x_value: None, mode: None,
...
use crabomination::game::{Attack, AttackTarget, Target};
...
fn tinkers_tote_gnomes_and_sac() {
    ...
    g.players[0].mana_pool.add(Color::White, 1);
    g.perform_action(GameAction::ActivateAbility {
        card_id: tote, ability_index: 0, target: None, additional_targets: vec![], x_value: None, mode: None,
    }).expect("sac");
    drain_stack(&mut g);
...
fn the_belligerent_grants_play_from_top() {
    ...
    let top = g.next_id();
    g.players[0].library.insert(0, crabomination::card::CardInstance::new(top, catalog::lightning_bolt(), 0));
...
fn the_ancient_one_descend_gate() {
    ...
    for _ in 0..8 {
        let id = g.next_id();
        g.players[0].send_to_graveyard(crabomination::card::CardInstance::new(id, catalog::grizzly_bears(), 0));
    }
...
fn the_ancient_one_mills_by_discarded_mv() {
    ...
    g.players[0].mana_pool.add_colorless(2);
    g.perform_action(GameAction::ActivateAbility {
        card_id: one, ability_index: 0, target: Some(Target::Player(1)), additional_targets: vec![], x_value: None, mode: None,
    }).expect("activate");
    drain_stack(&mut g);
...
fn soulcoil_viper_reanimates_with_finality() {
    ...
    g.players[0].mana_pool.add(Color::Black, 1);
    g.perform_action(GameAction::ActivateAbility {
        card_id: viper, ability_index: 0, target: Some(Target::Permanent(dead)), additional_targets: vec![], x_value: None, mode: None,
    }).expect("activate");
    drain_stack(&mut g);
...
fn reckless_detective_attack_loots_and_pumps() {
    ...
    g.perform_action(GameAction::DeclareAttackers(vec![crabomination::game::types::Attack {
        attacker: det, target: crabomination::game::types::AttackTarget::Player(1),
    }])).expect("attack");
    drain_stack(&mut g);
...
fn digsite_conservator_exiles_graveyard() {
    ...
    g.perform_action(GameAction::ActivateAbility {
        card_id: dig, ability_index: 0, target: None, additional_targets: vec![], x_value: None, mode: None,
    }).expect("sac to exile");
    drain_stack(&mut g);
...
fn thousand_moons_crackshot_taps_on_attack() {
    ...
    g.priority.player_with_priority = 0;
    g.perform_action(GameAction::DeclareAttackers(vec![crabomination::game::types::Attack {
        attacker: crack, target: crabomination::game::types::AttackTarget::Player(1),
    }])).expect("attack");
    drain_stack(&mut g);
...
fn envoy_okinec_ahau_makes_gnome() {
    ...
    g.players[0].mana_pool.add_colorless(4);
    g.perform_action(GameAction::ActivateAbility {
        card_id: envoy, ability_index: 0, target: None, additional_targets: vec![], x_value: None, mode: None,
    }).expect("activate");
    drain_stack(&mut g);
...
fn oaken_siren_taps_for_artifact_mana() {
    ...
    g.priority.player_with_priority = 0;
    g.perform_action(GameAction::ActivateAbility {
        card_id: siren, ability_index: 0, target: None, additional_targets: vec![], x_value: None, mode: None,
...
fn hoverstone_pilgrim_bottoms_graveyard_card() {
    ...
    g.players[0].mana_pool.add_colorless(2);
    g.perform_action(GameAction::ActivateAbility {
        card_id: pilgrim, ability_index: 0, target: Some(Target::Permanent(gy_card)), additional_targets: vec![], x_value: None, mode: None,
    }).expect("activate");
    drain_stack(&mut g);
...
fn dauntless_dismantler_destroys_artifacts_by_mv() {
    ...
    g.perform_action(GameAction::ActivateAbility {
        card_id: dd, ability_index: 0, target: None, additional_targets: vec![], x_value: Some(2), mode: None,
    }).expect("activate");
    drain_stack(&mut g);
...
fn orazca_puzzle_door_digs_two() {
    ...
    g.perform_action(GameAction::ActivateAbility {
        card_id: door, ability_index: 0, target: None, additional_targets: vec![], x_value: None, mode: None,
    }).expect("activate");
    drain_stack(&mut g);
...
fn cavernous_maw_needs_three_caves() {
    ...
    let res = g.perform_action(GameAction::ActivateAbility {
        card_id: maw, ability_index: 1, target: None, additional_targets: vec![], x_value: None, mode: None,
    });
...
fn uchbenbak_returns_at_descend_8() {
    ...
    let res = g.perform_action(GameAction::ActivateAbility {
        card_id: uch, ability_index: 0, target: None, additional_targets: vec![], x_value: None, mode: None,
    });
...
fn sage_of_days_keeps_one_mills_two() {
    ...
    let keep = g.next_id();
    g.players[0].library.insert(0, crabomination::card::CardInstance::new(keep, catalog::grizzly_bears(), 0));
    g.add_card_to_library(0, catalog::lightning_bolt());
    g.add_card_to_library(0, catalog::lightning_bolt());
...
fn bringer_of_the_last_gift_mass_swap() {
    ...
    let mut ctx = crabomination::game::effects::EffectContext::for_ability(bringer, 0, None);
...
fn nicanzil_land_and_nonland_explores() {
    ...
    fn explore(g: &mut GameState, scout: CardId) {
        let ctx = crabomination::game::effects::EffectContext::for_ability(scout, 0, None);
        let evs = g.resolve_effect(&crabomination::effect::Effect::Explore { who: crabomination::effect::Selector::This }, &ctx).unwrap();
        g.dispatch_triggers_for_events(&evs);
    }
    // Nonland on top → explore gives Nicanzil a +1/+1 counter.
    let nid = g.next_id();
    g.players[0].library.insert(0, crabomination::card::CardInstance::new(nid, catalog::lightning_bolt(), 0));
    explore(&mut g, scout);
    drain_stack(&mut g);
    assert_eq!(
    ...
    let lid = g.next_id();
    g.players[0].library.insert(0, crabomination::card::CardInstance::new(lid, catalog::forest(), 0));
...
fn chimil_uncounterable_and_end_step_discover() {
    ...
    let spell = crabomination::card::CardInstance::new(g.next_id(), catalog::lightning_bolt(), 0);
...
fn abuelo_blinks_own_permanent() {
    ...
    g.perform_action(GameAction::ActivateAbility {
        ...
        target: Some(Target::Permanent(buddy)), additional_targets: vec![], x_value: None, mode: None,
...
fn gishath_reveals_and_drops_dinosaurs() {
    ...
    let ctx = crabomination::game::effects::EffectContext::for_ability(_gishath, 0, None);
    let evs = g.resolve_effect(&crabomination::effect::Effect::RevealTopNPutMatchingToBattlefield {
        who: crabomination::effect::PlayerRef::You,
        count: crabomination::card::Value::Const(3),
        filter: crabomination::card::SelectionRequirement::Creature
            .and(crabomination::card::SelectionRequirement::HasCreatureType(crabomination::card::CreatureType::Dinosaur)),
    }, &ctx).unwrap();
    g.dispatch_triggers_for_events(&evs);
    drain_stack(&mut g);
...
fn the_mycotyrant_cda_and_end_step_fungi() {
    use crabomination::card::{CreatureType, Subtypes, TokenDefinition, CardType};
    let mut g = two_player_game();
...
fn view_surfaces_descended_this_turn_count() {
    let mut g = two_player_game();
    let v0 = crabomination::server::view::project(&g, 0);
    assert_eq!(v0.players[0].descended_this_turn_count, 0);
    for _ in 0..3 {
        let id = g.next_id();
        g.players[0].send_to_graveyard(crabomination::card::CardInstance::new(id, catalog::grizzly_bears(), 0));
    }
    let v1 = crabomination::server::view::project(&g, 0);
