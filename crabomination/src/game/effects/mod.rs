    ...
    fn run_effect(
        ...
        match effect {
            ...
            | Effect::SearchLibraryOrGraveyard { .. }) => {
                use crate::decision::Decision;
                let (who, picker_ref, filter, to, include_graveyard) = match e {
                    Effect::Search { who, filter, to } => (who, None, filter, to, false),
                    Effect::SearchLibraryOrGraveyard { who, filter, to } => {
                        (who, None, filter, to, true)
                    }
                    Effect::SearchPickedBy { who, picker, filter, to } => {
                        (who, Some(picker), filter, to, false)
                    }
                    _ => unreachable!(),
                };
                let Some(p) = self.resolve_player(who, ctx) else { return Ok(()); };
