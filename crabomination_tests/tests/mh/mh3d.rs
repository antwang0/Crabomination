...
fn ondu_knotmaster_grows_on_modified_death() {
    ...
    g.battlefield_find_mut(ally).unwrap().add_counters(crabomination::card::CounterType::PlusOnePlusOne, 1);
...
fn hydroelectric_specimen_stats_and_back_face() {
    ...
    assert!(back.card_types.contains(&crabomination::card::CardType::Land), "back is a land");
}
