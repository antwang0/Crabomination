...
9. **Seeds: TWO blocks this run, 24 cells / 118,400 games / 0 failures, `cap 0 / board 0 / stuck 0 / draw 16` on each** (pools `cube all sealed`, seeds 1354..1357 and 1358..1361, `target-audit/overflow` with `-C debug-assertions=yes` and `CRAB_ANSWER_LOG=strict`). They audit `(-324)`'s new lane assert, `(-323)`'s exact-tax assert and the nine loops that now keep their lanes across a write. **FRONTIER 1362 — read it from PERF's Baseline, not from this line.** Standing audits all 0 (`audit_panics` 0 bare / 70 sites, `audit_engine_stack` 0, `audit_stash_in_loop` 0 unexplained, `audit_seat_from_selector` 0 open, `audit_target_walkers --check` 0, `audit_doc_drift` 0/0/0, `audit_incomplete` 0 structural, `audit_stubs` 0).
