...
8. **Seeds: TWO blocks this run, 24 cells / 118,400 games / 0 failures, `cap 0 / board 0 / stuck 0 / draw 0` on both. FRONTIER 1354.** ⚠ **The prior NEXT said "frontier 1342" and that was STALE — PERF's Baseline is the authority and it said 1350.** 1342..1345 was therefore a RE-TAKE (last swept at the `(-318)` tip); it is still the block that audits `(-320)`'s new `debug_assert!` on dflt-pilot boards the suite never builds, and 1350..1353 is the frontier block. Both on `target-audit/overflow` with `-C debug-assertions=yes` and `CRAB_ANSWER_LOG=strict`. **Read the frontier out of PERF's Baseline, not out of this line.** Standing audits re-run at the start of the run and all 0 (`audit_panics` 0 bare / 70 sites, `audit_engine_stack` 0, `audit_stash_in_loop` 0 unexplained, `audit_seat_from_selector` 0 open, `audit_target_walkers --check` 0, `audit_doc_drift` 0/0/0).
9. **Bugs/robustness: nothing open.** Structural audits (`audit_incomplete` 1 finding = Elite Interceptor, triaged not-a-gap; `audit_stubs` 0), determinism, panic census all clean. TODO's robustness section has no open entries.
10. **Best next moves, in order.** (a) **READ the combat/block functions the way `(-321)` was found** — `declare_blockers` (1.71 % self), `declare_attackers_banded` (1.73 %), `blocker_pair_block` and `attack_block_keyword_tax` are all in view now; open each and look for whole-board walks with definition-only predicates that no table can name. That is the checklist's real population. **Price the population first** (line 5: a printed-shape gate is worth only its printed-negative fraction, and `(-321)` is what a small one looks like — take it anyway, it is free and compounding). (b) The fresh-seed sweep as the FIRST gate after any epoch/memo change — frontier 1354, **read it from PERF's Baseline**. (c) The three structural rows (line 6) are a count/structure question, not a line. (d) leftover context: `audit_dropped_may` 199 (`LookPickToHand` not safe at this granularity — put 73, exile 20, return 16, collect 12, begin 12); `audit_keyword_drift`'s last 4 each need a PRIMITIVE; card skips with a card behind them: `nocolors` 950, `noback` 60, `nosubtypes` 43, `nokeywords` 29.
