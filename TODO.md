...
10. **Cards and decks, what is left:** ⏳ 32 of 33 "can be your commander" non-creatures are unimplemented — *cards are chosen to complete decks*, so they wait on a sixth target deck wanting one. ⏳ `audit_each_opponent`'s residuals are down to **six** (Malcolm closed: the subject-keyed batch makes "a Treasure for each opponent dealt damage" exact); none of the six is in a target deck. ⏳ **CARD_BACKLOG's "Target-deck card defects" table is down to ONE**: Ghost Vacuum (three decks, and a real build — CARD_BACKLOG sizes it: the exile link exists, but the set selector is singular, no `CounterType` routes into `KeywordCounters` for the flying counter, and the "1/1 Spirit in addition to its other types" is a layer effect on the set the return just made). ⚠⚠ **THREE of the eight rows were wrong about the code** — Simian Spirit Guide and Arcane Signet had already shipped, and Delver of Secrets' "transform half is approximated" was about an optional *reveal* the engine answers optimally, not about the transform. ⚠ It used to say the eight were blocked on pool membership; **that was wrong on the facts** and the table now says so.
