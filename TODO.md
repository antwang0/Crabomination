...
1. **FIRST:** `git fetch origin claude/modern_decks && git checkout -B claude/modern_decks origin/claude/modern_decks`. Rebase, never force; code before tracker prose; **fetch before every push AND before you start**. ⚠⚠ **TWO SESSIONS RAN THIS BRANCH CONCURRENTLY on 2026-09-16 and both measured `(-336)`** — see PERF's "the block-planner session" block for the three conventions that made it survivable: a perf commit whose A/B has not finished **says so in its own message** and its numbers land in the next PERF commit; fetch before rebasing into an in-flight build; and **never `git stash` while a build of those sources is running** (it cost a binary that could not be proven to match either tree). Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure" + "Standing rules"**, ⚠ the 2026-09-12/13 detail (printed-body skip taxonomy, the `EventKind` fan-out census, the modal census, the collision log, the seed discipline) is in **ENGINE_BACKLOG's "The 2026-09-12/13 handoff detail, moved verbatim"** — read it before taking a skip kind or a seed. NEXT is 15 lines and stays 15.
2. **Gates at the `3a3daa5a` tip, all re-run:** `--bench` **195,806 / 27.49 / 611.9 / 0 stalls** byte-identical to the committed invariant, `determinism ok`, `thread_determinism ok (3 vs 1)`; suite **19,575 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, 12 golden traces unmoved); clippy **0** both feature sets; `cargo check --profile release-fast` clean. ⚠⚠ **CLIPPY WAS 1 FROM `1cfa4713` UNTIL `3a3daa5a` AND TWO COMMITS SHIPPED SAYING "check clean": `cargo check --all-targets` IS NOT CLIPPY.** Run clippy after any commit that rewrites code mechanically, not just at the start of a run. ⚠ And **read PERF's `(-339)` entry before planning perf work**: its largest row was found by `grep`, not a dump — a hand-written `iter().find()` inlines into its caller and never becomes a symbol, so the self table, `cg_calls.py`, `cg_edges.py` and the line profile are all structurally blind to it. `(-340)` is that grep run on the second spelling.
3. **Perf — SEVEN ROWS at this tip.** ⚠ A **third** concurrent agent landed **`(-342)` — `restore_payment_state`'s per-permanent snapshot scan becomes one merge walk, fixed +0.001 / cube -0.249 / sealed -0.046 %** (restore self **-75.7 % `cube` / -64.0 % `sealed`**, snapshot row unmoved to the digit). Its A/B was taken at the `(-339)` tip and **re-measured on the `3c2b4e50` tip to the digit**, which is the reusable half: a row whose callees did not move re-measures identically — it is the TIP that moves, so re-take the tip's absolutes and never derive them from a cumulative percentage (`3cfdc7ac` put 0.023-0.026 % on all three pools after `(-339)`'s numbers were filed). **Eleventh-box tip absolutes at `3c2b4e50`: fixed 578,037,636 / cube 1,512,279,098 / sealed 1,631,233,534.** And the rest: **SIX ROWS at this tip.** The concurrent session's four (`(-336)`..`(-339)`, cumulative fixed -0.879 / cube -1.725 / sealed -0.908 % against the `(-335)` tip) plus **`(-340)`/`(-341)`, fixed -0.174 / cube -0.142 / sealed -0.023 %**: `(-339)`'s grep run on the OTHER spelling — 52 `battlefield.iter().any(|c| c.id == x)` existence scans — then the two *search-everywhere* probes reverted to the unhinted scan. Per-row numbers in PERF's Log.
3a. ⚠⚠ **`(-340)`'s ATTRIBUTION REFUTED THE CENSUS THAT CHOSE ITS TARGETS, and this is the entry to read before the next class conversion.** Sites were ranked by enclosing function against the cube self table first; that ranking picked exactly the wrong two. The win was `check_state_based_actions_into` -2.02 M and `resolve_top_of_stack_inner` -529 k — both dismissed beforehand as "per-death" / "only in its arm" — while `find_card_anywhere_mut`, predicted as a real target, **regressed +466 k**. **A hinted lookup is priced by its caller's HIT rate**: `find_by_id` costs a load, a compare and (on a hit) a store, so a probe that walks battlefield -> stack -> hand -> graveyard -> ... pays it on top of the scan it was going to make anyway. Reverting those two restored the row to the pre-`(-340)` value *to the digit*. **Convert the class and measure; do not argue about which members are hot.** And ⚠ the regex edit swallowed `&& c.blitzed` / `&& c.is_token` into two id arguments and matched two `Vec<PermanentView>` test sites — all four caught by `cargo check --all-targets`, none by reading the diff: **a grep-driven conversion needs the compiler as its ratchet.**
4. **The device that found `(-337)`/`(-338)`: rank the dump by SELF PER CALL, not by self.** `declare_blockers` (5,740 Ir a call) and `declare_attackers_banded` (4,381) are the two largest *bodies* in the program and neither was in `(-329)`'s checklist, which ranks by share. ⚠ And `kws_of` / `computed_kw` are a **linear find over the gated subset** before they are an `OverlayList::get` — that is why a six-ask collapse here paid 3-4x `(-335)`'s five-ask collapse. **Price an accessor by its body, not by its name.**
4a. 🔎 **A SECOND DEVICE, AND IT FOUND `(-342)`: GROUP THE LINE PROFILE BY SOURCE LINE ACROSS THE WHOLE PROGRAM, not by function.** `cg_lines.py <instr dump> <binary> --rows 0`, then sum the `file:line` column. `card.rs:13-14` — `CardId`'s derived `PartialEq` — is **22,214,350 Ir = 1.47 % of `cube`** over ~90 functions, i.e. `(-339)`'s whole class stated as ONE NUMBER instead of grepped for; `restore_payment_state` reading **590 id compares a call** is an O(n x m) shape written down in one line, in a function no self table ranks. ❌ **And the first lead the grouping produced was REFUTED BY `objdump` FOR FREE, which is the rule to carry: `core/src/sync/atomic.rs` reads 41,046,116 = 2.71 %, 7.40 M of it inside `compute_permanents::{{closure}}` alone — and `find_by_id`'s whole hint mechanism there is TWO INSTRUCTIONS** (`mov 0x2f8(%r12,%rdx,4),%edi` / `mov %edi,0x2f8(%r12,%rcx,4)` at `0x89022d` / `0x8902ac`). The DWARF inline record for the atomic accessor covers a range the surrounding block falls into, so the *scan* is charged to `atomic.rs`. **A line row in an `#[inline]`-heavy build is an ADDRESS RANGE, not a statement** — `objdump -d --start-address=<sym>` the enclosing symbol and count the instructions the row claims before ranking it. No build, the binary is on disk. ❌ And `debug_flags`' `{:?}` scan (`is_contained_in` + `SmallVec` Debug + `String::write_str`) is ~12.6 M / **0.83 %** and is ONCE PER DISTINCT CARD NAME PER PROCESS — it saturates, no 10-30 k-game run pays it, **do not rank it**. ⚠⚠ **AND THE TRAP IN THAT DEVICE COST A BUILD: a line attributed to `card.rs:13` INSIDE a function does not tell you WHICH scan it is.** `find_by_id` is `#[inline]`, so its own compares land on the caller's row under the caller's source line. `computed_permanent_hinted`'s 2.15 M read as four compares a call against the freeze scope's `perms` list; a `[u8; 16]` slot table on `LayerFreezeState` (the `FIND_HINTS` contract, validated against the entry's id so needing no invalidation) reads **cube +0.151 / fixed +0.230 / sealed +0.242 %** and leaves `computed_permanent_hinted` self **unchanged to 0.05 %** — the 2.15 M was the inlined battlefield scan, and the 16 bytes went into a struct `GameState::clone` copies 22,034 times. **Confirm the scan is the caller's own before front-ending it.** 🔎 Also measured: **`profiling-lines` is codegen-identical to `profiling-fast`** (1,514,412,257 vs 1,514,422,113 on `cube`, **0.0007 % apart**) and costs **8m19s cold**, not the ~28 min the candidates entry assumed.

5. ✅ **THE TENTH BOX REPRODUCES THE NINTH'S ABSOLUTES** — `(-335)` tip rebuilt reads +0.00007 / -0.00001 / +0.00007 % against the filed numbers, `pick_blocks_inner` self identical to the digit on all three pools, after three boxes in a row that did not. **"Absolutes do not cross boxes" is not a law**; a disagreeing absolute means the toolchain, the profile or the tree moved. Confirm on the counter anyway — it costs nothing — but do not discard a filed absolute unread. Timings this box (4 cores, 15 GB): cold `profiling-fast` **8m58s**, engine-only **2m43s**, cold `overflow` in a second target dir **8m01s**, three-pool callgrind ~40 s in parallel, debug suite 90 s, workspace clippy ~5 min, `cargo check --profile release-fast` 1m20s. **Box timings (ninth/eleventh box, 4 cores, 15 GB):** cold `profiling-fast` **11m02s**, a `crabomination_base` one-liner **10m57s** (a base change rebuilds the catalog, and the catalog IS the cost — batch them), engine-only ~3 min, cold `release` **35m43s**, three-pool callgrind 25 s in parallel, debug suite 129 s after a ~4 min build, workspace clippy ~5 min, `cargo check --profile release-fast` 1m59s. ⚠ **Disk: `target/debug` reached 18 GB and left 8 GB free; `rm -rf target/debug/incremental` + `find target/debug/deps -maxdepth 1 -size +50M -delete` returned it to 1.2 GB and rebuilt nothing.** Do it before a `release` build, not after it dies.
6. ⚠⚠ **THE CONTAINER IS SUSPENDED BETWEEN TURNS — background waits DO NOT PROGRESS.** A `sleep` in a backgrounded Bash call advances the wall clock barely at all while the model is idle; an 11-minute build took six turns of polling to finish. **Run builds, sweeps and A/Bs in the FOREGROUND** with the tool's 600 s timeout, chaining `build && cp && for p in ...; do valgrind ... & done; wait` in one call. Also: `pgrep -f valgrind.bin` inside a wait-loop **matches its own command line** and never terminates (CLAUDE.md warns about this for `pkill -f`). ⚠ **And when a foreground build overruns the 600 s tool timeout and is moved to the background, RE-ISSUE THE SAME `cargo build` IN THE FOREGROUND** — it blocks on the artifact-directory lock and that block *is* the wait, returning when the background build finishes. `Monitor`, `until`-loops and polling the task's output file all fail: the container advances ~18 s of wall clock across several model turns.
7. **Perf — the sized queue is in PERF's "Perf candidates", top first. ⚠ (A) IS RE-PRICED AND THE TRAP LIST IS NOW MEASURED, by `scripts/inline_check.py` (new, free, no build).** (A) `ManaCost::cmc` — its 0.28 / 0.24 / 0.31 % is call **plus** body and **`release` already inlines the call away**, so a `CardData::printed_cmc` memo's ceiling is the 25-instruction body on ~90 k reachable calls, not the filed row; price that against **two ~11-minute base+catalog builds** before taking it. (B) `clear_end_of_turn_effects` — **RE-SIZED and the write side is priced: it is NOT the blocker.** 51,750 calls, **83.5 Ir a call, `out/call` 0.04** (96 % of calls make no call at all — the guard runs all 26 probes and returns), 19.5 permanents a cleanup; the dirty bit's width is ~30 k Ir total and its store is free (every one of the 26 fields is written through a `&mut` that already paid the unshare). **The blocker is correctness — 26 scattered write sites, one missed write = a stale EOT effect past cleanup** — so build it with `end_of_turn_effects_are_clear` kept as a `debug_assert!` on the not-dirty path, `(-337)`/`(-338)`'s shape. ❌ Both cheap placements are dead: a `DerefMut` flag on `CardInstance` is true for nearly every permanent by cleanup (untap, damage, counters), and a `GameState` per-turn counter is set by combat damage on every creature in combat; **114 write sites censused across the 26 fields**, so an exact bit is a refactor, not a mechanical change. 💡 **The shape that needs no write-site change is in PERF's (B): 11 of the 26 live in `CardCold`, which is a `CowBox` built with `CowBox::default()` per card — give that default ONE process-wide `Arc` and `Arc::ptr_eq` is a conservative 1-instruction "never wrote cold" gate worth ~0.12 % of `cube`.** Price its hit rate (cold also holds `named_card` / `keyword_counters` / `front_face`, and a card that writes any of them is unshared for the rest of the game) and remember it is a `crabomination_base` change at ~11 min a build. ❌ (E) **`pick_blocks_inner` — LINE-PROFILED THIS RUN AND CLOSED AS DIFFUSE:** 11.32 M self on `cube`, and the largest `server/bot.rs` line in the whole body is **467,020 Ir / 0.03 %**; the rest is sixty-odd std-generic rows of a few hundred thousand. There is no row in it — the body is the cost. ✅ **NEW and rankable: `CardInstance::power` (44,976 calls) and `toughness` (79,198) are REAL** — both survive thin LTO at byte-identical size — where this file had listed them as artifacts. The confirmed artifacts are `cmc`, `ManaPool::is_empty`, `counter_count`, `can_block`, `LocalKey::with`. Still unread with `(-329)`'s checklist: `resolve_top_of_stack_inner` 0.81 %, `advance_step` 0.67 %, `cast_candidates` 1.06 % (read this run — plain loops, one shared `SweepMana`, `BoardFacts` gathered once; already worked).
8. ⚠⚠ **Perf — the `release-fast` trap, MEASURED at last, and my first write-up of it was WRONG — read the correction.** `scripts/inline_check.py` (new, free: the `--bench` gate already leaves `target/release/bot_ladder` on disk) reports each symbol's presence and BODY SIZE in both binaries. The filed suspect list was wrong in both directions: `ManaCost::cmc` (25 insn), `ManaPool::is_empty` (32), `counter_count` (16), `can_block` (19) are confirmed artifacts, but **`CardInstance::power` (75 insn, 44,976 calls) and `toughness` (68 insn, 79,198) survive thin LTO at byte-identical size and are REAL and rankable.** ⚠⚠ **And the rule is narrower than I first filed it: "inlined away" is a statement about the SYMBOL, never about the cost.** I claimed `LocalKey<T>::with`'s 0.67 % was "an artifact of the profile"; it overstates by ~6x. Its 102-instruction body is mostly `cp_pool::alloc`'s own work (hence `drop_in_place<ComputedPermanent>` as its *callee*), which `release` does too — what `release` removes is ~2 call/return pairs an access, ~0.1 %. A 91-instruction `Arc::clone_from_ref_in` that "vanishes" is the CoW family's real 2.3 %, not a free win. **Judge by the body the script prints.** ⚠ Grep disassembly with `-P '\bcall\b'`; `'\tcall'` matches nothing, reads as "no calls", and is how the wrong version got written.
8a. ❌ **The battlefield adapter class is CLOSED: `.position(|c| c.id == x)` censused and declined, no build spent.** 10 sites, 5 of them test bodies and one cold; of the four that run, **two are search-everywhere probes that expect to MISS** (`move_card_to`'s "try battlefield first", `activate_ability_inner`'s `bf_pos`, which also serves graveyard and hand activations) — exactly what `(-341)` reverted at +466 k — and `run_effect`'s is inside the phasing arm and removes in a loop. That leaves ONE always-hit warm site worth ~0.005 %. Grep also finds zero production `find_map` / `filter(..).next()` battlefield-by-id sites. ❌ **Also declined with no build:** `computed_permanent(x.id)` -> `_on(x)` at 58 sites — the hint is consulted only on a `perms` MISS (the memo lookup returns first) and misses are the minority, and each site needs a per-site judgement that the card is a battlefield permanent because of `computed_permanent_hinted`'s `debug_assert!`. **The allocation picture is DIFFUSE** — 756,267 allocations a six-game `cube` run, `finish_grow` (Vec regrowth) 150,669 of them with its largest single site at 7 % — and its Ir is the *system* allocator's, which is not what ships. `String::write_str`'s 166,925 calls are the `wants_converge` `{:?}` scan, already documented as not scaling with game count. All four censuses are in PERF's candidates.
9. ❌ **Perf — still closed, do not re-open without a device none of the prior ones was:** `perform_action_inner`'s five `is_cast()` asks (a `matches!` over many variants is a 3-instruction bitmask — disassemble before ranking a family), `permanent_value_with`'s printed-shape gate (`(-320)`), the `ComputedPermanent` memo lane (`(-312)`), `available_mana` (`(-317)`), the target enumerator, the three `continuous_effects` duration scans, and the three structural rows `dispatch_triggers_for_events_slow` / `compute_permanent_pass` / `check_state_based_actions_into` (line-profile verdict FLAT). ⚠ And the `release-fast` trap stands: a non-generic `crabomination_base` leaf's CALL is a profile artifact — rank by the BODY.
10. **Seeds: ONE block this run, 12 cells / 59,200 games / 0 failures, `cap 0 / board 0 / stuck 0 / draw 4`** (seeds 1382..1385, pools `cube all sealed`, `target-audit/overflow` with `-C debug-assertions=yes` and `CRAB_ANSWER_LOG=strict`). It is the **ratchet for `(-336)` and `(-340)`**, and both needed one: `(-336)` widened `card_has_gate_keyword`'s union (whose `board_keyword_matching` `debug_assert!` re-derives the computed board on every `false`), and `(-340)` moved 50 existence checks onto `find_by_id` (whose `debug_assert!` asserts no two battlefield permanents share a `CardId`). **Neither assertion can fire on a board the suite builds** — that is what the sweep is for. ⚠ **The third agent ran the SAME block (1382..1385, 12 cells / 59,200 games / 0 failures, `cap 0 / board 0 / stuck 0 / draw 4`) on a binary carrying `(-342)` instead**, so the two blocks ratchet different assertions and neither covers the other's: `(-342)`'s merge walk carries a `debug_assert!` against the by-id scan it replaces, element for element and in board order. **Two agents took the same seeds — say which seeds you are taking in the commit that builds the sweep binary.** **FRONTIER 1386.** Standing audits not re-run (`audit_incomplete` / `audit_stubs` were 0 and no card changed).
11. **Bugs/robustness: ONE FIXED, and it was found by READING a function the perf work was already touching** — every `audit_*` was clean and this line said "nothing open" when it was found. `AttackerFacts::poison` summed Toxic/Poisonous off `definition.keywords` while the Infect leg one line above read the instance's four sources; the damage step reads both off the **computed** set, which lifts eot grants, pushes keyword counters (CR 122.1b) and keeps Toxic/Poisonous **cumulatively** (CR 702.180b/702.70b) — so a granted or stacked toxic scored zero on `poison_threatened`, the flag that turns on chump-blocking. Fixed at `3cfdc7ac` with the fold shared with the damage step (`combat::toxic_poison_value`) so the two walkers cannot drift; suite **19,575 / 0 / 5**, traces unmoved. ⚠ **Still open, same family:** `has_toxic` / `has_modular` read definition + `granted_keywords_eot` and stop — no counters, no removals. ENGINE_BACKLOG's twenty-seventh find has it; close it as a class. **Clippy was NOT 0 when the concurrent run started** — re-check at the START of a run, never trust this line.
12. **Best next moves, in order.** (a) Candidate **(H)**, the other zones' by-id scans — **already censused, read it before writing code**: 138 production sites (`hand` 68 / `exile` 29 / `graveyard` 29 / `library` 12), 73 of them in functions that never execute in a six-game run, and the obvious fix is wrong (`FIND_HINTS` is 64 bytes a pile; an eight-pile table adds ~512 bytes to every `GameState` clone, already 1.05 % of `cube`). ⚠ **And `(-341)` now bounds it from the other side: `find_card_anywhere`'s 247 Ir/call over 25,900 calls IS those zones, but every leg it walks past is a MISS, so a hint helps only the leg that finds the card.** A one-entry `Cell` hint (4 bytes a pile, not 64) is the shape that answers the clone objection — price the clone side first either way. (b) Queue item (E), `pick_blocks_inner`'s body: still 0.74 % self on `cube` and never line-profiled, and `(-319)`'s "no more `profiling-lines` builds" verdict names three other functions, not this one. (c) A fresh-seed block from **1382** as the FIRST gate after any lane/memo change. (d) leftover: `audit_dropped_may` 199; `audit_keyword_drift`'s last 4 each need a PRIMITIVE; card skips `nocolors` 950 / `noback` 60 / `nosubtypes` 43 / `nokeywords` 29.
