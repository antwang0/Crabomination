...
5. 🔥 **Perf — `(-313)` LEAVES THE ROW THAT IS ACTUALLY BIGGEST IN THAT TREE AND `(-314)` ONLY SHAVED IT: `available_mana` under `OnceCell::try_init`, 31.9 M on `cube` / 31.6 M on `sealed`, 1.8-2.0 % of the pool** (`(-314)` took its per-permanent relax scan for -0.046..-0.058 %, 6.2 % of the row, and its lesson is that **a presence gate in front of a probe of an EMPTY collection is priced by that collection's header loads — about two instructions — not by the walk it imagines**, so the rest of this row will not fall to more of the same), against ~5-6.6 M for every other callee of the affordability read combined. It is a whole-battlefield walk **per `SweepMana`** (15,884 on `sealed` against 17,016 `cast_candidates` calls — the `shared:` parameter that exists to amortise it is passed `None` by both hot callers). ⚠ Half of them are `sim_spell_action_inner`'s, over a *simulated* state that genuinely cannot share; `main_phase_action_with`'s 8,280 is the half that could. Sizing, its callee tree (`grant_scan` 4.98 M / `tap_ability_summoning_sick` 3.63 M / `grants_nothing_slow` 1.75 M on `cube`) and the two questions it needs are in PERF's candidates head. ⚠⚠ **Price any memo here against `(-312)` first** — the bot writes between every pair of asks, so the win is in the plumbing, not the key.
