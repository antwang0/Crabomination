...
4. **Perf — the transferable rule (same as `(-317)`, one function up): a whole-board walk that reads the COMPUTED type line only needs the PRINTED one when nothing can change a type. Ask `card_type_change_in_scope()` once for the board.** The instrument is the callee table, not the self table: `computed_permanent_hinted` is called from 230,016 sites; rank its callers, take the one whose per-permanent question the printed shape already answers.
...
8. **Seeds: frontier 1342 — NOT swept this run** (the run went to the `legal_blockers` row instead). Take 1342..1345 on the `(-320)` tip with `target-audit/overflow` (`-C debug-assertions=yes`), which exercises `(-320)`'s new `debug_assert!` on every board — coverage the suite can't reach. `scripts/fresh_seed_sweep.sh "cube all sealed" "1342 1343 1344 1345"`. Standing audits were all 0 at the `(-319)` tip and code paths they cover are unchanged: re-run `audit_panics`/`audit_engine_stack`/`audit_target_walkers --check`/`audit_doc_drift` if you touch their areas.
...
10. **Best next moves, in order.** (a) Run the callee-table checklist to the end (line 4): rank `computed_permanent_hinted`'s remaining callers (`pick_blocks_inner` 20,214, `pick_attacks_inner` 14,678, `blocker_can_block_attacker` 11,152 on cube) and every `battlefield.iter().any(` over ~0.5 %, asking which per-permanent question the printed shape answers — but PRICE THE POPULATION FIRST (line 5's lesson: a printed-shape gate is worth only its printed-negative fraction). (b) The fresh-seed sweep (line 8) as the FIRST gate after any epoch/memo change, and for `(-320)`'s new `debug_assert!`. (c) The structural rows (line 6) are a count/structure question, not a line. (d) leftover context: `audit_dropped_may` 199 (`LookPickToHand` not safe at this granularity — put 73, exile 20, return 16, collect 12, begin 12); `audit_keyword_drift`'s last 4 each need a PRIMITIVE; card skips with a card behind them: `nocolors` 950, `noback` 60, `nosubtypes` 43, `nokeywords` 29.
