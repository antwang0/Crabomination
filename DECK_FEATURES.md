...
| Job Select (CR 702.182) | ✅ | Equipment ETB mints a 1/1 colorless Hero token and self-attaches (living-weapon shape — `job_select_equipment`), carrying the "is a [class] in addition to its other types" rider via `EquipBonus.add_creature_types`. The FIN cycle: Monk's Fist, Bard's Bow, Paladin's Arms, Samurai's Katana, Warrior's Sword, Dark Knight's Greatsword, Thief's Knife, Ninja's Blades, Red Mage's Rapier, Black Mage's Rod, Sage's Nouliths, Dragoon's Lance, Machinist's Arsenal, Astrologian's Planisphere. |
| Tiered (Final Fantasy) | ✅ | `Effect::Tiered` — "choose one additional cost", sharing the `GameAction::CastSpellSpree` cast path with an exactly-one-mode rule. Fire / Thunder / Ice Magic, Tifa's Limit Break, Restoration Magic. |
| Fortify (CR 702.67) | ✅ | `Keyword::Fortify` routed through the equip action against a land host; the CR 704.5n unattach SBA distinguishes Equipment (creature host) from Fortification (land host). Darksteel Garrison. |
...
| Sacrifice-as-cost effects | 🟡 | Thud ✅; variable-count sacrifice ✅ (`Effect::SacrificeAnyNumber`); flashback-with-additional-cost ✅ (Lava Dart, Dread Return); reflexive "you may sacrifice X; if you do, …" ✅ (`Effect::MaySacrifice` — Bloodcrazed Socialite, Gut); reflexive "you may discard X; if you do, …" ✅ (`Effect::MayDiscard` — Toph, Hardheaded Teacher; the bot screens its payoff). |
