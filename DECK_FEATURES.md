...
| Drown in the Loch | {U}{B} | 🟡 | Instant. ChooseMode([CounterSpell, Destroy(Creature)]) (creature-only per Oracle; not PW). The "mana value ≤ cards in the target's controller's graveyard" gate collapses (no per-target dynamic-MV restriction). Tests: `drown_in_the_loch_mode_zero_counters_a_spell`, `drown_in_the_loch_mode_one_destroys_creature`. |
