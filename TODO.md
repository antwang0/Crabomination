...
3. **Perf — THREE ROWS TAKEN. `(-317)` is the big one: `available_mana` reads its per-permanent answer off the definition, fixed -0.842 / cube -0.496 / sealed -0.540 %, the row -43.9 / -35.6 / -36.8 %.** For an untapped, non-sick permanent granted nothing, the loop's whole job is a pure function of the printed ability list, so `mana_summary_of` packs it into the word's last free bits — `AM_COLORS` (51-55), `AM_COLORLESS` (56), `AM_BEST_ONE` (57), `AM_EXACT` (58). `AM_EXACT` clear means "run the walk", so the pack can only **decline**, never mislead. **`(-315)`** gated `grant_scan`'s one ungated pass (fixed -0.188 / cube -0.113 / sealed -0.203 %, row -36.4 / -29.5 / -47.1 %) and **`(-316)`** the CR 602.5g haste walk (-0.002 / -0.044 / -0.028 %, row halved). The branch moved **fixed -1.02 / cube -0.64 / sealed -0.76 %** between the `(-314)` tip (616,268,574 / 1,626,125,678 / 1,713,378,250) and the `(-317)` tip (609,992,918 / 1,615,703,398 / 1,700,369,346) — which also carries a concurrent session's two census commits, measured at ±0.0004 %, and the move's +0.011 %.
4. **Perf — four transferable rules, and two of them are new.** **(i) Size a presence gate by what the loop body does on a MISS, not by the loop** — `(-314)` gated a probe of an empty `Vec` (two instructions a permanent, 0.05 %); `(-315)` gates two `Option` discriminants out of a large `CardDefinition` and is thirty times that. **(ii) ~40 % of what a lane takes off a row comes back** (the lane read at each call site plus `walk_and_store`'s walk per invalidation). **(iii) Ask for the WEAKEST precondition that makes a packed answer sound, and ask for it ONCE** — `(-317)` gated on `grants_nothing` twice over and read -0.69 / -0.36 / -0.42 %; what the walk needs is only `granted.is_empty()`, and asking `granted_abilities_of` once both widened the population and deleted a duplicated evaluation, for a fifth of the win. **(iv) A mechanical file move is NOT free under `release-fast`** — cgu 16 with no LTO makes a cross-module callee a call: moving four predicates out of `bot.rs` read **+0.179 / +0.097 / +0.100 %** until each got `#[inline]`, and took `accumulate_mana_colors` from 53,844 Ir to 2,320,396. **Check the moved symbols' rows after any module split.**
...
6. 🆕 **Perf — THE NEXT MOVE IS NEWLY SIZED AND NOBODY HAS EVER NAMED IT: `fire_combat_damage_triggers` is the FOURTH biggest self row on `cube`** — fixed 6,920,234 (1.13 %), cube **32,699,392 (2.02 %)**, sealed 22,660,994 (1.33 %), 22,008 calls on `cube` at ~1,486 Ir of *self* each and a whole callee tree of only ~2.1 M, so the self **is** the work. Its first act is a whole-battlefield walk that does not short-circuit, folding `any_attached` and `soulbond_pair` once per call. ⚠ **Both halves read INSTANCE fields, so `(-315)`'s definition lane does not apply** — the sound key is `(-306)`'s `battlefield.writes()`, the way `sba_fold` already uses it. ⚠⚠ **CENSUS THE HIT RATE FIRST**: the gate only pays on a board with zero `attached_to.is_some()`, and one Aura or Equipment defeats it — this is the shape `(-312)` refuted one lane over. If the rate is low, the cost is in the three trigger phases below the walk and that wants a `profiling-lines` dump, cold.
7. ❌ **Perf — sized and DECLINED, so nobody spends a build:** `main_phase_action_with`'s `has_magecraft` (bot.rs:7662) and `has_opus` (bot.rs:7706) are ungated whole-battlefield walks with definition-only predicates, but once *per tick* and with a `controller` compare plus a length load for a body — `is_opus_trigger::branches` is **60,168 Ir on `sealed` (0.0035 %)**, 2,210 on `cube`. 0.02-0.05 % for the pair. ⚠ **And the mana word has NO free bits left**: 43-50 are `PLAIN_TAP`/`PLAIN_LAND`/`LAND_MANA_REPLACER`, 51-58 `(-317)`'s, 59-62 the marker bits, 63 the memo's — **a sixth per-definition family needs a sixth `CardMemo` word**, which is a `crabomination_base` change and a ~70-minute cold rebuild, so batch it. Only lane shifts **60 and 62** are left on the battlefield word. The `ComputedPermanent` lane stays CLOSED (`(-312)`), as do `(-309)`, `(-307)` and the **target enumerator**.
