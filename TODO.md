...
11. **Bugs/robustness: nothing open.** No `.bak`/stray files, no open `FIXME`, the one `#[ignore]` is the documented expensive A/B ladder, and the three `TODO` mentions in the catalog are citations of this file. **Clippy was NOT 0 when this run started** — two warnings in `crabomination_ml` from `2648ac08` that the `(-334)` NEXT had claimed clean; both fixed (`PackedGroup` alias, an `#[allow]` matching the tree's other wide helpers). Re-check clippy at the START of a run rather than trusting the previous handoff.
