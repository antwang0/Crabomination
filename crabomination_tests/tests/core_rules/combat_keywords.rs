...
use crabomination::card::{CardDefinition, CardType, Keyword, Subtypes, TriggeredAbility};
...
fn decayed_creature(name: &'static str) -> CardDefinition {
    CardDefinition { keywords: vec![crabomination::card::Keyword::Decayed], ..body(name, 2, 2, vec![]) }
}
...
fn cr_702_39_provoke_doesnt_force_when_unable_to_block() {
    ...
    def.keywords.push(crabomination::card::Keyword::Flying);
...
fn combat_preview_flags_a_losing_trade() {
    ...
    g.perform_action(GameAction::DeclareBlockers(vec![(blk, atk)])).expect("block");
    drain_stack(&mut g);
    let prev = crabomination::server::view::project(&g, 0).combat_preview.expect("preview");
...
fn create_token_attacking_joins_combat_tapped() {
    use crabomination::card::{CreatureType, TokenDefinition};
...
fn cr_724_become_monarch_effect_via_etb() {
    ...
    def.cost = crabomination::mana::cost(&[crabomination::mana::generic(1)]);
...
fn cr_509_1d_block_tax_auto_taps_lands() {
    ...
    use crabomination::card::{CardDefinition, CardType, StaticAbility};
    use crabomination::effect::{StaticEffect, Value};
...
fn cr_500_7_view_flags_extra_combat_and_end_step() {
    ...
    assert!(!crabomination::server::view::project(&g, 0).extra_phase, "first combat isn't extra");
    // Second (looped) combat: flagged.
    g.combat_phases_this_turn = 2;
    assert!(crabomination::server::view::project(&g, 0).extra_phase, "second combat is extra");
    // Outside combat the combat count doesn't flag the phase.
    g.step = TurnStep::PostCombatMain;
    assert!(!crabomination::server::view::project(&g, 0).extra_phase, "main phase never extra");
    // A repeated end step is flagged too.
    g.step = TurnStep::End;
    g.end_steps_this_turn = 2;
    assert!(crabomination::server::view::project(&g, 0).extra_phase, "second end step is extra");
}
