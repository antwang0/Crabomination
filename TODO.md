...
3. 🔎🔎 **THE FIND, AND IT IS A RULES CLASS WITH 89 ROWS LEFT: "As this ~ enters" IS A REPLACEMENT (CR 614.12), NOT A TRIGGER.** An `EntersBattlefield` trigger puts the permanent on the battlefield *first*, so a conditional tapper leaves a priority window its controller can tap the land in, a static keyed on the choice reads an unchosen value across an SBA check, and CR 614.12's own example (a token copy choosing as it is created) has no trigger to fire. `scripts/audit_as_enters.py` is the census: **89 cards in five buckets** — choose-a-type 27, choose-a-colour 20, pay-life-or-tapped 19 (all ten shocklands), other 14, choose-a-name 9. ENGINE_BACKLOG's **forty-eighth find**.
4. 📐📐 **AND WHY IT IS OPEN RATHER THAN A BATCH OF CARD EDITS — THE ONE MISSING PRIMITIVE.** `apply_enters_tapped_replacement` (`actions.rs:4209`) is where this shape is applied and it reads a `Predicate`: it can test the state, it cannot **ask**. ⚠⚠ **The synchronous shortcut is a REGRESSION, not a shortcut**: the tree has precedent (`chosen_mana_color`, `ManaPayload::AnyColors`) for resolving a choice through `self.decider` with no pending state, and it would work for self-play — but a shockland's choice is a real `Decision::ChooseMode` prompt in the client **today**, and routing it through the decider takes that choice away from a human seat. The work is a **suspendable** replacement application, and the function returns `()` from three call sites inside `move_card_to`-shaped paths.
5. ✅ **THE SUB-CLASS THAT DID CLOSE IS THE TEMPLATE FOR THE REST.** The fifteen "you may reveal a [X] card from your hand, else enters tapped" lands ask *nothing* — `IfRevealFromHand` peeked at the hand and always accepted, because declining only buys the printed downside — so a `StaticEffect::EntersTappedUnless` over a hand predicate is the whole card, no new primitive. **Ask first whether the printed "may" is a real choice**; when it is not, the replacement already exists.
