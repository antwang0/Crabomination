...
1. ⚠⚠ **TWO SESSIONS SHARE THIS BRANCH. FETCH BEFORE YOU START AND BEFORE EVERY PUSH.** Rebased once mid-run (a client session landed seven commits: deck import, pod table awareness, the command-zone HUD). `CR_COVERAGE.md` is **generated** — take either side of a conflict and re-run `scripts/cr_coverage.py`, never merge it by hand. `cargo nextest` fail-fast hides the rest, so `--no-fail-fast`. Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure"**, the 2026-09-12/13 detail in **ENGINE_BACKLOG's "moved verbatim"**. ⚠ **DO NOT EDIT THE TREE WHILE A BUILD IS RUNNING** (markdown is safe — nothing `include_str!`s a tracker; verified). ⚠⚠ **DISK IS THE RECURRING TAX AND `CARGO_INCREMENTAL=0` IS THE FIX**: this box hit `No space left on device` **three times**, twice as a bogus "could not compile … (test)" with no error body, and `target/debug/incremental` was the whole of it each time. Export it for the session, not per command.
2. **Gates at the closing tip `5a4bc0d6`:** suite **20,273 / 0 / 6** — and at `00d0f08b` — ⚠⚠ **TWO NUMBERS, BOTH MEASURED, AND THE GATE LINE HAS TO SAY WHICH COMMAND: 20,085 without `crabomination_client`, 20,285 with it.** `crabomination_client` holds exactly **200** `#[test]`s and is the only member CLAUDE.md's documented suite excludes (`--workspace --exclude crabomination_client`), so 20,085 + 200 = 20,285 and neither reading was ever wrong: a session that can build the Bevy stack runs the wider one, a session that cannot gets the narrower. The line read 20,285, was "corrected" to 20,085 as a transposition, and was corrected back — two sessions each re-ran the suite and each got its own right answer, because a bare count does not carry its command. **Quote the exclusion with the number, every time.** (The neighbouring inference held up and is worth keeping: a one-line tracker edit cannot move a count, and nothing `include_str!`s a tracker.) — (`CRAB_ANSWER_LOG=strict`); clippy **0** over the workspace; `--bench` **195,806 / 27.49 / 611.9 / 0 stalls — byte-identical to the committed invariant**, `determinism ok (all pairs split)`, `thread_determinism ok (3 vs 1)`, `peak_rss_mib` 31.7; `audit_panics` **0 bare**; every mirror column 0 (`seat_walks` 0, `retarget` 0, `each_player` 1-allowlisted, `target_opponent` 1 known, the three invented-* columns 0, `loop_splice` 13/13/0/0) and `audit_as_enters --gate` clean. CR_COVERAGE **146/146**. **Pod smoke at a FRESH seed 9102: 2,000 games at each of 2..8 seats = 14,000 games, 14,000 DECIDED, every `undecided_by` column zero, zero panics.** turns/game 19.37 / 32.35 / 45.30 / 56.88 / 64.74 / 77.85 / 93.34. **FRONTIER 9103.**
3. 🔎🔎 **THE FIND, AND IT IS A RULES CLASS: "As this ~ enters" IS A REPLACEMENT (CR 614.12), NOT A TRIGGER.** An `EntersBattlefield` trigger puts the permanent on the battlefield *first*, so a conditional tapper leaves a priority window its controller can tap the land in, a static keyed on the choice reads an unchosen value across an SBA check, and CR 614.12's own example (a token copy choosing as it is created) has no trigger to fire. `scripts/audit_as_enters.py` is the census and the ratchet: **89 cards when it was written, 57 by the end of the same day** — choose-a-type 23, pay-life-or-tapped 19 (all ten shocklands), other 15. ENGINE_BACKLOG's **forty-eighth find**. 🔎🔎 **AND THE SIBLING CLASS IS BIGGER AND NEEDS NOTHING: "enters tapped" IS A REPLACEMENT TOO (CR 614.1c), IN A CLAUSE THAT AUDIT CANNOT SEE** because it does not open with "As". `scripts/audit_enters_tapped.py`, ENGINE_BACKLOG's **forty-ninth find**: **83 cards when written, 0 now — the class is CLOSED and `--gate` fails on any row.** `StaticEffect::EntersTapped` / `EntersTappedUnless` had shipped the whole time, so every row was a catalog edit: `slow_land` (10), `etb_tap()` over 33 call sites including `tri_land` and `cycling_dual` (32), the `etb_tap_then_*` family split into `enters_tapped()` + `etb_{scry,gain,surveil}_one()` behind `tapland_untyped`/`tapland_typed` (26), `fastland_etb_conditional_tap` (17), `land_tapped_unless` (2), and 26 hand-written bodies. ⚠ **Two of the eight conditional ones KEEP a trigger** — Dwarven Mine's "when this land enters **untapped**, create a Dwarf" and Mystic Sanctuary's recursion are a real trigger whose intervening-if is the same predicate; the replacement goes beside it, not instead of it.

4. ⚠⚠ **AND THE ENTRY'S FIRST ANSWER WAS WRONG, WHICH IS THE MORE USEFUL HALF: THE ASKING REPLACEMENT ALREADY SHIPPED.** It said the class needed a new primitive because `apply_enters_tapped_replacement` reads a `Predicate` and cannot ask. True of *that applier*, false of the engine — `CardDefinition::as_enters_effect` resolves inside the battlefield hop through **`resolve_effect_driven`**, which suspends, and forty shipped cards already ask through it. **A class audit that only asks "which cards ship the wrong shape" prices the fix off the wrong denominator; ask "which shapes does the engine already have" in the same pass.** A concurrent session then funnelled all three as-enters appliers through all four entry paths (`game::as_enters`), so a choose-a-* conversion is now a catalog edit. ⚠ **That session owns this class right now — check `git log origin/<branch>` before taking a bucket.**
5. 💡 **THE ONE BUCKET STILL BLOCKED, AND A LEAD ON IT.** `pay-life-or-tapped` is a choice about a **cost**, not about the board, so `EntersTappedUnless`'s `Predicate` cannot say it. It may still need no new primitive: `as_enters_effect` asks, and `Effect::MayDoElse { body: LoseLife 2, else_: Tap(This) }` under an `Effect::If` on the life total is the printed line (CR 119.4 — paying life *is* losing that much, and a player below the amount cannot pay). ⚠ **What to check first is ORDER**: where the funnel runs relative to `apply_enters_tapped_replacement` and the ETB trigger gather, because the permanent must never be on the battlefield untapped with a priority window open — that is the whole point of the class. ⚠ And the synchronous `self.decider` shortcut is a **regression** whichever applier is used: a shockland's choice is a real `ChooseMode` prompt in the client today.
 ✅ **The sub-class that DID close here is the template for the cheap half of the rest:** the fifteen "you may reveal a [X] card from your hand, else enters tapped" lands ask *nothing* — `IfRevealFromHand` peeked at the hand and always accepted, because declining only buys the printed downside — so a plain `StaticEffect::EntersTappedUnless` over a hand predicate is the whole card. **Ask first whether the printed "may" is a real choice**; when it is not, the predicate form already exists and no ask is needed at all.
6. 📐📐 **FIVE LAND CYCLES, ONE BODY EACH, AND THREE OF THE FIVE HAD DRIFTED — a cycle written N times is N places for the Nth card to differ.** hybrid filter (`sets::hybrid_filter_land`) 3 cards / **2 shapes** → 10/10; pay-one filter (`pay_one_filter_land`) → 10/10; reveal-or-tapped (`reveal_or_tapped_land`) 10 cards / **3 shapes** → 15/15; Shards tri-lands (`tri_land`, already shared) → 10/10; triomes (`decks::modern::triome`) → 10/10. **32 cards re-pointed, 23 added.** CARD_BACKLOG carries the class.
7. ⚠⚠ **QUERY THE ORACLE, NOT THE CYCLE'S SHAPE — the one row this run got wrong.** Cabaretti Courtyard sits in the same set, the same top-1000 slice and over the same three colours as the SNC triomes, and its oracle is "sacrifice it, search for a basic Mountain, Forest, or Plains". Jetmir's Garden is the Naya triome. The batch was sized off a **type line**; the oracle is the only thing that settles it.
8. 🔎 **AND THE SAME LESSON FROM THE ENGINE SIDE, WHICH IS THE MORE TRANSFERABLE HALF: ONE HELPER, TWO PURPOSES, AND A FALLBACK THAT IS RIGHT FOR ONLY ONE.** `GameState::commander_identity_colors` answers **all five colours** for a seat with no commander — deliberate, so `ManaPayload::AnyColorInCommanderIdentity` keeps a fixing land fixing in a cube. Read as a *cost*, that fallback is **five life for a card**: War Room outside a Commander game took the seat from 20 to 15, and the test that caught it was the "no commander" one. `Value::CommandersColorIdentityCount` reads the raw identity instead. **Before reusing a helper, read what it does at its EDGE, not in the middle.**
9. ✅ **Cards: 23 lands + 5 spells/creatures this run, every oracle read from `scripts/.scryfall_cache.json`.** Commander-matters in `sets::cmdr`: War Room (EDHREC **140**, the highest-ranked gap), Bastion Protector, Loyal Apprentice. Ordinary staples in the new `sets::decks::recent330`: Parallel Lives, Avacyn. Lands in `decks/lands.rs` and `decks/recent13.rs`. ⚠ **`sets::cmdr` is for text ABOUT the format** (command zone / a commander / colour identity); an EDHREC staple that is an ordinary card goes in `recent330`, whose header says so.
10. 📐 **THE ONE ENGINE FIELD, AND THE RULE IT FOLLOWED.** `ActivatedAbility::life_cost_value: Option<Value>` — "pay life equal to [a value]" in the **cost** line, evaluated once so the pre-flight gate and the payment cannot disagree. A `condition` plus an `Effect::LoseLife` would have been ENGINE_BACKLOG's own twenty-third find ("a condition is not a cost"). `None` on every other ability, and `--bench` is byte-identical across it.
11. **Standing leads, in the order they look worth taking:** ① the CR 614.12 pay-life bucket (item 5) — 19 cards and the shocklands are top-100 — and the 51 rows of the 614.1c sibling beside it, which need nothing at all. ② ⚠ **A LEAD THAT WAS WRONG AND WAS TAKEN ANYWAY, WHICH IS THE POINT:** this line said `Keyword::Equip` carries one cost, so Commander's Plate and Blackblade Reforged needed a restricted second equip cost. `CardDefinition::equip_filtered_cost` already held a `(filter, cost)` pair and `equip` preferred it whenever the host matched — both shipped as catalog edits, alongside the Plate's colours-outside-the-identity protection set. **Grep the field before filing an engine gap**; this is the second time in one run a "missing primitive" was already in the tree (item 4 is the first). ③ `audit_dropped_may` **198** and `audit_incomplete` 858 are still the two big unmined columns. ④ 32 of 33 "can be your commander" non-creatures remain unimplemented — ordinary card work now that the eighth deck exists.
12. **Cards left in COMMANDER_BACKLOG's top-1000 that need no new primitive** (the `recent330` queue): Mirror Entity, Teleportation Circle, Eerie Interlude, Second Harvest, Archivist of Oghma, Champion of Lambholt, Nesting Grounds, Treasure Vault, Witch's Cottage, Gavony Township's neighbours. ⚠ `COMMANDER_BACKLOG.md` is **generated and now stale** — it still lists the 23 lands this run added; re-run `scripts/commander_backlog.py` before trusting a count from it.
13. ⚠ **POOL MEMBERSHIP IS A MEASUREMENT, NOT A VETO, AND IT COVERS ENGINE EDITS TOO.** `--bench` plays `--decks fixed` = `bot_ladder::archetypes()` (basic lands and 34 spells), `golden_trace.rs` has two more decks. Not one of this run's 51 cards is in either, and the one engine field is `None` everywhere but War Room — which is why a run this size moved **no** committed number. ⚠⚠ **When the committed pod table and the aggregate disagree, the aggregate wins**: `cr_903_seeded_pod_outcomes_match_the_committed_table` is three games, and its re-bless this run moved 55→47 / 101→67 / 57→49 turns while the 14,000-game aggregate moved ≤0.4 at every seat count.
14. ⚠ **Five gotchas that each cost a round.** ① A `TriggeredAbility` has no intervening-`if` field — the gate goes inside the effect as `Effect::If`. ② `g.perform_action` returns `Result<Vec<GameEvent>, _>`, not `Result<()>`. ③ A test that draws needs `add_card_to_library` first; an empty library draws nothing and the assertion reads as a card-logic failure. ④ New Commander cards need `python3 scripts/gen_factory_lists.py`; a new `decks/*.rs` also needs **two** lines in `decks/mod.rs` (`mod` and `pub use`). ⑤ `ZoneDest` is `crate::effect`, `CounterType` is `crate::card` — the two imports that cost a compile each.
15. ⚠ **The rules table is DONE and that is the handoff's main claim**: FEATURE_ROADMAP's Commander (CR 903) and multiplayer (CR 800-series) sections are all ✅ bar CR 800.4i (unreachable by construction) and an N-seat observation encoder (⛔ by design). What is left in Commander is **cards and the 614.12 class**, not rules.
