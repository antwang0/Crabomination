...
use crabomination::card::{CounterType, EventKind, Keyword};
...
fn cast(g: &mut GameState, id: crabomination::card::CardId, target: Option<Target>) {
    fill_mana(g);
...
fn guardian_manifests_on_modified_death() {
    ...
    let mut evs = Vec::new();
    g.deal_damage_to_from(crabomination::game::effects::EntityRef::Permanent(bear), 3, None, &mut evs);
...
fn ghostfire_slice_cost_reduction_and_damage() {
    ...
    let mut g = two_player_game();
    let inst = crabomination::card::CardInstance::new(g.next_id(), catalog::ghostfire_slice(), 0);
