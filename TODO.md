...
1. **FIRST:** `git fetch origin claude/modern_decks && git checkout -B claude/modern_decks origin/claude/modern_decks`. Rebase, never force; code before tracker prose; **fetch before every push AND before you start** (three sessions have run this branch concurrently). Gotchas in **CLAUDE.md**, measurement in **PERF's "How to measure" + "Standing rules"**, ⚠ the 2026-09-12/13 detail (printed-body skip taxonomy, the `EventKind` fan-out census, the modal census, the collision log, the seed discipline) is in **ENGINE_BACKLOG's "The 2026-09-12/13 handoff detail, moved verbatim"**. NEXT is 15 lines and stays 15.
2. **Gates at the `ac1381ed` tip, all re-run:** `--bench` **195,806 / 27.49 / 611.9 / 0 stalls / peak_rss 25.2** byte-identical to the committed invariant across all eleven commits, `determinism ok`, `thread_determinism ok (3 vs 1)`; suite **19,595 / 0 / 5** (`CRAB_ANSWER_LOG=strict`, 12 golden traces unmoved); clippy **0**; `cargo check --profile release-fast` clean. Sweep: **42 cells / 207,200 games / 0 failures** over seeds 1398..1411, `stuck 0 / draw 26 / cap 2` (both `cube` MAX_BATTLEFIELD runaways on a saturated seat — the script's carve-out), **FRONTIER 1412**. `audit_stubs` 0 of 21,797, `audit_panics` 0 bare, `audit_doc_drift` 0/0/0.
3. 🔎🔎 **THE HIGHEST-YIELD QUESTION OF THIS RUN WAS NOT A WALKER DIFF — IT WAS "LIST THE FAMILY THIS FUNCTION ALREADY HAS AND ASK WHICH MEMBERS ARE MISSING".** The battlefield walker builds a computed-view closure per characteristic (`has_type`, `has_ctype`, `has_atype`, `has_ltype`, `has_stype`). **Seven characteristics were being asked about and only five had a closure** — the keyword and colour leaves read the instance and the print. So every "creature with flying" filter in the catalog missed a Levitation grant and honoured a layer-6 strip. ENGINE_BACKLOG's fortieth find.
4. 🔎 **The other two devices, both still live.** (a) **Diff the arms of two walkers over one enum** (thirty-fifth find: 165 vs 205 arms, 92 differing, 33 real). (b) **Then grep for who calls the weaker one** — that is what turned 33 divergences into a 61-site class. Twelve finds for the family so far; five walkers.
5. ⚠⚠ **AN ALLOWLIST DOCUMENTS A CONTRACT, NEVER THE CALLERS — three passes read the P3 allowlist as "deliberate" and all fourteen drifts were live on the board.** ⚠⚠ **AND THE MIRROR LESSON: A RAW `c.power()` IS NOT AUTOMATICALLY A DEFECT.** Of ~26 remaining reads on a battlefield permanent only eleven were rules reads; the rest are auto-decider orderings (a *strength* change with no gate) and one is a per-card pump measurement `effective_power` would actively break. **Ask whose power the card is asking about.**
6. 📐 **THREE NEW ACCESSORS, AND THEY RE-PRICE THE THIRTY-FOURTH FIND.** `effective_power_on` / `effective_toughness_on` go through `computed_permanent_on`, so a `max_by_key` over the board pays no by-id find: **seventeen computed reads cost +0.001 / +0.022 / +0.001 %** where `Value::PowerOf`'s ONE read through `effective_power` cost `fixed +0.118 %`. `computed_is_creature` is gated and reads **negative**. Use `_on` when already walking the battlefield; gate a type read the way `printed_requirement_impl` gates its own.
7. ⚠⚠ **A RULES FIX CAN COST 0.6 % OF A POOL AND `--bench` WILL NOT SAY SO** — it and all 144 golden traces were byte-identical across every commit of this run. 📐 **Price the ROUTING separately from the walk:** two-thirds of the card-walker gate's cost was the entry (`battlefield_find` + `PrintedGates` per *ask*), which `(-343)` took back (**cube -0.279 %**). ✅ **Run total: fixed -0.075 / cube +0.038 / sealed -0.005 % — five rules-correctness class fixes for NOTHING, and two pools end faster than they started.**
8. ✅ **`(-345)` — THE KEYWORD/COLOUR LEAVES' PRESENCE GATES, filed and taken the same day: fixed -0.242 / cube -0.121 / sealed -0.125 %, which recovers the whole of the correctness fix's cost.** 📐 **What made it sound AND cheap is asking WHICH DIRECTION the layers can move the answer**: `AddKeyword` is the only additive keyword modification, so a permanent without the keyword can only gain it (`keyword_grant_in_scope`, lane-backed, the common ask) and one with it can only lose it (the new `keyword_removal_in_scope`, a board walk, the rare branch). ⚠ **A computed read on an UNFROZEN state gathers once per distinct state** — the ungated version was 370 extra gathers on `fixed`. ⏳ Still owed: `card_color_change_unscoped` is a bare board walk (101,066 Ir a `fixed` run) and its own doc asks a hot caller for a valid flag; this is that caller.
9. ✅ **`(-344)` — the CR 104.4b loop watch settles off the digest's scalar head: fixed -0.072 / cube -0.240 / sealed -0.071 %.** 📐 **EVERY presence gate and memo this run landed carries a recompute-and-compare `debug_assert!` on its cheap path**, so the suite and every sweep cell audit it — `(-303)` point 2 and `(-311)`'s lesson. **A gate without that ratchet is a silent wrong answer waiting for the board that reaches it.** 📐 **A memo census has TWO numbers**: how often the cheap branch is *reachable* (54.4 % of `cube` samples) and how often the cheap key *separates* on it (97.5 %). Measuring only the first is `(-329)`'s mistake. The shortcut is **audited, not argued** — a `debug_assert_ne!` recomputes the skipped digest, so the suite and every sweep cell are the ratchet; 59,200 games, no fire.
10. ⚠ **`cow::tests::game_state_stays_small` IS 1,680 NOW (was 1,672) AND THE RAISE IS THE PRECEDENT, NOT THE NUMBER.** `(-344)`'s 8-byte field costs 529 k Ir a `cube` run and the A/B that asked for it already carries that. **Raise it again only with a measurement of that shape.**
11. **Perf queue: (J) and (K) CLOSED as diffuse, (L) taken as `(-344)`, (M) taken as `(-345)`.** `resolve_combat_into` (1.57 %, 276 lines, top line 0.09 %) and `do_untap` (0.53 %, 89 lines, top 0.04 %) are both "the body IS the cost". ⚠ **Both reads cost NOTHING and saved two builds:** `cg_lines.py` needs no `profiling-lines` build — one 16-second `--dump-instr=yes` callgrind line-profiles every body at once. **Line-profile a candidate before building for it.** Also open: (A) `ManaCost::cmc`, (B) the `CardCold` `Arc::ptr_eq` gate, (C) `blocker_pair_block`'s pair count, (H) the other zones' by-id scans.
12. **THE PROFILE IS FLAT AND PERF'S CANDIDATES HEAD SAYS SO WITH THE GLOBAL LINE TABLE — do not re-derive it.** Largest single source line **0.97 %** (`Arc::clone_from_ref_in`'s memcpy), then 0.80 / 0.56 / 0.41 %, and **12,537 lines hold 86.9 %**. Rank by **self per call**, not by self.
13. **Tip absolutes at `ac1381ed` (`profiling-fast --no-default-features`, system allocator): fixed 579,232,689 / cube 1,510,834,067 / sealed 1,632,366,971.** ✅ The twelfth box's closing absolutes **reproduced to six significant figures**. **"Absolutes do not cross boxes" is about the CPU MODEL, not the box.** ⚠ **Build times are CLOSED as a lever** — PERF's own measurement kills the file-split idea (a 138x file-size difference buys nothing, and a split of a hot body without `#[inline]` across the new seam COSTS ~0.1 %).
14. ⚠⚠ **THREE TRAPS, ALL LIVE.** `cargo build --profile profiling-fast -p crabomination --bin bot_ladder --no-default-features` — **both flags load-bearing**; `nm <bin> | grep -cE " (T|t) (_)?mi_"` = 0 before filing a number. **Background waits do not progress between turns**: run builds in the FOREGROUND and **re-issue the same `cargo build`** when one overruns the 600 s tool timeout — it blocks on the artifact lock and that block *is* the wait. ⚠ **Never overlap two sweep blocks on four cores: 3m55s becomes 29m13s.** Disk: a run this size reaches 83 %; `find target/debug/deps -maxdepth 1 -type f -size +50M -delete` plus `rm -rf target/*/incremental` returns ~17 GB.
