...
fn overzealous_muscle_indestructible_on_crime() {
    ...
    assert!(!g.computed_permanent(muscle).unwrap().keywords.contains(&crate::card::Keyword::Indestructible));
    g.dispatch_triggers_for_events(&[GameEvent::CommittedCrime { player: 0 }]);
    drain_stack(&mut g);
    assert!(
        g.computed_permanent(muscle).unwrap().keywords.contains(&crate::card::Keyword::Indestructible),
...
fn outlaws_fury_pumps_and_impulses() {
    ...
    rogue.subtypes.creature_types = vec![crate::card::CreatureType::Rogue];
