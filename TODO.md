...
2. **Gates at this run's tip (2026-09-25, session `015cKV6u` run 7):** suite **22,327 / 0 / 7** after two sign-offs (Tip the Scales prints its sacrifice as the effect; Forth Eorlingas!' per-dealer monarch grant is idempotent); workspace clippy **0**; `--bench` **195,806 decisions, 27.49 turns — byte-identical** through every deck of the run; pods for seats 132-149 all 1,000 / 1,000 bar one genuine simultaneous-loss draw. ⚠ **DISK:** the task-output tmpfs filled mid-suite (ENOSPC swallowed the output) — clear `target/*/incremental` before a long run.
