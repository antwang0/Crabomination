...
W=.ladder/deckwork; R3=$W/r3; S=$R3/search4; G=./target/release-fast/deck_gauntlet
mkdir -p $S
echo "=== r3b search start $(date)"
...
python3 scripts/deck_search.py --base decks/real_build_converge4.txt --pool decks/sealed_pool.txt \
  --cards $W/cards.json --work $S --gens 4 --pairs 500 --threads 20 --tag r3b
echo "SEARCH EXIT $?"
F=$S/final.txt
...
rm -rf target/deckwork/replays/r3b_final_search
$G --deck $F --pairs 25 --seed 43 --threads 12 --pilot mcts256 --opp-pilot mcts256 \
   --replays target/deckwork/replays/r3b_final_search --out $S/games_final_search.txt 2>/dev/null | grep -E "^RESULT"
...
python3 scripts/replay_scan.py target/deckwork/replays/r3b_final_search --deck-prefix final > $S/scan_final_search.txt 2>&1
python3 $R3/whohit.py target/deckwork/replays/r3b_final_search final "Arcane Omens;Traumatic Critique;Together as One;Divergent Equation;Bind to Life" > $S/whohit_final.txt 2>&1
echo "=== r3b search done $(date)"
