...
8. ✅ **Spend-restricted mana now pays (`e12ccee0`, CR 106.6):** a payment that comes up short floats the payer's untapped restricted sources whose restriction allows the `SpellKind` and pays again (`pay_with_restricted_sources`, failure path only) — Secluded Courtyard / Unclaimed Territory / Ancient Ziggurat / Vodalian Arcanist were colorless or dead to bots. ⚠ Still not taken: a restricted source whose ability **costs mana** (Castle Garenbrig, Lord of the Forsaken's graveyard mana) — the retry only fires `is_countable_mana_ability` sources. ⚠ **Two cast-origin bug classes closed the same day:** `Predicate::CastFromGraveyard` and River Kelpie read "not cast from hand", so every **command-zone cast** counted as a graveyard cast (Ash Zealot hit every commander caster); now `CardInstance/EffectContext::cast_from_graveyard`. Cards that MEAN "not from hand" say `Not(CastFromHand)`.
