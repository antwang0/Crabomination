...
3. **Gates at the closing tip `61067dd2`:** suite **20,038 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, golden traces unmoved); clippy **0** over the workspace (`--all-targets`); `--bench` **195,806 / 27.49 / 611.9 / 0 stalls**, `determinism ok`, `peak_rss_mib` 29.2 — **byte-identical to the committed invariant**, including across an `affected_from_requirement` change in the layer gather. Pod smoke on `release-fast`: **6,000 games** over 2/4/6/7 seats (seeds 8202/8204/8206/8207, 1,500 apiece), **100 % decided, every `undecided_by` column zero**. **FRONTIER 8208.** ⚠ No assertions sweep taken at this tip — the engine change is a *narrowing* of a layer filter and the suite covers it, but the next run should fold one in. ⚠ **`cargo nextest --cargo-profile release-fast` DOES NOT BUILD**: `land_tap_fast_path.rs` imports `PLAIN_LAND_TAPS`, which is `#[cfg(debug_assertions)]`. The suite is a debug-profile artifact; do not spend a build finding that out again.
