...
fn malevolent_witchkite_sacs_and_draws() {
    ...
    g.step = TurnStep::PreCombatMain;
    g.priority.player_with_priority = 0;
    g.add_token_to_battlefield(0, &crate::game::effects::food_token());
    g.add_token_to_battlefield(0, &crate::game::effects::treasure_token());
...
fn realm_scorcher_bargained_ramps() {
    ...
    g.step = TurnStep::PreCombatMain;
    g.priority.player_with_priority = 0;
    let token = g.add_token_to_battlefield(0, &crate::game::effects::treasure_token());
...
fn tough_cookie_food_and_animate() {
    ...
    assert!(f.card_types.contains(&crate::card::CardType::Creature), "now a creature");
}
