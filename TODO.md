...
8. 📐📐 **A POOL IS PRICED BY THE ALLOCATIONS IT RECOVERS *PER CALL*, AGAINST ~2 TLS TOUCHES PER BUFFER — `(-367)` WINS AT 0.96 A CALL AND `(-368)` LOSES AT 0.55.** `cast_candidates` allocates on barely half its calls (an empty accumulator never allocated), so its `take` is pure overhead on the rest; it removed exactly its 6,426 allocations and read **+0.180 / +0.126 / +0.152 %**. `(-359)`'s ~52 Ir a `LocalKey::with` pair is the constant to multiply. 📐 **AND AN ALLOCATION HAS A PRICE RANGE, NOT A PRICE: 111 / 222 / 245 / 278 / 509 Ir**, all measured by deletion (`(-355)` / `(-362)` / `(-364)`). A buffer a caller builds is 222; one `GameState::clone` copies is 278 (the fill loop and the drop go too); one carrying a `String` and six `Vec`s is 509 (the `memcpy` goes too). **Size a lead by the BYTES it copies, not only by the count.** ⚠ **AND THE BOX WAS A NON-EVENT, WHICH IS ITSELF A READING**: the base missed its prediction by **+0.017 / +0.010 / +0.016 %**, an order of magnitude under the two previous fresh-container steps. **A fresh container is a fresh box, but "fresh box" does not mean "the last offset" — re-take the base and read what it says.**
