...
5. 🔎 **Perf — THE NEXT ROW IS `permanent_value_with` (`eval_material_inner`'s per-permanent value), the largest remaining `computed_permanent_hinted` caller — 32.1 M / cube, 38,972 calls — BUT IT NEEDS A PRIMITIVE THAT DOES NOT EXIST.** It reads the computed view for `card_types().contains(Creature)`, `.contains(Planeswalker)`, `supertypes().contains(Legendary)`. For a printed artifact/enchantment with no type change in scope the first two are printed-derivable, but the legendary term (`+2*w.unit`) needs a **legendary-grant presence gate** (`AllNonlandPermanentsAreLegendary` static + `ring_temptations>=1`; AddSupertype has no `mod_families` bit). Add a `SUPERTYPE` family bit + a `supertype_change_in_scope()` slot, then the `(-320)` shape gates it. Est 0.3-0.8 % of `cube`. See the candidates section.
...
10. **Best next moves, in order.** (a) `permanent_value_with` + the `supertype_change_in_scope()` primitive (line 5) — the clean continuation of `(-320)`, one presence-slot addition then the same shape. (b) Run the callee-table checklist to the end (line 4): every `computed_permanent_hinted` caller and every `battlefield.iter().any(` over ~0.5 %, asking which per-permanent question the printed shape answers. (c) The fresh-seed sweep (line 8) as the FIRST gate after any epoch/memo change. (d) The structural rows (line 6) are a count/structure question, not a line. (e) leftover context: `audit_dropped_may` 199 (`LookPickToHand` not safe at this granularity — put 73, exile 20, return 16, collect 12, begin 12); `audit_keyword_drift`'s last 4 each need a PRIMITIVE; card skips with a card behind them: `nocolors` 950, `noback` 60, `nosubtypes` 43, `nokeywords` 29.
