...
13. 💡 **Standing leads, best first:** ① the six remaining 614.12 rows, by mechanic (a coin flip and a random pick would share one primitive). ② The `as_enters_effect` ask on the cast path (item 10) — three resume contexts, and it is the last human-seat gap in the class. ③ `audit_dropped_may` **198** and `audit_incomplete` are the two big unmined columns. ④ COMMANDER_BACKLOG's top-1000 gap after the land cycles closed.
