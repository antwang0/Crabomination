...
| Stadium Tidalmage | {2}{U}{R} | Creature — Djinn Sorcerer | 4/4 | Whenever this creature enters or attacks, you may draw a card. If you do, discard a card. | 🟡 | Push XV: ETB + Attacks loot triggers now use the new `Effect::MayDo` primitive. The "you may" prompt asks the controller via `Decision::OptionalTrigger` — `AutoDecider` says no, `ScriptedDecider::new([Bool(true)])` for tests. Promoted from "always loot" approximation to faithful opt-in. |
