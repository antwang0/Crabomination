...
2. **Gates at the tip, all re-run this run:** suite **19,548 / 0 / 5** (`CRAB_ANSWER_LOG=strict`), clippy **0** (`--workspace --all-targets --exclude crabomination_client`), golden traces unmoved, `--bench` **195,806 / 27.49 / 611.9 / 0 stalls byte-identical to the committed invariant** with `determinism ok` and `thread_determinism ok (3 vs 1)`. ⚠⚠ **THIS BOX IS 4 CORES AND READS 635-678 games/s across two runs of the same binary** against the previous session's 418 on its own 4-core box — **no absolute in PERF is comparable across boxes, and a single `games_per_s` is not comparable to itself; the counters are the invariant.** ⚠ `profiling-fast` inherits `release-fast`, so every A/B build already saw `debug-assertions = false`.
3. **Perf — the run took -0.52 / -0.95 / -0.59 % (fixed / cube / sealed) in two commits, both on the candidates head's #1 row and neither of them the device the head named.** `(-304)`: `compute_permanents` re-derived the CR 613.8 gate set per call (`SecondPass::of`) when the freeze scope's memo **already stores those bits beside the list**, and looked cards up with a linear `bf.iter().find` where `Battlefield::find_by_id` is hint-cached — **-0.371 / -0.337 / -0.302 %**, the hinted lookup five times the larger half. `(-305)`: that function's `collect()` sits behind a `filter_map`, which reports `(0, Some(n))`, so the Vec grew from zero 20,484 times a `cube` run — `Vec::with_capacity(ids.len())` + a `for` loop is **-0.148 / -0.619 / -0.294 %**, bigger on `cube` than `(-304)` for five lines.
4. **Perf — the two transferable shapes, and the refutation to read before re-taking the head's #1.** (a) **A batch path that looks a card up by id per element is a grep target**: `find_by_id` is hint-cached and the linear scan is not. (b) **A `collect()` with ANY filtering adapter over a known-length source is a grow loop wearing an allocation-free spelling.** `SpecFromIterNested` was 57.6 M self / 3.4 % of `cube` and is 35.2 M / 2.1 % after `(-305)`; its remaining hot callers are `check_state_based_actions_into` (16.0 M inclusive), `bot::pick_attacks_inner` (14.0 M), `dispatch_triggers_for_events_slow` (10.6 M), `declare_attackers_banded` (6.4 M) — none taken, and the grow path under them is FLAT (top `grow_one` caller is 10,806 calls / 0.08 %), so expect ~0.1 % each, not another 0.6 %. ⚠ `with_capacity` allocates where an all-rejecting `collect()` would not.
5. **Perf — `(-304)` REFUTED the head's actual #2 device and the refutation is the transferable half.** "Route `compute_permanents` through the freeze scope's `perms` memo" reads **+0.230 / +0.383 / +0.226 %** and **its hit rate is ZERO**: 36,258 ids asked, 36,258 layer passes run, not one already in `perms`. The batch path and the single-card path are asked about **disjoint** sets. **A memo shared between two call sites is priced by the OVERLAP of what they ask**, and the head had sized it off the size of the row it would serve. ⚠ The head's #1 (the state-level gathered-effect memo, ~1.8-2.5 % on every pool) is **still open and still blocked on the same thing `(-303)` named**: `self.players` at 45 read sites has no mutation chokepoint the way the `CowBox` zones do, so the prerequisite is a `players_mut()`-shaped accessor, not the memo.
6. **Bugs — a second target-walker CLASS closed, one level up from last run's 5c.** `requires_target` is exhaustive, so an arm that recurses into an inner body **says a target there is chosen at cast time**; `target_filter_for_slot_in_mode_kicked` ends in `_ => None`, which `check_target_legality` reads as the RESTRICTIVE default. **22 wrappers were in the first and not the second** (`AtNextEndStep`, `VillainousChoice`, `FlipCoinsChooseCount`, `EscalatingThisTurn`, …) — latent, no catalog card nests one yet. ⚠ **The judgement did not have to be re-made per wrapper**: `audit_target_walkers.py --check` now DERIVES the set from the census and counts only the disagreement (**22 of 50 -> 0**); the other 28 are visibly correct (CR 603.7d). ENGINE_BACKLOG rule **5d**, test `target_walkers::every_wrapper_the_census_recurses_into_surfaces_its_slot_filter`. Cost-free (±0.000 %). **`all` 1281 is still open and its field is `ctr`.**
7. **Seeds: frontier 1300 — THIS RUN TOOK 1296..1299: 12 cells / 59,200 games / 0 failures**, `cap 0 / board 0 / stuck 0 / draw 0` — a clean block, no 50,000-action re-run needed. `target-audit/overflow` with `-C debug-assertions=yes` and `CRAB_ANSWER_LOG=strict` at the `(-304)` tip. Say which block you are taking in this line *before* you take it.
8. **Best next moves.** (a) ⚠ **`cg_ratio.py` and the self table are both spent for this pair of pools** — the queue's top two were taken this run and the third (`SpecFromIterNested`) is now flat per line 4. The biggest untaken engine rows on `cube` are `dispatch_triggers_for_events_slow` **74.9 M / 4.5 %** (73,736 calls, 1016 Ir each — `(-300)` gated out the 47 % empty ones, and the open half is **merging `perform_action_inner`'s batches**, which is a CR 603.3b ORDERING question before it is a perf one) and `check_state_based_actions_into` + `sba_board_scan` **79.9 M / 4.8 %** (⚠ `sba_census` already refuted repeat-sweep skipping: the fingerprint costs as much as the sweep). (b) ⚠ **`Arc::clone_from_ref_in` is 36.0 M / 2.2 % on `cube` and is a WORKED LANE, not a fresh lead** — the Log has a long run of passes grinding `make_mut_slow` from 202,162 to 193,112 calls for fractions of a point each; read them before opening it, and use `--demangle=no` (How to measure) or you will read eleven monomorphizations as one row. (c) ⚠ **DO NOT re-take `(-295)`'s class as a defect fix — `(-297)` laddered it and it LOSES** (pooled 49.58 % on sealed). (d) The skips with a card behind them: `nocolors` **950**, `nosubtypes` 43, `noback` 60, `nokeywords` 29 — say which you are taking. (e) `audit_keyword_drift`'s last 4 each need a PRIMITIVE (bestow / escalate / flashback / kicker, one card each). (f) `audit_dropped_may` reads **258** after this run's CR 701.19c exemption cut the largest bucket (56 library searches, 52 of them false positives — a hidden-zone search may always fail to find, so the printed "may" is already modeled and the one survivor, Sift Through Sands, is a MISSING EFFECT already filed in CARD_BACKLOG). It is still the largest unexamined defect list on the branch; the next buckets are "put a creature/land card" (30) and "draw a card" (18).
