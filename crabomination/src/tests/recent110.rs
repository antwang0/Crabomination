...
fn saffi_returns_dying_creature() {
    ...
    let events = g.resolve_effect(
        ...
        &crate::game::effects::EffectContext::for_spell(1, Some(Target::Permanent(bear)), 0, 0),
    )
    .unwrap();
...
fn restore_balance_levels_everything() {
    ...
    let events = g.resolve_effect(&crate::effect::Effect::Balance, &ctx).unwrap();
    g.dispatch_triggers_for_events(&events);
...
fn hurkyls_recall_bounces_artifacts() {
    ...
    let ctx = crate::game::effects::EffectContext::for_spell(0, Some(Target::Player(1)), 0, 0);
...
fn slippery_scoundrel_with_blessing() {
    ...
    assert!(!cp.keywords.contains(&crate::card::Keyword::Hexproof), "no blessing yet");
    g.players[0].city_blessing = true;
    let cp = g.computed_permanent(rogue).unwrap();
    assert!(cp.keywords.contains(&crate::card::Keyword::Hexproof));
    ...
    let events = g
        .resolve_effect(
            &crate::effect::Effect::Draw {
                who: crate::effect::Selector::Player(crate::effect::PlayerRef::You),
