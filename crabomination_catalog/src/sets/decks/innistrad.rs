...
use crate::card::{
    ...
    CreatureType, Effect, EnchantmentSubtype, EquipBonus, EventKind, EventScope, EventSpec,
...
pub fn suspicious_stowaway() -> CardDefinition {
    let mut stowaway = werewolf_dfc(
        ...
        (2, 1),
        vec![Keyword::Unblockable],
        vec![on_combat_damage_to_player(Effect::Draw {
            who: Selector::You,
            amount: Value::Const(1),
        })],
    );
...
pub fn clarion_cathars() -> CardDefinition {
    CardDefinition {
        ...
        triggered_abilities: vec![etb(Effect::CreateToken {
            who: PlayerRef::You,
            count: Value::Const(1),
            definition: std::sync::Arc::new(white_human_token()),
        })],
        ..Default::default()
    }
}
    ...
    CardDefinition {
        ...
        triggered_abilities: vec![etb(Effect::If {
            ...
            then: Box::new(Effect::CreateToken {
                who: PlayerRef::You,
                count: Value::Const(1),
                definition: std::sync::Arc::new(flying_bat_token()),
            }),
            else_: Box::new(Effect::Noop),
        })],
        ..Default::default()
...
pub fn brood_weaver() -> CardDefinition {
    CardDefinition {
        ...
        triggered_abilities: vec![on_dies(Effect::CreateToken {
            who: PlayerRef::You,
            count: Value::Const(1),
            definition: std::sync::Arc::new(reach_spider_token()),
        })],
        ..Default::default()
    }
}
    ...
    CardDefinition {
        ...
        triggered_abilities: vec![
            ...
            etb(Effect::CreateToken {
                who: PlayerRef::You,
                count: Value::Const(1),
                definition: std::sync::Arc::new(boar_3_1_token()),
            }),
        ],
        ..Default::default()
    }
    ...
    CardDefinition {
        ...
        effect: Effect::Seq(vec![
            Effect::CreateToken {
                who: PlayerRef::You,
                count: Value::Const(2),
                definition: std::sync::Arc::new(green_insect_token()),
            },
            Effect::GainLife {
                who: Selector::You,
                amount: Value::Const(2),
            ...
            effect: Effect::Seq(vec![
                Effect::Sacrifice {
                    ...
                    filter: SelectionRequirement::Creature.and(SelectionRequirement::Not(
                        Box::new(SelectionRequirement::HasCreatureType(CreatureType::Demon)),
                    )),
                },
                Effect::If {
    ...
    CardDefinition {
        ...
        triggered_abilities: vec![TriggeredAbility {
            ...
            effect: Effect::CreateToken {
                who: PlayerRef::You,
                count: Value::Const(1),
                definition: std::sync::Arc::new(tapped_zombie_token()),
            },
        }],
        ..Default::default()
    }
...
pub fn ominous_roost() -> CardDefinition {
    ...
    let make_bird = move || Effect::CreateToken {
        who: PlayerRef::You,
        count: Value::Const(1),
        definition: std::sync::Arc::new(bird.clone()),
    };
    CardDefinition {
        name: "Ominous Roost",
        cost: cost(&[generic(2), u()]),
