...
2. **Gates at the closing tip:** suite **20,223 / 0 / 6** (`CRAB_ANSWER_LOG=strict`, `--workspace --exclude crabomination_client`, which holds exactly **200** more `#[test]`s on a box that can build Bevy — the same tree reads 20,0xx ↔ 20,4xx with the box, not with the tree); clippy **0**; `--bench` **195,806 / 27.49 / 611.9 / 0 stalls — byte-identical**, `determinism ok`, `thread_determinism ok (3 vs 1)`, `peak_rss_mib` 29.3; **every audit gate 0**, the four added this session included. CR_COVERAGE **146/146**. **Pod: 2 fresh seeds × 2..10 seats × 1,000 = 18,000 games, all 18 blocks 1,000/1,000 decided, every `undecided_by` column zero, zero panics.** **Two-player: 4 fresh seeds × cube/sos/sealed × 300 = 30,000 games, 0 panics, 3 draws.** **FRONTIER 9802 / 3305.**
3. ⚠⚠ **`--bench` CAME BACK BYTE-IDENTICAL AFTER EVERY COMMIT THIS SESSION, INCLUDING THE SEVEN THAT CHANGED HOW A CUBE CARD PLAYS. THAT IS NOT EVIDENCE.** `--decks fixed` is four hand-built archetypes and holds almost none of the ~50 cards this run touched. The 30,000-game cube/sos/sealed run is the only two-player guardrail with anything to say about them; the pod run is the only one that exercises an **elimination**, which is what half this session's finds were about. **Pick the guardrail by what the diff touched, not by what is cheapest to run.**
4. 🔎🔎 **THE QUESTION THAT FOUND THE MOST: "THIS RULE IS IMPLEMENTED TWICE — READ THE SECOND ONE."** `PlayerWithMostLife` filtered `is_alive()` and read `effective_life`; its four siblings (`LowestLife`, `HighestLife`, `MostCardsInHand`, `MostCreatures`) did neither, and a departed seat is at or below zero life so **`LowestLife` was always the dead seat** (sixtieth find, CR 800.4a, 14 sites). `Effect::Vote` walked `seats_in_turn_order_from`; `run_council_card_vote` — the same CR 701.38a — open-coded `controller + opponents_of`, i.e. **seat-index order** (sixty-first find). Neither is subtle: both are a correct answer written once and then re-derived. **Ask it of every helper that has more than one caller.**
5. 🔎🔎 **THE TOOLING FIND WAS THE BIGGEST ONE AND IT INVALIDATED FOUR "0" COLUMNS.** `audit_dropped_may.defs_in`, the reader six audits import, keyed every card on a `name:` field and skipped the rest: **7,030 of 22,134 factories never walked.** Now 22,050. `another_target` went 0/0 → 24 open, `dropped_may` 178 → 242, `target_zone`'s denominator 455 → 647. 📐 **Both of its own bugs were invisible in the totals** — an unanchored call match took the **token** a card mints (Conqueror's Pledge read as "Kor Soldier"), and `slug` turned `'` into `_` so the preference missed all 1,162 possessive names. Found by asking *how many yielded names slug-match their factory*, which no row count can answer. **A ratchet's denominator is a number to check, not to trust.**
6. 🔎 **FIND: "any player may X. If a player does, Y" CLOSED ON THE FIRST ACCEPTANCE** (fifty-fourth). Argothian Wurm's 2007 ruling and Desecration Demon's 2024 one both say every player is still asked and the consequence happens **once**. `PlayersMayAccept` now splits `on_accept` (per accepter) from `if_any` (once). 📐 **The ask loop had to move before the apply loop and that is CR 101.4a, not style** — it is also the only replay-safe order, since a `wants_ui` seat suspending mid-apply would re-deal damage on the re-run.
7. 🔎 **FIND: "DEFENDING PLAYER" IS ONE SEAT AND 32 CARDS HAD IT AS "ANY OPPONENT"** (fifty-sixth, CR 506.2). `ControlledByDefendingPlayer` + `audit_defending_player`, 0/32. Blaze of Glory carried **no** controller filter at all. 📐 **The clause is often not on the attacker** — an instant cast in combat, an Aura/Equipment whose host attacks, a trigger firing off *another* creature's attack, and a post-teardown trigger whose attack record is gone. The first three go through `defending_player_in_combat`; the fourth (Kusari-Gama) needs the damaged **blocker's controller**, which is that seat by definition.
8. 🔎 **FIND: "MILL, THEN RETURN" WAS CHOOSING BEFORE THE MILL** (fifty-ninth, CR 608.2). Grapple with the Past over an empty graveyard: graveyard 0 → 4, hand unchanged — a pure three-card self-mill. Six cards; `audit_resolution_order`, 0/23. 💡 **Three of the six came from the ratchet, not from the census that found the first three**: the census keyed on "mill N cards", the ratchet on "fills a zone, then reaches into it". **Write the audit against the shape of the bug, not the wording of the example.**
9. ✅ **EXERT IS A RULES FEATURE NOW** (fifty-seventh, CR 701.43 / 508.1g / 607.2h). `EventKind::Exerted` + `on_exert` + `GameAction::DeclareAttackersExerting`; six cards. 📐 **The policy is only askable because the ability became linked** — `exert_pays_off` declines when the bonus has no legal target, and an unconditional `Attacks` trigger carries nothing that says which trigger the exert is buying. ⚠ Residual: no modal, so a `wants_ui` seat cannot decline one its policy takes.
10. ✅ **The tenth target deck is Adriana, Captain of the Guard (RW)**, built because a census found **not one of the nine decks played a single multiplayer-native mechanic** — monarch (CR 725), goad (701.15), melee (702.121), voting (701.38), tempting offer + join forces (207.2c), the initiative (726), myriad (702.116), all implemented, all unreached by self-play. 27 such cards; `--seats 10`. ⚠ **It bends the turn curve**: linear at ~12.9 a seat, the tenth adds **+24.4**, reproducibly. Grand Melee was the suspect and the A/B **refuted it** (124.15 → 123.07). 💡 Remaining suspects: Protector of the Crown and the monarch's extra card a turn. The seat wins ~**5 %** of ten-seat pods against par 10 %.
11. 📐📐 **RESTORING A PRINTED "may" IS TWO EDITS AND THE SECOND IS THE POLICY.** `AutoDecider` declines *every* `OptionalTrigger`, so a `MayDo` wrapper without an answer is a card turned **off**. Mask of Memory proved it: `effect_imposes_self_cost` folds a `Seq` with `any` and a self-discard is on the list, so the wrapper alone would have left the Equipment a vanilla +0/+0. **A loot is not a cost** (`is_net_positive_loot`). Assert new rows through `optional_trigger_beneficial`, never through the wrapper's existence. `audit_dropped_may` 242 → **208**.
12. ⚠⚠ **A PROBE THAT SHOWS NOTHING HAPPENING IS A CLAIM ABOUT THE FIXTURE UNTIL IT IS A CLAIM ABOUT THE CARD.** Three lied in one hour: `destroy_permanent` into a discarded `Vec` throws the death events away; `move_card_to_battlefield_for_test` does not reach a constellation watcher (it needs a **cast**); and an existing test that scripts `DecisionAnswer::Target` proves the scripted path and says nothing about the headless one. Two cards were "fixed" on the strength of those before the side-by-side `enumerate_legal_targets_xc` comparison showed both shapes enumerate the same 2 targets. **Measure the old and new shapes together before calling anything a bug.**
13. 💡 **THE OPEN QUEUE.** ① `audit_dropped_may` **208** — the self-cost subset is mined out (13 rows, 1 real), so the next cut is a different question, not more of the same. ② ⚠ **A lead this file carried and that turned out to be wrong, checked and closed:** the council ballot's bot policy was fine all along. `decide_choose_target` already prefers a permanent the voter does **not** control and only gives up its own when every candidate is its own — which is right for a ballot, an edict and free-floating removal alike, and its doc said so. Now asserted by `a_council_ballot_never_votes_for_the_voters_own_permanent_when_another_is_legal`. **Read the policy before filing the lead.** ③ The 69-row non-Aura "invented target slot" census in INCOMPLETE_CARDS (the karoo shape), a divergence only under shroud or a response. ④ COMMANDER_BACKLOG's remaining top-1000 rows. ⑤ `target_walkers`'s invariant is **one-directional** — it proves every `TargetFiltered` is answerable, never that every `Target(n)` has one.
14. ⚠ **Gotchas that each cost a round.** ① A **"may"** needs `always_yes`/a scripted `Bool(true)` or the AutoDecider declines and the test sees nothing — a silent pass-looking failure. ② **Script exactly as many bools as the card asks**: `ScriptedDecider` falls back to `AutoDecider` when the script runs out, so four bools where one is wanted ate the following discard's card pick (`DecisionAnswerMismatch`). ③ `Selector::EachPermanent(filter)` does **not** thread the ability's source, so a source-relative atom inside one is always false; the mass-selection form is `Selector::ControlledBy { who, filter }`. ④ A new `SelectionRequirement` needs an arm in `evaluate_requirement_static_hinted` **and** `evaluate_requirement_on_card_inner`; the other walkers end in a permissive `_ =>` and it compiles either way. ⑤ `R` is not aliased in every catalog file.
15. ⚠ **The rules table is still DONE and that is still the main claim**: FEATURE_ROADMAP's Commander and 800-series sections are all ✅ bar CR 800.4i (unreachable) and an N-seat encoder (⛔ by design). Monarch and initiative **succession** on elimination (CR 725.4 / 726.4) is implemented in `stack.rs` — checked this run, not a gap. What is left is **cards, and the tooling that finds what is wrong with them**. `CR_COVERAGE.md` is generated: take either side of a conflict and re-run `scripts/cr_coverage.py`.
