...
///
/// ✅ Push XLIII: 🟡 → ✅. The "pay X life" additional cost is now
/// paid at *cast* time via the new `CardDefinition.additional_life_cost
/// = Some(Value::XFromCost)` primitive — the controller's life is
/// debited X (read from the spell's `{X}` pip) before the spell goes
/// on the stack. Loss-of-life triggers (Lifelink-payoff, pain-land
/// derivatives) fire pre-resolution. The body now contains only the
/// destroy-X-or-less mass-removal sweep. Loss-of-game from crossing
/// zero life is enforced by the next SBA pass per CR 119.4.
pub fn vicious_rivalry() -> CardDefinition {
    ...
    CardDefinition {
        ...
        toughness: 0,
        keywords: vec![],
        // Destroy all matching permanents whose mana value ≤ X. We
        // run a single `ForEach` over (Artifact ∨ Creature) with each
        // iteration gated by `Predicate::ValueAtMost(ManaValueOf(self),
        // X)`. The cast-time life cost is paid via `additional_life_cost`
        // below, so the body is now strictly the destroy half.
