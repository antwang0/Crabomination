...
5. **Seed frontier 1262** — this session took **1232..1241** (pre-merge tip, `cap 0 / board 0 / stuck 0 / draw 14`, the first block under the tightened verdict) and **1242..1251** (merged tip, `-C debug-assertions=yes`, `draw 12`), 60 cells / 296,000 games / 0 failures between them. ⚠⚠ **THE FOURTH SAME-SEED COLLISION: BOTH sessions swept 1222..1231 on 2026-09-13**, independently, and both read **30 cells / 148,000 games / 0 failures / `cap 0 board 0 stuck 0 draw 4`** — byte-identical verdicts off two different builds, which is a determinism cross-check nobody asked for and an hour each nobody needed to spend. This one ran under `-C debug-assertions=yes`, so the new `resolve_player` fan-out gate ran on every decision of all 148,000 games; theirs did not. **TAKE THE NEXT SEEDS FROM PERF'S TABLE, NOT FROM THE TOTAL, AND SAY IN THIS LINE WHICH BLOCK YOU ARE TAKING BEFORE YOU TAKE IT** — that is now four collisions in two days and the table is the only thing that stops it.
