...
11. **Standing leads, in the order they look worth taking:** ① the CR 614.12 primitive (item 4) — 89 cards, and the shocklands are top-100. ② `Keyword::Equip` carries ONE cost, so Commander's Plate (EDHREC 478) and Blackblade Reforged both need a **restricted second equip cost**, and the Plate also needs protection from a *computed* colour set. ③ `audit_dropped_may` **198** and `audit_incomplete` 858 are still the two big unmined columns. ④ 32 of 33 "can be your commander" non-creatures remain unimplemented — ordinary card work now that the eighth deck exists.
