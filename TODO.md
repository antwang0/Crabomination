...
3. **Perf — FOUR ROWS TAKEN. `(-317)` is the big one: `available_mana` reads its per-permanent answer off the definition, fixed -0.842 / cube -0.496 / sealed -0.540 %, the row -43.9 / -35.6 / -36.8 %.** For an untapped, non-sick permanent granted nothing, the loop's whole job is a pure function of the printed ability list, so `mana_summary_of` packs it into the word's last free bits — `AM_COLORS` (51-55), `AM_COLORLESS` (56), `AM_BEST_ONE` (57), `AM_EXACT` (58); `AM_EXACT` clear means "run the walk", so the pack can only **decline**, never mislead. **`(-318)`** put `fire_combat_damage_triggers`' opening fold behind a `writes`-keyed presence gate (fixed -0.061 / cube -0.417 / sealed -0.195 %, row -5.7 / -20.7 / -14.9 %). **`(-315)`** gated `grant_scan`'s one ungated pass (-0.188 / -0.113 / -0.203 %, row -36.4 / -29.5 / -47.1 %) and **`(-316)`** the CR 602.5g haste walk (-0.002 / -0.044 / -0.028 %, row halved). The branch moved **fixed -1.08 / cube -1.06 / sealed -0.95 %** between the `(-314)` tip (616,268,574 / 1,626,125,678 / 1,713,378,250) and the `(-318)` tip (609,619,688 / 1,608,960,836 / 1,697,060,293).
...
6. **Perf — what `(-318)` leaves, and the instrument it needs.** `fire_combat_damage_triggers` is still 6.52 M / 25.92 M / 19.29 M (1.07 / 1.61 / 1.14 %), and the walk is off it — what is left is the **three trigger phases below it** (the dealer's own list, the equipment/aura grants, the graveyard leg). Its whole callee tree is ~2.1 M, so the self is the work and the next look wants a **`profiling-lines` dump, cold**, with ⚠ `(-310)`'s rule about inlined frames. `combat_census` is in the tree and re-runnable (`--features trig-census`, `CRAB_COMBAT_CENSUS=1`), so the population numbers cost one run, not a build.
